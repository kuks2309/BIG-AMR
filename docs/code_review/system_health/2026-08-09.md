...
> **본 사본은 LGIT MOMA(Mobile robot + Manipulator) 이식본이다.** 상류(Ford-CATL `Big-AMR`)와 트리를
> 같게 유지하되, `read_fan`(#7)·`read_gpu`(#25)에 **x86 경로 탐색**을 더했다(분기 아님 — Tegra 경로를
> 먼저 보고 없을 때만 범용 경로를 본다). 시그니처는 그대로이고 전역 상수 2개가 늘었다(G42·G43).
> 실기(lgit-c6-4) 검증 결과는 §이식 검증 참조.
...
| # | 함수 | 입력 | 출력 | 기능 | 위치 |
| --- | --- | --- | --- | --- | --- |
| 1 | `_read_text` | `path` | `str \| None` | 파일 읽어 strip, 실패 시 None | sysfs.py:73 |
| 2 | `_read_int` | `path` | `int \| None` | 파일을 정수로, 실패 시 None | sysfs.py:81 |
| 3 | `_unique_key` | `key, seen` | `str` | 동일 type 노드 키 충돌 회피(`X#1`) | sysfs.py:92 |
| 4 | `read_temperatures_c` | 없음 | `dict[str,float]` | thermal zone 온도(°C) 전수 | sysfs.py:105 |
| 5 | `read_cooling_states` | 없음 | `dict[str,int]` | 냉각장치 `cur_state`(개입 여부) | sysfs.py:123 |
| 6 | `FanInfo` (frozen dataclass) | `pwm, rpm` | 인스턴스 | 팬 상태 값객체 | sysfs.py:147 |
| 7 | `read_fan` | 없음 | `FanInfo` | 팬 PWM/RPM(읽기 전용). Tegra `pwm-fan` → 범용 hwmon `fan1_input`/`pwm1` 순 탐색 | sysfs.py:161 |
| 8 | `CpuTimes` (frozen dataclass) | `idle, total` | 인스턴스 | `/proc/stat` 누적 시간 | sysfs.py:189 |
| 9 | `CpuSnapshot` (frozen dataclass) | `total, per_core` | 인스턴스 | 한 시점 CPU 누적 시간 | sysfs.py:197 |
| 10 | `CpuUsage` (frozen dataclass) | `total_pct, per_core_pct` | 인스턴스 | 구간 CPU 사용률 | sysfs.py:214 |
| 11 | `_parse_cpu_line` | `fields` | `CpuTimes \| None` | cpu 행 파싱(idle=idle+iowait) | sysfs.py:221 |
| 12 | `read_cpu_times` | 없음 | `CpuSnapshot \| None` | 전체·코어별 누적 시간 수집 | sysfs.py:238 |
| 13 | `_usage_pct` | `prev, cur` | `float` | 두 누적 시간의 구간 사용률 | sysfs.py:270 |
| 14 | `cpu_usage_pct` | `prev, cur` | `CpuUsage` | 스냅샷 2장 차분 → 사용률 | sysfs.py:279 |
| 15 | `read_cpu_freqs_khz` | 없음 | `tuple[int,...]` | 코어별 현재 주파수(kHz) | sysfs.py:312 |
| 16 | `MemoryInfo` (frozen dataclass) | 5 필드 | 인스턴스 | 메모리·스왑 사용량(MB) | sysfs.py:337 |
| 17 | `read_memory` | 없음 | `MemoryInfo \| None` | `/proc/meminfo` 파싱 | sysfs.py:351 |
| 18 | `DiskInfo` (frozen dataclass) | `path,total,free,used_pct` | 인스턴스 | 파일시스템 사용량(GB) | sysfs.py:390 |
| 19 | `read_disk` | `path` | `DiskInfo` | statvfs(`f_bavail` 기준). **예외 전파** | sysfs.py:399 |
| 20 | `ProcessInfo` (frozen dataclass) | `pid,name,rss_mb` | 인스턴스 | 프로세스 1건 | sysfs.py:429 |
| 21 | `ProcessScan` (frozen dataclass) | `count,top_rss,names,pids_by_name` | 인스턴스 | `/proc` 순회 결과 | sysfs.py:438 |
| 22 | `_read_process` | `pid` | `ProcessInfo \| None` | `/proc/<pid>/stat` 1건(rpartition) | sysfs.py:458 |
| 23 | `scan_processes` | `top_n=5` | `ProcessScan` | `/proc` 전수 순회 | sysfs.py:478 |
| 24 | `GpuInfo` (frozen dataclass) | `load_pct,freq_hz,max_freq_hz` | 인스턴스 | GR3D 상태 | sysfs.py:513 |
| 25 | `read_gpu` | 없음 | `GpuInfo` | GPU 사용률(천분율÷10)·주파수. Tegra devfreq → i915 `gt_*_freq_mhz`(MHz→Hz) 순 탐색 | sysfs.py:532 |
| 26 | `PowerRail` (frozen dataclass) | `name,voltage_mv,current_ma` | 인스턴스 | 전원 레일 1개 | sysfs.py:567 |
| 27 | `PowerRail.power_mw` (property) | self | `float` | mV×mA÷1000 = mW 계산값 | sysfs.py:582 |
| 28 | `read_power_rails` | 없음 | `tuple[PowerRail,...]` | INA3221 라벨 있는 채널만 | sysfs.py:587 |
| 28a | `EnergyCounter` (frozen dataclass) | `name,energy_uj,max_range_uj` | 인스턴스 | powercap 도메인 **누적** 에너지(신규) | sysfs.py:622 |
| 28b | `read_energy_counters` | `root=None` | `tuple[EnergyCounter,...]` | powercap(RAPL) 도메인 판독. 권한 없으면 빈 튜플. `root` 는 시험 주입점(신규) | sysfs.py:639 |
| 28c | `power_watts` | `prev,cur,elapsed_s` | `dict[str,float]` | 누적 에너지 차분 → 도메인별 W. `max_range` 로 되감김 보정(신규) | sysfs.py:669 |
| 29 | `SwapCounters` (frozen dataclass) | `pswpin,pswpout` | 인스턴스 | 누적 스왑 페이지 | sysfs.py:703 |
| 30 | `read_swap_counters` | 없음 | `SwapCounters \| None` | `/proc/vmstat` 파싱 | sysfs.py:716 |
| 31 | `swap_rate_pages_s` | `prev,cur,elapsed_s` | `(float,float)` | 스왑 in/out 속도(되감기 시 0) | sysfs.py:734 |
| 32 | `CanInterface` (frozen dataclass) | 8 필드 | 인스턴스 | socketcan 상태·누적 카운터 | sysfs.py:756 |
| 33 | `CanInterface.total_errors` (property) | self | `int` | rx+tx 에러·드랍 합 | sysfs.py:781 |
| 34 | `read_can_interfaces` | 없음 | `tuple[CanInterface,...]` | `type==ARPHRD_CAN` 으로 판별 | sysfs.py:786 |
| 34a | `read_can_interfaces.counter` (이너) | `field` | `int` | statistics 필드 읽기(None→0) | sysfs.py:803 |
| 35 | `can_error_rate` | `prev,cur,elapsed_s` | `float` | 에러 증가율(건/초) | sysfs.py:821 |
| 36 | `count_dds_segments` | 없음 | `int` | `/dev/shm` DDS 세그먼트 수(판정 없음) | sysfs.py:838 |
| 37 | `read_load_average` | 없음 | `tuple \| None` | 1·5·15분 부하 평균 | sysfs.py:856 |
| 38 | `read_uptime_s` | 없음 | `float \| None` | 부팅 후 경과 시간 | sysfs.py:864 |
...
| 63 | `SampleState` (frozen dataclass) | `stamp,cpu,swap,can,pids,energy` | 인스턴스 | 차분·비교용 직전 표본 묶음 | sampler.py:77 |
| 64 | `collect` | `prev, disk_paths, proc_scan, top_rss, fan_daemon_name, expected_processes, now` | `(record, SampleState)` | 한 시점 자원 record 생성 | sampler.py:95 |
| 65 | `_format_findings` | `findings` | `str` | 경보를 사람이 읽을 줄로 | sampler.py:256 |
| 66 | `_sleep_until` | `deadline` | None | 0.2s 청크 분할 대기(정지 반응) | sampler.py:261 |
| 67 | `_load_threshold_overrides` | `path` | `dict` | 임계 JSON 로드(실패 시 SystemExit) | sampler.py:270 |
| 68 | `_write_thresholds` | `path, th` | `int` | 임계값을 주석 포함 JSON 으로 저장 | sampler.py:289 |
| 69 | `_parse_args` | `argv` | `Namespace` | sampler CLI 해석 | sampler.py:324 |
| 70 | `run` | `args` | `int` | 샘플 루프 본체 | sampler.py:378 |
| 71 | `main` | `argv` | `int` | sampler 진입점 | sampler.py:497 |
...
| 84 | `filter_since` | `records, since` | `list[dict]` | ISO 시각 이후만 | report.py:279 |
| 85 | `format_report` | `log_dir, interval_s, prefix, since` | `str` | 사람이 읽는 6절 보고문 | report.py:297 |
| 86 | `_parse_args` | `argv` | `Namespace` | report CLI 해석 | report.py:386 |
| 87 | `main` | `argv` | `int` | report 진입점 | report.py:400 |
...
| G44 | `read_energy_counters`(#28b) | **`_POWERCAP_ROOT` (상수 경로, 신규)** — `/sys/class/powercap`. x86 의 전력 관측점 | sysfs.py:51 |
| G45 | `power_watts`(#28c) | **`_UJ_PER_J` (상수, 신규)** — μJ→J 환산 | sysfs.py:52 |
...
## 이식 검증 (LGIT 실기 lgit-c6-4, x86_64)

원본 장비(Jetson Orin NX, aarch64)와 하드웨어가 다르므로 **읽히는 항목이 달라진다**. 아래는 이식본을
실기에 올려 직접 실행한 결과다(서비스 등록 없이 수동 실행).

| 항목 | 실기 실측 | 비고 |
| --- | --- | --- |
| 회귀 | `python3 -m pytest test -q` → **220 passed in 11.85s** | 실기에서 실행 |
| 온도 | `acpitz 39.1` · `pch_cannonlake 35.0` · `x86_pkg_temp 38.0` | thermal zone 3개 |
| GPU | `freq_hz` 367,000,000 / `max_freq_hz` 1,050,000,000 | i915 경로 적중. 직접 노드 판독값 367/1050 MHz 와 일치. 사용률 노드는 없어 `load_pct` None |
| 팬 | `pwm` None · `rpm` None | 이 장비는 `fan*_input`·`pwm*` 노드를 노출하지 않는다(`ls /sys/class/hwmon/hwmon*/fan*_input` → 0건) |
| 전원 | 항목 미기록 | INA3221 없음. RAPL 은 있으나 `energy_uj` 가 `-r------- root` 라 서비스 계정으로 판독 불가 |
| CPU · 메모리 · 디스크 | 10.8 % / 8 코어 · 7,807 MB · 227 GB(35.6 %) | 공통 procfs 경로 |
| DDS | 세그먼트 95개 | 기록만, 판정 없음 |
| 판정(기본 설정) | `ERROR ['fan_daemon']` | `nvfancontrol` 은 Jetson 전용이라 이 장비에서 **상시 오탐**이 된다 |
| 판정(`fan_daemon_name=thermald`) | **`OK`, findings 0** | 실기에서 `thermald.service` 구동 확인(comm=`thermald`) |

**포트 충돌 (이식으로 드러난 결함)**: 원본 기본 포트 8770 은 이 장비에서 **Drawer 콘솔(uvicorn)이
`0.0.0.0:8770` 으로 이미 점유**하고 있어 대시보드가 `OSError: [Errno 98] Address already in use` 로
죽는다. 조치로 유닛에 `@PORT@` 자리표시자를 두고 `config/system_health/service.env` 의 `HEALTH_PORT`
로 장비별 지정하게 했다(이 장비 8771). `install_service.sh --apply` 는 설치 전 포트 점유를 경고한다.
실측: 렌더된 유닛 `--port 8771` · 자리표시자 잔존 0 · `systemd-analyze verify` exit 0 ·
8771 기동 후 `/`·`/api/latest`·`/api/report` 전부 HTTP 200.

**미실행**: `install_service.sh --apply`(상시 등록)는 `sudo` 암호가 필요해 하지 않았다. 지금 실기에는
상주 서비스가 없고 수동 실행으로만 검증했다.

**전원 감시 부재의 성격**: 하드웨어 부재가 아니라 **권한** 문제다. RAPL 노드는 존재하며 root 로는
읽힌다. 승격하려면 udev 규칙 등 로봇 시스템 변경이 필요하고, 그것은 "관측 전용·최소권한" 원칙과
맞물리므로 별도 결정 사항으로 남긴다.
