...
severity 분포: Critical 0 / High 2 / Medium 3 / Low 1 / Info 1
...
**#5 의존성 — [품질][신규] `package.xml` 의 `csm` 은 본 노드가 쓰지 않는다 (Medium)**
   재현: `seer_pose_publisher/package.xml` 에 `<exec_depend>csm</exec_depend>` 가 있으나
   `pose_node.py` 어디에도 csm 사용이 없다(grep 0건). 불필요한 런타임 의존은 배포 환경에서
   설치 실패를 만든다.
   권고: 제거하거나, 쓰는 곳이 있으면 근거를 주석으로 남길 것.
