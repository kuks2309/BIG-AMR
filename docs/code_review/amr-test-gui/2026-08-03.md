...
| 18 | `MainWindow._steer_axis` | `node,deg` | `float` | 0x607A + 0x6040=0x3F 2프레임. **목표를 `_steer_counts` 에 남겨 폴 루프가 재송신한다**(2026-08-14 — 단발이라 프레임 유실 시 그 축이 지령을 통째로 놓쳤다) | gui.py:552 |
...
| 21 | `MainWindow._fill_row` (classmethod) | `table,node,values` | None | 노드 행에 각도·회전·전류 기입. **`None` 은 `—` 로 덮어쓴다**(2026-08-14 조치 — 종전 코드는 칸을 건너뛰어 옛 숫자가 남았고 이 표의 서술과 어긋나 있었다). 이식본 `ui/app.py` 와 같은 규칙 | gui.py:615-626 |
...
| 45 | `MainWindow._jog` | `label` | None | 조그 진입 게이트. **제어권 보유 시 CAN 직결, 미보유 시 Seer API 경로(#45a)로 분기**(2026-08-14). `정지` 는 즉시 처리 | gui.py:1226-1253 |
| 45a | `MainWindow._jog_seer` | `label` | None | **2026-08-14 신설** — 제어권 없을 때의 조그. `정지` 는 Seer 2000 즉시, 그 외는 #45d 워커 기동 | gui.py:1267 |
| 45b | `MainWindow._seer_velocity` (staticmethod) | `steer_deg,raw_sign,mmps` | `(vx, vy)` m/s | **2026-08-14 신설** — `JOG` 표(조향각·raw 부호)를 차체 속도로 환산. `v = −raw_sign × (cos θ, −sin θ)` · mm/s → m/s | gui.py:1255 |
| 45c | `MainWindow._run_seer_ctrl` | `method,*args` | None | **2026-08-14 신설** — Seer 제어 포트(19205) 호출 1회를 작업 스레드에서. 결과·실패를 로그로 | gui.py:1288 |
| 45c-1 | `_run_seer_ctrl.work` (inner) | — | None | 그 호출 본체 | gui.py:1290 |
| 45d | `MainWindow._jog_seer_run` | `label,vx,vy` | None | **2026-08-14 신설** — Seer 조그 워커. `SEER_JOG_PERIOD_S` 마다 개루프 운동(2010) 재송신, 종료 시 2000 송신 | gui.py:1308 |
| 45e | `MainWindow._seer_ctrl_client` (staticmethod) | — | `RobokitClient` | **2026-08-14 신설** — 제어용 Seer 클라이언트(상태 폴링과 별개 연결 — 19205 는 배타 포트) | gui.py:1300 |
...
| 50 | `MainWindow._wait_settle` | `target,tol,timeout,honor_jog_stop` | `bool` | 두 축 모두 허용치 안이어야 정착. **`honor_jog_stop=False` 면 조그 정지 래치를 보지 않는다**(2026-08-14 — 호밍은 조그가 아닌데 잔류 래치가 0° 복귀 판정을 즉시 실패시켰다) | gui.py:1510 |
| 51 | `MainWindow._loop` | — | None | **poll 스레드** — heartbeat + 4객체 폴 + 응답 파싱 + **구동·조향 재송신**(2026-08-14 조향 추가, 호밍 중 제외) | gui.py:1532 |
| 52 | `MainWindow._on_motor_data` | `data` | None | 표·`_meas_deg`·`_status` 갱신. 호밍 중 각도 미갱신 | gui.py:1064-1083 |
| 53 | `main` | — | `int` | **중복 실행 가드(#54) 확인 후** 해제 4경로 배선 + 신호 pump 타이머 + 이벤트 루프. 이미 실행 중이면 창을 만들지 않고 `1` 반환 | gui.py:1682 |
| 54 | `acquire_single_instance` | `path` | `(파일객체\|None, 선점자 PID\|None)` | **2026-08-14 신설** — `flock(LOCK_EX\|LOCK_NB)` 로 한 번에 하나만 띄운다. 판다는 한 프로세스만 열 수 있어 두 창이 동시에 USB 를 잡으면 둘 다 못 쓴다. `flock` 이라 프로세스가 죽으면 커널이 자동 해제 — 찌꺼기 PID 파일이 다음 실행을 막지 않는다 | gui.py:1654 |
...
| G18 | #54 | `SINGLE_INSTANCE_LOCK="/tmp/amr_test_gui.lock"` (상수) — 중복 실행 가드용 잠금 파일 경로 | gui.py:1651 |
| G16 | #45d | `SEER_JOG_DURATION_MS=600` (상수) — 개루프 운동(2010) 시한. 재송신이 끊기면 이 안에 로봇이 선다 | gui.py:86 |
| G17 | #45d | `SEER_JOG_PERIOD_S=0.2` (상수) — 그 시한보다 짧은 재송신 주기 | gui.py:87 |
...
결함 11건 수정으로 함수 4개가 늘었다. 표에 반영한다.

| # | 함수 | 입력 | 출력 | 기능 | 위치 |
| --- | --- | --- | --- | --- | --- |
| 54 | `_load_steer_home` | — | `(dict, str)` | 조향 0° counts 를 **정본 YAML 에서** 읽는다. 실패 시 사본 + 그 사실을 출처 문자열에 남긴다 | gui.py:45-71 |
| 55 | `MainWindow._set_meas` | `node,deg` | None | 실측 1건 반영 — **값과 시각을 함께** 남기는 단일 기록지점 | gui.py:648-656 |
| 56 | `MainWindow._on_poll_died` | — | None | 폴링 스레드 사망 시 제어권 표시를 내린다(표시↔동작 불일치 차단) | gui.py:1097-1103 |
| 57 | `MainWindow._append_log` | `msg` | None | 로그 위젯을 만지는 **유일한 지점**(GUI 스레드 슬롯) | gui.py:768-772 |

### 전역 delta

| # | 사용처(함수) | 기능 | 위치 |
| --- | --- | --- | --- |
| G16 | #55·#30 | `MEAS_TTL_S = 1.0` (상수) — 이보다 오래된 실측은 없는 것으로 친다 | gui.py:50-52 |
| G17 | #51 | `RX_TTL_S = 1.0` (상수) — 응답이 이보다 오래 없으면 구동 0 | gui.py:53 |
| G18 | #54 | `_STEER_HOME_FALLBACK`·`_MACHINE_YAML`·`STEER_HOME_SOURCE` (상수/파생) — 정본 로드 실패 시 사본과 그 출처 | gui.py:45-71 |

⚠ `STEER_HOME`(G4)은 이제 **리터럴이 아니라 `_load_steer_home()` 의 반환값**이다.
`test_steer_home_sync.py` 의 사본 규칙도 `_STEER_HOME_FALLBACK` 기준으로 바뀌었다.

---

---

## 조치 후 검출력 검사 (2026-08-04)

소견 11건을 모두 고친 뒤 `111 passed` 로 보고했으나, **통과는 「고쳤다」의 증거가 아니다.**
회귀가 대상 코드를 지나가지 않으면 고쳐도 통과하고 되돌려도 통과한다. 11건을 하나씩
수정 전 상태로 되돌려 회귀가 잡는지 확인했다(`Tools/amr_test_gui/mutation_check.py`).

**첫 실행에서 3건 미검출** → 그중 2건은 **돌연변이 지렛대가 틀린 것**이었다:
`MEAS_TTL_S`·`RX_TTL_S` 를 시험이 `monkeypatch` 하므로 기본값을 바꿔도 무력하다.
로직 자체(`_meas_angle` 의 TTL 분기, 워치독 분기)를 되돌리자 즉시 검출됐다.
**남은 1건이 진짜 공백**이었다 — 아래.

| 항목 | 되돌린 내용 | 판정 | 잡은 회귀 |
|---|---|---|---|
| H1 | `_meas_angle` 의 실측 신선도(TTL) 검사 무력화 | ✅ | `test_stale_measurement_is_not_used` 외 2 |
| H2 | heartbeat 를 `_can_lock` 밖으로 | ✅ | `test_heartbeat_and_sdo_do_not_overlap` 외 1 |
| H3a | 폴 루프의 구동 재송신 제거 | ✅ | `test_drive_command_is_resent_by_poll_loop` 외 1 |
| H3b | 응답두절 워치독 분기 무력화 | ✅ | `test_watchdog_zeroes_drive_when_bus_goes_silent` |
| M1 | 조향 0° 정본 YAML 로드 실패시킴 | ✅ | `test_steer_home_comes_from_canonical_yaml` 외 1 |
| M2 | Seer 경로 환경변수 주입 제거 | ✅ | `test_seer_path_is_overridable_by_env` |
| M3 | 판다 2대 이상도 1대를 열도록 복귀 | ✅ | `test_two_pandas_block_usb` |
| M4 | 반환 시 정지 실패를 조용히 삼킴 | ✅ | `test_stop_failure_on_release_is_logged` |
| **M5** | **`poll_died` 방출 제거(핸들러는 그대로)** | **첫 검사 ❌ → 보강 후 ✅** | `test_poll_thread_death_actually_emits_the_signal` |
| L1 | `_can_lock` 을 재진입 불가 Lock 으로 | ✅ | `test_can_lock_is_reentrant` |
| L2 | `_sdo_write` 판다 미연결 가드 제거 | ✅ | `test_sdo_write_guards_missing_panda` |
| L3 | 로그 위젯 쓰기 경로 이중화 | ✅ | `test_log_widget_has_single_writer` |

**M5 가 왜 뚫렸나** — 기존 회귀 `test_poll_death_drops_the_control_toggle` 은 핸들러
`_on_poll_died()` 를 **직접 부른다.** `_loop` 안의 `self.poll_died.emit()` 은 지나가지 않으므로,
그 한 줄을 지워도 111개가 전부 통과했다. 핸들러는 검증됐고 **배선은 검증되지 않은** 상태였다.
→ `test_poll_thread_death_actually_emits_the_signal` 신설: 폴링을 실제로 죽여 신호가 나오는지 본다.
(기록: `docs/claude-mistake/2026-08-04-001_regression-without-detection-power.md`)

**새 시험을 쓰면서 잡은 두 함정** — 둘 다 「통과했다」로 넘어갔으면 못 봤다.

1. PyQt5 는 평범한 콜러블을 **송신자 스레드로 큐잉**한다. 기본 연결로는 `th.join()` 이 끝날
   때까지 호출되지 않아 방출을 관측할 수 없다 → `Qt.DirectConnection` 으로 연결한다.
2. 첫 판정 근거였던 `lab_status` 문구는 **Seer 폴링 스레드도 쓰는 공유 위젯**이라,
   단독 실행은 통과하고 전체 실행에서만 깨졌다
   (실측: `'Seer 192.168.44.82 · 폴링 실패 (RobokitError)'` 가 덮어씀).
   판정을 `btn_take` 토글로 바꿔 안정화 → 전체 3회 반복 `112 passed`.

```
$ python3 Tools/amr_test_gui/mutation_check.py
✅ 12개 항목 전부 검출 — 각 수정이 실제로 회귀로 고정돼 있다.
exit=0

$ python3 -m pytest test -q      # 3회 반복
112 passed in 17.65s / 17.70s / 17.68s
```


---

## 2026-08-08 delta — 호밍 후 조향 0° 복귀 (§6 후속갱신 이행)

ADR `docs/adr/2026-08-08-steer-zero-return-after-homing.md`.

**고친 사실**: 이 GUI 의 호밍 경로는 **0° 복귀 지령을 한 번도 내지 않았다.**
`_homing_run` 은 `0x6040=0x86` → `0x6099` → `0x60FB:04=1` 을 보내고 `_wait_homed()` 로
`0x6041` bit15 만 확인한 뒤, 성공 메시지에 **"…초 소요. 조향 0° 복귀까지 확인하세요."** 라고 적었다 —
지령이 아니라 **육안 확인 요청**이었다. 이 경로에는 펌웨어 시퀀서의 `GOZERO` 에 해당하는 단계도 없다.

(참고 — 펌웨어 시퀀서 경로에도 같은 문제가 있다. 그쪽 `GOZERO` 목표
`SEER_HOME_ZERO_N3/N4`(`safety_seer_gate.h:212-213`)는 **호밍 후 정착값**이며 실측 0°에서
**+0.178° / +0.331°** 떨어져 있다. 그래서 `can_relay` 에도 같은 0° 복귀를 넣었다.)

### 함수 delta

| # | 함수 | 입력 | 출력 | 기능 | 위치 |
| --- | --- | --- | --- | --- | --- |
| 58 | `MainWindow._steer_zero_return` (신규) | — | `(bool,str)` | 호밍 뒤 **조향 0° 복귀** — `_steer_to(0.0)` 로 지령하고 `_wait_settle(0.0, STEER_ZERO_TOL_DEG, STEER_ZERO_TIMEOUT_S)` 로 도달 확인. 미도달이면 축별 실측을 사유에 적는다 | gui.py:`def _steer_zero_return` |
| 48 | `MainWindow._homing_run` (계약 변경) | — | None | 원점 신호 확인 후 **#58 을 이어서 호출**한다. 0° 미도달이면 로그가 「호밍 완료」가 아니라 「호밍 미확인」이다. ⚠ **2026-08-14 정정** — #58 **직전에** `_homing` 을 내린다. 종전에는 `finally` 에서만 내려 판정 구간이 플래그 안에 있었고, `_on_motor_data` 가 호밍 중 0x6064 를 각도로 반영하지 않으므로 **실측이 영원히 없어 항상 미확인**이었다(지령은 나갔다). 근거·실측: ADR 2026-08-08 §2026-08-14 부기 | gui.py:`def _homing_run` |
| 49 | `MainWindow._wait_homed` (문구 변경) | — | `(bool,str)` | 성공 사유가 `"…초 소요. 조향 0° 복귀까지 확인하세요."` → `"원점 신호 확인(N초 소요)."` — **육안 위임 문구 제거**. 판정 로직은 불변(bit15 0→1 2상) | gui.py:`def _wait_homed` |
| 58a | `MainWindow._check_seer_agreement` (계약 변경) | `node,deg` | None | CAN 실측 ↔ Seer 판독 대조. ⚠ **2026-08-14 정정** — 대조 상대를 **잡기 직전 Seer 각도**(`_seer_at_take`)로 바꾸고 **축별 세션당 1회**만 본다. 종전에는 살아 있는 `_seer_deg` 와 매 표본 대조했는데, **제어권을 쥐면 Seer 는 모터를 못 읽어**(intercept) 굳은 값과 비교하는 것이라 거짓 경보만 났다(실기 11:16:03). 폴링이 제어권 보유 중에만 도니 그 대조에는 유효한 창이 없었다 | gui.py:1032 |
| 58b | `MainWindow._jog_seer` · `_seer_velocity` · `_run_seer_ctrl` · `_jog_seer_run` · `_seer_ctrl_client` (신규) | — | — | **2026-08-14 신설** — 제어권이 없을 때 조그를 **Seer 개루프 운동(2010)** 으로 보낸다. 제어권(4005) 획득 → 재송신 → 정지(2000) → 반납(4006). 상세는 함수표 #45a~#45e | gui.py:1248-1355 |

### 전역 delta

| # | 사용처(함수) | 기능 | 위치 |
| --- | --- | --- | --- |
| G19 | #58 | `MainWindow.STEER_ZERO_TOL_DEG = 0.1` (클래스 상수) — 0° 도달 판정 폭. ⚠ **선택값이며 실측에서 나온 수가 아니다.** 사용자 정착 허용치 슬라이더(0.5~10°)를 쓰지 않는 이유는 그 폭이 바로잡으려는 편차(+0.178°/+0.331°)보다 커서 **판정이 무의미해지기** 때문이다 | gui.py `MainWindow` §호밍 |
| G20 | #58 | `MainWindow.STEER_ZERO_TIMEOUT_S = 10.0` (클래스 상수) — 0° 정착 대기 상한 | gui.py `MainWindow` §호밍 |

0° 값 자체는 **새로 만들지 않았다** — `_steer_to(0.0)` → `steer_counts()` → `STEER_HOME`(G4,
정본 YAML 런타임 로드)이 그대로 쓰인다.

### 시험 delta

| 파일 | 건수 | 고정하는 것 |
| --- | --- | --- |
| `test/test_steer_zero_return.py` (신규) | 10 | 0° 지령 송신(`0x607A = STEER_HOME`)·`0x6040=0x3F` 동반·**트리거보다 뒤에 나감**·**GOZERO 정착값을 도달로 인정하지 않음**·실측 부재 시 미완료·호밍 실패 시 0° 지령 없음·0° 출처가 `STEER_HOME` |
| `test/test_homing.py` | — | `win` 픽스처에 `STEER_ZERO_TIMEOUT_S = 0.5` (호밍 성공 경로가 0° 복귀까지 가므로) |

⚠ 새 시험을 쓰며 잡은 함정: 대본의 **`bit15=0` 국면이 `_wait_homed` 폴 주기(0.1 s)보다 짧으면**
개시를 관측하기 전에 완료로 덮여 「개시 신호를 못 봤습니다」로 떨어진다. 0.4 s 로 늘려 고정했다.

### 검출력 검사

`mutation_check.py` 에 **Z1~Z4** 추가. **「회귀로 고정했다」는 이 출력으로 말한다**
(근거: `docs/claude-mistake/2026-08-04-001`).

```
$ python3 Tools/amr_test_gui/mutation_check.py Z1 Z2 Z3 Z4
✅ 검출  Z1   호밍 뒤 0° 복귀 호출 제거 — 종전처럼 육안에 위임
✅ 검출  Z2   0° 지령을 빼고 대기만 함 — 「기다리면 알아서 온다」 복귀
✅ 검출  Z3   판정 허용오차를 사용자 정착 허용치로 되돌림 — GOZERO 정착값이 0° 로 통과
✅ 검출  Z4   0° 미도달을 완료로 적음 — 실패를 성공으로 보고
✅ 4개 항목 전부 검출 — 각 수정이 실제로 회귀로 고정돼 있다.
```

### 남는 것

- **실기 검증 0** — GUI 기동은 오프스크린 시험뿐이고 장치 접속·실모터 구동 0.
  이 경로는 `GOZERO` 단계가 없어 **호밍 직후 축이 어디 서는지 본 세션이 실측하지 않았다.**
  이동량이 예상(≤0.34°)보다 클 수 있으므로 **첫 실기는 이동구역 확보 또는 잭업 상태**에서 할 것.
  `STEER_ZERO_TOL_DEG` 0.1° 와 `STEER_ZERO_TIMEOUT_S` 10초의 적정성도 실기에서 정한다.
- **범위 밖 — 리팩터 진행분의 라벨이 여전히 틀리다**: `Tools/amr_test_gui/amr_test_gui/panda_can_bus.py:135-139`
  는 펌웨어 GOZERO 상태를 「0° 지령」·「0° 복귀 중」·「완료 (조향 0°)」로 적고,
  `ui_main.py:303·314-318·532-575` 도 같은 서술이다. 그 경로에는 0° 복귀가 **아직 없다** —
  본 delta 는 `gui.py` 와 `can_relay` 두 곳만 고쳤다.
