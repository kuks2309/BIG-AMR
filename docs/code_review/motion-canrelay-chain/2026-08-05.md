...
| `wheel_radius_m` | double | **기본 없음(필수)** | translator:27 | #7·#8 |
| `gear_walk` | double | **기본 없음(필수)** | translator:28 | #7·#8 |
| `gear_steer` | double | **기본 없음(필수)** | translator:29 | #7·#8 |
| `pulses_per_rev` | double | 65536.0 | translator:30 | #7·#8 |

> 기하 3종은 **기본값을 두지 않는다.** 종전 기본값(0.08 / 20.0 / 265.5)은 이식 원본
> Carrier AGV(Automated Guided Vehicle) 것이라, params 파일을 안 먹인 실행이 **조용히 다른
> 기체 기하로 돌았다**(조향 판독 1.19배·지령 0.84배). 미지정 시 `RCLCPP_FATAL` 후 예외로
> 기동을 중단한다 — 값이 틀린 채 도는 것보다 죽는 편이 안전하다.
> 재발 경위는 `docs/issues_and_fixes/issues_and_fixes.md` 2026-08-16 entry.
...
**이 한 번의 대조가 두 가지를 동시에 확인한다** — 원점(137° → ≈0) ·
조향 부호(`direction_steer = −1`). Low 항목이던 「부호 미검증」이 이로써 근거를 얻었다.
스케일(57,344) 판독은 이 대조가 확인하지 못한 **열린 항목**이다 — 2026-08-16 실기에서
−90° 자세를 Seer 는 −90.0°, `/wheel_motor_state` 는 −106.78° 로 읽어 약 1.19배 계수차가
미확정으로 남았다(`code_updates/2026-08-16-line-follow-first-real-run.md` §10). 작동값
57,344 자체(홈↔90° 스윕 실측 5,160,960c/90°)는 유효하나, 이 정지 대조가 그것을
확인했다는 주장만 철회한다.
...
### ⑦ SIL 검증 (2026-08-05) — 크랩 yaw 보정 권한은 직진 대비 1/13

사용자 지시 「실기 안 해도 됨 … SIL 이면 충분함」에 따라 SIL 로 검증했다.

**먼저 드러난 것: 체인이 첫 홉에서 끊겨 있었다.**
`origin/main` 의 2WS 는 `trnav_2ws_msgs` 를, mux 는 `trnav_msgs` 를 써서 같은 토픽에
두 타입이 등록돼 전달이 0 이었다. 첫 크랩 골이 Phase 0 조향정렬 5 초 타임아웃으로 중단됐다.
두 메시지 패키지가 완전 동일함을 확인하고 통합했다(`a100676`) — 그 뒤 체인이 끝까지 흐른다.

**두 번째: 플랜트가 회전을 모델링하지 못했다.**
`translate_sim_odom` 의 역산이 `ω = (vx₁−vx₂)/(y₂−y₁)` 로 대각 배치 전제라, inline(2WS)은
분모가 0 이라 `ω=0` 으로 강제됐다. QD 에서는 문제가 없는 코드다. 분모가 큰 쪽을 고르도록
고쳐(`884579f`) spin SIL 이 처음으로 성립했다 — 90° 지령에 실측 89.765°.

**측정 결과 (크랩 2 m, 초기 yaw 외란 +10°, target_yaw 0°)**

| 항목 | 값 |
| --- | --- |
| 전/후 조향 차동 (PD 출력) | 평균 **−4.705°** — 보정은 정상 작동 |
| 그 차동의 이론 회전 | `ω = v(sinθf−sinθr)/L` = **−0.0445 °/s** |
| 실측 yaw 변화 | **−0.045 °/s** (13.8 s 에 −0.620°) — 이론과 일치 |
| 같은 차동이 **직진 자세**였다면 | **−0.561 °/s** |

⇒ **측면 크랩의 yaw 보정 권한은 직진 대비 약 1/13 이다.** 결함이 아니라 기구학이다 —
`sin θ` 가 90° 근처에서 평평해 같은 조향 차동이 만드는 회전이 급감한다. 그 결과 10° 외란은
2 m 주행으로 되돌아오지 않는다(13.8 s 에 0.62° 회복).

⚠ 종전 보고에서 「최종 heading error 0.0」을 yaw 유지 성공의 근거로 읽었던 것은 **오독**이다 —
그때 플랜트가 회전을 못 했으므로 yaw 가 변할 수 없었다. 재현: `Tools/motion_chain_check/
measure_crab_yaw_authority.py`.

### ⑧ SIL 확장 — spin·turn 성립 (2026-08-05)

**spin** — 플랜트 역산 정정(`884579f`) 전에는 성립하지 않던 기동이다. 돌연변이로 확인했다:

| 조건 | 결과 |
| --- | --- |
| inline 기하 + 수정본 | 90° 지령 → **89.741°**, 8.36 s, SUCCEEDED |
| inline 기하 + 종전 코드(y성분 쌍 차단) | status −3, **actual_angle 0.0°**, 60 s 타임아웃 |

⚠ 종전 보고(`884579f`)의 spin 성공은 **증명이 아니었다** — 그 실험은 `sil_spin` 이 쓰는
`translate_sim_odom/config/sim_params.yaml` 의 **Carrier AGV 대각값**으로 돌았고(기동 로그
`w1=(0.330,0.135)`), 대각에서는 종전 코드도 정상 동작한다. 플랜트 기본 설정까지 Foil_A082 로
맞춘 뒤(`a64b819`) 재실험해 위 표를 얻었다.

**turn** — SIL 런치가 저장소에도 상류(`kuks2309/TR_Nav_ros2_ws`)에도 **없어서 신설**했다
(`sil_turn.launch.py`, source id 5). turn 은 `v = ω·R` 로 **병진과 회전이 함께** 나가므로
플랜트가 둘 다 되돌려야 성립한다 — spin 이 회전만 보는 것과 다르다.

R=1.0 m 원호로 90° 선회 (시작 pose `(4.952, −2.327, 0°)`):

| | x [m] | y [m] | yaw [°] |
| --- | --- | --- | --- |
| 실측 종료 | 5.9534 | −1.3252 | +90.181 |
| 기하 예측 | 5.9520 | −1.3270 | +90.000 |
| 오차 | **+1.4 mm** | **+1.8 mm** | **+0.181** |

ICR 까지 실측 거리 **1.0014 m**(목표 1.000). 각도뿐 아니라 **반경까지** 맞았다 —
turn 원호 기구학·플랜트 병진회전 동시 역산·Foil_A082 기하(휠베이스 1.2 m)가 함께 검증된다.

### ⑨ 전 체인 SIL — translator·can_relay 를 처음으로 체인에 넣었다 (2026-08-06)

종전 `sil_*.launch.py` 는 플랜트가 `/motor/wheel_cmd` 를 **직접** 받아 translator·can_relay 를
건너뛰었다 — SI→raw 환산과 조향 원점·클램프가 체인 안에서 한 번도 함께 돌아본 적이 없다.
`motion_chain_sil.launch.py` 로 그 구간을 넣고 turn R=1.0 m·45° 를 돌렸다(성공, 45.17°).

**`/motor/low_cmd` 실측 149 메시지 — 예측과 1 count 이내로 일치**

| 축 | 예측 | 관측 | 차 |
| --- | --- | --- | --- |
| 조향(node3) | `(89.87° − (−1.676°)) × 57,344 × (−1)` = **−5,249,452** | −5,249,451 | **1 count** (0.000017°) |
| 구동(node1) | `175.4 mm/s × 24.4462 × (−1)` = **−4,288** | −4,288 | **0** |

구동이 차체중심 150 mm/s 가 아니라 **175.4 mm/s** 인 것도 맞다 — 휠 궤도반경이
`hypot(R−y, x) = 1.1694 m` 로 차체중심(1.0 m)보다 크다. 기하가 체인 끝까지 반영된다.

**새 사실 — 클램프 ±115° 는 크랩 전용이 아니다.** 위 조향 지령은 counts 상 **−91.54°** 다.
IK 각 자체는 89.87°(±90° 이내)인데, translator 가 영점 오프셋 `steer_offset = −1.676°` 를
빼면서 **raw 는 90° 를 넘는다.** can_relay 의 클램프는 counts 로 판정하므로,
종전 ±90° 였다면 **spin·turn 의 미세보정 자세도 1.54° 잘렸다.**
→ 시뮬레이션 ⑦(「90° 를 넘겨 요구하는 것은 crab 뿐」)은 **IK 출력 각 기준**의 결론이었고,
  **영점 오프셋을 거친 raw 기준으로는 spin·turn 도 넘는다.**

⚠ 실행 시 반드시 도메인을 격리한다(`ROS_DOMAIN_ID`). 같은 도메인에 다른 can_relay 인스턴스가
있으면 `/motor/low_cmd` 를 그쪽도 구독한다 — 그 인스턴스가 실 하드웨어에 제어권을 쥐고 있으면
**실차가 움직인다.** 2026-08-05 에 타 세션 종료 후에도 `can_relay_node` 가 14시간 살아남아
판다를 점유한 사례가 있다. 절차: `Tools/motion_chain_check/RUNBOOK-chain-sil.md`
