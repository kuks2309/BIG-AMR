...
severity 분포: Critical 1 / High 5 / Medium 5 / Low 2 / Info 3
...
**함수 #33·G2·#48 — [논리] 크랩 wrap 임계(±115°)가 can_relay 클램프(±90°)를 넘는다 (High)**
   재현: `WRAP_MARGIN = 25°`(qd_crab_inverse_kinematics.cpp:35) 라 `wrapSteer` 는 **±115°
   까지 wrap 하지 않는다** — 즉 90°~115° 조향각이 그대로 하류로 나갈 수 있다. can_relay 는
   `steer_limit_deg: 90.0` 으로 클램프하므로(can_relay.yaml:18, #48) 그 구간은 **90° 로 잘린
   채 지령**된다. 크랩 주행 중 의도와 다른 자세가 된다.
   권고: 두 한계를 한 값으로 묶거나(±90°), can_relay 가 초과분을 **거부**로 처리(클램프 금지)
   할지 결정. 어느 쪽이든 ADR 필요.
...
| High 액션 서버 기하 | **[잔존]** | 2WS `<action>_params.yaml` 소관 — 별건 |
| High 크랩 ±115° vs ±90° | **[잔존]** | C4 가 WARN 으로 상시 노출. 결정 필요(ADR) |
| High 통합 런치 부재 | **[잔존]** | — |
| Critical 원점 정규화 | **[해결]** | §조치 ⑤ — 지령·피드백 양방향, 회귀 19건 + 돌연변이 확인 |

⚠ **연결이 안전해졌다는 뜻은 아니다.** Critical 은 §조치 ⑤ 에서 닫혔으나 **실기 검증이 0건**
이다. 남은 High 3건(액션 서버 기하 · 크랩 ±115° · 통합 런치)도 그대로다.
...
4. **통합 런치 + 브링업 순서** 신설.
5. **크랩 ±115° vs can_relay ±90°** 한계 통일 결정.
