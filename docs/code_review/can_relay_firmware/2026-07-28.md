...
| 12 | `seer_home_zero_target` | `uint8_t node` | `int32_t` | node==3 → N3(7882020), **그 외 전부** N4(7859062) | `board/safety/safety_seer_gate.h:452` |
...
| 221·222 | 함수 #12 | **(상수)** `SEER_HOME_ZERO_N3 7882020` / `_N4 7859062` — **차량별 실측 상수** | `board/safety/safety_seer_gate.h:419-420` |
...
**[H1] 함수 #23 `seer_homing_tick` — [논리][safety][신규] 스테일 캐시로 호밍/도달을 허위 완료 판정 (High — `[존재]` 확정 / `[동작]` 조건부)**
   ...
   재현 B(허위 0° 도달): `SEER_HOME_GOZERO_W` 도 동일 패턴 — tick 1 이 읽기 미발행(`:668-675`)한 채 `:676-688` 에서 캐시된 `0x6064` 를 평가한다. WAIT 상태는 `0x6064` 를 폴하지 않으므로(`:611-612` 는 0x6041·0x6000 만) 캐시가 호밍 전 값이면 **축이 전혀 움직이지 않았는데 `SEER_HOME_DONE`**.
