...
> ⚠ **앵커 안내(후행 추가)** — 본 문서의 `파일:줄` 앵커는 **이 리뷰 시점 기준**이다
> (당시 `backend.py` 844줄, 현행 1,067줄). 현행 앵커의 권위는
> `src/Comm/CAN/can_relay/docs/function_table.md` 다. 본문 판정·근거는 그대로 유효하며
> 날짜=버전 스냅샷이므로 소급 수정하지 않는다.
...
**함수 #106 — [테스트][신규] `~/stop` 의 실제 경로인 `stop_all` 에 회귀가 없다 (Medium)**
   ...
   구성요소인 `stop`·`halt_steer` 에는 시험이 있으나 **둘의 결합(반환값이 조향 정지 성패를 반영하는지)** 은 미고정이다.
...
| 115 | `RelayBackend.halt_note` | — | `str` | 직전 `halt_steer` 가 못 잡은 사유(「실측 미확보」/「이동 중 보류」 구분) | backend.py:744-747 |
...
`link.homing_cancel`, method 35 경로는 `stop`+`halt_steer`) · `backend.py:615-644`.
...
| # | 사용처(함수) | 기능 | 위치 |
| --- | --- | --- | --- |
| G23 | #105·#113 | `RelayConfig.stationary_tol_counts = 200` (상수) — `halt_steer` 가 「정지」로 인정하는 폭. 근거는 마스터 캡처 12,396건의 \|Δ\| ≤ 200 c 대역 | backend.py:101-107 |
...
⚠ **새로 생긴 제약(2026-08-04 사용자 결정 「현행 유지」)**: `halt_steer` 는 **정지 확인된 축에만**
...
ADR `docs/adr/2026-08-08-steer-zero-return-after-homing.md`. 호밍만으로는 축이 조향 0° 에
서지 않는다 — 펌웨어 `GOZERO` 목표(`safety_seer_gate.h:212-213`)는 **호밍 후 정착값**이고
실측 0°(`[7871815, 7840086]`)에서 **+0.178° / +0.331°** 떨어져 있다. 펌웨어 상수를 바꾸지 않고
(재플래시 회피) **호스트가 0° 복귀 지령을 이어서 낸다.**

### 함수 delta

| # | 함수 | 입력 | 출력 | 기능 | 위치 |
| --- | --- | --- | --- | --- | --- |
| 117 | `RelayBackend.home` (계약 변경) | `speed,poll_s,timeout_s` | `(bool,str)` | **래퍼가 됐다** — `_home_sequence` 성공 뒤 `steer_to_zero()` 를 이어 붙인다. **0° 미도달은 전체 실패.** method 35 경로·`steer_zero_after_home=False` 에는 붙이지 않는다 | backend.py:`def home` |
| 118 | `RelayBackend.steer_to_zero` (신규) | `timeout_s` | `(bool,str)` | 0° 목표를 세우고(`set_steer_deg(0.0)`) 두 축이 `steer_zero_tol_deg` 안에 들어올 때까지 대기. **기존 안전 게이트를 그대로 지난다**(E-stop·호밍 중·`homed_effective`·±limit) — 우회로 없음 | backend.py:`def steer_to_zero` |
| 119 | `RelayBackend._home_sequence` (개명) | `speed,poll_s,timeout_s` | `(bool,str)` | 종전 `home()` 본체(펌웨어 시퀀서 폴링 / method 35 분기). 0° 복귀는 포함하지 않는다 | backend.py:`def _home_sequence` |

⚠ **#55 `RelayBackend.home` 을 「호밍 시퀀스만 수행」으로 인용하지 말 것** — 그 본체는 이제 #119 다.

### 전역 delta

| # | 사용처(함수) | 기능 | 위치 |
| --- | --- | --- | --- |
| G25 | #117·#118 | `RelayConfig.steer_zero_after_home = True` (상수) — 호밍 후 0° 복귀 수행 여부. ROS 파라미터 동명 | backend.py `RelayConfig` |
| G26 | #118 | `RelayConfig.steer_zero_tol_deg = 0.1` (상수) — 0° 도달 판정 폭. ⚠ **선택값이며 실측에서 나온 수가 아니다.** `settle_tol_deg`(3.0°)를 쓰지 않는 이유는 그 폭이 바로잡으려는 편차(0.178°/0.331°)보다 커서 **지령 전에도 「도달」로 읽히기** 때문이다 | backend.py `RelayConfig` |
| G27 | #118 | `RelayConfig.steer_zero_timeout_s = 10.0` (상수) — 0° 정착 대기 상한 | backend.py `RelayConfig` |

ROS 파라미터 3종 신설: `steer_zero_after_home`(true) · `steer_zero_tol_deg`(0.1) ·
`steer_zero_timeout_s`(10.0) — `driver_node.py` 선언부·`RelayConfig` 전달부.

### 시험 delta

| 파일 | 건수 | 고정하는 것 |
| --- | --- | --- |
| `test/test_steer_zero_return.py` (신규) | 12 | 0° 지령 송신·`0x6040=0x3F` 동반·**GOZERO 정착값을 도달로 인정하지 않음**·미도달 시 전체 실패·피드백 부재 시 실패·끄기 동작·method 35 미적용·E-stop 거부·0° 출처가 `steer_home` |
| `test/conftest.py` | — | `FeedingLink`·`sdo_response` 추가 — `MockLink.inbox` 는 1회 소비라 **여러 주기에 걸친 정착 판정**을 시험할 수 없다 |
| `test/test_backend.py` | — | `make_homed()` 추가. `test_home_uses_firmware_sequencer_not_direct_sdo`·`test_only_done_is_success` 가 이를 쓴다(호밍 성공 경로가 0° 복귀까지 가므로) |
| `test/test_port_equivalence.py` | — | `test_homing_path_differs_by_design` 은 `steer_zero_after_home=False` — 대상이 **호밍 경로의 바이트**뿐이다 |

### 검출력 검사 (신규 도구)

`src/Comm/CAN/can_relay/mutation_check.py` — `Tools/amr_test_gui/mutation_check.py` 와 같은 원리.
**「회귀로 고정했다」는 이 출력으로 말한다 — 통과 숫자로 말하지 않는다**
(근거: `docs/claude-mistake/2026-08-04-001`).

```
$ python3 src/Comm/CAN/can_relay/mutation_check.py
✅ 검출  Z1   호밍 뒤 0° 복귀 호출 제거 — 종전 동작(정착값에 정지)으로 복귀
✅ 검출  Z2   0° 지령을 빼고 대기만 함 — 「기다리면 알아서 온다」
✅ 검출  Z3   판정 허용오차를 settle_tol_deg 로 되돌림 — GOZERO 정착값이 0° 로 통과
✅ 검출  Z4   0° 미도달을 성공으로 보고 — 호밍 성공만으로 완료 처리
✅ 검출  Z5   피드백 없어도 도달로 인정 — 「모르면 됐다고 친다」
✅ 5개 항목 전부 검출 — 각 수정이 실제로 회귀로 고정돼 있다.
```

### 남는 것

- **실기 검증 0** — 장치 접속 0 · 판다 플래시 0 · 실모터 구동 0. 본 delta 의 어떤 문장도
  실차 거동을 확정하지 않는다. 특히 **축이 0° 지령에 얼마 만에 정착하는지**(→ `steer_zero_timeout_s`
  10초가 충분한지)와 `steer_zero_tol_deg` 0.1° 의 적정성은 실기에서 정해야 한다.
- **debt-034 · debt-016 은 닫히지 않는다.** 상수명이 사실과 어긋나는 문제(`SEER_HOME_ZERO_*`)와
  펌웨어 도달 허용오차 1.0° 가 편차를 검출하지 못하는 문제는 그대로다 — 본 변경은
  「0° 로 **보내는** 것」만 해결한다.
- ⚠ **본 delta 작성 시점의 워킹트리 선존재 결함**(이 변경과 무관): `RelayConfig` 에서
  `steer_limit_bench_deg`(벤치/체인 이중 한계)가 사라져 `test/test_steer_origin.py` **7건이 실패**한다.
  HEAD 판 `backend.py` 에는 존재한다(`git show HEAD:… | grep -c` → 5). 다른 세션의 미커밋 변경분으로
  보이며 **본 세션은 손대지 않았다.**

### ❌ 2026-08-08 16:20 정정 — 위 delta 의 「남는 것」 3·4번째 항목

**(가) 「워킹트리 선존재 결함(`steer_limit_bench_deg` 소실)」 — 해결됐다.**
원인은 「다른 세션의 미커밋 변경분」이 아니라 **공유 워킹트리 사본이 HEAD 보다 옛 판**이었던 것이다.
빠진 것은 둘 — ① 체인/벤치 이중 조향 한계(2026-08-05) ② `release_steer_target` 의 2026-08-05 판.
사용자 결정(「내 트리를 먼저 동기화」)에 따라 `git checkout HEAD -- backend.py driver_node.py` 로
되돌린 뒤 0° 복귀를 **재적용**했다. `test_steer_origin.py` 7건 해소(377 → 384 passed).
⚠ 동기화 없이 커밋했다면 c9ea2414 세션의 이중 한계 기능을 **되돌릴 뻔했다.**

**(나) 「실기 미검증」 — 실기 기록을 확인했고, 결론은 「미검증」보다 강하다.**
**본 변경은 실기에서 한 번도 실행되지 않았다.** 오늘 실기 호밍 3회(14:35 · 15:31~15:33 · 15:42)는
전부 수정 전 동작이다. 실기 노드가 **다른 워크트리**에서 돌기 때문이다 —
`ps` → `Big-AMR-ses-c9ea2414/install/can_relay/lib/can_relay/can_relay_node`.
그 트리에 `steer_to_zero`·`steer_zero_after_home` **0건**. 15:42 호밍은 `DONE` 뒤 종료 로그가
`조향 목표 해제 — (걸어 둔 목표 없음)` 이라 조향 목표가 걸린 적이 없음이 확인된다.

### 2026-08-08 16:20 추가 반영

| # | 함수/전역 | 내용 |
| --- | --- | --- |
| 120 | `CanRelayNode._srv_home` (계약 변경) | **호밍 결과를 노드 로그에 남긴다**(`호밍 결과 — {why}`, 실패는 `error`). 종전에는 `res.message` 에만 담겨서, 0° 복귀가 게이트에 거부돼도 **로그가 조용**했다 — 2026-08-08 실기 조사에서 사유를 찾지 못한 직접 원인이다 |
| 121 | `CanRelayNode.__init__` | `steer_zero_after_home=false` 면 **기동 시 경고**. 사용자 요구(「호밍후에 반드시 0도 조향을 줘야 함」) 이탈을 드러낸다 |

회귀 `test_gui_node.py::test_home_result_is_logged_not_only_returned` 신설, 돌연변이 **Z6** 추가.

**검사기 자체의 거짓 통과를 잡았다** — `mutation_check.py` 가 rclpy 미소싱으로 시험이
`1 skipped` 된 것을 **「검출」로 집계**했다(Z6 첫 실행). 부모 환경을 물려주고 **실패 1건 이상일
때만** 검출로 인정하도록 고쳤다 — 0건 실행·전량 skip 은 `검사 불가`(실패로 집계).
「시험을 추가한 것 ≠ 시험이 검출하는 것」(`docs/claude-mistake/2026-08-04-001`)이 검사기 자신에게도
적용된 사례다.
