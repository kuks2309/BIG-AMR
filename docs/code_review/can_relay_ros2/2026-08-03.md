...
| 105 | `RelayBackend.halt_steer` | `reason` | `bool` | 현재 실측 위치를 0x607A 목표로 덮어써 붙들려 시도(⚠ 2026-08-05 반증·제거 — commit b0af895·debt-040·mistake 2026-08-05-001: 실제로는 축을 세우지 못하고 bit15=0 구간엔 +69.3° 지령이 될 수 있었다. 벤더 PP 정지 명령 0x6040=0x03/0x05 가 정본. 현행은 release_steer_target 의 재송신 중단뿐) | backend.py:315-349 |
| 106 | `RelayBackend.stop_all` | `reason` | `bool` | 구동 + 조향 동시 정지(`~/stop` 의 실제 경로) — 단 조향 부분은 #105 `halt_steer` 와 동일 반증(⚠ 2026-08-05 반증·제거 — commit b0af895·debt-040·mistake 2026-08-05-001: 조향축을 실제로 세우지 못했고 벤더 PP 정지 명령 0x6040=0x03/0x05 가 정본. 현행은 release_steer_target 재송신 중단뿐) | backend.py:375-383 |
...
⚠ **2026-08-05 commit b0af895 로 이 「현행 유지」(2026-08-04 사용자 결정) 결정은 뒤집혔다**
(debt-040·mistake 2026-08-05-001; #105·#106 반증과 동일) — `halt_steer` 는 제거됐고
`stop_all`·`estop`·`shutdown` 은 조향축에 프레임을 보내지 않고 `release_steer_target`(우리 목표
재송신 중단)만 한다. 「축이 직전 PP 목표까지 계속 회전한다」는 결과는 알려진 제약으로 그대로 유효하다.
