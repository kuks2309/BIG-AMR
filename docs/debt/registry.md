...
| debt-017 | 이해 | Tools/amr_test_gui/tongyi_can.py `decode_frames` | **호밍 중 `0x6064` 에 섞이는 정확한 0 의 발생원이 미확정.** 2026-07-29 실기(우리 GUI 가 intercept 를 쥐고 직접 폴링)에서 탐색 중에도 `0x6064` 는 실제 엔코더 값을 계속 보고했고(node3 7,871,817 / node4 7,840,084 대다수) 정확한 0 이 간헐적으로 섞였다. 드라이브의 재기준 순간인지, 두 마스터(Seer·PC) 폴링 경합인지 가려지지 않았다. 또한 이번 run 은 조향이 이미 홈 근처였어서 **대각도 스윙 중 매끄러운 추종은 미확증**이다 — '지속 0 아님'만 확증됐다. ⚠ 2026-07-27 캡처에서는 같은 구간 3,105/3,105 표본이 전부 0 이었으나 그것은 **Seer 폴링에 대한 응답을 수동청취**한 것이라 조건이 다르다 | 2026-07-29 | 미해결(증상 무해화됨) | 표시·정착 판정은 **정확한 0 만 버리는 필터**로 해결했다(어느 발생원이든 무해). 발생원 규명은 별건 — 호밍 중 0x6064 를 고빈도로 덤프해 0 의 시간 분포를 보고, Seer 폴링을 끊은 상태(단독 마스터)와 대조하면 경합 여부가 갈린다. 대각도 스윙 추종은 홈에서 ±90° 호밍을 걸어 시계열로 확인 |
