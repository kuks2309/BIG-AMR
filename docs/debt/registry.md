...
## [2026-08-08 조치] 호밍 후 조향 0° 복귀 — 호스트 지령으로 추가 (debt-034 · debt-016 은 **유지**)

ADR `docs/adr/2026-08-08-steer-zero-return-after-homing.md`.

**무엇을 고쳤나** — 호밍 완료 후 **0° 복귀 지령**(`0x607A = steer_home_counts` → `0x6040=0x3F`)을
호스트가 이어서 내도록 두 경로에 넣었다. 펌웨어 상수·설정 상수는 **하나도 바꾸지 않았다**(재플래시 없음).

- `src/Comm/CAN/can_relay` — `RelayBackend.home()` 이 래퍼가 되어 `steer_to_zero()` 를 이어 붙인다.
  0° 미도달은 **전체 실패**로 보고한다. 파라미터 `steer_zero_after_home`(기본 true) 로 끌 수 있다.
  method 35 경로에는 붙이지 않는다(그쪽은 현재 위치를 홈으로 재선언하므로 0° 지령이 그 설계와 충돌).
- `Tools/amr_test_gui/gui.py` — `_homing_run` 이 원점 신호 확인 뒤 `_steer_zero_return()` 을 호출한다.
  종전에는 이 경로가 **0° 지령을 한 번도 내지 않고** "조향 0° 복귀까지 확인하세요"라고 육안에 위임했다.

**닫히지 않는 것 — 아래를 근거로 부채를 닫지 말 것**

| 부채 | 왜 유지되나 |
| --- | --- |
| **debt-034** | 등록 사유는 「`SEER_HOME_ZERO_N3/N4` 라는 **이름**이 사실과 어긋나고, 도달 허용오차 1.0° 가 그 오인을 막지 못한다」이다. 본 조치는 **0° 로 보내는 것**만 해결했고 상수명·허용오차는 그대로다. 상환계획 ①(개명)·②(0° 대조 분리)는 미이행 |
| **debt-016** | 「+0.178° / +0.331° 가 어느 게이트로도 검출되지 않는 영구 미검출 오프셋」이라는 등록이 그대로다. 본 조치는 **펌웨어가 그것을 검출하게 만들지 않았다** — 호스트가 그 뒤에 한 번 더 지령할 뿐이다. 펌웨어 단독 동작(호스트 없이 Seer 가 지휘)에서는 편차가 그대로 남는다 |

**검증 상태** — 단위·회귀만. **실기 검증 0**(장치 접속 0 · 플래시 0 · 실모터 구동 0).
검출력은 `mutation_check.py` 로 확인했다(can_relay Z1~Z5 5/5 · amr_test_gui Z1~Z4 4/4).
⚠ 미확정으로 남긴 값 2개: `steer_zero_tol_deg` **0.1°** 와 `steer_zero_timeout_s` **10초**는
**선택값이며 실측에서 나오지 않았다** — 실기에서 정해야 한다.
