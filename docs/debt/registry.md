...
| debt-126 | 이해 | `src/Comm/seer_tcp_ip/src/api.cpp` `SeerApi::setParams` (편호 4001/4002) · 로봇 Foil_A082 | **4001 이 `ret_code 0` 을 돌려주고도 값이 바뀌지 않는다 — 「수리됨」과 「반영됨」이 다르다.** 2026-08-24 실측: `NetProtocol.RobotNote`(현재 `""`, `mutable=1`, 자유 메모 칸)에 4001 로 문자열을 쓰면 응답이 `{"ret_code":0}` 인데 직후 1400 되읽기가 **옛 값 그대로**다. 제어권(4005)을 쥐고 다시 써도 같다. 같은 조회에서 4003(reload)은 **거부**된다 — `ret_code 40012`, `err_msg "dispatching... , can't execute any standalone operation"`. 그런데 로봇은 유휴다(1020 `task_status=0`·`running_status=0`, 1003 `mode=0`). **기전 미규명**: ⓐ 배차 문맥이라 설정 쓰기가 조용히 무시되는가 ⓑ `RobotNote` 만 실제로는 읽기 전용인가 ⓒ 4001 이 별도 적용 트리거를 요구하는가 — 어느 쪽인지 이 조회로는 갈리지 않는다. 반례 하나는 있다: `RobotName` 이 기본값 `AMB-01` 이 아니라 `Foil_A082` 다 — **어떤 경로로든 파라미터는 쓰인 적이 있다** | 2026-08-24 | 미해결 | ① `setParams` 는 이미 계약을 고쳤다 — **`ret_code 0` 을 반영으로 읽지 말 것**을 헤더에 명시하고, 되읽기 왕복 도구 `tools/param_probe` 를 남겼다(재실행 가능, 로봇 미동작, 허용 목록 밖 파라미터는 거부). ② 갈래를 가르려면 **다른 플러그인의 파라미터 하나**를 같은 방식으로 왕복시킨다 — ⓑ(RobotNote 한정)라면 그쪽은 반영된다. ③ 4002(디스크 저장)는 시도하지 않았다 — 원복 실패가 영구가 되므로 잭업·승인 후에만. ④ 그 전까지 **파라미터 쓰기에 의존하는 기능을 만들지 않는다** — 지금 이 저장소에 `setParams` 호출자는 없다 |
