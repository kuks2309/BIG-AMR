...
| debt-003 | 이해 | src/Actuators/motor_control/motor_control/backend.py:272-300 | freewheel servo-off(0x6040=0x05)를 1회만 assert하고 Node Guarding RTR은 계속 폴링 — 드라이브가 guarding 활동으로 서보를 조용히 재-enable/재기동하지 않는다는 가정이 코드로 확인 불가(HW/펌웨어 거동). ADR은 실측 §8 "무재초기화 재개" 인용하나 벤치 미확인 | 2026-07-26 | 미해결 | 실 Tongyi 드라이브 벤치: freewheel 유지 동안 서보가 Switch-On-Disabled 유지·노드 alive 확인. 조용히 재-enable되면 `fw_active` 중 주기적 `CW_DISABLE` 재-assert 추가 · ⚠ **2026-08-03 부분상환**: runaway 결과(진입 전 잔류속도로 조용한 재-enable 시 급발진)는 코드로 완화됨 — servo-off 직전 각 구동 노드에 `Target_velocity=0` device write(commit `8dc16a5`, 외부 리뷰 APPROVE). **본 부채 미종결** — servo-off 지속성(조용한 재-enable 여부)·free-spin 자체는 여전히 잭업 벤치 미검증(주기 재-assert 미도입) |
...
| debt-025 | 의도 | `src/Comm/CAN/can_relay/package.xml` (패키지명 `can_relay`) | **이름 충돌 — 알고 감수한 것.** 저장소에서 `can_relay` 는 지금까지 판다 펌웨어 프로젝트를 뜻해 왔다(`Tools/Can_Relay/`, `docs/can_relay/`, `docs/code_review/can_relay_firmware/`, `README.md:1` "CATL-Ford CAN-Relay"). 신설 ROS2 드라이버가 같은 이름을 쓰면 세 번째 맥락이 생긴다. 10인 감사의 리뷰어 8·변호인 D1 이 **둘 다 개명을 권고**했고, 근거는 이 저장소에서 이름 공유로 **debt id 오참조가 실제 발생한 기록**(본 registry `id 충돌 목록`)이다. 사용자가 원안(이름 유지)을 명시 선택했으므로 진행하되 위험을 등록한다 | 2026-07-29 | 미해결(의도적 수용) | 완화책 유지: `package.xml` `<description>` 첫 단락이 펌웨어와의 구분을 명시하고, 문서·주석 인용 시 **어느 `can_relay` 인지 반드시 병기**한다. 오참조가 1건이라도 실제 발생하면 개명을 재상신한다(후보: `seer_relay_bridge`). 근거 ADR `docs/adr/2026-07-29-can-relay-ros2-package.md` §Decision 2 |
| debt-026 | 이해 | `src/Comm/CAN/can_relay/config/can_relay.yaml` `steer_home_counts` · `can_relay/safety.py` `DEFAULT_STEER_HOME` | **조향 홈 상수 하드코딩 계승 (debt-007 파생).** `Log/homing_capture_220350.jsonl` 전수 디코드 결과, Seer 가 유지한 0° 목표는 `7882020`(node3)/`7859062`(node4) 이고 config 값 `7871815`/`7840086` 과 `+10,205`/`+18,976` counts(= **+0.178°**/**+0.331°**) 다르다. 두 값 모두 `homing_tol` 5°·`settle_tol` 3° 어느 게이트로도 검출되지 않는 **영구 미검출 오프셋**이다. 또한 `7871815` 계열은 캡처에서 `0x6041=0x9450`(Operation enabled 아님) 구간에만 나타나 **드라이브가 수행한 적 없는 목표**로 보인다. debt-007 상환계획 ③("실측 없이 값 변경 금지") 때문에 값을 바꾸지 않고 계승했다 | 2026-07-29 | 미해결 | 근본 해법은 하드코딩 폐기 — 호밍 완료(`0x6041` bit15 0→1) 직후 드라이브가 정착한 `0x6064` 를 **런타임에 취득**해 홈으로 삼는다. 그러면 상수 소유자가 사라지고 debt-007·debt-026 이 함께 상환된다. 그 전까지 파라미터로만 노출하고 기본값을 바꾸지 않는다 |
...
| debt-032 | 의도 | `src/MES/mini_mes/mini_mes/runtime/tasks/` (three tasks present) ↔ `ARCHITECTURE.md` §5.3 (four drawn) | **The fourth FSM — a per-robot handler — is not built, and the design compensates for its absence rather than waiting for it.** Dr. Shim's 2026-07-29 whiteboard shows four state machines. Three exist: `equipment_monitor`, `dispatcher`, `job_tracker`. The fourth would drive one robot and collect its reports, which requires addressing robots individually — and the MES → ACS interface is still undecided (JSON path files / a new API / Seer's own; `ARCHITECTURE.md` §10). `DispatcherTask` was therefore written to never ask how many robots are free: it grants **one** permit at a time and treats `BUSY` as "wait", which needs no interface the ACS may not have. That is a deliberate workaround, not the design. **Consequence if left**: the MES cannot see fleet state, so it cannot prefer a nearer robot, cannot react to one going offline mid-job (only the `t5` timeout catches that, after `job_timeout_s`), and cannot report *which* robot is carrying a job. **Also unconfirmed**: whether Dr. Shim's four names match these three plus a robot handler at all | 2026-08-03 | 미해결 | ① Settle the MES → ACS interface with Dr. Shim (open question Q3). ② Confirm the four FSM names against the whiteboard before adding the fourth — the split determines everything below it. ③ Then add `RobotHandlerTask`, one instance per robot, registered with the same `Supervisor`; the `wakes` wiring in `runtime/mes_app.py` is the only file that should need editing. ④ Until then, do **not** infer fleet state anywhere above the `AcsAdapter` interface — `BUSY` is the only capacity signal that exists |
