...
기술·이해·의도·**지능** 부채의 등록·추적. **항목은 append, 해결도 기록(덮어쓰기 금지).** 코드 마커는 여기 `id` 를 참조한다 (`# TODO(debt-001): ...`).

### 유형 — **지능(intelligence)** 부채 (2026-07-29 신설, 사용자 지시)

**간단히 풀리는 문제를 못 풀거나 크게 돌아가서 생긴 비용.** 지식 공백이 아니라 **판단 실패**다.

| | 이해(understanding) | 지능(intelligence) |
| --- | --- | --- |
| 원인 | **몰라서** | **알 수 있었는데 판단이 낮아서** |
| 상환 | 지식·문서 보강 | **"가장 먼저 볼 것" 을 자산에 고정** |

같은 사건에 둘 다 걸리면 **지능이 우선**이다 — 지식 보강으로는 닫히지 않으므로.
`claude-mistake` 와도 다르다: mistake 는 **사건 기록**, 지능 부채는 **남아 있는 비용**
(그 판단 습관이 안 고쳐져 다음에 또 든다). 보통 짝으로 남는다.

**식별 신호** — 사용자가 한 문장으로 해결한 것을 내가 여러 시도로 못 풀었다 / 점검 한 줄이면
끝날 것을 대규모 조사로 돌았다 / 이미 해결된 것을 미해결로 열어 뒀다 / 묻지 않고 혼자
어려운 길을 골랐다.

⚠ 유형 enum 의 SSOT 는 `docs/claude_guideline/debt/debt.md`(**git 미추적** — 설치된 번들)이다.
거기엔 아직 3유형만 있다. 이 확장은 여기에 먼저 적었으므로 **번들과 드리프트 상태**다.
...
| debt-022 | 기술 | Tools/amr_test_gui/tongyi_can.py `take`·`ensure_drives_enabled` | **제어권 획득 시 구동축 운전 상태를 갖추지 않았다.** Seer 에 반환했다 되찾으면 node1 이 `Switch On Disabled` 로 떨어져 있어(2026-07-29: 13:24:35 반환 → 13:33:54 재획득 시 `enabled {1: False, 2: True}`, fault 없음) 조향만 되고 구동이 취소됐다. **제어권을 잡는 쪽이 필요한 상태를 갖추는 것이 정상 책임 분담인데** `take(True)` 는 릴레이·폴링만 세웠다 | 2026-07-29 | **해결 (2026-07-29, 커밋 2a7f4f6)** | `take(True)` 가 `ensure_drives_enabled()` 로 획득 직후 운전 상태를 점검하고 필요하면 enable 한다(fault 가 있으면 자동으로 켜지 않고 `구동축 활성화` 버튼의 확인을 요구). 회귀 `test_drive_enable.py` 3건으로 고정. 실기 확인: 획득 시 n1 `0x8050`→`0x8037`, 크랩 주행 정상(사용자 확인). **이전 마스터가 왜 내려 두는지는 규명 대상이 아니다** — 인수인계 시 상태를 갖추는 것으로 충분하다 |
| debt-023 | 지능 | (판단) 구동 이상 진단 순서 | **"구동이 안 된다" 신고에 상태가 아니라 지령 경로부터 팠다.** 2026-07-29: CAN 프레임·리팩터 차분·컨트롤워드 bit4 엣지·폴링 과부하·점프 크기·프레임 유실을 차례로 조사했다 — 하드웨어 시험 6회 이상, 가설 5종 반증, 약 500회 지령. 실제 원인은 `operation enabled`(상태워드 bit2)=0 이었고 **읽기 한 줄이면 나왔다**. 조향축엔 `0x6040` 을 보내면서 구동축엔 안 보낸다는 비대칭이 코드에 그대로 있었는데 보지 않았다 | 2026-07-29 | 미해결 | **진단 순서를 고정한다** — 액추에이터가 '지령을 받는데 안 움직인다' 면 ① 상태워드(operation enabled·fault) ② 동작 모드 ③ 에러코드 를 **먼저** 읽고 그 다음 지령 경로를 본다. `drives_ready()`·`drive_faults()` 가 이미 있으니 다음엔 한 줄이다 |
| debt-024 | 지능 | (판단) 부채 프레이밍 | **해결된 문제를 '원인 미규명' 미해결 부채로 열어 뒀다.** debt-022(제어권 재획득 시 구동축이 내려가 있음)를 '누가 `0x6040` 을 내리는지 수동청취로 규명해야 한다' 며 이해 부채로 등록했다. 사용자가 한 문장으로 닫았다 — "제어권 획득시에 점검해서 enable 하면 간단히 해결되는 문제임". 잡는 쪽이 필요한 상태를 갖추면 끝나는 문제였다 | 2026-07-29 | 미해결 | **부채 등록 전 자문한다** — (1) 내 쪽에서 갖추면 끝나는가? (2) 원인 규명이 운용에 필요한가, 호기심인가? (3) 사용자에게 한 줄로 물어볼 수 있는가? 하나라도 예면 부채가 아니라 **작업**이다 |
