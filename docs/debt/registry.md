...
| debt-017 | 이해 | Tools/amr_test_gui/tongyi_can.py `_loop`·`decode_frames` | **호밍 리밋 탐색 구간에서 드라이브가 `0x6064` 에 0 만 돌려주는 이유가 미확정.** 실측(2026-07-29, 우리 GUI intercept 보유 직접 폴링): 탐색 ~31 s 동안 표본 1,363(N3)/1,368(N4) **전부 0**. 리밋 도달 판독(−137.27°)과 137° 복귀 스윙(−130.57→−20.70°, 0.5 s 간격)은 실값이 나온다. 별도 고속 폴링 시험에서는 스윙 초반 2 s 에 비영 132/463 이 잡힌 회차도 있어 **탐색 중 실값 확보 가능성은 회차마다 다르다**. 대체 객체는 전부 미지원(0x6063·0x6062·0x60FC → SDO ABORT 184건). 드라이브 재기준인지 freeze 게이트인지 두 마스터 폴링 경합인지 미확정 | 2026-07-29 | 미해결(표시 영향 최소화됨) | 표시는 0 필터 + 호밍 중 조향 `0x6064` 고속 폴링으로 **얻을 수 있는 실값은 모두** 그린다. 탐색 구간 정지는 장치 동작이라 GUI 로 못 없앤다. 규명하려면 Seer 폴링을 끊은 단독 마스터 상태와 대조하고, 판다 freeze 게이트를 끈 채 같은 구간을 덤프해 게이트 기여를 분리한다 |
