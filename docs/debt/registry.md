...
| debt-022 | 이해 | Tongyi 구동축 node1(FrontWalk) · 제어권 인수인계 경로 | **제어권을 Seer 에 반환했다 되찾으면 node1 이 `Switch On Disabled` 로 떨어져 있다.** 2026-07-29 실기: 13:24:32 정상 주행 → 13:24:35 반환 → 13:33:54 재획득 시 `operation enabled {1: False, 2: True}`, `fault {1: False, 2: False}`. node2 는 유지되므로 **node1 만의 현상**이고 fault 도 없다. Seer 가 내리는 것인지 드라이브가 자체적으로 떨어지는지 **가리지 않았다**. 과부하(debt-021)와 같은 축이라 관련 가능성이 있으나 미확인 | 2026-07-29 | 미해결(증상 자동 복구됨) | `ensure_drives_enabled()` 가 제어권 획득 시 자동 복구하므로 운용은 막히지 않는다. 규명하려면 반환~재획득 구간을 수동청취해 **누가 `0x6040` 을 내리는지** 프레임으로 확인한다(Seer 발신이면 0x601 요청이 잡힌다). node2 와의 비대칭 원인도 함께 본다 |
