...
| **debt-108** | 의도 | `src/Navigation/mcl2d_ros2/launch/bringup.launch.py` ↔ 신설 `odom_imu_ekf` | **융합 노드를 만들었으나 측위 입력에 배선하지 않았다.** 레거시는 `MCLoc` 이 `RobotPosEKF` 의 융합 오도를 받는데(배선 정본 §A.1), 우리 `bringup` 은 아직 `icp_odometry` 오도를 `mcl2d` 에 바로 물린다. 배선을 바꾸면 **라이브 스택 거동이 바뀌고**, 그 런치는 다른 세션이 방금 재작성해(loc 스택 단일 진입점화) 동시 수정 시 충돌 위험이 있다. 그래서 노드만 추가하고 배선은 남겼다 | 2026-08-17 | 미해결 | ① `bringup` 에 `use_imu_fusion` 인자를 넣어 `odom_imu_ekf` 를 끼우고 `mcl2d` 입력을 `/odom_fused` 로 돌린다(기본값은 현행 유지 — 켤 때만 경로가 바뀌게). ② IMU 미수신 시 측위가 이동량을 전혀 못 받으므로 런치 문서에 그 실패 모드를 명시한다. ③ 휠 오도를 소스로 쓸 경우 `debt-004`·`debt-007`(조향 부호·원점) 선결 |
