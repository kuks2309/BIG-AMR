...
| debt-111 | 기술 | `src/Comm/seer_tcp_ip/seer_tcp_ip/api.py` 쓰기 계열 24종 · `control.py` 전체 | **쓰기 API 가 실기 미검증이다 — 단위 시험은 「우리가 올바른 편호·포트·JSON 을 만든다」까지만 보증한다.** 미검증 목록: 4005·4006·2000·2001·2003·2010·3001·3002·3003·3050·3051·3052·3055·3056·3066·4000·4001·4002·4003·4004·4010·6000·6001·6004. 조회 경로는 실기 확인했다(인벤토리 §6). 이 구분이 흐려지면 「구현했다」가 「동작한다」로 읽힌다. ⚠ 3055·3056 은 **필드명 자체가 미확인**이라 저수준 dict 만 받는다 — 편의 인자를 만들면 발명이다 | 2026-08-18 | 미해결 | ① 제어권 경로(1060→4005→2010→2000→4006)를 **잭업 상태**에서 먼저 확인한다 — `JogKeepalive` 의 dead-man 이 실제로 로봇을 세우는지(프로세스를 죽여 확인)가 핵심이다. ② 내비게이션(3051·3066)은 지도·스테이션이 준비된 상태에서만. ③ 3055·3056 은 필드명을 실기 응답으로 확정한 뒤에야 편의 인자를 만든다. ④ 확인한 편호는 인벤토리 §6 표로 옮기고 이 행에서 지운다 |
