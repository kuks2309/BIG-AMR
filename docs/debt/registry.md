...
| debt-014 | 기술 | Tools/amr_test_gui/gui.py `_on_take` | **종료 경로에서 제어권 반환이 건너뛰어질 수 있다.** `_on_take` 는 `btn_take.setText()` 를 CAN 조작보다 **먼저**, 그리고 `try` 블록 **밖에서** 호출한다. `atexit`·미처리 예외 경로처럼 Qt C++ 위젯이 이미 파괴된 시점에는 그 호출이 `RuntimeError` 를 던져 뒤따르는 auth=Seer·passthrough 복구가 통째로 실행되지 않는다. `safe_release` 가 예외를 잡아 USB 는 닫지만 **릴레이가 intercept 로 남아** Seer 가 로봇을 되찾지 못할 수 있다. 3계층 분할(ADR 2026-07-28-gui-three-layer-split) 중 식별했으나 '값·순서 보존' 원칙에 따라 손대지 않고 등록만 한다. debt-013 은 다른 세션이 선점해 014 를 쓴다 | 2026-07-28 | 미해결 | 위젯 갱신을 `try` 안으로 넣거나 CAN 복구 뒤로 미뤄 예외가 반환 경로를 끊지 못하게 한다. 회귀는 `test_safe_release.py` 에 'setText 가 던져도 `_on_take` 의 CAN 복구가 수행된다' 케이스 추가 |
...
| debt-017 | 이해 | Tools/amr_test_gui/tongyi_can.py `_loop`·`decode_frames` | **호밍 탐색 구간(~31 s)에 조향 위치를 CAN 으로 얻을 수 없다.** 2026-07-29 실기 4가지로 확인: ① SDO 폴링 — `0x6064` 표본 1,314/1,445 가 0(t=4~34 s 비영 0건) ② 조향축 단독 고속 폴링 — 스윙 초반 2 s 만 비영 132/463, 이후 0 ③ 버스 전체 청취 — **TPDO 0건**(0x180/0x280/0x380/0x480 전무), SDO 응답·요청·heartbeat 뿐 ④ 대체 객체 전수 — `0x6063`·`0x6062`·`0x60FC`·`0x60F4`·`0x6065` 모두 **ABORT 0x06020000(객체 없음)**. 위치 객체는 `0x6064` 하나뿐이다. 판다 freeze(`seer_is_motion_obj`)는 `seer_send_bus0` 로 **bus 0(Seer)** 에만 합성 응답을 보내므로 우리 bus 2 판독과 무관하다(펌웨어 `safety_seer_gate.h:76-100` 확인). 같은 구간에 `0x606C`(−250 rpm 일정)·`0x6078`·`0x6041` 은 살아 있다 | 2026-07-29 | 미해결 | 표시는 '탐색 중' 으로 바꿔 **동결된 값을 현재값처럼 보여주지 않게** 했고, 회전(rpm) 칸이 살아 있어 움직임은 그쪽으로 보인다. 위치를 얻으려면 CAN 밖의 경로가 필요하다 — EasyDRIVE 등 벤더 도구로 드라이브 파라미터에 호밍 중 위치 보고 설정이 있는지 확인하거나, 드라이버 엔코더 신호를 직접 읽는 경로를 검토한다. 속도 적분 추정은 **하지 않는다**(측정 아닌 추정을 실측처럼 표시하는 것은 claude-mistake 2026-07-28-003 과 같은 실패) |
