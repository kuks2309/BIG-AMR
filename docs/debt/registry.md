...
| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-125 | 이해 | `src/Sensors/Positioning/pgv_ros2/` (PGV 드라이버) | 신규 PGV 드라이버의 **실기 부분 검증에서 남은 미확인 4건**: ① 레인/태그 **실판독 미검증**(테이프·태그 실물 부재 — 통신·프레이밍·방향결정만 실증) ② 위치/각도 해상도의 **장치 설정값 미확인**(매뉴얼 DOCT-3707D 에 공장 preset 서술 없음 — 파라미터 기본 0.1 mm/0.1° 는 추정) ③ 컬러 테이프 모드 상대각(-45°~+45°, §3.2)의 **14bit ANG 인코딩 방식 미서술** — 현재 드라이버는 무부호 해석만 적용 ④ `pgv/set_color` 서비스 실기 미호출(현장 테이프 색 미정) | 2026-08-24 | 미해결 | 레인 테이프 실장 시: ① 테이프 위에서 `pgv/position` 의 x/y/angle 실측 대조 ② Vision Configurator 또는 코드 카드로 장치 해상도 확인 후 `config/pgv_driver.yaml` 정합 ③ 컬러 테이프에서 읽기 헤드를 좌/우로 기울여 ANG raw 부호 표현 실측 ④ 현장 색으로 `set_color` 1회 호출·응답 확인. 좌표: `pgv_ros2/README.md` §검증 상태 |
