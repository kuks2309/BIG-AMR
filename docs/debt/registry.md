...
| debt-084 | 기술 | `src/Comm/CAN/can_relay/can_relay/home_and_zero.py` | **운용 CLI 잔여 결함 5건**(적대적 리뷰 2026-08-15). ① **시간축 혼용** — `elapsed()` 는 ROS 시계, 신선도는 `time.monotonic()`. `_await_zero` 의 유일한 탈출구가 `elapsed()` 라 `use_sim_time:=true` 이고 `/clock` 이 없으면 **0° 를 지령한 뒤 영원히 반환하지 않는다**(실증: 실시간 0.3 s 경과에 `elapsed()=0.000s`). ② **하드코딩** — `STEER_NODES=(3,4)` 와 `can_relay_node/...` 상대 이름이 상수다. 드라이버는 `steer_nodes` 파라미터를 쓰고 같은 세션의 `supervisor.py` 는 `target_node` 파라미터를 두었으므로 패키지 내부에서도 일관되지 않는다. 기체 YAML 이 다른 축을 쓰면 **0° 를 이미 보낸 뒤** 거짓 실패를 낸다. ③ **지령 수용 미확인** — `~/steer_deg` 는 volatile 1회 발행이고 드라이버는 거부를 자기 로그로만 남기므로, 게이트 거부(E-stop·`_home_failed`)로 버려져도 CLI 는 「발행했습니다」로 끝난다. ④ **파라미터 무검증** — `-p tol_deg:=180` 이면 어떤 자세든 「복귀 완료」·코드 0. 음수·0 이면 영구 미도달. ⑤ 100°+ 스윙 전 확인 절차·카운트다운 없음 | 미해결 | ①은 `elapsed()` 를 `time.monotonic()` 으로 통일(신선도와 같은 축) ②는 `target_node`·`steer_nodes` 를 파라미터로 선언해 `main()` 에서 한 곳 주입 ③은 대기 중 멱등 재발행 또는 진단의 `steer_target_deg` 대조로 「미수용」 종료코드 분리 ④는 상·하한 검증(예 `0 < tol <= 5.0`) ⑤는 `confirm` 파라미터 또는 카운트다운 |
