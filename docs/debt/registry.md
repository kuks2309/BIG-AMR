...
| debt-043 | 이해 | `src/Navigation/mcl2d_core/src/motion_model.cpp` `supplyControlVar()` dθ 정규화 | **원본 대조에서 dθ 만 1 ulp 어긋난다(잔여 17/1800, 다른 항목은 전부 비트 일치).** 원본 경로는 `Normalize(cur.angle − prev.angle)`(libfoundation 0x18750, floor 기반 `y = x − 2·floor(x/2π)·π`, `(π>y)?y:y−2π`) 뒤에 조건부로 `atan2(sin,cos)` 재정규화(`33d91c` → 분기 `33d93c` → `33dca3`)다. 후보 6개를 400표본 비트 대조한 결과 **while 루프 384 · 원본식 재현 349 · floor 단독 335 · atan2 단독 300** 으로 어느 것도 400/400 이 아니어서, 최선(while)을 유지하고 추측 변경을 하지 않았다. 분기 조건(`33d92b` 의 dt 비교 상수 `0x59ce38`)과 `Normalize` 의 π 상수·연산 순서가 미확인 | 2026-08-06 | 미해결 | ① `0x59ce38` 상수와 `rbk::foundation::math::PI` 실값을 읽어 분기·상수를 확정 → 식 재현 후 오라클 재실행(`-DMCL2D_MOTION_ORACLE=ON`). ② 400/400 이 되기 전에는 「비트 일치」라고 쓰지 않는다(현재 표기: 99.06% 일치, dθ 1 ulp 잔여) |
