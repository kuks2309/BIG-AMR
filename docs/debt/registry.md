...
| **debt-107** | 이해 | 원본 `rbk/plugins/libRobotPosEKF.so` (`estimation::OdomEstimation`) | **위치추정이 소비하는 오도가 무엇인지 절반만 안다.** 배포 배선(`rbk/rbk.plugin`)상 `MCLoc` 은 `OdoCalculator` 의 휠 오도를 직접 받지 않고 **`RobotPosEKF` 가 오도와 IMU 를 융합해 재발행한 `Message_Odometer`** 를 받으며, 이 기체는 `RobotPosEKF.UseIMU = 1` 로 **융합이 켜져 있다**. 심볼로 확인된 것은 인터페이스뿐이다 — `addOdoMeasurement(Message_Odometer)` · `addImuMeasurement(Message_IMU)` · `update()` · `getFilterOdometer(Message_Odometer&)` · `angleOverflowCorrect(double&,double)`, 그리고 레이저 오도 경로(`setLaserOdom`/`isEnableLaserOdom`/`getTargetMsgLaser(Message_AllLasers)`). **융합식·상태벡터·공분산·IMU 축 정렬·`UseIMU=0` 일 때의 거동은 전부 미조사**다. 이 공백 때문에 「Seer 오도는 엔코더만 쓴다」는 서술이 한 번 틀렸다(`docs/claude-mistake/2026-08-16-001`) | 2026-08-16 | 미해결 | ① `estimation::OdomEstimation::update()` 를 역어셈블해 상태벡터·예측/갱신식·공분산 초기값을 확정한다. ② `ReadIMUParam()` 이 읽는 파라미터와 IMU 축·부호 규약을 배포값과 함께 확정한다. ③ `UseIMU=0` 경로가 통과인지 별도 경로인지 확인한다. ④ 그 전까지 「위치추정 입력 = 휠 오도」로 서술하지 않는다 — 배선 정본은 `Tools/seer_re/docs/legacy_runtime_wiring.md` |
