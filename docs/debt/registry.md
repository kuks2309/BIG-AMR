...
| debt-043 | 이해 | `src/Navigation/mcl2d_core/src/motion_model.cpp` `supplyControlVar()` | **원본 대조 잔여 1 ulp — 원인 확정: 소스가 아니라 빌드 최적화였다.** 2026-08-07 에 dθ 정규화를 원본과 맞춰 17 → 2 로 줄였고, 2026-08-08 에 남은 2건을 격리했다: **`-O0` 로 빌드하면 1,800/1,800 완전 비트 일치**, `-O1` 이상에서만 2건(약 0.1%)이 1 ulp 어긋난다. 번역단위 교차 실험으로 원인을 `motion_model.cpp` 코드 생성으로 특정 (하네스 O2 + 모션모델 O0 → 0 / 하네스 O0 + 모션모델 O2 → 2). FMA 계약 아님(`-ffp-contract=off` 로도 2건), 벡터화·인라인 플래그와도 무관. `-O2` 에서 `normalizeAngle` 이 인라인되며 호출 9→5·`comisd` 8 추가로 코드 구성이 바뀐다 | 2026-08-06 | **해결(2026-08-08)** | 오라클 타깃을 `-O0` 로 고정(`CMakeLists.txt` `target_compile_options(test_motion_oracle PRIVATE -O0)`) — 오라클은 **소스 동치**를 재는 도구이므로 최적화를 끈다. 배포 라이브러리는 `-O2` 유지: 1 ulp 는 파티클 산포(mm 단위) 대비 무의미하다. **표기 규칙**: 「소스 동치 확정(1,800/1,800 @ -O0)」이라 쓰고, 「빌드 산출물이 비트 일치한다」고는 쓰지 않는다 |
| debt-044 | 기술 | `src/Navigation/mcl2d_ros2` · `Tools/mcl2d_standalone` (원본 `MCLoc::DoMoveAction` 대비) | **원본이 오도 콜백마다 수행하는 `moveRobotAccordingToMotion` 을 이식하지 않았다.** 원본은 파티클과 **별개로** 로봇 추정 자세(StateVar2D)를 같은 결정론 식으로 전진시키고(`0x33f4b0`, `DoMoveAction` @0x3d7d39), 파티클 평균은 스캔 갱신 주기에만 반영한다. 우리는 매 오도 콜백에 전체 필터를 돌려 파티클 평균만 낸다 — 출력 자세의 시간 특성이 다를 수 있다(특히 스캔이 오도보다 느릴 때). 같은 계열로 `doOffsetMove`/`setLaserMoveOffset`(라이다 장착 오프셋 보정, 조건부 경로)·`moveAccordingToMotion`·`getRealControlVar` 도 미이식 | 2026-08-06 | **해결(2026-08-08) — 단, 잔여 3건은 아래 참조** | ① **판정 완료**: 원본 `MCLoc::PublishLoc(Message_Odometer)`(0x20b9e0–0x20ce10)가 `Message_Localization` 을 만들어 **발행**하며 `OdometerMoveAction`·`DoOdoDataProcess` 를 각 1회 호출한다 → 그 자세는 우리 `/mcl_pose` 와 **같은 소비자**다. ② 따라서 이식했다(ADR `docs/adr/2026-08-08-mcl2d-two-rate-pose.md`): 파사드를 `advanceWithOdom`(오도 주기: kMove + 발행 자세 전진) / `updateWithScan`(스캔 주기: 산포·우도갱신·추정·리샘플 + 자세 재설정)로 분리하고, ROS2 노드는 `/odom` → 전진·발행, 스캔 콜백 → **양쪽 스캔이 새로 왔을 때만** 보정하도록 배선했다(스캔 재사용 D2 동반 해소). 회귀 `Tools/mcl2d_standalone/test/test_two_rate.cpp` — 돌연변이 4건(자세 전진 제거·자세 재설정 제거·is_stop 분기 제거·스캔 주기에 kMove 삽입) 전부 검출, 원복 시 0. ⚠ **잔여**: `doOffsetMove`/`setLaserMoveOffset`·`moveAccordingToMotion`·`getRealControlVar` 는 여전히 미이식이며 본 항목에서 다루지 않았다. 스캔 타임아웃(`ScanLostTimeThresh`)은 **debt-048** 로 분리 등록 |
...
| **debt-048** | 기술 | `src/Navigation/mcl2d_ros2/src/mcl2d_localization_node.cpp` · `Tools/mcl2d_standalone` (원본 `ScanLostTimeThresh` 대비) | debt-044 이식으로 **스캔이 끊겨도 오도만으로 자세가 계속 전진·발행**되는 구조가 됐다(원본도 같다). 다만 원본에는 스캔 두절 게이트 `ScanLostTimeThresh`(robot.param 기본 300 ms)가 있고 **우리는 그것을 이식하지 않았다** — 라이다가 죽어도 측위가 「정상」으로 보이며 흘러간다. ⚠ 원본에서의 정확한 동작(발행 중단인지, 상태만 강등인지)은 **미확인**이다. 값 300 ms 는 파라미터 파일에서 읽은 것이고 소비 지점 역어셈블 확인 전이다 | 2026-08-08 | 미해결 | ① 원본 `ScanLostTimeThresh` 소비 지점을 역어셈블로 특정해 초과 시 무엇이 바뀌는지 확정(발행 중단 / 상태 강등 / 재위치추정 유발). ② 확정된 동작을 노드에 이식하고, 스캔을 인위로 끊는 회귀 + 돌연변이 확인 |
