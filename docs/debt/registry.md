...
>   node4 **7,859,065**(σ≈3.2c) 로 σ≈3 counts 에 재현된다 ⇒ **재현되는 정착 동작이다(σ≈3c 로 10회 재현)** — 다만 「결함이 아니다」로 단정하지 않는다: 상수 SEER_HOME_ZERO_N3/N4 의 적정성은 실측 밖이며 debt-016 의 「영구 미검출 오프셋」 등록과는 별건이다(근거 §0-0-3 · debt-034 E7 · debt-016 E7).
