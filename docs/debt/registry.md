...
| debt-014 | 이해 | `Tools/amr_test_gui/gui.py:1058` (세션 8bfbdf1d 소유 — 본 세션은 진단만) | **호밍 중 조향각 표시 경로 미확정.** GUI 는 호밍 중 `0x6064` 를 읽지만 `not self._homing` 가드로 폐기한다(각도 미표시). `0x6064` 는 호밍 중(0x6041 bit15=0) 정확히 0 이 맞다(debt-007 §①, 3,080/3,080 샘플) — 따라서 0x6064 로는 호밍 중 각도 불가. 그러나 TR_Nav 는 UI 로 호밍 중 각도를 표시하며(다른 드라이브, 벤더 PDO `0x2106`), Tongyi Handbook V7.0 에도 `0x60FB.3 slAbsAngle`(32bit 절대각, txt:9656)·기본 TPDO4(`0x60FB.02`+`0x6064`, txt:8055) 가 있어 **호밍 중 유효한 각도 경로가 존재할 가능성**이 열려 있다. ⚠ `0x6063` 이 그 객체라고 단정 금지 — Handbook grep 0건(미확인) | 2026-07-29 | 미해결 | ① `0x60FB.3` 등 후보를 **실제 호밍 이동 중**(홈 아닌 위치에서 시작) 실기 판독해 유효성 확인(이번 판독은 조향이 이미 홈이라 무효) ② 유효 객체 확정 시 gui.py 가드를 그 객체 기반으로 교체. 관련 mistake `docs/claude-mistake/2026-07-29-001` |
