...
## 채번 규약 (ID collision 방지)

병렬 세션이 같은 번호를 서로 다른 부채에 배정하는 사고가 반복됐다(079 → 087 개번,
CSM 034·035 → 088·089 개번, 017~020 의 중복 재등록 027~030 은 별칭화). 규약:

1. 채번 전 `grep -o 'debt-[0-9]*' docs/debt/registry.md | sort -V | tail -1` 로 최대 번호를 확인하고 +1.
2. **다음 채번: debt-101** — 새 항목을 등록하면 이 숫자를 +1 로 갱신한다(이 줄이 곧 카운터다).
3. 병합에서 같은 번호가 서로 다른 부채로 충돌하면 **나중 등록을 개번**하고, 개번 전 저장소 전체에서 참조를 전수 확인한다.
...
| debt-004 | 이해 | src/Actuators/motor_control/config/tongyi_amr.yaml:15 `kin_steer_sign: 1  # ⚠ 가정` | 조향 counts 증가가 물리적으로 CCW(+θ)인지 CW(−θ)인지 미확정. 두 실측(홈 raw음수=전진 / +90°counts+raw양수=좌)은 **CW 일 때 정합**하므로 `kin_steer_sign=-1` 이 시사되나 직접 관측으로 확인되지 않았다. 영향 범위는 `driver_node` 의 twist→모듈 변환·오도메트리 경로이며, raw 언어로 직접 지령하는 `Tools/amr_test_gui` 는 무관 | 2026-07-27 | **부분 상환(2026-08-25 실기)** — **조향 부호만 확정**: 제자리 스핀에서 CCW(counter-clockwise) 회전 ⇒ steer3 −90°·steer4 +90°, CW 는 반대. counts 폭이 양 축 정확히 180.00°(57,344 counts/°). 회전 방향은 **IMU 가 독립 측정**하고 축 부호는 **모터 피드백 counts** 로 읽어 지령값이 개입하지 않는다. `steer_two_phase_260803_131305.jsonl` 의 지령각↔counts 동부호 선형과 이어져 **counts ↔ 물리 회전 방향 사슬이 닫힘**. ❌ **전후 축 배정은 여전히 미판정** — 후진 R-turn 조향 정착값이 `atan(x/R)` 예측과 0.003° 일치했으나 **그 각을 서버가 같은 기하 정본으로 계산해 지령**했으므로 순환이다(확인된 것은 지령 도달까지). 초판에 「설정이 뒤집혔음이 확인됐다」고 쓴 것은 **철회**했다. 근거: `docs/verified_facts/2026-08-25-steer-sign-and-axle-assignment-field.md` **존치**: ① 전후 배정 판정에는 **지령에서 파생되지 않는 독립 관측**이 필요하다 — 한 축만 조향한 채 주행해 회전 중심을 측위 궤적으로 읽는 식. ② `kin_steer_sign` 설정값 정정과 그 영향 범위(twist 변환·오도 yaw·crab·spin) 재검증 미수행 | 잭업 상태에서 조향 +90° counts 지령 후 바퀴 회전방향을 육안 확인(CCW/CW) → `kin_steer_sign` 확정하고 config 주석의 `⚠ 가정` 해제. 확정 전까지 `driver_node` 의 crab/스핀 twist 사용 금지 |
...
| debt-027 | (별칭) | — | **debt-017 과 동일 부채의 중복 등록이었다** — 본문은 debt-017 이 정본이다. 외부 문서(README·issues 등)가 027 로 인용한 이력이 있어 번호는 별칭으로 유지한다 | — | 별칭(정본 debt-017) | debt-017 을 볼 것 |
| debt-028 | (별칭) | — | **debt-018 과 동일 부채의 중복 등록이었다** — 본문은 debt-018 이 정본이다 | — | 별칭(정본 debt-018) | debt-018 을 볼 것 |
| debt-029 | (별칭) | — | **debt-019 와 동일 부채의 중복 등록이었다** — 본문은 debt-019 가 정본이다 | — | 별칭(정본 debt-019) | debt-019 를 볼 것 |
| debt-030 | (별칭) | — | **debt-020 과 동일 부채의 중복 등록이었다** — 본문은 debt-020 이 정본이다 | — | 별칭(정본 debt-020) | debt-020 을 볼 것 |
...
| debt-072 | 의도 | `src/Comm/seer_tcp_ip/` (broker 노드 부재) · `api.py:63-81` (`SeerApi.transport` 의 `allow_guarded` 우회구) | **Seer 지령 포트(19205 제어 · 19206 내비 · 19207 설정 · 19210 DO)의 단일 소유 broker 를 만들지 않았다 — 지금은 「명시하면 열린다」로만 막혀 있다.** ADR `docs/adr/2026-08-07-seer-api-tcp-hal.md` §Decision 3 은 broker 단일 소유를 정했으나 **본 작업 범위 밖으로 미뤘다** — 이관한 소비자 2건이 둘 다 19204 조회여서 broker 없이도 정책 위반이 없기 때문이다. 남는 구멍은 `SeerApi(..., allow_guarded=True)` 로, 단발 도구를 위해 열어 둔 것이지만 **두 도구가 동시에 켜지면 그대로 동시 지령**이다. ❌ **등록 사유 2건이 2026-08-10 실측으로 반증돼 정정한다**(원문은 이력으로 보존): ① ~~「이 네 포트는 동시연결 1 이라 선점되면 RoboShop 을 포함한 타 클라이언트가 거부된다」~~ → **틀렸다.** 한도는 **5** 이고(19204·19301 은 10), 초과 시 거동은 **거부형**이다 — 신규만 거부되고 **기존 연결은 살아남는다** (19204 실측: 9번째부터 `type=19204 / ret_code=61001 / "reach the maximum of status api connection limitation"`, 직후 기존 #1 정상 응답). 따라서 **선점 사고 위험은 존재하지 않는다.** broker 의 근거는 소켓 희소성이 아니라 **지령 중재**(두 주체 동시 지령)로 옮겨갔다. ② ~~「실기 프로토콜 판본 미확인 → 보수적으로 v1.2.1 값 채택」~~ → **질문 자체가 틀렸다.** 한도는 판본이 정하는 상수가 아니라 로봇의 **런타임 파라미터**(`Robot<카테고리>APITCPServerMaxConnections`, `uint32`, `minValue` 1 ~ `maxValue` 20, `advanced`)이며 API 1400 으로 직접 조회된다. 근거 2경로 일치 — 실기(192.168.44.82) 6건 + 원본 하드 `robot.param` SQLite `NetProtocol` 테이블(amap-server `sdb2`; 그 하드의 `rbk/product.version.h` = `3.4.5.22` 로 **실기와 동일 버전**). 조회기 `Tools/seer_re/seer_param.sh` 신설(양쪽 동시 조회). 경위: `docs/claude-mistake/2026-08-07-002` | 2026-08-07 | **미해결(축소)** — ②는 해소, ①은 근거 교체 후 존치 | ① 지령 포트를 쓰는 실사용(제어·내비·설정 자동화)이 생기는 시점에 broker 노드를 만든다 — 소켓은 노드가 소유하고 타 노드는 ROS 서비스로 요청. **판정 기준이 바뀌었다**: 「연결이 부족한가」가 아니라 「두 주체가 동시에 지령할 수 있는가」로 본다. ② 그 전까지 `allow_guarded=True` 는 **단발 CLI 도구에서만** 쓰고 상주 노드에 넣지 않는다. ③ ✅ **해소** — 한도는 상수로 신뢰하지 않고 `SeerApi.get_max_connections()` 가 로봇에 묻는다(돌연변이 `A13` 이 상수 우회를 차단). `ports.OBSERVED_MAX_CONNECTIONS` 는 참고값임을 스스로 선언한다. ④ 게이트 집합을 한도에서 파생하지 않는다 — 한도가 5 라 `n<=1` 파생은 빈 집합이 되어 게이트가 조용히 사라진다(돌연변이 `P2` 가 고정) |
| debt-073 | 이해 | HAL 경계 메시지 계약 (토픽·서비스·액션 목록 부재) · `docs/adr/2026-08-07-seer-api-tcp-hal.md` §Decision 2 | **HAL 경계를 「ROS 인터페이스에 둔다」고 위치만 정하고 그 목록을 정하지 않았다.** ADR 은 Seer 교체 가능성을 위해 상위 알고리즘 패키지가 `seer_api` 를 직접 import 하지 않는다고 정했으나, **그 대신 무엇을 쓰는지**(`/odom` 을 Seer 1004 로 채울지 우리 MCL(`src/Navigation/mcl2d_*`)로 채울지, 배터리·알람 토픽의 이름과 메시지 타입, nav 를 action 으로 감쌀지 service 로 둘지, 맵 배포 경로 4011 → `mcl2d_map` 를 어떻게 이을지)가 미정이다. 계약이 없으면 다음 소비자가 각자 편한 이름을 만들고, 그 시점에 경계가 사실상 사라진다 — `seer_api` 를 직접 import 하는 상위 노드가 하나라도 생기면 「디렉토리 하나만 지우면 된다」는 ADR 의 이득이 무효가 된다. 현재 소비자 2건은 TF 발행이라 이 문제를 건드리지 않는다 ⚠ **2026-08-13 조사 — 반대 방향 절반이 이미 구현돼 있다.** LGIT 장비 `lgit-c6-4`(tailscale, 계정 `tc`, tailscale SSH 미제공)의 `/home/tc/LGIT_C6_MoMa/LGIT-C6-MOMA/src/Comm/seer_task_bridge/seer_session` 이 **로봇 → PC 작업 위임** 규약을 C++17 로 구현해 두었다 — 우리 `seer_api`(PC=클라이언트, 16B 바이너리 헤더, 포트 19204~19210)와 **방향이 반대**다: 거기서는 **PC 가 TCP 서버**(bind/listen/accept, 포트 20101, systemd `seer-session-runner.service`)이고 로봇이 노드 도착 시 `customType`·`waitTimeout`·params 를 실어 접속한다. 선로는 **개행 종단 JSON 한 줄**, 프레임 7종 START/ACCEPT/REJECT/PROGRESS/RESULT/CANCEL/PING. 층 구성은 `frame`(값 타입)·`codec`(프레임↔JSON 한 줄)·`line_buffer`(부분 수신, 8192B 초과는 세션 종료)·`session`(방향·순서 FSM: AwaitingStart→AwaitingDecision→Running→Closed)·`tcp_transport`(논블로킹, 블로킹 호출 0, poll) — 전부 ROS 무의존. ⇒ **본 부채의 계약 목록을 정할 때 이 7종 프레임을 우리 ROS 인터페이스로 어떻게 올릴지가 그대로 논점**이며, 새로 설계하기 전에 이것을 먼저 본다. ⚠ 미확인 2건 — 그 코드가 정본으로 인용하는 `docs/protocol.md`·`ADR-006`(결정 2·4·9)을 **`/home/tc` 아래에서 찾지 못했다**(그 장비 그 홈만 조회 — 「없다」로 단정 금지), `kDefaultMaxLineBytes=8192` 는 선행 구현 `dock_listener_v050.py:32` 답습이며 코드가 스스로 「최적값 미검증」이라 밝힌다. 상세 분석은 코드 리뷰 미수행(사용자 중단) — 접속법·구조 요지는 메모리 `biguamr-lgit-seer-task-bridge`. | 2026-08-07 | 미해결 | ① 세 번째 소비자(Seer 상태를 ROS 로 흘리는 첫 노드)가 생기기 **전에** 토픽·서비스 목록을 ADR 로 확정한다 — 그 노드가 사실상 계약을 정해버리기 때문이다. ② 확정 시 기존 상류(`trnav_msgs` 계열)와 타입을 맞춘다(메시지 재정의 금지 — `trnav_2ws_msgs` 중복 폐기 선례). ③ 그 전까지 `seer_api` 를 import 하는 곳이 늘면 이 행에 추가 기록한다(현재 2곳: `seer_lidar_tf_node.py`, `seer_read_lidar_install.py` — 둘 다 조회 전용) |
| debt-110 | 이해 | `References/Seer-Driver/robokit_tcp_api.md:133` ↔ `T-Robot_seer_gui/seer_core/client.py:50,239` | **편호 2022 의 뜻이 두 자료에서 어긋났다.** 참조 문서는 `robot_control_scan_req`(스캔 시작), 실사용 클라이언트는 `API_SWITCH_MAP`(활성 지도 전환)으로 썼고, 실기 확인 없이 어느 쪽도 채택하지 않았다 | 2026-08-18 | ✅ **해결 2026-09-02 — 클라이언트 쪽이 맞다. `2022 = robot_control_loadmap_req`(지도 전환)이다.** 실기 실증: 편집한 `260709_test` 를 4010 으로 올린 직후 2022 `{"map_name":"260709_test"}` 를 보내 `1300 current_map_md5` 가 `92e73074…` → `0319831d…` 로 바뀌고 `1022 loadmap_status=1` 이 유지됐다. 스캔은 별도로 `6100`/`6101`@19210 이다. 조치: `SeerApi::loadMap(mapName)` 추가(빈 이름 거부, 제어 포트 19205), 시험 4건 고정, 참조 편호표의 2022/2023 정정 + 4010/4011 행 추가(debt-095 ①③ 동반 상환) | 근거: `tools/map_upload` 실행 기록, `src/Comm/seer_tcp_ip/docs/seer_tcp_ip_code_updates.md` 2026-09-02 (4) |
| debt-111 | 기술 | `src/Comm/seer_tcp_ip/src/api.cpp` 쓰기 계열 · `src/control.cpp` | **우리 라이브러리의 쓰기 경로가 실기에서 호출된 적이 없다.** 단위 시험은 「우리가 올바른 편호·포트·JSON 을 만든다」까지만 보증한다. ⚠ **주의 — 이것은 「Seer 쓰기 API 가 미검증」이라는 뜻이 아니다.** 저장소의 다른 도구들이 같은 API 를 실기에서 이미 썼다: `Tools/seer_jog`(2010 프레임·응답, 2000 정지 — 3초 펄스 +2.078° 실측 / vx 병진·맵 폐루프 2.02 m·28.7° 복귀, 잔차 36 mm·0.76°), `Tools/amr_test_gui/gui.py`(4005→2010 `duration` 600 ms·200 ms 갱신→2000→4006, 현장 검증본), `Tools/docking_field_kit/orin_steer_two_phase.py`(B 국면에서 Seer 가 주행, `vy` 사용). **미검증인 것은 API 가 아니라 우리 코드가 그 API 를 부르는 경로다** — 둘을 섞어 적으면 아는 것을 모른다고 보고하게 된다 | 2026-08-18 | 미해결(부분 상환) | ✅ **실기 확인된 것**: 4005·4006·1060·2000 과 `ControlSession`(`tools/control_probe`, 2026-08-23) · **지도 계열 3종**(`tools/map_upload`, 2026-09-02) — **4011** 다운로드(md5 대조 통과) · **4010** 업로드(되받아 값 대조 일치) · **2022** 전환(md5 갱신·로드상태 유지 확인). **설정 계열 4종 전부**(`tools/param_probe` + 실기 확인, 2026-09-02) — **4100** setparams 왕복 PASS, **4101** saveparams 디스크 저장·되읽기·원복 PASS, **4102** 지정 대상 공장값 복원 `ret_code 0`, **4300** clearfatal `ret_code 0`(fatal 0건 상태). ⚠ **남은 것**: 2001·2002·2003·2010·3001·3002·3003·3050·3051·3052·3055·3056·3066·4000·6000·6001·6004 및 `JogKeepalive`·`relocateAndConfirm` — 이 중 2010·2000 은 **타 도구가 실기 확인했고 우리 라이브러리 경로만 미호출**이다. 3050·3051·3052·3066(내비 태스크)은 저장소 전체에서 실사용 0건이라 **진짜 미검증**이다. ① 2010 은 잭업 상태에서 `JogKeepalive` 의 dead-man 이 실제로 세우는지(프로세스 kill) 확인. ② 내비 계열(3050·3051·3052·3066)은 **전제 조건이 이미 충족돼 있다** — 2026-09-02 실기 조회: 지도 `260709_test` 로드 완료(1022 `loadmap_status=1`), 스테이션 **LM1·LM2** 실재(1301, LocationMark·좌표 보유), 측위 성공(1021 `reloc_status=1`). 막는 것은 준비가 아니라 **로봇이 실제로 주행한다는 것**과 제어권(현재 타 PC `DESKTOP-I1MJLBT` 보유)이다. 3050(자유 내비)이 가장 작은 단위이며, 의미·인자는 `docs/seer/2026-08-08-seer-path-control.md` 가 벤더 원문 대조로 이미 확정해 뒀다 (⚠ 그 문서가 정정한 함정 — `3052` 는 평동이 아니라 **순찰**이다). ③ ✅ **2026-09-02 해소 — 3055·3056 의 필드명은 「미확인」이 아니었다.** 벤더 원문이 표로 적어 뒀다(`robotkit-netprotocol-l-1.2.1.txt:3192-3246`·`:3248-3300`), 그리고 `docs/seer/2026-08-08-seer-path-control.md` 가 그 줄번호까지 인용해 두었는데 읽지 않고 「미확인」이라 적었다. 3055 = `dist`(m, **절대값**)·`vx`(m/s, 부호가 방향)·`mode`, 3056 = `angle`(rad, **절대값**, 2π 초과 가능)·`vw`(rad/s, +반시계)·`mode`. `translateBy(dist, vx)`·`turnBy(angle, vw)` 를 만들고 음수 거리·각도와 속도 0 을 송신 전에 거부한다. `mode` 는 보내지 않는다 — `1`(자기측위)을 벤더가 「현재 사용 불가」로 명시하므로 기본 `0`(오도메트리)만 쓴다. ⚠ 따라서 이 두 기동은 **측위 폐루프가 아니다**(거리·각도가 커질수록 오차 증가), 둘은 **동시 수행 불가**다 |
| debt-112 | 의도 | `src/Comm/seer_tcp_ip/seer_tcp_ip/control.py` (제어권 세션) ↔ debt-072 (broker 부재) | **제어권 세션이 생기면서 동시 4005 사고가 실제로 가능해졌다.** 4005 는 기존 소유자를 뺏고 반납해도 원 소유자에게 자동 복귀하지 않는다. broker 가 없는 상태에서 두 프로세스가 각자 `SeerControlSession` 을 열면 뒤에 연 쪽이 앞을 뺏고, 앞은 자기가 잃은 줄 모른 채 지령하다 40020 을 만난다. debt-072 는 「broker 미착수」를 적었지만 그때는 제어권을 잡을 수단 자체가 없어 **위험이 잠재적**이었다 — 이제 실재한다 | 2026-08-18 | 미해결 | ① broker 노드(debt-072)의 우선순위를 올린다 — 소켓과 **제어권**을 노드가 소유하고 타 노드는 ROS 서비스로 요청. ② 그 전까지 `SeerControlSession` 은 **단발 CLI 도구에서만** 쓰고 상주 노드에 넣지 않는다. ③ 지령 직전 `get_control_owner()` 로 자기 `nick_name` 을 확인하는 가드를 검토한다(레이스를 없애지는 못하지만 창을 좁힌다) |
| debt-123 | 기술 | `src/MES/csm/csm/seer_client.py:43,45` | **Seer 16B 헤더의 세 번째 자체 구현이 남아 있다.** `SYNC = 0x5A` · `HEADER = struct.Struct(">BBHIH6s")` 로 같은 프로토콜을 독자 구현한다. 2026-08-07 이관 때 라이다 2건만 걷었고 이건 놓쳤다 — 그때 「헤더 재구현 2곳 → 0곳」이라 보고했는데 **실제로는 1곳이 남아 있었다**(조사 범위를 이관 대상으로 한정한 결과). 그 구현은 seq 를 대조하는지·부분 수신을 처리하는지·한도 거부를 알아보는지 확인되지 않았다. ⚠ **다른 세션 소유 코드**라 손대지 않았다 | 2026-08-23 | 미해결 | ① 소유 세션과 합의 후 `seer_tcp_ip` 로 이관한다 — 다만 그쪽은 Python 패키지(`ament_python`)라 C++ 라이브러리를 직접 못 쓴다. 언어 전환이나 ROS 인터페이스 경유가 선행돼야 하므로 **debt-073(HAL 계약)과 묶인다.** ② 그 전까지 「저장소에 Seer 헤더 구현이 하나뿐」 이라고 인용하지 않는다 — 사실이 아니다. ③ 이관 시 그 구현이 무엇을 안 하고 있었는지 먼저 기록한다 |
| debt-124 | 의도 | `src/Comm/seer_tcp_ip` ↔ lgit `LGIT_C6_MoMa/src/Comm/seer_api_comm` | **같은 로봇을 상대하는 Seer C++ 클라이언트가 두 저장소에 각각 있다.** 우리 것은 50편호·232시험, 그쪽은 8편호(1000·1021·1040·2002·2003·4005·4006·4011)·자체 `sim/fake_robokit.cpp`(가짜 로봇 258줄) 보유. 같은 Foil_A082 를 두 PC(젯슨·lgit-c6-4)가 함께 쓰므로 **동일 로봇에 두 구현이 붙는다.** 그쪽은 모션(2010)을 의도적으로 열지 않는 반면 우리는 전부 열었다 — 범위가 다르다. 지금은 서로 모르고 각자 4005 를 잡을 수 있다 | 2026-08-23 | 미해결 | ① 제어권 충돌이 실제 문제가 되는 시점(둘 다 모션을 쓰는 때)에 **소유 규약**을 먼저 정한다 — debt-112(동시 4005)와 같은 문제의 저장소 간 판이다. ② 그쪽 `fake_robokit` 는 우리에게 없는 자산이다 — 쓰기 API 실기 검증(debt-111) 전에 그것으로 대체 검증이 가능한지 본다. ③ 통합·일원화는 지금 판단하지 않는다(저장소 소유가 다르고 범위도 다르다) |
| debt-126 | 이해 | `src/Comm/seer_tcp_ip/include/seer_tcp_ip/api.hpp` 설정 편호 4종 · `References/Seer-Driver/robokit_tcp_api.md:171-174` | **4001 이 `ret_code 0` 을 돌려주고도 값이 바뀌지 않았다.** 2026-09-02 원인 확정 — **기전 문제가 아니라 편호가 틀렸다.** 우리 `kConfigSetParams` 는 4001 이었는데 공식 편호는 **4100**(`robot_config_setparams_req`, 응답 14100)이다. 4101 saveparams·4102 reloadparams·4300 clearfatal 도 각각 4002·4003·4004 로 틀려 있었다. 근거는 **이미 저장소에 있던** 공식 PDF 추출본 `References/Seer-Driver/github_sdk/robotkit-netprotocol-l-1.2.1.txt:3320,3401` 과 SEER 위키 `Set Robot Params Temporarily`(API number 4100 (0x1004)). 로봇은 정의되지 않은 4001 요청에 `ret_code 0` 만 돌려줬다 — **응답했다는 사실이 편호가 맞다는 근거가 되지 않는다.** 당시 세운 가설 3개(배차 문맥·RobotNote 읽기전용·적용 트리거)는 **전부 틀렸다** | 2026-08-24 | ✅ **해결 2026-09-02** — 편호 4종 정정(4100/4101/4102/4300), 근본 원인인 파생 참조표도 정정, `param_probe` 재실행으로 실기 왕복 **PASS**(`1400 ""` → `4100` 쓰기 → `1400` 반영 확인 → `4100` 원복 → 확인). 함께 철회한 것: `ports::kDispatchingRetCode`(40012) — 그 코드는 **존재하지 않는 편호 4003 에 대한 응답**이었고 공식 오류표에 없다(`grep 40012` → 0건), 의미를 단정할 근거가 없어 상수와 시험 고정을 삭제했다. 부작용 점검: 1060 조회 결과 조종권은 타 PC(`DESKTOP-I1MJLBT`)가 보유 — 우리 오호출이 조종권을 남기지 않았다 | 경위: `docs/claude-mistake/2026-09-02-001` |

| debt-113 | 기술 | `coding-inventory-gate.py` 의 기본-통과 조건 (`docs/claude_guideline/coding/coding.md:53`) · 표 미도입 모듈 전역 | **표를 만들지 않는 위반이 그 위반을 잡을 게이트를 무력화하는 자기참조 구멍.** coding SOP §2 는 코딩 전 함수표·전역변수표 선독을 요구하고 `coding-inventory-gate.py` 가 `.claude/settings.json` 에 **실제 등록돼 있으나**, coding.md:53 이 「표가 아예 없으면 **통과가 기본값**」으로 규정한다(인벤토리 미도입 프로젝트 전면차단 회피 목적). 따라서 표를 한 번도 만들지 않은 모듈은 영원히 통과한다 — `src/AI/yolo_detector` 가 그 경로로 최초작성(2026-07-28)부터 2026-08-06 까지 무검사였고, 그 결과 반증된 전제(「GPU 는 한 번에 한 프레임」) 위에 구조가 세워졌다. 경위: `docs/claude-mistake/2026-08-06-003_coding-sop-inventory-step-skipped.md`. `CODING_GATE=strict` 로 기본값을 뒤집을 수 있으나 미설정 | 2026-08-06 | 미해결 | ① `yolo_detector` 는 **표 신설로 개별 해소**(`docs/code_review/ai-yolo-detector/2026-08-06.md` + 패키지 병기) — 그 모듈은 이제 게이트가 실제로 막는다(수정 직전 차단 실증). ② 전역 해소는 `CODING_GATE=strict` 설정이나 **모든 세션의 코드 수정을 차단**하므로 사용자 승인 필요 — 승인 전에 표 없는 모듈 목록을 뽑아 영향 범위를 제시할 것. ③ `coding.md` 는 번들 SSOT 라 다운스트림 직접 수정 금지(드리프트), 기본값 자체를 바꾸려면 번들 갱신·재설치. ⚠ 본 행은 2026-08-06 에 `debt-042` 로 최초 등록했으나 **커밋 전 타 세션이 `registry.md` 를 덮어써 유실**됐고 2026-08-21 에 `debt-113` 으로 재등록했다(그 사이 042 번대는 재배정됨) |
| debt-118 | 의도 | `src/MES/csm/csm/runtime/capacity.py` `LineCapacity(redundancy=)` · `sim_node.py --line-redundancy` | **The §2.15 redundancy term is a number nobody has given us.** The CCS manual describes it as the knob that balances lines — positive lets extra pallets wait upstream, negative keeps a line deliberately short — but the value per leg is a customer decision. It defaults to 0, which makes the ceiling exactly ports + rack slots. With the documented capacities that gives A=6, B=17, C=34, and one robot per leg will rarely reach 17 or 34, so on B and C the ceiling does not bind in a three-robot sim. That is the honest consequence of implementing their formula rather than picking a number that makes our graph look calm | 2026-08-20 | 미해결 | ① Ask CATL for the per-line redundancy actually configured at XM. ② Until then leave the default at 0 and use `--line-redundancy` negative in the simulator when a test needs the ceiling to bind. ③ Do NOT invent a tighter formula — if the plant ceiling is the wrong shape for a three-robot cell that is a question for them, not a constant for us |
| debt-119 | 이해 | `src/MES/csm/csm/runtime/capacity.py` `in_flight()` ↔ CCS manual §2.15 | **One of the four terms in the customer's ceiling has no analogue in our model.** Their sum is AGV tasks in flight + material at turntable entrances + material on the line's buffer racks + material posted awaiting transport. We count in-flight jobs and parked material; **the turntable entrance term is simply absent**, because we do not model turntables. It is left out rather than approximated with something that merely looks similar, so our ceiling is looser than theirs by however much sits at an entrance | 2026-08-20 | 미해결 | ① Decide whether our plant model gains turntables at all — the deck's Big AGV flow docks at machine ports, not turntables, so it may genuinely not apply to this line. ② If it does not, record that as an answer and close this. ③ If it does, the term is a count of material at each leg's entrance ports and belongs in `in_flight()` beside `parked` |

| debt-120 | 기술 | `src/MES/csm/csm/records.py` `RackSlot.material_ref` ↔ CCS manual §2.2, §3.4 | **We model a slot as holding ONE material; the plant holds a PAIR.** The manual is emphatic — *"Every roll is half of a PAIR. A rack carries roll A and roll B"* — and the rack PLC table carries `RollerNumberA`/`RollerNumberB` and `TrayStatusA`/`TrayStatusB` as separate fields. Almost every failure in manual §5 is one of the two being somewhere the other is not. `material.TrayStatus` now expresses the pair's STATES (single vs double), but `rack_slot` still cannot say WHICH roll is on which side | 2026-08-20 | 미해결 | ① Add `material_ref_a` / `material_ref_b` to `rack_slot`, keeping `material_ref` as the A side so nothing already reading it breaks. ② Then §6 items 5-8 of the daily check become answerable and `ui/health.py` can drop its last four UNKNOWNs. ③ Blocked on nothing — the schema is documented in `References/local/rack-plc-interface.md` |
| debt-121 | 이해 | `src/MES/csm/csm/material.py` `attribute_matches` ↔ `records.py` resting default | **Two 'unknown' defaults now point opposite ways, deliberately, and that is a trap for the next reader.** Unknown resting time counts as READY (manual §4.6.12 documents a shipped default of 0, so unknown really does mean 'not configured'). Unknown material attribute counts as NO MATCH (nothing says an unrecorded attribute means 'any face will do', and a machine loaded wrong-side-out costs more than a deferred call). Both are defensible; together they mean 'unknown' has no single meaning in this codebase | 2026-08-20 | 미해결 | ① Leave both as they are — each is right for its own field. ② When the customer answers open decision #6 (who owns curing elapsed time), revisit the resting side; the attribute side needs no answer. ③ Anyone adding a third unknown-valued field must state which way it falls and why, in the field's own comment |

| debt-122 | 이해 | `src/MES/csm/csm/pda.py` ↔ CCS manual §3.4 (재고 대조 목록) | **We have no reconciliation.** Their PDA has a screen listing the four ways records and reality disagree — a roll in inventory but on no live rack; a roll on a rack but not in inventory; the inventory's rack number differing from the live one; the same roll appearing twice — and every one resolves by going to look at the physical rack and re-entering the truth. The CCS notes say it plainly: *"We have no equivalent. Our records are assumed to be the truth."* That assumption was harmless while CSM minted almost no material; since 2026-08-20 it mints one per collection, so the records now make claims that can be wrong | 2026-08-20 | 미해결 | ① The four checks are queries over `materials` + `rack_slots` and need no customer input — `ui/health.py` already runs one of them (§6 item 12, orphaned parked material). ② Add the other three there rather than in the PDA, since the daily check is where a human looks. ③ The RESOLUTION path (re-supplement and re-inbound the truth) is PDA work and comes after `debt-115`, because three of the four are about a pallet's two sides |
...
| debt-069 | 기술 | `amr_safety_watchdog` ↔ 모션 체인 | `/safe_to_move` **구독자 0** (실행 중 실측). 워치독은 `/safety/estop`·`/safety/lidar`·`/localization/healthy` 를 구독해 `/safe_to_move` 를 내지만 **아무도 소비하지 않아** 모션 체인을 막지 못한다. 게다가 그 세 입력을 발행하는 코드가 **SIL 런치에만** 있다 (실기 발행자 부재). 즉 실기에서 안전 게이트가 배선되지 않은 상태다 | 미결(설계 결정) | ① 입력 3종의 실기 발행자를 정한다(SICK 필드 → `/safety/lidar` 변환, E-stop 소스) ② `/safe_to_move` 를 mux 또는 supervisor 가 소비하도록 배선 |
| debt-070 | 이해 | 2WS 모션 스택 전역 (`src/Control/Motion_Control/2WS/**`, 16,174줄) | **주석이 코드와 어긋난 채 누적돼 있었다 — 3라운드 감사에서 180건 정정.** 밀도 ≈ **90줄당 1건**. 유형: 상수·임계값 오기(예: crab wrap 마진 주석 ±20°/±110° ↔ 코드 25°/115°), 부호 반전(`e_d` `+ left` ↔ 실제 경로 우측), 폐기된 알고리즘 라벨(MPC 코드에 `Pure Pursuit update`), 존재하지 않는 심볼·경로·파라미터 접두(`trnav_2ws_core::loadGeometry`, `translate_reverse_*`, rosdep `nlopt`), 형제 모듈 복사 잔재(reverse 런치에 `id=5 turn`), 자기 파일 안 모순(`0.2 s` ↔ `1.0 s`). **이자**: 이 주석들이 판정 근거로 인용되면 잘못된 결론이 나온다 — 실제로 본 저장소 실수 기록 `2026-08-05-001`(±115 vs ±90)이 그 형태였다. 코드는 무변경이라 빌드·테스트가 이 부채를 전혀 검출하지 못한다(67 tests PASS 상태에서 180건이 살아 있었다) | 부분해결(2026-08-13 감사분 180건 정정 + 검사기 신설 — 단 재현율 9%) | ① **주석↔코드 재도출 검사기** → `Tools/comment_check/` **신설 완료**(anchor·path·symbol·const 4종 + 회귀 테스트). **실측: 2WS 113파일에서 오탐 0, 알려진 결함 180건 중 25건 검출 = 재현율 14%**(`path` 16 · `symbol` 9) — 나머지 91%는 서술·부호 규약·동작 설명이라 기계 재도출이 불가능하다(`anchor`·`const` 는 알려진 결함 0건 검출: 줄이 존재하지만 엉뚱한 줄 / 상수가 선언 꼬리가 아닌 블록 주석에 있음). ⚠ **`symbol` 은 처음에 0건이었다** — 마지막 이름만 대조해 `trnav_2ws_core::loadGeometry` 가 빈 통과했고, codex 적대 검토(2026-08-13)가 이를 반증했다. 스코프 대조로 고쳐 9건이 됐으며 **그중 2건은 36 에이전트 감사가 놓친 실제 결함**이었다(`crab_linear_params.yaml:3`·`spin_params.yaml:3` — 같은 커밋에서 정정). `history` 검사(주석 속 이력 서술)도 신설했으나 현재 트리에 기존 42건이 걸려 **옵트인**이다. **따라서 이 검사기는 에이전트 감사를 대체하지 못한다** — 회귀 가드 + 이식 게이트 1차 체로 쓴다. 남은 상환: 의미 판정 자동화는 미해결이며 현재 수단은 적대적 에이전트 감사뿐 ② **QD 는 본 저장소에서 감사하지 않는다** — 주석 속 수치·심볼·경로를 코드/설정에서 재도출해 대조(`Tools/motion_chain_check/check_chain_contract.py` 방식). 최소 3종부터: (a) 주석이 인용한 `파일:줄` 앵커 실재 검사 (b) 주석 속 심볼(`::`·`*_params` 접두)의 실재 grep (c) 주석 속 상수와 같은 스코프 `constexpr`/yaml 값 대조 ② **QD(Quad-Drive) 스택은 본 저장소에서 감사하지 않는다** — LGIT 에서 검증 후 이식될 예정이므로, 통제점은 감사가 아니라 **이식 시점 게이트**다: 이식된 QD 트리에 ①의 검사기를 통과시키기 전에는 머지하지 않는다. ⚠ **2WS 는 QD 의 사본이 아니라 실기검사를 마친 독립본이다**(2026-08-13 사용자 확인) — 이식되는 QD 를 2WS 주석·서술의 근거로 인용하지 말고, 두 스택을 각자 자기 코드로만 대조한다 ③ 검사기 설치 전까지는 `code_review` SOP 에 「주석 인용 좌표 검증」 항목으로 수동 강제 |
| debt-071 | 지능 | 본 세션 커밋 `6fb9663` (2WS 주석 111건) | **주석을 고치면서 「무엇을 고쳤는지」를 주석 안에 적어, 되돌리는 데 감사 1라운드를 통째로 썼다.** 3라운드 69건 중 **22건이 1·2라운드가 심은 이력 서술**이었다(「종전 …이었다」·「2026-08-10 교체됐고」·「(2026-08-09 확인)」·「… 는 이 저장소에 없다」·「In-repo trace: …」). 즉 3라운드 작업의 **약 1/3 이 자초한 것**이다. 판단 실패의 형태: 감사 근거를 남기려는 의도가 「주석은 현재 동작만 적는다」는 원칙을 이겼고, 그 결과 **주석이 또 다른 낡음의 원천**이 됐다(날짜 하나는 실제로 틀렸다 — `2026-08-06` ↔ 실제 `2026-08-05`). 사용자 정정: 「기능만 넣기 이력은 넣지마시길」·「코드의 주석은 이력을 넣지 않습니다. 대신 code update 문서에서 만드는 것이 주석으로 인한 오류를 줄일 수 있습니다」 | 해결(2026-08-13 — 22건 전량 제거, 이력은 각 패키지 `docs/*_code_updates.md` 로 이관) | **가장 먼저 볼 것을 고정**: 주석을 쓰기 전 「이 문장이 1년 뒤에도 참인가」를 묻는다. 참이 아니면(날짜·「종전」·「정정했다」·조사 결과 보고) 주석이 아니라 `code_updates` 소관이다. 부재 사실은 **서술하지 않고** 인용을 삭제한다. 자산 반영: `docs/claude-mistake/2026-08-13-001_history-narrative-injected-into-code-comments.md` |
| debt-074 | 의도 | 세션 브랜치 4개 (삭제됨 — 복원 좌표는 사유란) | **미회수 세션 브랜치를 회수하지 않기로 판정하고 삭제했다. 같은 조사를 반복하지 않도록 근거를 남긴다.** 판정 기준은 「병합 가능한가」가 아니라 **「회수 가치가 있는가」** 였다 — 가치 없는 회수는 부채를 늘린다(사용자 지적). ① `session/c9ea2414`(`9ee8dac`) — 고유 파일 0, 머지 시뮬레이션 `0 files changed`, 충돌 96건 전부 `add/add`. 2WS 가 추적되기 전 브랜치라 병합하면 주석 감사 76파일이 **회귀**한다(예: crab wrap `±25°/±115°` → `±20°/±110°`). 브랜치의 실제 수정은 이미 main 에 있다(`steer_home` 16개 동일, 체인 검사기는 main 이 343→559줄로 앞섬). ② `session/17558ff1`(`d4aebcb`) — 실수기록 `2026-07-29-001` 이 이미 main `INDEX.md` 에 반영됐고 `2026-07-29-002/003` 도 main 에 존재. 더하는 `debt-013`·`debt-014` 는 현행 번호대(073까지)와 충돌한다. ③ `session/489e9c4e-full`(`a029810`) — **회수 금지**. 저장소가 「인용 금지」로 폐기하고 삭제한 `docs/verified_facts/2026-08-02-steer-home-closed.md` 가 `add/add` 로 부활하며(registry.md:14 가 폐기를 선언), 안전 펌웨어 `Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h` 포함 25건 충돌. ④ `origin/session/56a709a5-tools`(`9100ebe`) — main 의 `gui.py` 가 876→**1,579줄**로 앞섰고 같은 수정을 main 이 독립 커밋했다(브랜치 `2c57f7a` ↔ main `4aea32d` 「구동축 CiA402 운전 상태 복구」). 브랜치의 3계층 분할(876줄)은 main 최종본과 다른 갈래다 | 해결(2026-08-14 삭제) | **복원**: `git checkout <해시>` 로 언제든 되살아난다(`9ee8dac`·`d4aebcb`·`a029810`·`9100ebe`). ⚠ 남은 미회수 브랜치 6개(`50990c6a`·`5466b21a`·`56a709a5`·`6e5a2017`·`82e3631a`·`9988218d`)는 **활성 worktree 를 가진 타 세션 소유**라 손대지 않았다. 그 브랜치들은 2WS 변경 0건이라 병합해도 2WS 를 위협하지 않지만, 낡은 2WS 트리를 갖고 있으므로 **회수 시 먼저 `git rebase origin/main`** 할 것 |
| debt-075 | 기술 | `src/Comm/CAN/can_relay/{backend.py,driver_node.py,health.py,supervisor.py}` · `systemd/*.service` | **노드 health 감시·복귀 일체가 실기 미검증이다.** 전량 mock/순수 함수 회귀로만 확인했다(445 passed). 미확인 항목: ① ROS 실행기 정체 시 심박 억제가 실제로 펌웨어 fail-safe 를 부르는가 ② 재기동 후 `~/engage` 자동 복귀가 실기에서 성립하는가(`relay_off_latched` 해제 포함) ③ systemd 유닛이 실장비에서 오버레이를 제대로 소싱하는가. 설계 근거는 `docs/adr/2026-08-15-can-relay-node-health-supervision.md` | 2026-08-15 | **부분 해소 (2026-08-15 실측)** — ①②③ 중 **①(프로세스 소멸)과 재기동·복귀 전 경로가 실기에서 확인**됐다: 드라이버 기동 → `~/engage`(제어권 획득·fail-safe 무장) → `kill -9` → 감시자 `RUNNING → DEAD`(두절 3.2 s) → 재기동 → **자동 복귀 성공**(`engaged: true`, `homed_effective: true` — 재호밍 불요 확인) → `~/engage false` 로 정상 반환. **미해소**: ②(ROS 실행기 정체 주입으로 좀비 재현) · ③(systemd 유닛 실장비 기동) · 심박 억제가 실제로 펌웨어 fail-safe 를 부르는지 **분리 관측**(이번엔 프로세스 소멸이라 심박이 자연히 끊겼다 — 억제 경로 단독 검증 아님) **추가 해소 (2026-08-16 실측, `docs/verified_facts/2026-08-16-can-relay-supervisor-estop-field.md`)** — 병합판(`fe50b48`)으로 kill→복귀 전 경로 재확인 + **E-stop 인가 중 재기동 → HOLD 보류(≥20 s, 복귀 지시 0회) → 해제 후 복귀** 실기 PASS. 안정화 창 3 s·`prune_stamps` 창 밖 정리·수동 해제 불개입도 실기 관측. **추가 상환 (2026-08-16 동결 실험, `docs/verified_facts/2026-08-16-can-relay-zombie-freeze-field.md`)** — ②(실행기 정체)를 SIGSTOP 주입으로 실기 검증: ZOMBIE 판정 PASS. 그 과정에서 동결 후 감시자 영구 무수신 결함을 발견·수정(참여자 재생성)·재검증. **③ systemd 실장비도 상환 완료 (2026-08-16, `docs/verified_facts/2026-08-16-can-relay-systemd-field.md`)** — 오버레이 소싱·RuntimeDirectory·크래시 소생(Restart=always 로 정정 후 4 s)·소생 후 자동 복귀(kill→복귀 9.6 s) 실기 PASS. 유닛은 main 고정 배포 워크트리(`~/Project/Ford-CATL-AMR/Big-AMR-deploy`)에서 설치·갱신한다. 이로써 ①②③ 전 항목 상환. **마지막 잔여였던 버스 수준 직접 관측도 상환 완료 (2026-08-16, `docs/verified_facts/2026-08-16-can-relay-hb-failsafe-bus-field.md`)** — 판다 자체를 계측기로 쓰는 `Tools/can_relay_field/hb_failsafe_capture.py` 로 심박 중단 +1.6 s 에 0x60FF=0 드라이브 ACK 연발과 릴레이 개방(bus2 수신율 +61 %)을 직접 관측. **debt-075 전체 해결** — 남은 것 없음(점화 on 임계 4~5 s 구간만 미시험으로 한정 기재). **심박 억제 단독 관측은 상환 완료 (2026-08-16, `docs/verified_facts/2026-08-16-can-relay-hb-suppression-field.md`)** — 기본 콜백 그룹 점유 주입(sitecustomize, 제품 무변경)으로 ROS 정체 2.1 s 에 억제 발동·해소 시 자동 해제를 실기 확인. | 잭업 상태에서 ① `kill -9` 로 프로세스 소멸 ② 실행기 정체 주입(예: 콜백에 장시간 블로킹)으로 좀비 ③ `systemctl restart` 후 복귀. 각 경우 `0x60FF` · 릴레이 상태 · `/run/can_relay/state.json` 을 대조한다 |
| debt-076 | 이해 | `Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h:88-93` · `:136-138` (동결 스냅샷) | **릴레이 개방 직후 조향이 어디로 가는지 모른다.** 펌웨어 fail-safe 는 구동 노드(1~2)만 정지시키고 조향축(3~4)은 건드리지 않는다(`:554-559`). 개방 순간 Seer 의 `0x607A` 가 드라이브에 직결되는데, Seer 는 「현 위치 유지」 패턴이나(캡처 6종 38만 프레임 전수 — `master_command_census.py`) intercept 중 읽는 값이 **engage 시점 동결본**이라 그 「현 위치」가 실제 각과 다를 수 있다. 개방과 동시에 `seer_frozen_valid=0` 이 되어 ~28 ms 뒤 갱신되므로 결과는 **완전 복귀 스윙과 짧은 경련 사이 어딘가**다 | 2026-08-15 | **✅ 2026-08-15 해소 (실측)** — 릴레이 개방 시 **조향은 움직이지 않는다.** 실기 관측: 조향 `[−90.0°, +90.0°]`(±90° = 최대 조건)에서 제어권 획득 후 `kill -9` → 심박 상실 → 펌웨어 fail-safe → 릴레이 개방. Seer API 1005 를 0.15 s 주기로 45 s 계측한 결과 `steer_angles: [1.571, -1.571]` **전 구간 불변, 변화점 0건**. ⇒ 「engage 시점 동결본으로 되돌아간다」는 가설은 **반증**됐고, 캡처 분석이 시사한 「Seer 현 위치 유지」가 실기에서 확인됐다. ⚠ 세션 중 「0° 지령 시 ≈136.7° 스윙 가능」으로 경고한 서술은 **과장이었다** — 그 값은 상류가 관측한 `ERR_GOZERO` 사고(축이 −리밋에 선 채)의 크기이지 정상 개방 시 거동이 아니다. 계측 원자료 `/tmp/relay_field/steer_trace.jsonl`(휘발) | 잭업 + 조향 40° 에서 engage 후 `kill -9`. 바퀴가 제자리면 무해, engage 시점 각으로 돌아가면 crash-loop 이 스윙 반복이 되므로 `StartLimitBurst`(3회/120 s)가 유일 방어선임을 문서에 승격하고 펌웨어 동결 스냅샷 주기 갱신을 별도 ADR 로 검토한다 |
| debt-077 | 이해 | `src/Comm/CAN/can_relay/can_relay/health.py:37` `SupervisorConfig` | **복귀 차단 임계 3회/120 s 는 실측이 아니라 채택값이다.** 근거는 「반복 engage/release 가 죽은 채 있는 것보다 나쁘다」는 정성 판단뿐이고, 실제 crash-loop 주기·복구 성공률 분포를 관측한 적이 없다. 같은 성격으로 `diag_timeout_s`(3.0)·`ros_alive_timeout_s`(2.0)도 주기 배수에서 유도했을 뿐 실측 분포 기반이 아니다 | 2026-08-15 | 미해결 | debt-075 실기 검증에서 재기동 소요·복귀 성공률을 기록해 임계를 재산정한다. 그 전까지 값을 「실측」으로 인용하지 말 것. `recycle_after_s`(15 s)도 같은 성격의 채택값이다 |
| debt-078 | 기술 | `src/Comm/CAN/can_relay/test/test_gui_node.py` — `test_drive_command_is_republished_so_watchdog_does_not_kill_the_jog` · `test_stop_wins_over_republish` | **두 시험이 20~30 % 확률로 실패한다(간헐).** 실측: 두 건을 함께 10회 실행 — 통과 7 / 실패 3. `backend.py`·`driver_node.py` 를 stash 한 **baseline 도 통과 8 / 실패 2** 로 같은 수준이라 특정 변경의 회귀가 아니라 시험 자체의 성질이다. 단독 실행에서도 재현된다(4회 중 1회). 시험 주석이 「전체 시험을 함께 돌리면 타이머가 밀려 간헐 실패가 난다(2026-08-04 실측)」로 이미 기록했으나 **부하와 무관하게 깨지는 것**은 그 설명을 넘어선다. 이 rig 는 드라이버와 클라이언트를 한 `MultiThreadedExecutor` 에 올리므로 타이머 스케줄링이 판정에 직접 섞인다. ⚠ 실질 피해: 이 패키지의 회귀 결과(예: 「445 passed」)를 **증거로 인용할 수 없다** — 같은 스위트가 실행마다 445/444/443 을 오간다 | 2026-08-15 | 미해결 | ① 어느 단언이 깨지는지 확정(현재 미포착 — 전체 실행 출력을 `tail` 로 잘라 트레이스를 유실했다). ② 드라이버·클라이언트를 별도 실행기/프로세스로 분리하거나, 판정을 벽시계 창이 아니라 **발행 횟수 단조 증가**로 바꾼다. ③ 그때까지 회귀 인용 시 실행 횟수와 편차를 함께 적는다 |
| debt-079 | 지능 | `Tools/amr_test_gui/{gui.py,test/test_steer_zero_return.py,mutation_check.py}` · `src/Comm/CAN/can_relay/{can_relay/home_and_zero.py,test/test_home_and_zero.py,mutation_check.py}` | **`debt-071` 재발 — 형태가 하나 늘었다.** 071 이 닫은 것은 「주석에 이력 서술」이었는데, 이번에는 그 위에 **「주석이 타 계층의 현재 동작을 단정한다」**가 겹쳤다. 실제로 거짓이 된 사례: `home_and_zero.py` 모듈 docstring 이 「드라이버는 호밍 실패를 조향 게이트로 막지 않는다 — `homed_effective()` 가 bit15 만으로 True 를 돌려준다」고 단정했으나, 그 파일을 커밋하기 **전에** 커밋 `54ceea4` 가 드라이버에 `_home_failed` 래치를 넣어 막고 있었다. 즉 **작성 시점에 이미 거짓**이었다. 원인은 워크트리 base 를 최신화하지 않은 채 그 시점의 관측을 현재형 사실로 적은 것이다. 이력 서술도 함께 재발했다(`test_steer_zero_return.py` 「종전에는 … 육안에 위임했다. 그 문장이 사실이 아니었던 것이 이 작업의 출발점이다」). **이자**: 이 주석들이 다음 작업의 전제로 인용되면 없는 구멍을 막는 코드가 또 나온다 | 해결(2026-08-15 — 6파일 주석 재작성, `Tools/comment_check` `history` 포함 5종 검사 0건) | **가장 먼저 볼 것**: 주석에 **다른 파일·다른 계층의 동작**을 적으려 하면 멈춘다. 그 문장은 그 계층이 바꾸는 순간 거짓이 되고, 이쪽 빌드·테스트는 그것을 검출하지 못한다. 적어야 한다면 ⓐ 이 파일이 무엇을 하는지로 바꿔 쓰거나 ⓑ `code_updates`/ADR 로 옮긴다. 판정 한 줄은 071 과 같다 — 「이 문장이 1년 뒤에도 참인가」. 추가로 **워크트리 base 가 `origin/main` 보다 낡았는지 커밋 전 확인**한다(`git rev-list --count HEAD..origin/main`) |
| debt-080 | 기술 | `Tools/amr_test_gui/mutation_check.py` | **검출력 검사기가 두 가지 거짓 초록을 냈다.** ① **기준선 실패 무시** — 검출 판정을 `pytest` 종료코드로만 해서, 시험 묶음에 선존재 실패가 1건이라도 있으면 **모든 돌연변이가 `✅ 검출`** 로 찍힌다(워크트리 실측 기준선 7건). ② **0건 선택** — 없는 id 를 인자로 주면 아무것도 돌지 않고 `✅ 0개 항목 전부 검출` 을 출력한다. 같은 계열이 `src/Comm/CAN/can_relay/mutation_check.py` 에도 있었다(rclpy 미소싱으로 `1 skipped` 된 것을 검출로 집계) | 해결(2026-08-15 — ① 원본 상태의 실패 집합을 먼저 재고 **새로 깨진 시험이 있을 때만** 검출로 인정 ② 선택 0건은 `검사 불가`로 `exit 1`) | **남은 한계(정직 선언)**: 어떤 항목의 검출 시험 자체가 기준선 실패이면 그 항목은 판정 불가인데 현재는 `미검출` 로 표시된다(실측: `M5` 의 `test_poll_thread_death_actually_emits_the_signal` 이 그 경우). 항목별 「이 돌연변이를 잡아야 할 시험」을 명시해 그 시험이 기준선 실패면 `검사 불가`로 분리할 것 |
| debt-081 | 의도 | 세션 브랜치 `session/6e5a2017` (삭제됨 — 복원 좌표는 사유란) | **미채택 브랜치를 삭제했다. 왜 채택하지 않았는지를 남겨 같은 조사를 반복하지 않게 한다.** 내용: 호밍 후 조향 0° 복귀를 **드라이버(`RelayBackend.home()`)에 결합**하는 안 — `home()` 이 `steer_to_zero()` 를 이어 호출하고 0° 미도달을 전체 실패로 묶는다. 구현·회귀·돌연변이 검사까지 마쳤고 `0522d2d`(19파일 +1292/−12)에 있다. **기각 사유**: 드라이버는 `~/home` 과 `~/steer_deg` 를 이미 따로 노출하므로 **호출자가 조합하면 같은 결과**가 되고, 드라이버 표면을 늘리지 않는 쪽을 사용자가 택했다(2026-08-15 「드라이버에서 호밍 + 0도 조향 기능이 있으면 조합해서 사용하면 되지 않을까?」 → 「조합 + 호출자 가드만 추가」). 채택된 대안은 `can_relay/home_and_zero.py`(운용 CLI + 가드)이며 `8816096`·`134b093` 으로 main 에 있다. ⚠ 브랜치 base 가 일주일 낡아 현행 main 에 **9파일 충돌**(`git merge` 시뮬레이션 실측)이므로 그대로 병합하면 다른 세션 작업을 되돌린다 | 해결(2026-08-15 삭제) | **복원**: `git checkout 0522d2d` 로 언제든 되살아난다. 되살릴 이유가 생긴다면 그때는 **현행 `origin/main` 위에 재적용**할 것 — 그 브랜치를 병합하지 말 것(위 충돌). 관련 기록: `docs/adr/2026-08-08-steer-zero-return-after-homing.md` · `code_updates/2026-08-15-home-and-zero-caller-guard.md` |
| debt-082 | 기술 | `src/Comm/CAN/can_relay/mutation_check.py` · `Tools/amr_test_gui/mutation_check.py` | **검출력 검사기가 실 소스 트리를 그 자리에서 변형한다.** 원본을 덮어쓰고 시험 후 `finally` 로 되돌리는 구조라 `KeyboardInterrupt` 는 막히지만 SIGKILL·SIGHUP(SSH 두절)·OOM 은 막지 못한다. 적대적 리뷰 실증: 실행 0.35 s 뒤 `kill -9` → `home_and_zero.py` 가 `if False:` 상태(「호밍 실패면 0° 를 보내지 않는다」 가드가 꺼진 상태)로 남았다. 이 저장소는 pre-commit·CI 가 없어 `git status` 를 보지 않고 `colcon build` 하면 그 바이너리가 설치된다. 더해 `subprocess.run(...)` 에 `timeout=` 이 없어 돌연변이가 무한 루프를 만들면 노출 창이 무한이다 | 미해결 | 패키지를 `tempfile.TemporaryDirectory` + `shutil.copytree` 로 복사해 **사본에서만** 변형·실행한다. 최소 조치로도 `subprocess.run(..., timeout=…)` 과 SIGTERM/SIGHUP 핸들러 등록. ⚠ 그때까지는 검사기 실행 중 강제 종료 시 **반드시 `git status` 로 잔존 변형을 확인**할 것 |
| debt-083 | 이해 | `src/Comm/CAN/can_relay/can_relay/{driver_node.py,home_and_zero.py}` · `src/Actuators/motor_control/motor_control/driver_node.py` | **`joint_states` 를 두 노드가 같은 이름 규약(`steer_<n>`)으로 발행한다 — 판정 근거가 외부 발행자에 열려 있다.** `home_and_zero._on_joint_states` 는 받은 한 장으로 각도 dict 를 통째 교체하므로 단일 발행자 전제에서만 옳다. `motor_control` 쪽에는 문서화된 결함이 붙어 있다(그 파일 주석: 호밍 중 `0x6064` 가 0 이라 θ≈−137° 를 그대로 발행, bit15 유효성 게이트 미구현 — debt-007). 두 노드가 동시에 뜨면 마지막 발행자가 이기고 CLI 는 누구의 값인지 구분할 수단이 없다. ⚠ **두 노드의 동시 기동 여부는 미확인**이다(launch·systemd 만으로는 판단 불가) — 코드 사실만 근거 | 미해결 | ① 동시 기동 여부를 실기·런치 구성으로 먼저 확정 ② 겹친다면 `can_relay` 전용 경로에서 읽는다 — 드라이버 진단(`_on_diag_timer`)의 노드별 KeyValue 를 쓰거나 `can_relay_node/joint_states` 사설 토픽을 추가 |
| debt-084 | 기술 | `src/Comm/CAN/can_relay/can_relay/home_and_zero.py` | **운용 CLI 잔여 결함 5건**(적대적 리뷰 2026-08-15). ① **시간축 혼용** — `elapsed()` 는 ROS 시계, 신선도는 `time.monotonic()`. `_await_zero` 의 유일한 탈출구가 `elapsed()` 라 `use_sim_time:=true` 이고 `/clock` 이 없으면 **0° 를 지령한 뒤 영원히 반환하지 않는다**(실증: 실시간 0.3 s 경과에 `elapsed()=0.000s`). ② **하드코딩** — `STEER_NODES=(3,4)` 와 `can_relay_node/...` 상대 이름이 상수다. 드라이버는 `steer_nodes` 파라미터를 쓰고 같은 세션의 `supervisor.py` 는 `target_node` 파라미터를 두었으므로 패키지 내부에서도 일관되지 않는다. 기체 YAML 이 다른 축을 쓰면 **0° 를 이미 보낸 뒤** 거짓 실패를 낸다. ③ **지령 수용 미확인** — `~/steer_deg` 는 volatile 1회 발행이고 드라이버는 거부를 자기 로그로만 남기므로, 게이트 거부(E-stop·`_home_failed`)로 버려져도 CLI 는 「발행했습니다」로 끝난다. ④ **파라미터 무검증** — `-p tol_deg:=180` 이면 어떤 자세든 「복귀 완료」·코드 0. 음수·0 이면 영구 미도달. ⑤ 100°+ 스윙 전 확인 절차·카운트다운 없음 | **해결 (2026-08-16)** — 상환계획 5건 전부 구현: ① monotonic 통일 ② 파라미터 주입 ③ 멱등 재발행 + 진단 대조 「미수용」 코드 5 분리 ④ `validate_params` 상하한(위반 코드 6) ⑤ `confirm:=true` 게이트(부재 코드 7). 시험 20종·패키지 491 passed. 이력: `docs/issues_and_fixes/issues_and_fixes.md` 2026-08-16 | (해결 — 계획 이행) |
| debt-085 | 기술 | `src/Comm/CAN/can_relay/test/test_home_and_zero.py` · `mutation_check.py` | **주입 경계 바깥(`_RosClient`)에 시험도 돌연변이도 0 건이다.** 시험은 전부 `FakeClient` 를 주입하고 돌연변이 항목도 `ZeroReturnGuard`·순수 함수·`supervisor` 만 겨눈다. 결과적으로 F2 가 실제로 고친 자리(`_on_joint_states` 의 dict 통째 교체·`_angles_t` 갱신·`FEEDBACK_TTL_S` 결선)는 한 줄도 검증되지 않는다. 2026-08-15 적대적 리뷰가 찾은 C1·H4·M2 가 **전부 이 경계 바깥**에서 나왔다. 부수: 돌연변이 `F2a` 는 설명(「직전 값으로 채움」)과 실제 변형(전 축 항상 `None`)이 어긋나 라벨이 실제보다 큰 확신을 준다 | 미해결 | 가짜 node(`create_subscription`/`create_publisher`/`get_clock` 스텁)로 `_RosClient` 를 감싸는 단위 시험을 추가하고, 돌연변이 목록에 `_on_joint_states` 를 겨누는 항목을 넣는다. `F2a` 의 설명을 실제 변형에 맞게 고친다 |
| debt-086 | 이해 | `src/Comm/CAN/can_relay` (관측) · 미확정 기전은 `debt-004`(회전 부호) · `debt-036`(`0x6064`=0) 소관 | **호밍 실패 뒤 조향축이 움직이지 않는 상태로 남는 기전이 미확정이다.** 2026-08-15 17시 관측(제어권 획득 후 판독, 지령 0건): 호밍이 `ERR_GOZERO (30s, reached_mask=0x01)` 로 끝난 뒤 **node3** 은 `pos=7,882,021`·`DI=1`(정상 정착, 0° 대비 +0.178°), **node4** 는 `pos=0`·`DI=9`(**bit3 = −리밋 스위치 ON**)·`sw=0x9450`. `joint_states` 에는 node4 가 **−136.72°** 로 실렸다. ⚠ **그 −136.72° 는 측정값이 아니라 계산 산물**이다 — `(0 − 7,840,086)/57,344`. `0x6064` 가 0 이라 나온 수치이고 축의 물리 각도가 아니다. **사용자 가설(2026-08-15)**: 목표 −90° 를 **반대 방향(CW/CCW 오판)** 으로 돌아 −90+360=+270 쪽으로 가려다 한계 ±115° 에 걸려 +137° 에서 멈췄고, 그 뒤로는 한계 밖이라 0° 지령에도 반응하지 않았다. **부분 반증**: ① 부호가 관측(−136.72)과 가설(+137)이 반대다 ② **±115° 는 소프트웨어 클램프**라 지령을 만들 때 잘리므로 「거기 걸려 멈춘다」가 성립하기 어렵다(`safety.steer_deg_to_counts`) — 물리적 정지 근거는 클램프가 아니라 **−리밋 스위치**(DI bit3)다. **미반증 부분**: 「반대로 돌았다」는 기전 자체는 배제되지 않았다 — `debt-004` 가 「조향 counts 증가가 CCW 인지 CW 인지 **미확정**」으로 열려 있고, 두 실측이 CW 를 시사하나 직접 관측된 적이 없다. ⚠ 2026-08-15 작업(`home_and_zero`·호밍 실패 가드·신선도·감시자 수정)은 **「언제·무엇을 보낼지」만 다루며 회전 방향을 건드리지 않는다** — 이 항목은 그 작업으로 해결되지 않았다 | 미해결 | **가장 먼저 볼 것**: `debt-004` 상환계획 그대로 — 잭업 상태에서 한 축에 작은 각(예 +10°)을 주고 바퀴가 CCW/CW 어느 쪽으로 도는지 **육안 확인**(30초). 그것이 확정되면 「반대로 돌았다」가 성립하는지 즉시 갈린다. 그 전까지 이 가설을 근거로 코드를 고치지 말 것. 부수 확인: 갇힌 상태에서 `0x6064` 가 0 을 읽는 것이 `debt-036` 과 같은 현상인지(같은 축·같은 조건) |
| debt-100 | 이해 | Big-AMR `src/Comm/CAN/can_relay/can_relay/ui/{app,backend_direct}.py` ↔ lgit-c6-4/`LGIT-C6-Cobot` 동명 파일 | **UI 계층이 기체별 포크로 갈라져 있다(2026-08-16 이식에서 확인).** 포크(QD)는 `backend_direct` 를 기체 선택형(`CAN_RELAY_MACHINE` env + machine yaml 로드·타기체 폴백 차단)으로, `app` 을 QD 실측 JOG 로 진화시켰고, 정본은 foil 하드코드 직결 백엔드 + Foil JOG 다. 코어(backend·감시)는 정본 단일화 완료, UI 는 파일 경계로 공존(경계 목록: `docs/deployment/2026-08-16-can-relay-supervision-deploy.md` §2-1). 추가로 구 포크 backend 의 「호밍 내장 0° 복귀」 의미론이 정본 코어 채택으로 소멸 — QD 실기 확인 보류 중 | 2026-08-16 | 미해결 | ① 포크의 기체 선택 방식을 정본 `backend_direct` 에 역이식해 직결 백엔드 단일화 ② JOG 표를 machine yaml 로 내려 app 단일화 ③ QD 호밍 후 자세 실기 확인으로 `test_steer_zero_return` 판정 확정 |
| debt-087 | 기술 | `src/Comm/CAN/can_relay/can_relay/supervisor.py` (ROS 껍데기 전반) | **단위 회귀가 ROS 껍데기를 덮지 못한다 — 실증됨.** 판정을 `health.py`(순수)로 분리해 하드웨어 없이 시험 가능하게 만든 대가로, 껍데기(구독·타이머·서비스·파일 입출력·실물 메시지 타입)가 **시험 사각지대**가 됐다. 2026-08-15 SIL 하니스가 잡은 결함 5건이 **전부** 그 사각지대에 있었고, 그동안 단위 회귀는 매번 전건 통과했다(병합 시점 447 passed 상태에서 기능은 0). 결함 유형: 실물 메시지 타입(`DiagnosticStatus.level` 이 `bytes`) · rclpy 로거 호출 지점 severity 고정 · 프로세스 경계를 넘는 상태 승격 · 서비스 응답과 관측의 차이. 대응으로 판정 부분을 순수 모듈로 계속 이관했으나(`as_level`·`next_prev`·`next_was_down`·`is_outage`) **남은 껍데기에 대한 상시 검증은 `Tools/can_relay_sil/sil_health.py` 수동 실행뿐**이다 — CI(Continuous Integration) 부재라 아무도 돌리지 않으면 같은 부류가 다시 들어온다 | 2026-08-15 | 미해결 | ① SIL 하니스를 회귀 파이프라인에 넣는다(현재 저장소에 CI 자체가 없으므로 최소한 `README` 검증 절차에 명시). ② 같은 구조(순수 판정 + ROS 껍데기)를 쓰는 다른 노드에도 동일 사각지대가 있는지 조사. ③ 그 전까지 **감시자 코드를 고칠 때는 SIL 전종을 함께 돌린다** — 단위 통과만으로 기능을 주장하지 않는다. 재확인 사례(2026-08-16): recycle 이월 순서 결함이 단위·SIL 전건 통과 상태에서 실기 재현으로만 드러났다. **축소(2026-08-16)**: SIL 실험 11(재생성·승계 복귀)을 추가하고 돌연변이 검사로 검출력 확인 — 재생성 경로는 상시 회귀로 고정됐다. 잔여는 구조 항목뿐: CI(Continuous Integration) 부재라 SIL 전종이 여전히 수동 실행이다 |
| **debt-102** | 이해 | `src/Navigation/mcl2d_core/include/mcl2d_core/skid_detector.hpp` ↔ 런치가 고르는 `/odom` 소스 | **슬립 감지의 전제가 런치에 따라 무너진다.** `SkidDetector` 는 휠 오도 이동량과 레이저 위치추정 이동량을 대조해 미끄러짐을 판정하는데, `bringup.launch.py` 는 `icp_odometry`(레이저 정합)를 `/odom` 으로 물린다 — 그러면 레이저↔레이저 비교가 되어 두 값이 같은 원인으로 함께 틀리므로 **휠 미끄러짐을 원리적으로 검출할 수 없다**. 반면 `motor_control/driver_node.py` 는 휠 오도를 발행하고 있어 소스 선택만으로 의미가 갈린다. 주석에는 전제를 명시했으나 **코드·런치에는 아무 가드가 없다** | 2026-08-16 | **부분 상환(2026-08-17)** — 슬립 감지를 **성립시키지는 못했다**(휠 오도가 선결). 대신 **깨진 전제를 조용히 두지 않게** 했다: `odom_source` 파라미터(`wheel`|`laser`, 기본 `laser`)를 신설해 ① 값이 둘 중 하나가 아니면 configure 실패 ② `wheel` 이 아니면 기동 시 WARN (「판정은 계속 계산되지만 근거로 쓰지 말 것」) ③ `/diagnostics` 에 `skid_detection: degenerate (laser vs laser)` 로 상시 노출. 그 과정에서 **코어가 매 주기 판정하는 `reportState()` 를 노드가 꺼내지도 않고 버리고 있었음**을 확인해 함께 노출했다(`report_state`·`confidence`). 실기동 확인: 잘못된 값 → 종료코드 1 + FATAL / `laser` → WARN + degenerate / `wheel` → WARN 0건 + valid. **존치**: 실제 검출 능력은 휠 오도 확보(`debt-004`·`debt-007`) 후에야 생긴다 | ① 두 오도 소스 중 무엇을 측위 입력으로 삼을지 결정하고 런치에 못박는다. ② 레이저 오도를 쓰는 구성에서는 슬립 판정을 끄거나(파라미터) 휠 오도를 **별도 토픽으로 함께 받아** 원래 비교를 복원한다. ③ 어느 쪽이든 회귀로 고정한다 |
| **debt-103** | 기술 | 저장소 전반 (주석 규율 강제) | **주석 규율이 기계 강제되지 않는다.** `conventions.md §4`(이력 금지·낡은 주석 교정)와 `hooks/coding-comment-gate.py` 가 있으나 훅은 **이번에 추가된 텍스트만** 보고, 이미 들어간 주석은 아무도 훑지 않는다. odom 관련 16파일을 규칙에 맞춰 재작성하는 과정에서 파일 전체 스캔으로 **게이트 위반 33건**이 나왔고 그중 상당수가 날짜·이력 서술이었다. `Tools/comment_check` 는 인용 좌표·심볼·수치만 보므로(재현율 실측 ~14%) 이 부류를 대신 잡지 못한다 | 2026-08-16 | **부분 상환(2026-08-17)** — 「이미 들어간 주석을 아무도 훑지 않는다」를 도구로 메웠다: `Tools/comment_check/scan_gate.py`(+ `test_scan_gate.py`). 판정 규칙을 **훅에서 직접 불러 쓴다** — 복사본은 훅이 바뀌는 순간 조용히 어긋나 「통과」를 근거로 못 쓰게 되기 때문이고, 계약 시험이 훅을 임시 무력화해 도구 판정이 따라 바뀌는지 확인하므로 복사본으로 회귀하면 시험이 깨진다. `--strict` 는 게이트가 막지 않는 서술어까지 보되 **모드를 함께 적어야** 근거가 된다. 실측 기준선: `src`+`Tools` 736파일에 **위반 633건**(게이트 동일 모드). **존치 2건**: ① 이 저장소에는 pre-commit·CI 가 없어(CLAUDE.md 강제 장치 미설치 고지) 여전히 **수동 실행**이다. ② **가이드라인 번들이 git 미추적**이라 새 워크트리·새 클론에는 훅이 없고, 그러면 이 도구는 「규칙 비활성」로 판정을 거부한다(계약 시험은 SKIP 으로 빠진다 — 번들 부재가 도구 결함으로 오독되지 않도록). 즉 도구의 도달 범위가 번들 배포 문제에 묶여 있다 — `docs/claude-mistake/2026-08-06-003` 의 미해결 항목 ①과 같은 뿌리다 | ① 파일 전체를 훑는 스캐너를 `Tools/comment_check` 의 `history` 검사와 합치거나 별도 체크로 추가한다(현재 임시 스크립트로만 존재). ② 통과 기준을 정해 대상 트리를 단계적으로 넓힌다 — 저장소 전체에는 아직 다수 잔존하므로 한 번에 게이트로 켜면 노이즈가 된다 |
| **debt-104** | 기술 | `src/Sim/translate_sim_odom/config/sim_params.yaml` (휠 기하 자체 보유) · `src/Sim/trnav_2ws_gazebo/scripts/wheel_cmd_bridge.py` (코드 기본값) | **휠 기하 사본이 아직 둘 남았다.** 오도 경로는 정본 `trnav_2ws_core/config/robot_geometry_2ws.yaml` 주입으로 수렴시켰으나(ADR `2026-08-16-wheel-geometry-single-source`), ① `translate_sim_odom` 은 여전히 자기 `sim_params.yaml` 에 `w1_*`/`w2_*` 를 들고 있고 **QD SIL 런치도 같은 파일을 받는다**(즉 QD 플랜트가 2WS 기하로 돈다) ② `wheel_cmd_bridge.py` 는 정본과 갈린 `y=−0.0014` 를 코드 기본값으로 갖는다(런치가 정본을 주입하므로 실사용 경로는 정합되나 기본값 자체는 거짓) ③ `translate_sim_odom` 의 코드 기본값은 **QD 대각값**이라 파라미터 없이 띄우면 2WS 플랜트가 QD 기하로 돈다(현재 런치 19개는 전부 파라미터를 주므로 잠재 위험) | 2026-08-16 | **상환 완료(2026-08-18)** — ②③ 상환, ① 은 계획대로 QD 재이식 게이트로 이월. **기하를 공유 파일에서 걷어내고 런치가 플랫폼 정본을 얹게** 했다: `sim_params.yaml` · `sil_pose_adapter/config/crab_linear_sim_params.yaml`(**등록 시 누락됐던 세 번째 사본** — QD crab 런치도 이 2WS 값을 받고 있었다) 두 파일에서 `w1_*`/`w2_*` 제거, 런치 **19개** 전부에 `robot_geometry_2ws.yaml`/`robot_geometry_qd.yaml` 주입. 코드 기본값은 **제거**해 미주입을 기동 실패로 바꿨다(`translate_sim_odom_node.cpp` · `wheel_cmd_bridge.py` · `fleet_wheel_bridge.py` — 형제 `wheel_odometry.py` 와 같은 규약). 검증: 미주입 기동 → 종료코드 250 + 사유 로그 / 2WS 주입 → `w1=(0.604,0.000)` / QD 주입 → `w1=(0.330,0.135)` 실기동 확인. 신설 검사기 `Tools/motion_chain_check/check_sil_geometry.py` 가 런치 치환을 **실제 경로까지 풀어** 19개를 대조하며, 돌연변이 2종(플랫폼 뒤바꿈·주입 제거)을 검출한다. 기존 C6 도 불일치 0건. ⚠ 전수 조사에서 **컨트롤러 쪽 사본 20여 벌**이 드러나 `debt-109` 로 등록했다 | ① QD 는 LGIT 검증 후 재이식 예정이라 이 항목의 QD 부분은 그 시점 게이트로 넘긴다. ② 2WS 쪽은 `sim_params.yaml` 에서 기하를 걷어내고 런치가 정본 + 동특성 파일 둘을 얹는 형태로 바꾼다. ③ `wheel_cmd_bridge.py` 도 오도 노드와 같이 기본값 제거(미주입 시 기동 실패) |
| **debt-105** | 기술 | `src/Sim/trnav_2ws_description/CMakeLists.txt` | **패키지가 빌드되지 않는다** — `cmake_install.cmake:46` 에서 `src/Sim/trnav_2ws_description/launch` 를 설치하려는데 그 디렉터리가 없다(`file INSTALL cannot find`). 이 패키지에 의존하는 `trnav_2ws_gazebo` 가 함께 막혀 SIL 빌드 전체가 서지 않는다. 2026-08-16 휠 기하 작업과 무관한 선행 결함이며, 그 작업의 런치 수정을 실빌드로 검증하지 못한 이유다 | 2026-08-16 | **상환 완료(2026-08-17)** — `install(DIRECTORY …)` 목록에 없는 디렉터리(`launch`·`rviz`·`config`)가 들어 있어 죽던 것을, **존재하는 것만 설치**하도록 `foreach`+`IS_DIRECTORY` 로 바꿨다. 목록은 남겨 두어 나중에 만들면 그때부터 설치된다. 검증: `trnav_2ws_description` 빌드 성공(설치본에 `urdf/` 확인) → 이 패키지가 막고 있던 `trnav_2ws_gazebo` 도 빌드 성공. 소비자가 쓰는 것은 `urdf/foil_a082.urdf.xacro` 뿐임을 확인했다(`sim.launch.py`·`fleet.launch.py`) | `launch/` 를 만들거나 install 규칙에서 제외한다. 어느 쪽인지는 그 패키지 소유 세션이 판단 |
| **debt-106** | 기술 | `src/Navigation/mcl2d_ros2/src/mcl2d_localization_node.cpp` (원본 `ScanLostTimeThresh`·`OdoLostTimeThresh` 대비) | **입력 두절 워치독이 없다.** 원본 동작은 63G 원본 하드에서 확정했다(대조 문서 §8): 소비자는 `MCLoc::CheckScanUpdateTime()` 단 하나, `RunNew()` 에서 1회 호출되고 **반환값이 쓰이지 않는** 순수 부작용 함수다. 판정은 `thresh_ms × 1e6 ≥ now_ns − last_rx_ns` (기본 300 ms, `loadParam<int>` 기본값 `$0x12c`, 범위 [0,10000] — 설정 파일에는 이 키가 없다). 초과 시 하는 일은 **1회 로그 + `ErrorCodes::setError(0xcb86=52102)`** 가 전부이고 복귀 시 `clearError`, **발행 중단·상태 강등·재위치추정 유발은 하지 않는다** — 즉 **게이트가 아니라 워치독**이다. 오도 자매는 `OdoLostTimeThresh`(300 ms) → `52106` "odo data lost". ⚠ 한때 구현했으나 그 구현이 `/scan_front`+`/scan_rear` 두 콜백 구조(`session/5466b21a`) 위에 지어져 있었고, `main` 이 `dual_laser_merger` 병합 스캔 단일 구독으로 전환하면서 **되살리지 않고 폐기**했다. 분석은 남기고 코드는 버렸다 | 2026-08-16 | **상환 완료(2026-08-17)** — 병합 스캔 단일 구독 위에 워치독을 다시 얹었다. ① 판정은 ROS 무의존 순수 클래스 `mcl2d_ros2::LostWatchdog`(`include/mcl2d_ros2/lost_watchdog.hpp`) — 시각을 인자로 받아 시계 없이 시험한다. ② **발행을 막지 않는다** — 스캔 두절 중에도 `/mcl_pose` 가 19.999 Hz 로 계속 나가는 것을 실기동으로 확인. ③ 원본 에러 번호(52102·52106)를 `/diagnostics` key/value 로 보존. ④ 회귀 8항목 + **돌연변이 8/8 검출** (경계 완화·ns 환산 누락·진입판정 제거·회복 제거·클램프 제거·무수신 은폐·기본임계 변경·에러번호 훼손). 감시는 **타이머**가 돌린다 — 오도 콜백에 얹으면 오도가 끊긴 순간 감시도 멈춰 그 두절을 못 알린다. 실기동: 무입력→둘 다 ERROR(`no input received`), 오도 인가→OK + 회복 로그 1회, 오도 절단→`input lost`, 두절 로그는 진입당 1건(전 구간 2건) | ① 병합 스캔 노드 위에 워치독을 다시 얹는다 — 판정은 ROS 무의존 순수 함수로 두어 시계 없이 시험 가능하게 한다. ② **발행을 막지 않는다**(원본과 달라진다). ③ 우리에겐 `ErrorCodes` 버스가 없으므로 `/diagnostics`(DiagnosticArray)로 내보내고 원본 코드 번호를 key/value 로 보존한다. ④ 스캔을 인위로 끊는 회귀 + 돌연변이 확인 |
| **debt-107** | 이해 | 원본 `rbk/plugins/libRobotPosEKF.so` (`estimation::OdomEstimation`) | **위치추정이 소비하는 오도가 무엇인지 절반만 안다.** 배포 배선(`rbk/rbk.plugin`)상 `MCLoc` 은 `OdoCalculator` 의 휠 오도를 직접 받지 않고 **`RobotPosEKF` 가 오도와 IMU 를 융합해 재발행한 `Message_Odometer`** 를 받으며, 이 기체는 `RobotPosEKF.UseIMU = 1` 로 **융합이 켜져 있다**. 심볼로 확인된 것은 인터페이스뿐이다 — `addOdoMeasurement(Message_Odometer)` · `addImuMeasurement(Message_IMU)` · `update()` · `getFilterOdometer(Message_Odometer&)` · `angleOverflowCorrect(double&,double)`, 그리고 레이저 오도 경로(`setLaserOdom`/`isEnableLaserOdom`/`getTargetMsgLaser(Message_AllLasers)`). **부분 해소(2026-08-16)** — 역어셈블로 확정한 것: 상류 ROS `robot_pose_ekf` 이식본이고(BFL 클래스 실재), **상태벡터 6차원**(x,y,z,roll,pitch,yaw), 오도 관측 6×6·IMU 관측 3×3, 예측식 `x⁺=x+v·cosθ · y⁺=y+v·sinθ · θ⁺=θ+ω`(z·roll·pitch 는 예측 불변), 야코비안 비영 항은 yaw 열뿐(`F(1,6)=−v·sinθ`, `F(2,6)=+v·cosθ`). 상류와 다른 점: `update()` 가 **무인자**로 재작성됐고 **VO·GPS 융합은 멤버째 삭제**, 잡음 σ 를 `update()` 안에서 `AdditiveNoiseSigmaSet` 2회로 세팅하며, Seer 가 `double wzOdoAbsDeg` 멤버를 추가했다. 상세: `Tools/seer_re/docs/legacy_runtime_wiring.md` 부록 A. **추가 해소(2026-08-16 2차)**: ① **시스템 잡음 σ = 1000**(분산 1e6, 6축 동일, 생성자 `movabs 0x412e848000000000`) — 상류 `sysNoise_Cov(i,i)=pow(1000,2)` 와 같다. ② **`wzOdoAbsDeg` 확정** — `addOdoMeasurement` 이 `|Rad2Deg(odom.vel_rotate)|` 를 저장하고(`andps` 절대값 마스크 실측), `update()` 가 `m_imu_init && wzOdoAbsDeg > 1.0 deg/s` 일 때만 **IMU 측정 갱신을 수행**한다 — 정지·직진 중 자이로 바이어스 적분을 막는 Seer 고유 게이트이며 임계 1.0 은 **하드코딩**(파라미터 없음). ③ **IMU 장착·보정값 출처** = `robot.model` + `rbk::chasis::Calibration`(robot.param 아님). ④ **레이저 오도**: `rf2o::LaserOdometry2D` 가 플러그인에 `[존재]` 하나 `MCLoc.StartLaserOdo=0` · `LaserOdomDetectSkid=0` 로 **`[동작]` 비활성**. **3차 해소(2026-08-16)**: ⑤ **prior 공분산 = 1e-6**(σ 0.001, 6×6 전량 채움, `initialize()` 유일 즉치 `0x3eb0c6f7a0b5ed8d`) — 상류 `pow(0.001,2)` 와 동일. **첫 오도 메시지가 `initialize()` 를 부른다**. ⑥ `update()` 의 `AdditiveNoiseSigmaSet` 2회 인자 확정 — `odom_meas_pdf_`(멤버 0x18)에 `odom_covariance_`(0x48), `imu_meas_pdf_`(0x28)에 `imu_covariance_`(0x70)를 **멤버 주소 그대로** 넘긴다. ⚠ 상류의 `* pow(dt,2)` 스케일링에 해당하는 임시 객체가 없다 — **dt² 스케일링 부재**로 보이나 단정하지 않는다. **4차 해소(2026-08-16)**: ⑦ **관측행렬 확정** — 전 라이브러리에서 행렬 원소를 쓰는 함수는 `initialize()`(36회)와 생성자(33회) **둘뿐**이라 관측 모델은 생성자 상수다. `Hodom`(6×6): (1,1)·(2,2)·(6,6)=1 ⇒ **오도는 (x, y, yaw)만 관측**. `Himu`(3×6): (1,4)·(2,5)·(3,6)=1 ⇒ **IMU 는 (roll, pitch, yaw) 관측**. 값 1.0 은 `movabs $0x3ff0000000000000,%r13` 로 확인. 오도 관측잡음 6×6 대각=1.0, IMU 관측잡음 3×3 대각=1.0. ⇒ 두 관측이 겹치는 축은 **yaw 하나**이고 §A.6 의 `|ω|>1 deg/s` 게이트가 그 경합을 조절한다. ⑧ **관측 잡음 멤버가 채워지지 않는다** `[존재]` — 생성자는 `odom_covariance_`(0x48)에 `SymmetricMatrix(6)`, `imu_covariance_`(0x70)에 `SymmetricMatrix(3)` 로 **크기만** 잡고, 라이브러리 전량 조사에서 값을 넣는 경로가 없다(원소 쓰기는 `initialize()`·생성자의 **스택 지역변수** 대상뿐, `SymmetricMatrix::operator=(const&)` 호출 **0건**, `update()` 의 memmove 2회는 `*_meas_old_ ← *_meas_`). 그런데 `update()` 는 매 주기 그 멤버를 `AdditiveNoiseSigmaSet` 으로 넘긴다. **상류 `robot_pose_ekf` 는 이 자리를 메시지 공분산으로 채우는데 Seer 판에는 그 대입이 없다.** ⚠ 런타임 실효는 `[동작-미검증]` — `SymmetricMatrix(int)` 가 0으로 채우는지 미초기화인지에 갈린다. ⑨ **그 생성자는 값을 초기화하지 않는다** `[존재]` — `SymmetricMatrix::SymmetricMatrix(int)` @0x180660 은 24 명령 전문에 `memset`·0채움 루프가 없고 `operator new` 버퍼를 그대로 쓴다. ⇒ 두 공분산 멤버는 **초기화되지 않은 힙 메모리**를 담은 채 매 주기 측정모델에 들어간다. ⚠ 런타임 실제 값은 `[동작-미검증]`(초기 할당이면 커널 0페이지, 재사용 힙이면 임의값) — 실행 대조 전까지 「0이다」도 「쓰레기값이다」도 단정하지 않는다. ⑩ **IMU 는 선택이 아니라 필수 전제** `[존재]` — 필터를 구동하는 것은 `RobotPosEKF::run()` 뿐이고(addOdo/addImu/update/getFilterOdometer 전부 여기서 호출), 주루프가 **오도 수신 플래그(0x338)와 IMU 수신 플래그(0x339)가 둘 다 참일 때만** 진행한다(아니면 `nanosleep`). 0x338 은 매 반복 지워지는 「새 오도」 플래그지만 **0x339 는 생성자에서만 0이 되는 래치**라 「IMU 를 한 번이라도 봤는가」다. ⇒ **IMU 가 한 번도 오지 않으면 EKF 는 아무것도 발행하지 않고, `MCLoc` 은 이동량 입력을 전혀 받지 못한다.** ⑪ `UseIMU` 값(`RobotPosEKF+0x3b0`)을 **읽는 지점을 못 찾았다** — 「죽은 파라미터」로 단정하지 않는다(부재 증명 아님). ⑫ **`update()` 2회 호출 구조 해소** — `run()` 의 3연속 호출 블록 두 개는 성격이 다르다: ①`+637~+753` 은 뒤에 `Time::init()`·`VelocityEstimatorIMU()` 생성이 오고 `getFilterOdometer` 가 없어 **발행하지 않는 초기화 1회**, ②`+12381~+12535` 는 `getFilterOdometer()` 로 결과를 꺼내는 **주기 경로**다. ⑬ **곁가지 구성요소** — 같은 플러그인에 `VelocityEstimatorIMU`(IMU 기반 속도 추정, `LowPassFilter` 보유)가 있고 `run()` 이 실제로 쓴다 `[동작]` ⇒ IMU 는 자세 관측만이 아니라 **속도 추정에도** 쓰인다. `SkidDetector`(오버로드 2종 + 임계 설정자 4종)도 `run()` 에 호출지가 있으나 배포 `StartSkidDetection=0` 이라 실행 여부는 `[동작-미검증]`. `VarianceCalculator::cal` 3회의 결과는 스택 지역으로 가며 **EKF 공분산과 무관**하다. ⚠ **정정(9차 서술)**: `VelocityEstimatorIMU` 를 「run() 이 실제로 쓴다」고 적었으나 **틀렸다** — 호출은 일어나지만 출력 멤버 `0xdc0`·`0xdc8`·`0xdd0` 은 라이브러리 전량에서 **저장 6건 / 적재 0건**의 write-only 이고 속도 접근자도 없다. **계산은 돌되 결과는 소비되지 않는다.** ⑭ `SkidDetector::Update` 오버로드 선택은 멤버 `0xd50` bit0 (`test $0x1,%al`) 로 갈린다(세워지면 레이저 포함). 바깥 게이트(`StartSkidDetection` 소비 지점)는 특정 못 했다. ⑮ **슬립 감지 바깥 게이트 = 타임스탬프 신선도** — `SkidDetector` 블록 진입 조건은 오도·IMU 헤더의 `+0x20`(타임스탬프)이 직전보다 새로운가 두 번 검사하는 것뿐이고, `StartSkidDetection` 을 읽지 않는다. ⑯ **`run()` 진단 2건**: `setFatal(50400)`="gyro original data error"(**fatal**) · `setWarning/clearWarning(54300)`="imu noise is too large". 둘 다 `OpenIMUErrorDetect`(0x7f0) 게이트 아래이고 코드가 그 값을 **실제로 읽으므로** 배포값 0 → **`[동작]` 비활성 확정**(임계 3.0/20.0/10 미적용). ⑰ **슬립 판정 결과는 로그로만 나간다** — `Update` 반환값(`%ebx`)이 CalibStatus 플래그와 `and %bl,%al` 로 결합돼 분기하고, 참일 때 블록은 `stringstream`→`Logger::thread` **로깅뿐**이다(그 구간에 `setError`/`setWarning`/`setFatal`·상태 쓰기 없음). `Message_Odometer::set_detect_skid` 는 **호출 명령 0건**이라 발행 메시지의 `detect_skid` 도 세우지 않는다. ⇒ `StartSkidDetection` 을 읽지 않아 「끄지 못한다」던 우려는 **무해**하다 — 켜져 있어도 로그뿐이다. (측위 쪽 `MCLoc::CheckWheelSkid` → `setError(52718)` 은 별개 기구다.) ⑱ **미초기화 공분산은 실제로 필터에 도달한다** — `AdditiveNoiseSigmaSet` 은 `memmove` tail-call 로 인자 버퍼를 통째 복사한다(크기만 쓰는 것이 아니다). ⑲ **`UseIMU`·`StartSkidDetection` 미소비 근거 보강** — `MutableParam` 에 값 접근자(`operator T`/`get`)가 **없어** 값은 멤버 직접 접근으로만 읽히며, 적재 직후 다른 멤버로 복사하지도 않는다. 대조군 11개와 합쳐 근거가 강하나 인라인 가능성 때문에 `[존재]` 수준으로 둔다. ⑳ **임계 1.0 deg/s 는 근거 확인 불가** — `robot.param` 전 테이블·바이너리 문자열에 대응 항목 없음(하드코딩). ㉑ **EKF 입출력 100배 스케일은 대칭** — `addOdoMeasurement` 가 x·y 를 ÷100, `getFilterOdometer` 가 ×100 (각도는 양쪽 무배율). proto 는 `// m` 이므로 **발행 단위는 미터로 보존**되고 내부 상태만 1/100 스케일이다. 발행 메시지의 속도·`is_stop`·`motor_info` 는 `run()` 의 `CopyFrom` 으로 **원본 오도 값**이 실린다. **남은 미조사**: 두 공분산 버퍼의 **런타임 값**(정적 분석 한계 — 실기·원본 구동 대조 필요) · `VelocityEstimatorIMU` 가 버려지는 이유 · 내부 1/100 스케일의 이유 · `UseIMU=0` 경로 · 슬립 감지 비활성 사유 · IMU 이상감지 활성 시 거동 · 임계 1.0 deg/s 의 근거. 이 공백 때문에 「Seer 오도는 엔코더만 쓴다」는 서술이 한 번 틀렸다(`docs/claude-mistake/2026-08-16-001`) | 2026-08-16 | **부분 상환(2026-08-18)** — 상환계획 ②③ 상환, ①은 이미 상환(2026-08-16 역어셈블), ④는 이행 중. **② IMU 파라미터·축 규약 확정**(§A.28): `ReadIMUParam()` 은 `robot.param` 이 아니라 **`robot.model` 의 `controller` 디바이스**를 읽는다(`Model::getModelDevices("controller")` → `getModelParam<double>` + `Calibration::Instance()`). 키는 `Bax`/`Bay`/`Baz`(가속도계 바이어스) · `x`/`y`/`z`(장착 위치) · `qx`/`qy`/`qz`/`qw`(장착 자세) · `SSF`(자이로 감도). 배포값은 바이어스 0 · 위치 (0,0,0) · **단위 쿼터니언** ⇒ **IMU 프레임 = 차체 프레임, 축 교환도 부호 반전도 없다.** ⚠ 그 배포값은 **`Roll_A084`** 것이라(우리 기체는 `Foil_A082`) 값이 아니라 규약만 참고 대상이고, `controller` 디바이스는 `isEnabled: false` 다. 파일 실위치도 정정 — `/usr/local/etc/.SeerRobotics/rbk/resources/models/robot.model`(`rbk/` 아래가 아니다). **③ `UseIMU=0` 경로 확정**(§A.29): `robot.param` 의 `RobotPosEKF` 테이블에 `UseIMU b 1 Mutable=0` 로 실재하나 **이 플러그인은 그 이름 문자열을 참조하지 않는다** — 대조군 실험에서 형제 키 6개는 전부 정확히 1건씩 `lea` 참조되는데 `UseIMU` 만 0건이고, **Mutable=0 인 유일한 키**라는 점과 정합한다. ⇒ 끄는 스위치가 아니며 §A.20(주루프가 IMU 래치를 무조건 대기)과 합쳐 **IMU 는 전제**다. ⚠ 함께 돌린 「데이터 섹션 포인터」 검사는 세 키 모두 0건이라 **판별력이 없어 근거로 쓰지 않는다** — 그 사실도 문서에 남겼다. **존치**: 두 공분산 버퍼의 **런타임 값**은 여전히 정적 분석 한계다(실기 또는 원본 구동 대조 필요) · `UseIMU` 를 **다른 바이너리가** 읽는지는 미조사 | ① `estimation::OdomEstimation::update()` 를 역어셈블해 상태벡터·예측/갱신식·공분산 초기값을 확정한다. ② `ReadIMUParam()` 이 읽는 파라미터와 IMU 축·부호 규약을 배포값과 함께 확정한다. ③ `UseIMU=0` 경로가 통과인지 별도 경로인지 확인한다. ④ 그 전까지 「위치추정 입력 = 휠 오도」로 서술하지 않는다 — 배선 정본은 `Tools/seer_re/docs/legacy_runtime_wiring.md` |
| **debt-108** | 의도 | `src/Navigation/mcl2d_ros2/launch/bringup.launch.py` ↔ 신설 `odom_imu_ekf` | **융합 노드를 만들었으나 측위 입력에 배선하지 않았다.** 레거시는 `MCLoc` 이 `RobotPosEKF` 의 융합 오도를 받는데(배선 정본 §A.1), 우리 `bringup` 은 아직 `icp_odometry` 오도를 `mcl2d` 에 바로 물린다. 배선을 바꾸면 **라이브 스택 거동이 바뀌고**, 그 런치는 다른 세션이 방금 재작성해(loc 스택 단일 진입점화) 동시 수정 시 충돌 위험이 있다. 그래서 노드만 추가하고 배선은 남겼다 | 2026-08-17 | **부분 상환(2026-08-17)** — ①② 상환, ③ 존치. `bringup.launch.py` 에 `imu_fusion` 인자(기본 **false**)를 넣어 켤 때만 융합기가 끼워지고 측위 입력이 `/odom_fused` 로 바뀌게 했다. 청정 도메인 실기동으로 양쪽 다 확인 — `false`: 측위가 `/odom` 구독·융합기 부재 / `true`: `/odom`(구독 1=융합기) → `/odom_fused`(발행 1·구독 1=측위), 중복 노드 없음. 실패 모드는 런치 머리말과 인자 설명에 명시했다(IMU 미수신 시 융합기 무발행 → 측위가 이동량 상실). **존치(③)**: 융합 입력이 레거시의 휠 오도가 아니라 **ICP 오도**다 — 휠 오도는 `debt-004`·`debt-007`(조향 부호·원점) 확정 전까지 신뢰할 수 없다. 기본값을 true 로 돌리는 것은 **실기 검증 후** 결정 | ① `bringup` 에 `use_imu_fusion` 인자를 넣어 `odom_imu_ekf` 를 끼우고 `mcl2d` 입력을 `/odom_fused` 로 돌린다(기본값은 현행 유지 — 켤 때만 경로가 바뀌게). ② IMU 미수신 시 측위가 이동량을 전혀 못 받으므로 런치 문서에 그 실패 모드를 명시한다. ③ 휠 오도를 소스로 쓸 경우 `debt-004`·`debt-007`(조향 부호·원점) 선결 |
| **debt-109** | 기술 | `src/Control/Motion_Control/{2WS,QD}/*/config/*_params.yaml` (기하 20여 사본) · 각 액션서버 `safeParam("w1_x", …)` 코드 기본값 · `Tools/motion_chain_check/check_chain_contract.py:378` | **컨트롤러 쪽 휠 기하 사본이 20여 벌 남았다.** `debt-104`(SIL 플랜트) 를 상환하며 전수 조사한 결과, 기하는 플랜트뿐 아니라 **컨트롤러 params YAML 전량**(2WS 11개 · QD 9개)에 복제돼 있고 각 액션서버 코드도 `safeParam("w1_x", 0.6039)` / `("w1_x", 0.330)` 형태로 기본값을 들고 있다. 검사기 C6 이 YAML 사본을 정본과 상시 대조하므로 **드리프트는 잡히나**, ① 코드 기본값은 C6 대상이 아니어서 파라미터 미주입 시 조용히 옛 값으로 돌고 ② `check_chain_contract.py:378` 의 시험 픽스처 `_BASE_GEOM` 은 아직 `w1_y: -0.0014`(폐기된 실측 잔차)를 쓴다. 사본 자체가 20여 벌인 것이 근본 부담이다 | 2026-08-18 | 미해결 | ① 액션서버 기본값을 걷어내고 미주입은 기동 실패로 바꾼다(SIL 플랜트·`wheel_odometry.py` 와 같은 규약). ② params YAML 의 기하를 걷어내고 런치가 정본을 얹게 한다 — `debt-104` 에서 SIL 에 적용한 형태를 컨트롤러로 확장한다. ③ `_BASE_GEOM` 픽스처를 정본 값으로 맞춘다. ④ QD 쪽은 LGIT 검증 후 재이식 예정이므로 그 시점 게이트로 넘긴다(`debt-104` ①과 같은 이유) |
| **debt-126** | 기술 | `src/Actuators/motor_control/motor_control/driver_node.py:161-163` · `kinematics.py:152-190` | **생산 휠 오도가 레거시 산식과 세 곳에서 다르다.** 레거시 산식은 원본 바이너리와 **비트 대조를 마쳤고**(비교 문서 §15·§16), 그 기준으로 재면: ① **자세 누적 규칙** — 레거시는 각을 먼저 갱신하고 **그 각으로** 회전(end-point)하는데 우리는 갱신 전 각으로 회전한다. 100 m 주행 실측 차이는 1.0 m/s 에서 6.6 cm, 1.5 m/s 에서 9.9 cm(스텝 크기에 비례, yaw 차이는 0). ② **보정항 `cpx`/`cpy` 부재** — 레거시는 `(x+cpx, y+cpy)` 를 유효 휠 위치로 쓰는데 우리는 설계값만 쓴다. 크기는 이 기체 보정값 미확보라 **미확인**. ③ 선형 해를 닫힌형으로 전개(수학 동일, 연산 순서만 다름). 측위는 오도 **증분만** 쓰므로(§8.1) 실사용 영향은 제한적이나, **오도를 궤적으로 쓰는 소비자**(기록·재생·오프라인 분석)에는 그대로 남는다 | 2026-08-25 | 미해결 | ① 누적 규칙을 end-point 로 맞춘다 — 한 줄 변경이고 검증된 기준자(`seer_odom_core`)가 있다. ② 이 기체 `cpx`/`cpy` 를 확보해 크기를 재고, 유의하면 기하 정본에 넣는다. ③ 연산 순서는 실용 영향이 없으므로 그대로 둔다 — 비트 대조가 필요하면 `seer_odom_core` 를 쓴다. ④ 대조 도구: `Tools/seer_re/odo_oracle/compare_production.py` |

---

## [2026-08-15 19:40 정정·계층 결정] debt-086 — 「반대 방향으로 돌아 한계에 갇혔다」 가설 철회

### 철회 — 조향 지령 공간에는 wrap 이 없다

`debt-086` 에 실린 가설(목표를 반대 방향으로 돌아 ±180° 쪽으로 가려다 한계에 걸림)은
**modulo 공간을 전제**하는데, 실측 상수가 그 전제와 맞지 않는다.

| 항목 | 값 |
| --- | --- |
| 조향 1회전 | `360 × 57,344` = **20,643,840 counts** |
| 가동범위(±115°) 폭 | **13,189,120 counts** = 1회전의 **64 %** |
| node3 범위 | 1,277,255 ~ 14,466,375 (전부 양수) |
| node4 범위 | 1,245,526 ~ 14,434,646 (전부 양수) |

전 가동범위가 **1회전 안에 들어가고 음수가 없다.** 즉 `0x607A` 목표는 각도(mod 360)가 아니라
**선형 다회전 절대 counts** 다. `+110° → −110°` 는 `14,147,926 → 1,532,246` 으로 counts 가
**단조 감소**해 0° 를 지나갈 뿐, 최단경로 선택도 wrap 도 발생하지 않는다.
⇒ **「반대로 돌았다」로 137° 갇힘을 설명하지 않는다.** 관측(`DI` bit3 = −리밋 ON · `0x6064`=0)은
**호밍이 리밋을 찾은 뒤 `GOZERO` 복귀를 못 한 상태**와 정합하며, 그쪽이 남은 후보다.

### 계층 결정 (사용자, 2026-08-15) — 등가각은 운영 계층 소관

> 「이 부분은 운영쪽에서 할 내용입니다. CAN NODE 는 받아서 움직이면 됩니다」

**`can_relay` 는 raw 실행 계층이다.** 등가 2해 `(θ,+v) ≡ (θ∓180°,−v)` 중 무엇을 고를지,
목표가 한계를 넘는지 보고 방향을 정하는 일은 **모션·기구학 계층이 소유**한다.
`can_relay` 는 받은 각도를 클램프해 절대 counts 로 내보내는 것까지만 한다.

이 결정에는 기술적 근거도 있다 — **조향 단독 지령 경로에는 등가각을 넣을 수 없다.**
`~/steer_deg`·`~/steer_axis_deg` 는 조향만 받으므로 등가각을 고르면 **함께 뒤집어야 할 구동
부호를 건드릴 수단이 없다.** 조향만 ±180° 바꾸면 로봇이 반대로 간다.

상류에는 이미 있다 — `trnav_2ws_kinematics/src/qd_inverse_kinematics.cpp` 의 `normalizeAngle`
이 `[−90°,+90°]` 정규화로 등가 2해를 유일해로 확정하고 방향 부호를 함께 낸다.
따라서 **모션 스택 경로는 이미 최소 조향각을 고른다.** 남는 것은 IK 를 지나지 않는 경로
(`~/steer_deg` 벤치 · GUI 슬라이더 · `home_and_zero`)이며, 위 결정에 따라 **그 경로에
등가각을 넣지 않는다** — 그 경로들은 사람이 각도를 직접 지정하는 시험·운용 수단이다.

### 남는 확인 (선택)

`0x60F2`(위치결정 옵션코드)를 저장소가 한 번도 쓰지 않는다(전체 grep 0건). 위 counts 근거로
선형 동작이 강하게 시사되나 **드라이브 설정을 직접 읽어 확인한 적은 없다.** 읽기 1회면 끝나며
축은 움직이지 않는다 — 필요해지면 그때 확인한다.

---

## [2026-08-08 등록] Seer 지도생성 이식(`slam_karto_core`) — 신규 부채 10건

> 등록 당시 번호는 `debt-050`~`debt-059` 였다. main 병합 시 같은 번호가 다른 부채에
> 이미 배정돼 있어 채번 규약 §3(나중 등록을 개번)에 따라 **`debt-090`~`debt-099` 로 옮겼다.**
> 저장소 전체 참조 30건을 전수 갱신했다.

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-090 | 이해 | `src/Navigation/slam_karto_core/include/slam_karto_core/seer_mapper_config.hpp` (`scan_buffer_size`·uint/bool 6개) | **★ Seer 튜닝 5개 중 `ScanBufferSize` 만 재현되지 않았다.** 두 독립 디스어셈블(A1·A2)이 `double` 즉치만 뽑고 `Parameter<unsigned int>`·`Parameter<bool>` 은 건드리지 않아, 근거가 RE 문서 `2026-07-10.md §4` 의 교차검증 진술뿐이었다 | 2026-08-08 | **해결 (2026-08-08)** | **디스어셈블로 전량 확정 — 28개 파라미터 추출 실패 0건.** 방법: 블록 경계(`mov $0x50,%edi; call operator new` 정확히 28회) + std::string SSO 이름 길이 즉치 + 값 슬롯(`0x48(obj)` 스토어 28회, 1:1) + DWARF decodedline(줄 1842~2046 단조 증가) 4중 교차. 결과: **`ScanBufferSize=100`**(`e2136: movl $0x64,0x48(%r15)`, 상류 70) = **Seer 튜닝 확정** — RE 문서 진술이 옳았다. `LoopMatchMinimumChainSize=10`·`DoLoopClosing=true`·`UseScanMatching=true`·`UseScanBarycenter=true`·`UseResponseExpansion=false` 5개는 상류 stock 일치. 부수 해소 2건: ① `LoopSearchSpaceSmearDeviation`=0.03(stock) — 즉치가 아니라 `%r15` 재사용(`e2e27`→`e3171`, 구간 내 쓰기 0건)이라 이전 조사가 놓쳤고, **0.09 는 `DistanceVariancePenalty`** 였다 ② **`MinimumTimeInterval` 은 원본에 파라미터 자체가 없다**(블록 28 vs 상류 29, 바이트열 검색 0회, DWARF 멤버명 0회) → 신규 debt-096 로 분리. 판본 교차검증: 3.4.5.20 ≡ 3.4.8, 명령 1,925개 **차이 0건** |
| debt-091 | 이해 | `src/Navigation/slam_karto_core/include/slam_karto_core/seer_slam_mapper.hpp` (`rssi_threshold_`) | **원본 `RssiThres` 런타임값이 미확정인데 코드에 값이 들어가 있었다.** RE 문서가 「미확정」으로 남긴 항목이고, 이식본 기본값 0.0 은 구현자 선택이었다 | 2026-08-08 | **해결 (2026-08-08)** | **역어셈블로 확정 — `RssiThres = 150.0`.** `SlaMapping::run()` 이 매 메시지마다 리터럴로 기록한다: `7048b: movabs $0x4062c00000000000,%rax` / `70495: mov %rax,0x38(%rsi)` (SlaMapping.cpp:91). `KartoSLAM` 생성자는 이 필드를 **초기화하지 않으므로**(raw `operator new(0x40)`) `run()` 이 유일 출처다. 소비처도 확인: `ca5bc: cmpltsd` — `RssiThres < rssi[i]` 이면 100.0, 아니면 0.0 으로 빔을 이진화한다(KartoSLAM.cpp:112-115). 교차 확인: 위치추정 쪽 `robot.param` 의 `MCLoc.ReflectorRSSI` 도 **150.0**. 코드 반영 완료(`seer_runtime::kRssiThreshold`), 시험 픽스처의 합성 rssi 도 실측 규모(기저 50.0/반사판 200.0)로 교체 |
| debt-092 | 기술 | `src/Navigation/slam_karto_core/` (지도 산출 경로) | **⚠ 2026-08-09 재분류 — '후처리 미구현'이 아니라 '산출 경로 자체가 다름'이다.** 원본 `KartoSLAM::SaveMap`(`0xccc80`)의 호출 목록 전체에 `GetPointReadings` 가 **0건**이고, `OccupancyGrid::ComputeBoundingBox`(`0xcce29`) → `OccupancyGrid(w,h,offset,res)` 생성(`0xcced8`) → 셀 순회로 `vMapPoint`/`vRssi` 를 채운다. `ComputeBoundingBox` 반환이 `vector<int>&`·`vector<Vector2<double>>&` **복수형**이라 서브맵 여러 개로 쪼갠다. 즉 원본 지도 점군은 **점유격자 셀**에서 나오고, 우리는 스캔 점 판독을 그대로 낸다. 부수로 `HTLine`(Hough 벽각 보정)·`mapAngle` 도 없다. 현재 `buildMap()` 은 `GetPointReadings(true)`(필터본)를 쓰는데 이는 **오라클의 선택에 맞춘 것**이지 원본 `.smap` 산출 경로가 아니다 | 2026-08-08 | 미해결 | ① `OccupancyGrid` 래스터화(해상도 0.02 m)를 구현해 원본과 같은 계층에서 점군을 뽑는다. ② `ComputeBoundingBox` 의 서브맵 분할 규칙을 역어셈블로 복원한다(`0xcf270`, 상류 0건 = Seer 추가). ③ 벽각 보정(`HTLine`+`mapAngle`)을 붙인다 — 없으면 우리 지도가 원본 대비 회전한 채로 나온다. ④ 그 뒤에야 회수한 원본 `.smap` 15개와 **지도 수준** 대조가 가능해진다(현재는 `Mapper` 계층까지만 대조됨) |
| debt-093 | 기술 | `src/Navigation/slam_karto_core/test/mutation_check.py` (도달 불가 4건) | **합성 궤적으로는 검증할 수 없는 경로가 남았다.** 돌연변이 10/10 이 검출되지만 다음 4건은 시험 입력이 그 코드 경로를 밟지 않아 미검출이다(각 항목에 실측 근거 기재): ① `seer-tuning-response-coarse` — 합성 스캔은 상관 응답이 사실상 1.0 이라 게이트를 상류값으로 조여도 솔버 계측이 완전 동일 ② `information-identity` ③ `block-ordering` ④ `max-iterations`. **즉 ★ 튜닝 파라미터의 실제 효과가 ctest 회귀로 고정돼 있지 않다** | 2026-08-08 | **부분 상환 (2026-08-09)** | **상환된 것**: 실 로그 재생·대조 도구가 생겼다 — 원본 `.so` 직접 구동 오라클(amap-server) + 우리 재생기 `Tools/seer_rawmap/replay/`(`replay_ours` + `compare.py`). 26개 rawmap 전량의 오라클 출력이 `References/seer/slam_mapping/oracle/` 에 있다. 이 대조가 정적 분석이 못 잡은 결함 4건을 실제로 잡아냈다. **남은 것**: ① 그 재생·대조를 **ctest 에 회귀로 등록**해야 위 4건이 자동으로 잠긴다(현재는 수동 실행). ② 오라클 산출물은 저장소 미추적(`References/`)이라 재현에 amap-server 가 필요하다 — 대조 기준 출력만이라도 추적 가능한 곳에 둘지 결정한다. ③ 26개 중 1개(기준 파일)만 대조했다 — 나머지 25개로 넓힌다 |
| debt-094 | 이해 | 조사 결론 — 원본 지도 생성 주체 | **「Seer 지도는 온보드 Karto+g2o 가 만든다」가 확정되지 않았다.** 알고리즘 정체(`libSlaMapping.so` = open_karto + g2o)는 DWARF 소스경로로 확정됐으나(`/root/workspace/3.4.5.20/plugins/SlaMapping/src/./karto/{Karto,Mapper,KartoSLAM,g2o_solver}.cpp`), **실기 로그는 다른 경로를 보여준다** — 매핑 명령 `6100`/`6101`(포트 19210)은 `OnlineMapLogger` 로 라우팅되고 완성 `.smap` 은 PC 툴(Roboshop)이 만들어 `4010` 으로 업로드한다(2024-05-31 Foil AGV 세션 전 구간 로그 실측). 열람한 로그 파일들의 플러그인 태그 전수에 SlaMapping 태그가 없었다. 즉 온보드 경로는 `[존재]` 이고, 이 장비의 열람 범위에서 `[동작]` 근거는 얻지 못했다. **PC 측 알고리즘은 조사하지 않았다** — 우리 이식본이 재현 대상으로 삼는 것이 실제 지도 생산자와 다를 수 있다 | 2026-08-08 | 미해결 | ① `diagnosis/log/` 1,564개 `.log.gz` 전수에서 SlaMapping 태그·`SLAM start`·`finish map` 문자열을 grep 해 **온보드 경로가 쓰인 세션이 하나라도 있는지** 확정한다(현재는 2개 세션만 열람). ② `{"real_time":false}` 경로의 동작을 확인한다 — 그때는 로봇이 스스로 만들 가능성이 있다. ③ 그래도 미확정이면 「우리는 원본 *알고리즘* 을 재현하며, 원본 *지도 생산 경로* 와 동일하다는 근거는 없다」를 README·ADR 에 명시적 한계로 남긴다(현재 `slam_karto_core/README.md` §알려진 격차 5 에 기록됨) |
| debt-095 | 기술 | `References/Seer-Driver/robokit_tcp_api.md` (명령 코드 4건 오류) | **저장소 참조 문서의 Seer TCP API 명령 코드가 실기 펌웨어(3.4.5.22)와 어긋난다.** 정본은 원본 하드의 `Roboshop.db`(SQLite, `SCProtocol` 518행) + 실기 로그다. ① 스캔 시작 = 문서 `2022 robot_control_scan_req` → 실제 **`6100 robot_other_slam_req` @19210** ② 스캔 정지 = 문서 `2023` → 실제 **`6101` @19210** ③ 문서의 `2022` 는 실제로 **`robot_control_loadmap_req`**(지도 로드 전환) ④ 문서의 `4001/4002 = setparams/saveparams` 는 실제 **`4100/4101`** 이고 `4001/4002` 는 조종권 회수/이양. 원인은 동봉 PDF v1.2.1 자체에 있다 — 구 `2020/2021` 이 `(DEPRECATED)` 로 표기돼 있고 신 `6100/6101` 은 PDF 에 0건(`grep -n "610[01]"` → 0). 이 문서를 근거로 매핑을 자동화하면 엉뚱한 명령을 보낸다 | 2026-08-08 | 미해결 | ① `robokit_tcp_api.md` 의 4건을 정정하고, 각 항목에 **근거 출처**(`Roboshop.db` 테이블 + 실기 로그 인용)를 붙인다. ② PDF 기반 서술과 실기 확인 서술을 문서 안에서 등급 분리한다(현재 섞여 있다). ③ `4010 uploadmap`·`4011 downloadmap`·`1780 rawmaplist` 등 문서에 없는 신규 항목을 추가한다 ✅ **2026-09-02 ④ 상환** — `robokit_tcp_api.md:171-174` 의 4001/4002/4003 → **4100/4101/4102** 정정, `4300 clearfatal` 추가(중복 행 제거). **그리고 이 부채가 예고한 사고가 실제로 났다** — 2026-08-18 C++ 재구현이 이 표를 그대로 물려받아 설정 쓰기 4종을 틀린 편호로 내보냈고, 2026-09-02 실기 왕복 시험에서야 드러났다(debt-126). 본 행은 2026-08-08 에 이미 정답(4100/4101)을 적어 뒀다 — **등록된 부채를 구현 전에 읽지 않은 것이 원인이다.** ✅ **2026-09-02 ①③ 상환** — `2022`→`robot_control_loadmap_req`(지도 전환)·`2023`→`robot_control_loadmapobj_req` 정정, `4010 uploadmap`·`4011 downloadmap` 행 추가, 「스캔은 6100/6101@19210」 경고 삽입. **2022 = 지도 전환은 실기로 확인했다** — 4010 업로드 직후 2022 를 보내 `1300 current_map_md5` 가 새 값(`0319831d…`)으로 바뀌고 `1022 loadmap_status=1` 유지. ⚠ 남은 것: ②(PDF 기반 서술 ↔ 실기 확인 서술의 등급 분리) 및 본 행이 말하는 「`4001/4002` 는 조종권 회수/이양」의 확인 — 우리는 4005/4006 으로 조종권을 실기 검증했으므로 두 계열의 관계가 미확인이다 |
| debt-096 | 이해 | `src/Navigation/slam_karto_core/include/slam_karto_core/seer_mapper_config.hpp` (`minimum_time_interval = 3600.0`) | **원본에 대응 파라미터가 없는 값을 이식본이 설정한다.** Seer 의 `Mapper::InitializeParameters()` 는 상류 29개 중 `MinimumTimeInterval` 을 뺀 **28개만** 등록한다 — 근거 3중: ① 파라미터 블록 전수 28개 ② `.so` 18,437,896 B 전체 바이트열 검색에서 `"MinimumTimeInterval"` **0회**(대조군 `"MinimumTravelDistance"` 14회 검출) ③ DWARF 멤버명 0회. 3.4.8 판본도 동일. 값 3600.0 이 상류 기본값과 같아 **거동은 중립**이지만, 원본 충실 항목이 아닌 것이 충실 항목처럼 표에 섞여 있으면 다음 작업자가 이 값을 근거 있는 것으로 오인한다(리뷰 Medium 으로 지적됐던 항목이며, 실제로는 '값 미실측'이 아니라 '파라미터 부재'였다) | 2026-08-08 | 미해결 | ① Seer 가 시간 기반 스캔 게이팅을 **다른 곳에서** 하는지 확인한다 — `Mapper::Process` 는 이번 조사 범위 밖이었다(조사 구간 `0xe1c50`–`0xe3c5b` = `InitializeParameters` 전체). ② 하지 않는다면 우리도 `setParamMinimumTimeInterval` 호출을 빼서 상류 기본값에 맡기고, 코드에서 이 필드를 제거한다. ③ 한다면 그 값을 실측해 넣는다 |
| debt-097 | 기술 | `src/Navigation/slam_karto_core/third_party/open_karto/` (무개조 상류 동봉) | **Seer 의 Karto 는 RSSI(반사강도) 채널이 관통 배선된 포크이며, 우리는 무개조 상류를 동봉했다.** DWARF 라인테이블 실측: `Mapper.cpp` **2,701줄**(상류 2,168 → **+533, +24.6%**), `Karto.h` 6,792줄(상류 6,636). 구체 격차: ① `ScanMatcher::m_pRssiGrid` 가 상시 할당(`Create`@`0xdb48f/0xdb4b8`)·상시 스미어(`AddScan`@`0xdddc8/0xdddeb`)되고 응답 계산이 전량 **`GetResponseWithRssi`**(`0xdcdf0`)로 대체됨 — 상류 `GetResponse`(`0xdda00`)는 **호출자 0건으로 사장** ② `LaserRangeScan(name, range, angle, rssi)` 3채널화 + `m_pAngleReadings`(각도 실측 배열 보존) ③ `doSubmapLoop` 3번째 bool 이 `MatchScan`·`CorrelateScan`·`ComputeOffsets` 로 전파(`Mapper::Process` 경로에서 `mLoopNotFind`) ④ `OccupancyGrid` RSSI 레이어(`m_pCellRssi`) + pass/hit 카운터 `kt_int32u`→**`kt_int8u`**(255 포화). ⚠ 다만 RSSI 가중은 **조건부**다 — `ΣvID > nPoints && nPoints < pointSize` 일 때만 걸리고 아니면 상류와 **수치 동일**하다. 즉 「항상 다르다」가 아니라 「반사판이 유효한 스캔에서만 다르다」이며, 활성 빈도는 불명 | 2026-08-08 | 미해결 | ① `vID`(=`GetResponseWithRssi` 의 `id`) 의미를 역추적해 **활성 조건과 빈도**부터 확정한다(debt-099) — 빈도가 낮으면 격차 자체가 작다. ② 반사판이 있는 현장에서만 문제가 되므로, 우리 현장에 반사판이 있는지 먼저 확인한다. ③ 이식이 필요하다고 판단되면 동봉 Karto 에 **패치 파일로 분리** 적용하고 `VENDORED.md` 에 기록한다(무수정 원칙의 예외 절차). ④ `ComputePositionalCovariance`·`ComputeAngularCovariance` 는 줄 수·시그니처 **완전 동일**이고 Mapper 파라미터에 Seer 추가 튜너블이 **0개**이므로, 격차는 RSSI 축에 국한된다 |
> **2026-08-09 갱신** — 이 격차 중 **per-beam 각도(②)는 해소**했다. 동봉 Karto 에 패치를 적용해 실측 각도 배열 경로를 재현했고(`third_party/patches/0001-use-measured-per-beam-angles.patch`, `VENDORED.md` 기록), 오라클 대조에서 포즈 위치차 max **1.83 m → 0.026 m** 로 떨어졌다. 남은 것은 ①(RSSI 스캔매칭 응답)·③(`doSubmapLoop`)·④(점유격자 RSSI 레이어·카운터 8비트화)다. ①은 조건부(`sum(vID) > nPoints && nPoints < pointSize`)이며 활성 빈도는 debt-099 가 풀어야 한다.

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-098 | 이해 | `src/Navigation/slam_karto_core/third_party/open_karto/` (커밋 `922db50`) | **동봉한 상류가 Seer 의 Karto 기반보다 신형이다 — 역방향 격차.** 상류 `Mapper.h:1971` 에는 `m_pMinimumTimeInterval` 이 있는데 Seer 원본에는 그 파라미터가 없다(`InitializeParameters()` 블록 28개 vs 상류 29개, `.so` 전체 바이트열 검색 0회, DWARF 멤버명 0회). 즉 Seer 는 `MinimumTimeInterval` **도입 이전** 버전을 포크했다. 이 한 파라미터만큼 우리 동봉본은 원본과 다른 코드다. 다른 상류 변경분이 그 사이에 더 있었는지는 **미조사** | 2026-08-08 | 미해결 | ① open_karto 상류 히스토리에서 `MinimumTimeInterval` 도입 커밋을 찾아 **Seer 포크 시점의 상한**을 잡는다. ② 그 시점과 `922db50` 사이의 커밋 diff 를 훑어 거동에 영향 있는 변경이 더 있는지 확인한다. ③ 영향이 있으면 동봉 커밋을 그 시점으로 내리거나, 차이를 `VENDORED.md` 에 명시한다 |
| debt-099 | 이해 | 원본 `ScanMatcher::CorrelateScan` 의 `vID` (Mapper.cpp:533-535) | **RSSI 스캔매칭의 활성 조건을 좌우하는 변수의 의미가 미해독이다.** `GetResponseWithRssi` 는 `useRssi = (Σid > nPoints) && (nPoints < pointSize)` 일 때만 RSSI 가중(`pRssi[idx] * id[i]`)을 쓰고, 아니면 상류 `GetResponse` 와 수치가 같다. `vID` 는 `CorrelateScan` 안에서 외부 getter 호출 없이 인라인 계산돼 역추적이 끊겼다(반사판 ID? 강도 버킷? 불명). 이것이 풀려야 debt-097 의 실제 심각도를 판정할 수 있다 | 2026-08-08 | 미해결 | ① `CorrelateScan` @`0xdbb90` 의 `Mapper.cpp:530-538` 구간을 정밀 디스어셈블해 `vID` 생성 로직을 복원한다. ② `LocalizedRangeScan::m_RssiReadings` 와의 관계를 추적한다. ③ 확정되면 회수한 원본 `.rawmap` 의 rssi 분포(기저 50.0, 임계 150.0)에 대입해 **실제 활성 빈도**를 계산한다 |

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-125 | 이해 | `src/Sensors/Positioning/pgv_ros2/` (PGV 드라이버) | 신규 PGV 드라이버의 **실기 부분 검증에서 남은 미확인 4건**: ① 레인/태그 **실판독 미검증**(테이프·태그 실물 부재 — 통신·프레이밍·방향결정만 실증) ② 위치/각도 해상도의 **장치 설정값 미확인**(매뉴얼 DOCT-3707D 에 공장 preset 서술 없음 — 파라미터 기본 0.1 mm/0.1° 는 추정) ③ 컬러 테이프 모드 상대각(-45°~+45°, §3.2)의 **14bit ANG 인코딩 방식 미서술** — 현재 드라이버는 무부호 해석만 적용 ④ `pgv/set_color` 서비스 실기 미호출(현장 테이프 색 미정) | 2026-08-24 | **부분해결(2026-08-25)** — ① Data Matrix 태그 실판독 확정(태그 #0, x/y/angle 판독·정지 반복성 0카운트·11h 드리프트 ≤2카운트) ② 10 mm 실이동 대조로 **0.1 mm/카운트 확정**(PGV Δ121카운트=12.1 mm ↔ Seer 자기측위 10.7 mm). 기록: `pgv_ros2/docs/measurements.md` | 잔여 ③④ 는 **컬러 테이프 레인 추종 전용이라 현 용도(도킹 정밀도 계측, 2026-08-24 사용자 확언)에는 비적용** — 컬러 테이프를 도입하는 경우에만: ③ 컬러 테이프에서 읽기 헤드를 좌/우로 기울여 ANG raw 부호 표현 실측 ④ 현장 색으로 `set_color` 1회 호출·응답 확인. 좌표: `pgv_ros2/README.md` §검증 상태 |

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-127 | 기술+지능 | `Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h` (`seer_gate_fwd_hook`) | Seer emulate(읽기 대리·guard 대리·fake ack)가 fwd_hook 의 **포워딩 결정과 한 함수에 뒤엉켜** 있고, node guarding 응답이 **정적 캡처값 replay** 라 pc_authority(완전분리=전달 차단) 시 **토글(bit7)이 정지** → Seer node guarding 실패 → 단절·재호밍(engage 시 조향 스윙). 완전분리와 emulate 가 섞여 있어 「전달만 끊는」 반쪽 수정이 반복 실패했다(2026-09-03 세션에서 「완전 분리」를 전달 전면차단으로 오해 → Seer 단절 유발, 사용자 정정). | 2026-09-03 | 상환중 (2026-09-03) | emulate 를 fwd_hook 에서 **별도 emulator 함수로 분리** + 판다가 node guarding 토글을 **스스로 생성**(상태 0x7F \| 매 응답 반전 토글, 캡처 실측 형식). 전달 없이도 Seer 안정 = 완전 분리. 근거·Rollback: `docs/adr/2026-09-03-seer-motor-emulator.md`. 실기 검증(engage 시 Seer 단절·재호밍 0) 후 상태→해결. |

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-128 | 기술 | `Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h` (`seer_cache_store_resp`·`seer_freeze_snapshot`·`seer_cache_reply`) | **emulate 의 freeze 스냅샷이 "마지막 폴 노드"(node4)에서 미정착/비정규 값을 문다.** freeze 는 engage 순간의 캐시를 그대로 복사하는데, ① 모터가 degenerate 샘플(pos_act=0, statusword bit15=0)을 실측 ~1/3 비율로 뿌리고 ② node4 는 정착이 가장 느려, engage 가 이르면(예: 플래시/리셋 직후) freeze 가 불완전·비정규 값을 캡처한다. **2026-09-04 실측**: 06:38 가드 플래시 직후 1회차 node4 statusword=0x0050(bit15=0, 틀림)·cur_act/digital_in/error_code 미응답 → 캐시 정착 후 2회차 statusword=**0x8050**(bit15=1이나 idle 0x9450 과 bit12/10 차이=비정규)·digital_in 은 OK 로 개선, cur_act/error_code 는 **여전히 미응답**. pos_act 가드(0 덮어쓰기 금지)·statusword 가드(bit15=0 덮어쓰기 금지)는 상환됨 — 남은 건 statusword 비정규 샘플과 node4 cur_act/error_code 미충전. | 2026-09-04 | **해결 (2026-09-04)** | **상환 완료·실측검증.** ① 근본규명(2026-09-04 src-split, `posact_src_split.py` 필터확장): node4 cur_act/error_code 「미응답」은 **store/serve 실패가 아니라 플래시 직후 캐시 미정착 일시현상** — 정착 후 Seer요청·모터응답·판다서브 전부 OK(cur_act 39/17/16, error_code 45/20/19, digital_in 39/19/18). ② statusword 잔여(node4 0x8050 vs 0x9450): `seer_cache_store_resp` 의 statusword 가드에 **완전 operational(high byte 0x94 = bit15|12|10) 우선 유지** 조항 추가(`safety_seer_gate.h:71-75`) — 덜 operational 한 0x8050 이 캐시된 0x9450 을 못 덮게. ③ 재빌드·4e002c 플래시(sig device==file=True)·정착 8s 후 engage 검증: **node3·node4 statusword 판다서브=0x9450**(모터실값 일치), pos_act 실위치 일치. pos_act 0-가드·statusword bit15 가드와 함께 전부 실측 통과. **주의**: 가드 3종은 아직 작업트리 미커밋 — 커밋 시 `safety_seer_gate.h`. |

| id | 유형 | 위치 | 사유 | 식별일 | 상태 | 상환계획 |
| --- | --- | --- | --- | --- | --- | --- |
| debt-129 | 이해 | (Seer 레거시) `libDSPChassis`: `CMotorDriverBase::init`·`Motor::needCalib`(amap-server 63G) ↔ 우리측 intercept/handover (`safety_seer_gate.h`) | **제어권 반환 시 Seer 재호밍(조향 ~137° 스윙)의 근본.** 2026-09-04 확정: ① release bus0 캡처 — Seer 는 반환 시에만 `0x6099`(homing) 발행(engage 0회), 그리고 hold 목표는 **정확한 홈**(node3 7,871,815 / node4 7,840,086)이라 **위치 불일치 아님**. ② Seer API `steer_angles`(rad) 실측 2.396 rad=**137° 스윙**(2.4° 오차 아님 — 단위 라디안). ③ libDSPChassis RE: `Motor::needCalib`(0x824360)는 `_need_calibration`(0x1fd)·`_first_need_calib`(0x1fc) 검사; `_need_calibration` 는 **생성자(0x84a360)에서만 true 로 init**(전 바이너리에서 true 쓰는 곳 1곳); calib 상태(0x4f0)·`0x4ec` 는 **`CMotorDriverBase::init` 이 모터 config 문자열 비교로 세팅**; homing 기준은 **리밋스위치 DI**(`getDIStates` 0x538). ⇒ **모터 드라이버가 (재)init 될 때만 재호밍** — Seer 는 "제어권 잃음/얻음"을 모르고 드라이버 재init 을 보고 결심. 정상 운전(census 12,928회 hold, 재호밍 0)엔 init 없음. | 2026-09-04 | 해결 (2026-09-04) | ① **트리거 뒷받침(2026-09-04 handover 캡처)**: [존재] 심볼 `add_reconnect_timer`·`_checkOperationalT`·`CMotorDriverBase::doInit`·`resetbeforeNMT`/`sendNMT`·`decodeNMT`·`MINGZHI_CANOPEN_NODE_GUARD_MSG_COUNT`. [관측] release 전후 **node-guarding 불연속** — node3 guard 토글 stuck(0xFF 고정), node1·node2 **bootup(0x70n data0=0x00)** 발생. [동작 추정] 이 불연속이 operational/node-guard 검사를 실패시켜 재연결→doInit→homing. **2026-09-04 반증**: stuck 토글은 emulate 정적 replay 로 확정돼 **수정(매 응답 반전 생성, seer_guard_rtoggle) + 플래시**했고 실측상 emulate 토글이 실모터처럼 교대(전이 4→85)·bootup 소멸까지 확인 — **그러나 release 137° 재호밍은 그대로.** 즉 **토글은 debt-127 문제였고 debt-129 재호밍의 트리거는 아니다.** 새 관측: 스윙이 release ~2.7s 뒤(타이머성) + Seer 는 홈 카운트로 hold 만 보내는데 스티어가 137° = **intercept 중 모터 엔코더 기준이 이동(재referencing)** + Seer 주기 operational 검사(`_checkOperationalT`/`add_reconnect_timer`)가 handover 후 재init. **2026-09-04 근본 확정(engage 25s 모니터링)**: Seer 는 engage 내내 **반복적으로**(t=0.07·3.24·6.76·17.98s) 드라이버 재init/home 시퀀스를 보냄 — `cw=0x86(fault reset)`→`guard_time=500`·`life_factor=1`·`mode=1`·`homing_spd=2500`·`0x60FB=1`. 블록돼 engage 중엔 미실행, **release 에 제어권 복귀 시 실행 → 137° 스윙.** WHY: emulate 가 statusword 를 **frozen(0x9450)** 으로 내 Seer 의 controlword(fault-reset 0x86·enter-op)에 **응답하지 않음** → Seer "명령 미반영=모터 비정상" 판정 → 재init 반복. (passthrough 는 실모터가 controlword 응답 → 재init 0.) **⇒ 근본 fix = emulate 가 CiA402 상태기를 흉내내 controlword 에 statusword 로 응답**(현 frozen 응답의 구조적 한계). toggle fix(debt-127)는 무관하게 유효. 이건 substantial emulate 확장이라 현 동작 회귀 위험 — 범위/검증 신중히. ② intercept/disengage 가 그 트리거를 안 만들게(연결 seamless) 설계 — 반환 전 TX 선행·NMT/heartbeat 연속 유지 등(이전 disengage-handover-restore 시도의 정확한 표적). ③ 실측 검증: release 후 steer_angles 무변 10회(분리 후에만 유의미). **2026-09-04 근본 재확정 — 재호밍은 emulate 내용도 handover 재init 응답도 아니라 CAN topology 다.** [토폴로지 근거] `src` 인코딩(returned bit→+128) 분해: passthrough 0x584 가 **bus0 에서 RX**(외부노드 송신·판다 TX 아님)×969 = **실모터가 Seer 버스에 물리 연결**. engage 시 판다 emulate 0x584(TX0)+실모터 0x584(RX0/RX2) 동시 = **같은 COB-ID 충돌** → Seer 만 CAN 오류(판다 자기 can0/1/2 는 error 0·bus_off 0, 두 트랜시버끼리 ACK). Seer 알람 실측: `CAN1 Bit Recessive/Dominant error`·`Stuff error`·`Acknowledgement error,check CAN router`·`Motor response timeout`. [반증실험 3건] (a) **passive 판다**(emulate=false 강제플래시)로 engage 해도 위 오류+`Motor is calibrating`+137° 그대로 → **emulate 주입과 무관**. (b) 전체 읽기응답 테이블(statusword 토글·cur 홀드전류·pos 디더)을 passthrough 실측과 일치시켜도(2슬롯 replay) 137° 그대로 → **내용 무관**. (c) `SEER_GATE` 에 `set_intercept_relay(true)` 추가해도 0x584 여전히 **RX0×540** → **Seer↔모터 버스가 판다 릴레이 경로 밖**, 릴레이로 분할 불가. ⇒ **Seer·모터·판다 2트랜시버가 한 연속 CAN 버스**(4e002c 실측). 판다는 탭+주입만, 모터를 Seer 로부터 차단 불가 → engage 능동송신/브리지가 실모터와 충돌 → 재호밍. **펌웨어(emulate 내용·릴레이)로 상환 불가 — 상환 = 모터를 판다 뒤 전용 버스로 물리 분리(true inline MITM: Seer─bus0─판다─bus2─모터, 직결 절단)**. [[biguamr-canrelay-custom-board-bus-wiring]] "동시 MITM 불가"와 동일 제약. 종전 "CiA402 상태기 확장"·"seamless handover" 방향은 topology 앞에선 무효. 2슬롯 replay 정본은 scratchpad `seer_gate_2slot_backup.h`. **2026-09-04 해결 실측(종전 "topology 라 firmware 불가" 판정 철회)**: fwd_hook 에 `if (pc_authority) return -1;`(engage 중 포워딩·emulate 송신 전면 0) 넣고 플래시하니 **engage CAN 오류 0·release 재호밍 0(steer 0.000 rad, 알람 0)**. ⇒ 근본은 **"engage 중 판다가 버스에 송신(포워딩+대리응답)해 실모터 응답과 같은 COB-ID 충돌"** — Seer 만 Bit Recessive/Stuff/Form 오류 감지 → 재init→재호밍. 무송신이면 모터가 Seer 에 직접 응답해 충돌 0. **firmware fix 성립.** 옛 버전(athome_fix 8/3·957fa7e 8/10) 전부 재호밍이던 이유도 **다 engage 중 송신**했기 때문(내용 무관, debt-127/128 회귀 아님). **단 무송신 engage 는 능동 intercept(Seer 차단·PC 구동 은닉)를 하지 않음 → hold/release 전용 해결.** PC 가 engage 중 직접 구동까지 은닉해야 하면 별도 과제(그땐 실모터를 Seer 버스에서 못 떼는 현 배선이 제약). debt-128(내용)·debt-127(토글)은 유효하나 이 재호밍 축과 무관. **2026-09-04 릴레이 축 근본 해결**: 릴레이가 engage 때 안 끊긴 진짜 원인 = `black.h:143` 이 릴레이핀 **PC10 을 open-drain** 설정(HIGH 못냄) — 이 보드 릴레이는 active-high(Q31 NPN)라 정지. **push-pull 로 수정(main.c SEER_GATE→HIGH 절체·SILENT→LOW 패스스루) = 릴레이 물리 절체 성립**(engage 중 모터가 bus0 에서 0.5s 후 완전 격리 실측·CAN오류 40→2). + emulate **pos_act=Seer 명령목표(0x607A) 되돌림**(following error 0)·freeze 초기화 → 재호밍 **100%→~25%(3/4, 4e002c)**. **하드웨어 결함 아님(펌웨어 설정).** 상세·main.c 재적용코드 = [relay-pushpull-open-drain-fix-2026-09-04](../can_relay/relay-pushpull-open-drain-fix-2026-09-04.md). 종전 '무송신 hold/release 전용' 대체 — 릴레이 절체로 능동 intercept 가능. 잔여 ~25% 간헐(타이밍/레이스) 미결. **2026-09-04 세션 67ed5a48 분석**: 재호밍 = Seer 조향 재init 시퀀스(0x86→0x6099→0x60FB:04=1)가 release 를 걸쳐 실모터에 누설(rel8 cyc5 실측); Seer 가 engage 중 재init 을 반복한 이유는 미확정. **debt-130(하네스 방향 랜덤) 수정 펌웨어(DEV-8dcca835) 후 33/33 PASS(engage 4 s·6 s)·Seer 재init 시도 0건·EMCY 0**(`docs/can_relay/debt-129-rehoming-cause-analysis-2026-09-04.md` §8). 잔여 재호밍 재현 불가 → **2026-09-04 17:51 해결 확정**: 100사이클 확인 런 100/100 PASS·재init 0·EMCY 0·알람 0(누적 133/133, 사용자 종결 기준 100회 충족). 24 h 내구는 선택. |
| debt-130 | 이해 | `Tools/Can_Relay/panda-firmware/board/drivers/harness.h:54-66,20-33` · `board/boards/black.h:166-168,171-183` · `board/usb_comms.h:407` | **하네스 방향 판정이 부팅마다 랜덤(1/2)** — 이 보드는 PC0=CAN3_EN·PC3=미연결이라 ADC 판정이 흔들린다(이전 세션 관찰, 2026-09-04 14:4x health `car_harness_status=2`). FLIPPED 이면 ① `can_flip_buses(0,2)` 로 논리 bus0↔bus2 교체(emulate 응답이 모터측으로, Seer 요청은 drop) ② `0xe8` 의 `set_intercept_relay()` 가 릴레이핀 PC10 을 `!intercept` 로 구동해 push-pull 제어와 정반대. NORMAL 이면 PC11(도면상 미연결) 구동 = 무해. ⇒ intercept 성패가 부팅 복권에 걸려 있고, debt-129 통계(~13%)·e8 실험(0/8 → "set_intercept_relay 필수" 결론)이 이 교란을 통제하지 않았다. `Log/e8_260904_135849.jsonl` 8사이클 전부 bus2 에 Seer 요청만·모터 0건 = FLIPPED 서명. 분석: `docs/can_relay/debt-129-rehoming-cause-analysis-2026-09-04.md` §3 | 2026-09-04 | 해결 (2026-09-04) | ① ~~실기 E1~~ **완료(15:26): 예측 적중** — FLIPPED 에서 릴레이 미절체+브리징 루프 폭주(node4 EMCY 0x8110 ≈1000 f/s)·52111, SILENT 복귀 시 릴레이가 절체로 남아 Seer↔모터 분리(분석 §6) ② **2026-09-04 17:16 해결(플래시·검증)**: `black.h` 판정 제거·NC 고정·flip 삭제(ADR 2026-09-04-canrelay-harness-orientation-neutralize). 검증: 부팅 harness=0(리셋 2회 재현)·passthrough 복귀·1사이클 clean intercept(EMCY 0)·재호밍 0. 잔여: ③ e8 재현 불요(원인 확정) —  방향 판정 무력화(`car_harness_status` NC 고정 또는 `can_flip_buses`·`set_intercept_relay` 호출 제거) 후 debt-129 통계 재수집 ③ e8 실험 재현(변경 1개씩) |
| debt-131 | 기술 | `src/Comm/CAN/can_relay/config/can_relay.yaml:29`(`vel_max_units`) · `config/machine/foil_a082.yaml:25`(`drive_max_units`) | **구동 상한 29336(≈1.2 m/s)이 저장소에 회수됐으나 그 속도의 실주행 검증이 없다.** 값은 2026-09-05 커밋 `b8ea973` 로 배포 사본에서 회수했다(그전 저장소 값 4889 ≈0.2 m/s). 그러나 상한값 자체는 실측이 아니라 `drive_units_per_mmps 24.447` 환산이고, 이 기체가 그 속도로 주행한 기록이 없다. 2026-09-05 LM1↔LM2(6.26 m) 왕복 실측은 설정 0.6 → 최고 0.77 m/s, 설정 0.9 → 최고 1.27 m/s 로 **설정보다 20~40 % 높게 나왔고**, 정지 지령 후 추가 주행은 2~8 mm 였다. 다만 이 속도는 mcl 자세 미분값이라 확정 근거가 아니다 — 엔코더(`/motor/low_state.fb_vel`) 대조가 없다. 설정 1.2 로 올리는 회차는 자동 승인 분류기에 차단돼 미실시. | 2026-09-05 | 미해결 | ① 설정 0.9·1.2 왕복을 `Tools/docking_field_kit` 도구로 재실행하며 `/motor/low_state.fb_vel` 를 동시 기록해 **엔코더 기준 최고 속도**를 확정한다(mcl 미분값과 대조). ② 설정값 대비 초과분의 출처를 규명한다(액션 프로파일 오버슈트 vs 환산계수 오차). ③ 정지 거리를 속도별로 표로 남긴다. 전제: 충전기 분리·주행 구간 확보(사용자 지시 2026-09-05). |
