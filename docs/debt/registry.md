...
> ❌ **정정 2026-08-06 — 이 절이 든 근거 3개가 반증됐다(값 `[7871815, 7840086]` 채택은 유효).**
> Seer 1005·1040 은 판다가 엿듣는 **바로 그 `0x6064` 의 아핀 변환**이다
> (1040 기울기 ×57,344 = **1.000001**, 1005 = **1.000130**).
> ⇒ **「독립 경로」·「2회 독립 실행 동일」·「사용자 Seer 표시 0° 확인」은 근거가 되지 못한다** — 인용 금지.
> 역산 `0° = CAN + Seer°×57344` 는 **항등식**이라 자세와 무관하게 같은 값을 낸다.
> 유효한 것은 **「송신 0건(AST)」** 하나뿐이며, 값의 지위는 「물리 0° 실측」이 아니라
> **「Seer 좌표계의 조향 영점 + 공학적 채택」**이다.
> 현행 정본: `docs/homing/2026-08-03-can-relay-homing-assets.md` **§0**. 아래 원문은 이력 보존.
