...
- **broker 노드는 본 ADR 범위 밖**(다음 단계, debt-060). 지금 이관하는 두 소비자는 둘 다 19204
...
**남는 위험 / 미해결**
- **broker 노드 미착수** (→ **debt-060**) — 지령 포트를 쓰는 실사용(제어·내비·설정)이 생기면 그때 필요하다.
  현재는 `allow_guarded=True` 명시로 단발 사용만 열려 있고, 그 우회구로 두 도구가 동시에 켜지면
  **동시 지령**이 그대로 난다(연결은 5개까지 받아주므로 로봇이 막아 주지 않는다).
- **HAL 경계 메시지 계약 미확정** (→ **debt-061**) — 어떤 토픽/서비스를 우리 표준으로 둘지는 별도 결정.
