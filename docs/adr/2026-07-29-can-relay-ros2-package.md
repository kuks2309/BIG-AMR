...
아래 debt-025 가 충돌 위험을 등록한다. **문서·주석에서 인용할 때 어느 쪽인지 반드시 병기한다.**
...
2. **구동축 브링업 미검증**(debt-027) — `allow_bringup: true` 는 잭업에서만. `gui.py` 에는 이
...
- (−) `can_relay` 이름이 판다 펌웨어와 충돌한다(debt-025). 인용 시 병기 부담이 영구히 남는다.
