...
Status: ~~Proposed~~ → **Accepted — 소스 반영·플래시 완료(2026-07-27), Verification 3 항은 부분 검증(21 s)**
(사용자 지시 sess:56a709a5 2026-07-27: "펌웨어 수정(근본 해결)" · "모든 이상상태가 되면 relay 를 연결하면 안됨" · "부채0로 진행할 것" · "기술 부채가 있는 제안은 절대 금지")

> ### ⚠ 2026-07-27 Status 정정 (원문 이력 보존)
> Status 가 `Proposed` 로 남아 있으면 다음 작업자가 "아직 반영 안 됨(= 구 펌웨어 거동)" 으로 오독한다.
> 실제로는 Decision 1·2·3 이 **소스에 반영되고 플래시까지 끝났다**:
> - Decision 1 — `Tools/Can_Relay/panda-firmware/board/drivers/can_common.h` `bus_config[]` 3 버스 모두
>   `.can_speed = 2500U` (주석에 실측 근거·ADR 링크 포함).
> - Decision 2 — `board/main.c` heartbeat 타임아웃 블록에 `set_intercept_relay(false); pc_authority = false;` 추가.
> - Decision 3 — `board/safety/safety_seer_gate.h` `seer_is_motion_obj()` 가
>   `0x6064 / 0x606C / 0x6078 / 0x6041` 반환 (`0x6041` 추가됨).
> - `docs/verified_facts/2026-07-27.md` A-1 「**수정 후 재검증** — `can_speed` → `2500U` 로 고쳐 빌드·플래시한 뒤 …」,
>   A-2 「**적용**: `seer_is_motion_obj()` 에 `0x6041` 추가(**플래시 완료**)」.
>
> **단, Verification 3 항(호스트 SW 미실행 전원 사이클 **60 초 이상**)은 미완이다** —
> `verified_facts` A-1 의 재검증 로그는 `t+00s / t+07s / t+14s / **t+21s**` 까지만 기록돼 있다.
> ⇒ **부분 검증(21 s)**. 60 초 이상 관측을 남은 과제로 둔다.
> (Decision 4 — 펌웨어 소스 git 추적 전환 — 의 완료 여부는 본 정정에서 확인하지 않았다.)
...
| `0x6000` digital in | 66~68 (⚠ 12 s 스니핑 기준 — 아래 주 참조) | ~~아니오~~ → **조건부** (아래 주 참조) | 아니오 |

> ### ⚠ 2026-07-27 정정 — `0x6000` 의 「모션 노출: 아니오」는 조건부다 (원문 이력 보존. 펌웨어·Decision 3 은 변경하지 않음)
>
> **정정된 서술**: `0x6000`(digital in)은 위치·속도를 노출하지 않지만 **아무 것도 노출하지 않는 것은 아니다** —
> **bit3(−Limit, 음의 리밋 상태)는 조향축이 음의 리밋에 도달했을 때만 셋된다**. 즉 값 자체는 모션이 아니어도
> **모션의 결과 사건**이 Seer 에 노출된다.
>
> **근거**:
> - `0x6000` 은 **배열 오브젝트**이고 실제 입력값은 **sub 1** 이다(sub 0 = 항목 수 = 2). sub 1 의 비트는
>   **bit0 = ServoEnable, bit3 = −Limit** — 실기 캡처의 `0x01 ↔ 0x09` 전이로 확정
>   (`Log/homing_capture_220350.jsonl`, node3/4). 비트맵 원문: [Handbook V7.0 Appendix I(Object Dictionary), printed page 197].
> - 조향축에는 **리밋 스위치가 실재**하며 호밍 방식은 **Home 1(음의 리밋 트리거)** 이다 — 전 노드 `0x6098 = 1`
>   실기 판독으로 확정(2026-07-27). Home 1/2 는 이 리밋을 원점 신호로 쓴다 [같은 Handbook §4.6, printed page 116].
> - 실측에서 bit3 변화는 **조향 노드(node3/4)에서만** 관측되고 **구동 노드(node1·2)는 무변화**다
>   (`Log/homing_capture_220350.jsonl` t≈47.02 셋 → t≈49.42 해제). 구동축은 기계 원점이 없어 호밍하지 않는다.
>
> **측정조건 병기(폴 횟수)**: 표의 `66~68` 은 **12 초 스니핑** 기준이다. 별도 180 초 캡처
> (`Log/homing_capture_220350.jsonl`)에서는 `0x6000` 폴 주기가 약 0.4 s(≈2.5 Hz, 노드당 420 회 남짓)로
> 환산치가 서로 맞지 않는다 — **두 측정을 섞어 인용하지 말 것.**
>
> ⇒ **Decision 3(freeze 집합)은 바꾸지 않는다** — `0x6000` 은 여전히 freeze 대상이 아니다(실 고장·리밋 상태는
> Seer 가 봐야 한다). 다만 표만 보고 "`0x6000` 은 아무 상태도 노출하지 않는다" 로 읽으면 안 되며,
> **PC 구동이 조향을 리밋까지 몰 수 있는 시나리오를 허용하기 전에는 재평가가 필요하다**
> (동일 취지 주석: `Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h`).
...
2. **이상상태에서 릴레이 연결 금지** (사용자 요구) — heartbeat 상실 처리(`board/main.c` 의 `set_safety_mode(SAFETY_SILENT, 0U)` 블록)에 `set_intercept_relay(false)` + `pc_authority = false` 를 추가한다. ~~어떤 이상 상태에서도 릴레이가 intercept 로 걸린 채 남지 않고 **항상 물리 통과로 복귀**한다(fail-open).~~
   ...
   > ### ⚠ 2026-07-27 정정 — 위 취소선의 범위 한정 (원문 이력 보존. 펌웨어 값·로직은 변경하지 않음)
   >
   > **정정된 서술**: 「**heartbeat 상실(타임아웃) 경로에 한해** fail-open(intercept 해제 + `pc_authority=false`)이
   > 강제된다.」 "어떤 이상 상태에서도 / 항상" 은 코드가 뒷받침하지 않는다.
   >
   > **코드 근거** (`Tools/Can_Relay/panda-firmware/board/main.c`):
   > - fail-open 2 줄(`set_intercept_relay(false); pc_authority = false;`)은 `if (!heartbeat_disabled)`
   >   → `if (heartbeat_counter >= (check_started() ? HEARTBEAT_IGNITION_CNT_ON : HEARTBEAT_IGNITION_CNT_OFF))`
   >   블록 **안에만** 있다. 즉 분기 **1 곳**뿐이다.
   > - `set_safety_mode()` 의 `case SAFETY_SEER_GATE:` 분기는 릴레이를 **전혀 건드리지 않는다**
   >   → 직전 intercept 상태가 그대로 유지된다.
   > - `set_safety_mode()` 의 `default:` 분기는 오히려 `set_intercept_relay(true);` 다.
   >
   > ⇒ **커버되지 않는 이상 상태(미해결 gap)**:
   > 1. **heartbeat 가 계속 도착하는 동안 발생하는 이상** — 호스트 제어로직 고장·행(hang) 아닌 오동작,
   >    `board/faults.h` `fault_occurred` 경로 등. 이 경우 릴레이는 intercept 로 남는다.
   > 2. **SAFETY_SEER_GATE 유지 상태** — 모드 재설정만으로는 릴레이가 해제되지 않는다.
   > 3. **heartbeat 검사 자체를 끄는 경로** — `board/usb_comms.h:480` `heartbeat_disabled = true`
   >    (`ALLOW_DEBUG` 빌드 한정). 이 경로가 켜지면 위 fail-open 분기 자체가 도달 불가다.
   >
   > **판정/해소에 필요한 작업**: 위 1~3 을 덮으려면 별도 워치독(호스트 이상 감지) 또는
   > 모드 전환·fault 발생 시점의 릴레이 해제 훅이 필요하다. **본 정정에서는 펌웨어를 수정하지 않았다** —
...
- 변경 방향은 전부 **fail-open**(버스 유지 · intercept 해제)이다. ~~어떤 실패 경로에서도~~ Seer 가 모터를 잃지 않는 쪽으로만 움직인다.
  > ⚠ 2026-07-27 정정: 「어떤 실패 경로에서도」는 과장이다. **fail-open 이 강제되는 경로는
  > heartbeat 상실 타임아웃 1 곳뿐**이며, heartbeat 가 살아 있는 이상·`SAFETY_SEER_GATE` 유지·
  > `heartbeat_disabled` 빌드는 커버되지 않는다(Decision 2 정정 블록의 코드 근거 참조).
  > 변경 자체가 안전 방향인 것은 맞으나, **모든 이상 상태를 덮는다는 뜻은 아니다.**
...
3. **호스트 소프트웨어를 하나도 실행하지 않은 채 전원 사이클** → Seer 1050 폴링으로 `errors=[]` 및 `54022` 미갱신을 60 초 이상 확인. (이번 사건의 재현 조건 그대로.)
   > ⚠ 2026-07-27 상태: **부분 검증(21 s)**. `docs/verified_facts/2026-07-27.md` A-1 의 수정 후 재검증
   > 로그는 `t+00s / t+07s / t+14s / t+21s` 까지만 남아 있다(같은 시점 판다 `uptime=42s`, 8 초간 수신
   > 29,625 프레임, `rx_errs=0 faults=0`, 운전자 육안 "오류 안남"). **60 초 이상 관측은 미완** — 다음
   > 전원 사이클 때 마저 수행할 것.
...
- (+) ~~이상 상태에서~~ **heartbeat 상실 시** 릴레이가 intercept 로 남지 않는다(사용자 요구를 **부분 충족** —
  heartbeat 가 살아있는 이상·`SAFETY_SEER_GATE` 유지·`heartbeat_disabled` 경로는 미커버. Decision 2 정정 참조).
