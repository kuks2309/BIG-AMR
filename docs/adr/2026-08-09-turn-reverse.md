...
(각도 계상 회귀 `turn_angle_accounting_check.py`, 잔여각 프로브)을 그대로 재사용할 수 있다.
...
2  회귀   turn_angle_accounting_check.py 를 reverse 에도 적용 (검출력 확인 포함)   ✘ 미수행
...
2. **IMU 델타 계상이 후진에서도 같은 부호 규약으로 동작하는가** — 부호 규약은 정상(후진 다리가
   ...
   맵 절대 측위보다 양쪽 다리 모두 작다(0.3~0.5°). 누적기의 **0 클램프 래칫**을 의심하며
   `debt-048` 로 등록했다. 전진판 `turn` 도 **같은 코드**이므로 같은 성질을 갖는다.
3. **코드 중복 해소** — 범위 밖, 그대로 남는다. `debt-048` 상환 시 **두 파일을 함께** 고쳐야 한다.

**새로 드러난 미결** — `turn` 계열에 **허용 규격이 없다**(`spin` 은 ≤ 0.40° 사용자 승인).
실측 오차 n = 4: ±10° 에서 −0.19 ~ +0.38°, 90° 에서 −0.53 · +0.66°. 규격 결정은 사용자 몫이다.
