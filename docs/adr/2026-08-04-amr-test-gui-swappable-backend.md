...
**④ ~~원본 `Tools/amr_test_gui/gui.py` 는 건드리지 않는다.~~ → ❌ 뒤집힘 (2026-08-04)**

원 결정: 원본은 비교 기준선이자 유일하게 실기에서 돌아 본 코드이므로 동결한다(debt-039).

❌ **사용자 지시로 뒤집혔다** — 「**먼저 원본을 고치고 나서 ros2 backend 연결하는 것이 정석**」.
맞는 지적이다. **결함이 있는 코드를 기준선으로 삼는 것 자체가 틀렸다** — 그 위에 맞춘 이식본은
결함까지 같이 맞추게 된다. 그래서 원본의 결함 11건(High 3·Medium 5·Low 3)을 **먼저 고치고**
같은 수정을 `backend_direct` 에 반영했다(조치 내역: `docs/code_review/amr-test-gui/2026-08-03.md`
§조치, 이슈 기록: `docs/issues_and_fixes/issues_and_fixes.md` 2026-08-04).

바이트 동일성 회귀(`test_backend_swap.py` 33건)는 **그대로 유효하다** — 수정 3건(신선도·USB 락·
재송신)은 프레임 **바이트를 바꾸지 않고** 타이밍·반복만 바꾸기 때문이다.

여전히 `backend_direct` 가 원본과 같은 바이트를 내는지는 회귀로 고정한다.
