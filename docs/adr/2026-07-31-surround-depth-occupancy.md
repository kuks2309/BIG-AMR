...
### D10. 실사 색 입히기 — 컬러 5 fps + 소프트웨어 정합

D5 는 "depth 만, 컬러는 배타 운용" 이었다. 실측으로 **조건부 철회**한다.

| 구성 | depth 합계 | 카메라별 depth | 판정 |
| --- | --- | --- | --- |
| depth 전용 | 92.4 fps | 10.3~25.1 | 기준 |
| depth + 컬러 **25 fps** | 13.5 fps | **6대 중 4대가 0** | 불가 |
| depth + 컬러 **5 fps** | 62.1 fps | 9.6~10.9 (전 카메라) | **가능** |

컬러 프레임률이 결정적이다. 25 fps 면 depth 가 굶어 죽고, 5 fps 면 모든 카메라가 융합 주기
(10 Hz)를 충족하는 depth 를 유지한다. 카메라 CPU 합계는 129.2% 로 depth 전용과 사실상 같다 —
색은 대역 문제이지 연산 문제가 아니다.

**드라이버의 하드웨어 정합(`depth_registration`)은 이 모델에서 쓸 수 없다.** 켜면 depth·color
양쪽 `camera_info` 가 모두 깨진다(`fx=-0.000`, `cy=-1.7e38`). 대조 실험으로 확인했다 — 끄면
depth `fx=381.75`, color `fx=357.14` 로 둘 다 정상이다. 정합만 켜고 끄는 차이다.

대신 **소프트웨어 정합**을 융합 노드가 한다. 드라이버가 `/<camera>/depth_to_color` 로 외부
파라미터를 발행하며(회전 ≈ 단위행렬, 병진 12.5 mm), 역투영 → 컬러 좌표계 변환 → 컬러 내부
파라미터로 투영해 픽셀을 찾는다. 이미 하는 역투영에 변환·투영 한 번씩만 더한 것이다.
이 보정을 빼면 근거리 0.3 m 에서 약 15 픽셀이 어긋나 물체 가장자리에 배경색이 묻는다.

보셀 색은 **선착순**(그 보셀에 처음 들어온 점의 색)이다. 5 cm 보셀에 들어오는 점들은 대개 같은
표면이라 평균을 낼 이유가 약하고, 누산기가 필요 없다. 색을 못 구한 점은 회색으로 둔다 —
