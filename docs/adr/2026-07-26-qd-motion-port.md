...
- **취약점**: 수치최적화 순수 연산 라이브러리(네트워크·파서·직렬화 sink 없음). ~~알려진 CVE 없음.~~ 입력은 로봇 내부 상태(외부 신뢰경계 횡단 아님).
  > ⚠ **2026-07-27 정정 — "알려진 CVE 없음" 은 근거 없는 주장이다** (원문 이력 보존)
  > 조회 대상 DB·조회 일자·대상 버전 범위가 **하나도 인용되지 않았다**. 본 ADR 이 명시한 설치 버전은
  > 위 §설치 항의 `libnlopt-cxx-dev libnlopt-dev (Ubuntu 22.04 공식, **2.7.1-3build1**)` 인데,
  > 그 버전에 대한 취약점 조회 산출물이 **저장소 어디에도 없다**(`docs/` 하위에 NLopt 취약점 조사 문서 0 건).
  > ⇒ 정정된 서술: 「**2026-07-26 시점 미조회 — CVE DB 조회 기록 없음**」.
  > **해소 방법**: NVD(`nlopt` / CPE) 및 Ubuntu USN·Debian security tracker 를 **조회일과 대상 버전
  > (2.7.1-3build1)** 을 함께 명기해 인용하면 이 항목은 근거를 갖춘다.
  > (참고 — 위험도 판단 자체는 별개다: 네트워크·파서 sink 부재와 내부 신뢰경계 논거는 정정 대상이 아니며
  > 그대로 유효하다. 정정 대상은 "조회했다"고 읽히는 **"알려진 CVE 없음"** 문구뿐이다.)
