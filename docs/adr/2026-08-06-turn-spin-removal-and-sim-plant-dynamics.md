...
수치 사실:
- R=5 m 기준 추가 조향 이동 **173.1°**. 조향 슬루 57.1 deg/s(설정 유도) 기준 **3.03 s** —
  `fine_correction_timeout_sec` 이 **3.0 s** 이므로 예산 전부를 스윙에 쓰고 보정은 못 한다.
...
- `drive_*_mps2 = 0.0833` — **실측 유래**(50 mm/s → 0.57~0.65 s 역산).
  ...
  바 없다. 한 동작점에서의 역산이다.
- `steer_rate_dps = 57.1` — **설정 유도**(`steer_profile_velocity` 30000 = 0.1 rpm 단위 →
  모터 3000 rpm ÷ 조향 감속비 315 = 9.524 rpm). **실측 아님.**
