...
- 2026-08-06: §amap-1 → **§amap-server 로 개칭**(tailscale 호스트명 변경, IP 동일). 실측 정정 2건 —
  63G 원본 하드 device 를 `/dev/sda2` → **`/dev/sdb2`**(59.1G ext4, `lsblk` 실측)로 고치고,
  같은 장비에 별개로 마운트된 `/mnt/seer_128g_root`(sda3 LVM 40.4G)를 혼동 방지용으로 병기.
  **함정 2(Tailscale SSH 브라우저 재인증)** 신설 — 비대화형 ssh 가 URL 만 내고 끊기는 것도 거부가 아니다.
