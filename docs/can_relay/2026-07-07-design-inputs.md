...
> ## ❌ 정정 2026-08-03 15:40 — 호밍 확정 사실 반영 (원문·관측값은 전부 보존)
>
> 정본: `docs/homing/2026-08-03-can-relay-homing-assets.md` **§0**
>
> **[실측] 2026-08-03 15:33~15:40 호밍 10회 연속** (접지, 펌웨어 `DEV-cc5e0491-DEBUG`)
> — **10/10 성공**, 회당 **35.0 s**(WAIT −리밋 탐색 31.7 s → RESTORE → GOZERO → DONE),
> **10회 모두** DI(Digital Input) `0x01`→`0x09`(bit3 = −Limit) ⇒ **리밋 스위치 실재**,
> 실기 `0x6098` Homing method = **1**(Home 1, 음의 리밋), `counts/°` = **57,344**(실측 1.000000).
> 호밍 후 정착 counts 중앙(σ≈3c): node3 **7,882,021** · node4 **7,859,065**.
>
> - 본 문서 §3 정정 ②의 「호밍은 … 조향 **0°** 복귀까지」 라벨은 틀렸다 — 복귀 목표는
>   **호밍 후 정착값**이며 조향 0° 정본 `[7871815, 7840086]` 에서 **+0.178° / +0.331°** 벗어나 있다.
>   상세는 §3 정정 ② 아래 붙인 정정 블록.
> - 「호밍이 안 된다」·「`Switch on disabled` 라 호밍 불가」·「`0x6098`=0」·「RstStart 고착」·
>   「`0x6064`=0 전원 사이클 래치」·「이미 홈이면 즉시 완료」·「CAN↔Seer 독립 교차검증」·
>   `+0.334°`/`+0.332°` 표기는 **전부 폐기**됐다(인용 금지).
> - ⚠ 조향 0° `[7871815, 7840086]` 은 **Seer 좌표계 기준**이다. 물리적 직진 여부는 **미확인**
>   (Seer 1040·1005 는 둘 다 `0x6064` 유래라 독립 앵커가 아니다).

> ## ❌ 재정정 2026-08-03 17:00 — 바로 위 15:40 블록의 수치·판정 오류 (원문·관측값은 그대로 둔다)
>
...
>   두 노드가 같은 값이 아니다(`Log/steer_two_phase_260803_131305.jsonl` A국면 5지점 회귀).
>   config `steer_counts_per_deg: 57344.0` 은 node4 에 대해 **반올림 채택값**이다.
> - **「σ≈3c」** = **모표준편차** node3 **2.80** · node4 **3.21**(표본 2.95 / 3.38, n=10).
> - **「10/10 성공」은 유효**하나, 2026-08-03 전체로는 **시도 13 / 성공 12 / 실패 1**(09:58 ERR_TIMEOUT
>   120.26 s)이다. 「12회 연속 성공」식 서술은 쓰지 말 것. 15:33 summary 의 `baseline`(t=1.058)은 호밍이
>   아니라 **레지스터 스냅샷**이므로 회차에 넣지 않는다.
> - **폐기 목록의 「`0x6064`=0 전원 사이클 래치」 항 — 「단 1회·재현 안 됨」이라는 폐기 근거는 [거짓]이다.**
...
>   「전원 사이클로만 풀리는 래치」도 입증되지 않았다. (본 문서 §6 S6 계열의 「호밍 중 위치를 쓰지 않는다」
>   안전요건은 이 판정과 무관하게 유효하다.)
> - **조향 0° `[7871815, 7840086]` 값 유지**(`src/Comm/CAN/can_relay/config/machine/foil_a082.yaml:134`),
>   **근거는 강등** — 1040 역산은 **항등식**(Seer ≈0° 자세 보정항 node3 −4c / node4 +34c), `0x607A` 근거는
>   **선택 인용**(`Log/homing_capture_220350.jsonl` node3: 7,871,815 145/6,464 = **2.24%** ·
>   7,882,020 6,319/6,464 = **97.76%**). ⇒ 「실측 확정」이 아니라 「**공학적 채택값**」으로 적을 것.
...
>
> ❌ **정정 2026-08-03 15:40 — 위 ①②의 해석·라벨** (관측값·시각·counts·프레임은 원문 그대로 둔다)
> - **[확정 강화]** DI `0x01`→`0x09`(bit3 = −Limit) 전이는 2026-08-03 호밍 **10회 전부**에서 재현됐다.
>   ⇒ **리밋 스위치는 실재**하고, 호밍 방식은 실기 `0x6098` 판독 **1**(Home 1, 음의 리밋)로 확정됐다.
>   「emulate 시 DI 를 상수 `0x01` 로 하드코딩하면 안 된다」는 위 결론은 **그대로 유효**하다.
> - ❌ **라벨 오류**: ②의 「호밍은 … 원점 경유 후 **조향 0° 복귀**까지」에서 그 복귀 목표는 **0° 가 아니다.**
>   목표는 **호밍 후 정착값**(`Tools/Can_Relay/panda-firmware/board/safety/safety_seer_gate.h:212-213`
>   `SEER_HOME_ZERO_N3/N4` = `7882020` / `7859062`)이며, 조향 0° 정본 `[7871815, 7840086]` 에서
>   **+0.178° / +0.331°** 벗어나 있다.
>   ①이 관측한 `0x6064` **7,882,008**(`t=52.9354`)이 바로 그 정착값이며, 2026-08-03 호밍 10회 실측
>   정착 중앙값 node3 **7,882,021** / node4 **7,859,065**(σ≈3c)와 같은 자리다.
>   ⇒ **재현되는 설계 동작이며 결함이 아니다.** 서술은 「원점 경유 후 **정착 목표(≈직진)** 복귀까지」로 읽을 것.
>   - ❌ **재정정 2026-08-03 17:00**: 「**설계 동작**」은 과잉 확정이다. 원자료가 보증하는 것은
>     **10회 재현되는 정착 동작**과 그 목표가 펌웨어 컴파일 상수(`safety_seer_gate.h:212-213`)라는 사실까지이고,
>     **그 상수가 이 기체에 적정한지는 별건**(debt-016)으로 미판정이다.
>     ⇒ 「**재현되는 정착 동작(상수 적정성은 별건, debt-016)**」으로 적는다. 라벨 정정(복귀 목표는 0° 가 아니다)과
>     안전 함의는 그대로 유효하다. **σ≈3c** = 모표준편차 node3 **2.80** · node4 **3.21**(표본 2.95 / 3.38, n=10).
>   리밋에 얹힌 채 두면 그 방향 지령이 막힌다는 ②의 안전 함의는 **불변**이다.
> - ⚠ **[미확인]** 조향 0° `[7871815, 7840086]` 은 **Seer 좌표계 기준**이다. 물리적 직진과 같은지는
>   미확인이다 — Seer 1040·1005 는 둘 다 `0x6064` 유래(gain 1.000001 / 1.000130)라 독립 앵커가 아니다.
...
- [x] ~~Tongyi statusword 0x?050 비트 정의~~ → **해소(2026-07-07)**: Handbook §6.6.2 대조 완료 — 상태 = "Switch on disabled"인데 구동됨 = 하드웨어 enable 추정, [분석 보고서 §5](../../expriments/can_data/analysis/2026-07-07_tongyi_can_analysis.md) 참조
  ...
  - ✅ **재확인 2026-08-03 15:40 [실측]** — 이 판정은 **호밍 국면에서도 성립**한다. 2026-08-03 호밍
    **10/10 성공**(회당 35.0 s)은 조향 2축이 statusword 하위바이트 `0x50`("Switch on disabled")인 상태에서 이뤄졌고,
    같은 10회 모두 DI `0x01`→`0x09`(bit3 = −Limit)로 리밋에 도달했다.
    ⇒ ❌ 「`Switch on disabled` 라 호밍이 막힌다」는 주장은 **폐기**한다(인용 금지) —
    CiA402 상태머신은 이 드라이브의 벤더 호밍을 게이팅하지 않는다.
    근거: `docs/homing/2026-08-03-can-relay-homing-assets.md` §0-1 · §0-4.
  - ❌ **재정정 2026-08-03 17:00**: 「회당 35.0 s」는 **평균 35.07 s · 중앙 35.05 s**(최소 34.99 · 최대 35.16,
    n=10, `Log/home_experiment_260803_153319_summary.json`)로 적는다. 「10/10 성공」과 본 판정(폐기 결론)은
    그대로 유효하다.
