...
| 6 | **★ 2026-08-03 실측 확정**: 조향 홈(0°) = **`[7871815, 7840086]`**(Seer 좌표계 기준). `7882020 / 7859062` 는 **홈이 아니라 「호밍 후 정착값」**으로 0°에서 **+0.178° / +0.331°** 벗어남. **이 둘을 모두 "홈"이라 부른 것이 4주 재실험 반복의 원인.** 57,344 counts/°. | `docs/verified_facts/2026-08-02-steer-home-closed.md` · 값 정본 `src/Comm/CAN/can_relay/config/machine/foil_a082.yaml:126` |
...
| 파일:줄 | 값 | 기준선#6 대조 |
|---|---|---|
| `src/Comm/CAN/can_relay/config/machine/foil_a082.yaml` | `[7871815, 7840086]` | ✅ **정본** (2026-08-03 갱신 확인) |
| `Tools/amr_test_gui/gui.py:77` | `_load_steer_home()` → 정본 YAML 로드 (실패 시 코드 사본 `{3:7871815, 4:7840086}` + 경고) | ✅ **2026-08-06 재확인** — 이전 기재 `gui.py:46 {3:7871810, 4:7839894} (config 미연동 사본)` 은 **값·기전 둘 다 틀렸다**: 값은 폐기값이고, 현재는 하드코딩이 아니라 정본 YAML 연동이다 |
