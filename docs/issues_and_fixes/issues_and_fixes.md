...
- **상태**: 완료 — 실기 확인
  (제어권 획득 시 `n1 0x8050 enabled=0` → 자동 활성화 → `n1 0x8037 enabled=1`, 양축 운전 가능)
- **미해결**: **node1 이 왜 제어권 반환 구간에서 떨어지는지는 미규명.** Seer 가 내리는 것인지
  드라이브가 자체적으로 떨어지는지 가리지 않았다. 과부하(debt-021)와 같은 축이라 관련
  가능성이 있으나 확인되지 않았다 — **debt-022** 로 등록.
