...
- **설계 결함**: 오프셋을 한 번 잡고 끝까지 IMU 만 믿는다. `/robot_pose` 를 **주행 내내 구독하고
  있으면서도** 보정 yaw 와 맵 yaw 를 대조하지 않는다. localization watchdog 은 pose 두절·점프만 본다.
  `|보정 yaw − 맵 yaw| > 임계 → abort` 가 있었다면 **2초 안에 멈췄을 것**이다.
...
- **파급**: `turn`·`spin`·`yaw_control` 이 **전부 IMU 로 루프를 닫는다.** 이 현상이 재현되면
  세 기동 모두 같은 방식으로 조용히 실패한다.
