...
- **상태**: 완료 — 패키지 시험 311 passed / 0 failed(호밍 게이트 관련 102건 포함). 실기에서
  조향이 동작하기 시작해 crab 이 Phase 0 을 통과했고(3.88 s), spin 이 조향 F=−90.0°/R=+90.0°
  로 정상 기동했다.
...
- **판정**: 엔코더가 두 구동축의 비대칭을 그대로 보여준다.
  ...
  spin 은 산술이 세 곳에서 맞는다 — 회전 53.0°(0.925 rad)에 후륜 주행 1.1143 m 이면
  반경 **1.204 m ≈ 휠베이스 1.2 m**, 중심 이동 0.6039 × 0.925 = **0.559 m ≈ 실측 550 mm**.
  ⇒ 중심이 아니라 **전륜을 축으로 회전**했다.
...
- **경로·각도 의존성 배제**: GUI 직결 경로(`~/steer_axis_deg` + `~/drive_mmps`)로 같은 시험을
  0°·45°·70° 에서 반복했고 **전부 동일하게 실패**했다(node1 +0.0011 / +0.0010 / +0.0006 m,
  node2 +0.0615 / +0.0808 / +0.0842 m). ⇒ ROS2 경로 문제도, 조향 각도 문제도 아니다.
- **드라이브 자기보고**: `error_code 0` · `motor_enabled true` · 정지 중 `amps 452`.
  지령이 들어가는데 회전하지 않고 전류가 흐르는 것은 **구속(stall)** 의 전형이다.
- **상태**: **미해결 — 소프트웨어 원인 배제됨, 물리 점검 필요.** 같은 날 앞선 translate 에서는
  두 축이 0.6 mm 이내로 일치했으므로 **이 세션 진행 중 발생**했다. 손상 위험이 있어 구동 시험을
  중단했다. 확인 항목: 전륜 구동륜 수동 회전 시 구속 유무, 접지 상태, 배선·커플링, 드라이브
  전원 재인가.
- **부수**: 이 결함이 crab·spin 실패의 원인이므로, 같은 날 제기했던 「crab yaw 보정 부호가
  반대」 주장은 **철회**한다(아래 별도 항목).
