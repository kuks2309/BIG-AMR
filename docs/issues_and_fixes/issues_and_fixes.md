...
### [Open] **IMU 회전량이 맵 기준의 약 0.80 배** — 미해결, 오늘 최대 쟁점

정지 순간(완화 전) 값끼리 비교해도 IMU 와 mcl2d 가 계통적으로 어긋난다.

```
지령 −10°     IMU  −9.58   mcl2d −11.34    비 0.845
지령 −90°     IMU −89.09   mcl2d −112.41   비 0.792
지령 +112°    IMU +111.55  mcl2d +139.88   비 0.797
```

- 이 가설이 참이면 **오늘 「mcl2d 가 측위를 잃었다」고 한 제 판정이 틀렸다** — 90° 지령에 로봇이
  진짜 112° 돌았고 mcl2d 는 그것을 정직하게 보고한 것이 된다. 액션이 IMU 로 루프를 닫으므로
  IMU 가 덜 읽으면 **실제로 더 돈다.** 되짚기가 크게 빗나간 것도 이것으로 설명된다.
- **⚠ 반증이 하나 있다** — 같은 날 초저녁 ±10° 에서는 mcl2d +10.38 / IMU +10.30 으로 **0.08° 이내
  일치**했다. 스케일 오차가 상시라면 그때도 벌어졌어야 한다. 무엇이 달라졌는지 모른다.
- **파급**: 사실이라면 `turn` 뿐 아니라 **IMU 로 닫는 모든 기동**(`spin` · `yaw_control` 계열)이
  같은 비율로 틀린다. 오늘 쫓던 0.3° 급 튜닝보다 훨씬 크다.
- **다음 실험**: 제자리 `spin` 으로 큰 각을 돌려 IMU · mcl2d · Seer **3자 대조**. `spin` 은 병진이
  없어 회전만 분리된다. Seer 는 우리가 제어권을 놓아야 갱신되므로 기동 후 반납하고 재수렴을 기다린다.
- ⚠ **결론을 내리지 않는다.** 두 계통 중 어느 쪽이 참인지 아직 모르며, 반증 1건이 있다.
...
### [Diag] `turn` 의 각도 자기보고가 지도 기준 실제 회전보다 **작다** — 0 클램프 래칫 의심 (미확정)
