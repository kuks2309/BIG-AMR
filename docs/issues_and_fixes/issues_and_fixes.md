...
### 부차 확인 — 굳이 숫자로 봐도 같은 결론이다

**호밍이 조향축을 전 범위로 쓸고 지나간다.** `docs/homing/2026-08-03-can-relay-homing-assets.md:123-124`:

    소요  35.0 s (34.99~35.16, 편차 0.17 s) · 10/10 성공
    단계  ENABLE → SET_SPEED → START → WAIT(**31.7 s**, −리밋 탐색) → RESTORE
          → GOZERO → GOZERO_W → DONE

⇒ 137° 복귀(GOZERO·GOZERO_W)를 포함한 나머지 전 단계가 **≤ 3.3 s** 다.
   즉 조향축은 **≥ 41.5 °/s 로 움직일 수 있다** — 내가 쓴 10.3 °/s 의 4배다.

나는 「`max_steer_deg 82~90°` 는 실기 미실행」이라 적었으나, **큰 각 거동은 호밍에서 매번
관측되고 있었다.** 외삽할 필요가 없었다.

### 귀결 — `−8` 오탐 계산의 전제가 뒤집힌다

내 계산은 「지령 램프(20.05 °/s)가 축(10.3 °/s)보다 빠르므로 오차가 반드시 벌어진다」였다.
축이 ≥41.5 °/s 면 **축이 램프보다 빠르므로 오차가 자라지 않고** `gate_blocked` 가 지속될
이유가 없다. 되돌린 판단(`2026-08-10-005`)이 데이터로도 옳았다.

### ⚠ 남는 미확인 — 41.5 °/s 를 일반 조향에 그대로 옮기지 않는다

호밍 복귀는 `0x6081`(profile velocity)을 **따로 지정**한다(`protocol.py:226,233`).
일반 조향 브링업은 `0x6081 = 30000` 을 쓴다(`:190`). 두 값이 같은지 확인하지 않았으므로
**「축 capability ≥41.5 °/s」까지만 말하고, 일반 조향 구간의 실효 속도는 미확정**으로 둔다.
필요하면 호밍 로그(`Log/home_experiment_260803_095815.jsonl`, 66,614 프레임)에서
GOZERO 구간의 `0x6064` 시계열을 뽑으면 각속도가 직접 나온다 — 실기 재실행 없이 가능하다.
