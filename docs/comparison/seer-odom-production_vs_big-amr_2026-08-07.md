...
## 17. 우리 생산 코드와 레거시의 실제 차이 — 정량화 ✓

§15·§16 으로 레거시 산식이 **원본 대조로 확정**됐으므로, 이제 우리 생산 휠 오도
(`motor_control/driver_node.py` + `kinematics.py`)와의 차이를 값으로 말할 수 있다.

### 차이 3건

| 항목 | 레거시 | 우리 생산 코드 | 성격 |
| --- | --- | --- | --- |
| 선형 해 | `(AᵀA)⁻¹Aᵀ` 사전 역행렬 × b (Eigen) | 같은 정규방정식을 **닫힌형**으로 전개 | 수학 동일, 연산 순서 다름 |
| 휠 위치 | `(x+cpx, y+cpy)` — 보정항 포함 | `node_xy[n]` — **보정항 없음** | 구조 차이 |
| 자세 누적 | 각을 **먼저** 갱신하고 그 각으로 회전(end-point) | **갱신 전** 각으로 회전(start-point) | 거동 차이 |

### 누적 차이 실측 (자세 누적 규칙만 바꿔 100 m 주행)

`odom_rate` 25 Hz 기준. 스텝은 속도/주기다.

| 속도 [m/s] | 스텝 [mm] | 조향 [deg] | 위치차 [m] | 상대오차 |
| --- | --- | --- | --- | --- |
| 0.3 | 12 | 5 | 0.0198 | 0.020 % |
| 0.5 | 20 | 5 | 0.0331 | 0.033 % |
| 1.0 | 40 | 5 | 0.0662 | 0.066 % |
| 1.5 | 60 | 5 | 0.0994 | 0.099 % |
| 1.0 | 40 | 15 | 0.0317 | 0.032 % |

**yaw 차이는 전 조건에서 0** 이다 — 두 규칙은 각 누적식이 같고 **회전에 쓰는 각만** 다르므로
위치에만 나타난다. 직진·제자리스핀에서도 0 이다(전자는 회전이, 후자는 병진이 없다).

⇒ 차이는 **스텝 크기에 비례**한다(주기가 낮거나 빠를수록 커진다). 1.5 m/s 에서 100 m 당
약 10 cm.

### 판단

이 값만으로 「고쳐야 한다」가 되지는 않는다 — 측위가 오도의 **절대값이 아니라 증분만**
쓰기 때문이다(§8.1). 한 주기 증분에 섞이는 오차는 그 주기 몫뿐이고 파티클 필터가 매 주기
흡수한다. 다만 **오도 자체를 궤적으로 쓰는 소비자**(기록·재생, 오프라인 분석)에는 그대로 남는다.

⚠ **보정항 부재(`cpx`/`cpy`)가 더 큰 항일 수 있다.** 레거시는 설계값에 보정항을 더한 유효
위치를 쓰는데 우리는 설계값만 쓴다. 그 크기는 이 기체의 보정값을 확보해야 말할 수 있고,
아직 확보하지 않았다 — **미확인**.
