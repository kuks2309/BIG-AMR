...
-O0 빌드:  불일치 0 / 1800   → **완전 비트 일치**      (2026-08-08)
-O2 빌드:  불일치 2 / 1800 (max ulp 1)                (코드 생성 산물 — 아래)
```

**2026-08-08 원인 확정 — 잔여 2건은 소스가 아니라 빌드 최적화였다.** 번역단위를 교차해 격리했다:

| 빌드 조합 | 결과 |
| --- | --- |
| 하네스 `-O2` + 모션모델 **`-O0`** | **불일치 0** |
| 하네스 `-O0` + 모션모델 `-O2` | 불일치 2 |
| 둘 다 `-O2` | 불일치 2 |
| 둘 다 `-O0` | **불일치 0** |

⇒ 원인은 `motion_model.cpp` 의 최적화 코드 생성이다. FMA(Fused Multiply-Add) 계약 아님(`-ffp-contract=off` 로도 2건),
벡터화·인라인 플래그와도 무관(`-fno-tree-vectorize`·`-fno-tree-slp-vectorize`·`-fno-inline` 모두 2건), `-O1` 부터 발생.
`-O2` 에서 `normalizeAngle` 이 인라인되며 호출 9→5·`comisd` 8 추가로 코드 구성이 바뀐다.
오라클 타깃은 `-O0` 로 고정했다(CMake) — 오라클은 **소스 동치**를 재는 도구이기 때문이다. → debt-043 **해결**.

| 항목 | 결과 |
| --- | --- |
| `trans` · `direction` | **`-O0` 에서 300/300** — `-O2` 빌드에서만 표본 1개가 1 ulp(빌드 산물, debt-043 해결) |
