...
제자리 회전은 inline 2WS 구조상 ±90° 근처를 요구한다(`θ_spin = atan2(x_i, −y_i) = 89.87°`).
translator 의 영점 오프셋을 거치면 can_relay 가 보는 raw 는

```
스핀 요구 raw = |θ_spin| + |steer_offset| = 89.87 + 1.676 = 91.54°
```

**체인 클램프는 이 값 이상이어야 한다.** 현행 115° 는 충족한다(여유 23.5°).
...
[PASS] C4 조향 클램프 범위
       스핀 요구 91.54° ≤ 115.0° — 제자리 회전 허용
```

클램프를 91.54° 미만으로 내리면 C4 가 「**스핀이 잘린다**」로 FAIL 한다.
