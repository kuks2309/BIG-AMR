...
reflected_assets:
  ...
  - Tool/amr_test_gui/amr_test_gui/modes.py (모듈 docstring §정합성 메모 + 각 모드 verified=True 복원)
  - Tool/amr_test_gui/test/test_controller.py::test_crab_and_forward_share_one_motor_polarity
...
2. **debt-004 재정의** — "실측 2건 상호 배치"(허위 전제) → "`kin_steer_sign` 미확정(조향 +counts 가
   ...
   `Tool/amr_test_gui` 는 무관" 으로 교체. 상환 방법도 "오독분 폐기" → "잭업 상태에서 조향 +90°
