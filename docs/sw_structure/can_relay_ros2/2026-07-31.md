...
> ⚠ **상위 계약 정정됨 (2026-07-31-004)**: 이 문서는 Python→C++ 포팅 직전 스냅샷이라
> 현행 입력을 `cmd_vel(Twist)` 로 전제하나, 상위 계약은 `/motor/low_cmd(MotorCmdArray)`
> (벤치 경로는 `~/steer_deg`·`~/drive_mmps`)로 정정되었다. 동반 `2026-07-31-sequence.drawio`
> 의 e4 라벨은 이미 정정 반영됨. 본문의 `cmd_vel` 서술은 당시 스냅샷 기록으로 보존한다.
...
| `~/stop` | `std_srvs/Trigger` | `driver_node.py:295` | `stop_all()` — `stop()` 으로 구동을 0 으로 하고 `release_steer_target()` 을 호출한다. 이 호출은 **우리가 걸어 둔 조향 목표의 재송신을 멈출 뿐 조향축에 프레임을 보내지 않는다** — 드라이브가 이미 받은 목표까지는 못 세우므로 축은 직전 PP 목표까지 계속 회전한다. '현재 실측 위치를 새 `0x607A` 목표로 덮어쓰기' 방식은 벤더 매뉴얼·상류·마스터 캡처에 없고(engage 직후 0 위치 보고 구간에서 축을 크게 돌릴 위험) 채택하지 않는다(2026-08-05 결정; `backend.py:504-518,455-479`; 커밋 `efcd246`). 급정지 수단은 node guarding 폴 중단 또는 하드웨어 E-STOP 이다. ⚠ 2026-08-01 이전 서술 「조향은 현 위치 유지」는 **틀렸다** — 당시 `stop()` 은 `drive_nodes` 에만 프레임을 보냈고 조향축에 도달한 프레임이 0 이었다. 정지는 소프트웨어가 아니라 구동 완료·정지마찰로 끝났다 |
...
`auth(0xE9)` 를 `intercept(0xE8)` 보다 **먼저** 보낸다 — 펌웨어가 `pc_authority` 없이 온
intercept 를 무시하기 때문이다(`link.py:196-200`). ⚠ 2026-09-05 정정: 예전 서술이 근거로 든
「재engage 래치」는 **존재하지 않는다**. 근거였던 `relay_off_latched` 는 값을 쓰기만 하고
읽는 곳이 없는 죽은 변수였고 같은 날 펌웨어에서 제거했다(md5 `f5e3b6ae…`). 중간 실패 시 어중간한 상태로 두지 않고
