...
- 대상: ~~`src/` 아래 ROS2 패키지 **전체**(13 `package.xml` + 4 빈 stub 디렉토리)~~
  → ⚠ **2026-07-27 감사 정정(반증됨)**: '전체' 가 아니다. 실제 대상은 **본 분석이 커버한 13 패키지**이며,
  작성 시각(:1 `2026-07-26 21:30`) 이전에 이미 존재하던 **11 패키지가 미커버**다.
  실측: `find src -name package.xml | wc -l` → **24**. 미커버 11개(파일 mtime 기준, 전부 21:30 이전 생성):
  ...
  이 11개는 본 문서 ①~⑤ 및 ④ 표 어디에도 등장하지 않는다 → **커버리지 공백**.
  또 stub 개수 표기가 문서 내에서 **4 / 5(+2) / 7 세 값으로 갈린다**
  (정정 전 원본 행번호 기준: `:7` '4 빈 stub' vs `:9` 열거 **7개** vs `:514` 'stub 5(+2)' vs `:518-526` 표 **7행**).
  실제 열거 개수인 **7** 로 통일한다 — 그중 2WS 는 stub 이 아니므로(§'stub 5(+2) 구성' 정정 블록 참조)
  **실제 빈 디렉토리는 6개**다.
...
- **stub(코드 0, 빈 디렉토리) — 실측상 파일 없음**: `Comm/CAN/can_relay`(Black Panda relay), `Control/Seer`, `Safety`, `Sensors/Lidar/3D`, `Control/Motion_Control/{2WS,4IS,DD}`. → 시스템 아키텍처의 Seer·Panda·Safety PLC 체인은 **아직 코드 미구현**(HW/펌웨어·외부). 본 문서는 존재하는 코드만 그린다.
  - ⚠ **2026-07-27 감사 정정(반증됨)**: 위 열거 중 **`Control/Motion_Control/2WS` 는 stub 이 아니다.**
    실측 `find src/Control/Motion_Control/2WS -type f \( -name '*.py' -o -name '*.cpp' -o -name '*.hpp' -o -name '*.h' -o -name '*.c' \) | wc -l` → **67**,
    `find src/Control/Motion_Control/2WS -name package.xml | wc -l` → **6**(생성 2026-07-26 17:51, 본 문서 작성 21:30 보다 3h39m 앞섬).
    예: `src/Control/Motion_Control/2WS/trnav_2ws_core/src/localization_monitor.cpp`.
    → 2WS 는 **미구현이 아니라 본 문서의 커버리지 공백**이다. 상세는 §'stub 5(+2) 구성' 정정 블록 참조.
  - ⚠ 나머지 6개(`can_relay`·`Seer`·`Safety`·`Lidar/3D`·`4IS`·`DD`)는 2026-07-27 재실측에서 **파일 0 확인**(전체 파일 수 0). 이 항목은 유효.
...
- **stub(코드 0)**: ~~`can_relay`·`Seer`·`Safety`·`Lidar/3D`·`Motion_Control/{2WS,4IS,DD}` 는 빈 디렉토리(파일 0) — 아키텍처 정본의 Black Panda relay·Seer 주행제어·Safety PLC 안전체인·3D 인지·타 운동학 플랫폼은 **아직 미구현**.~~
  - ⚠ **2026-07-27 감사 정정**: ① **2WS 는 빈 디렉토리가 아니다** — 실측 6 `package.xml` / 67 코드파일(생성 2026-07-26 17:51). 본 문서의 커버리지 공백일 뿐 미구현이 아니다. ② 나머지 6개는 재실측에서 파일 0 유지. ③ '미구현' 판정의 범위 조건은 **`src/` ROS2 패키지 한정**이며, **Black Panda relay 는 `Tools/Can_Relay/panda-firmware` 에 실제 코드로 존재하고 `docs/sw_structure/panda-relay-firmware/2026-07-20.md` 에 구조가 실측 기록돼 있다.** 근거·상세는 §'stub 5(+2) 구성' 정정 블록 참조.
...
> ⚠ **2026-07-27 감사 정정 — 위 검증은 무효였다(근거없음).**
> `find` 의 `-name` 은 **단일 glob 만** 받고 `|` 는 리터럴 문자다. 따라서 이 패턴은 **어떤 파일에도 매치되지 않으며 항상 0 을 반환**한다.
> 실증(코드가 확실히 존재하는 디렉토리에 그대로 실행): `find src/Control/Motion_Control/QD -name '*.py|*.cpp|*.hpp|*.h|*.c' | wc -l` → **0**
> (QD 는 본 문서 § ① 파일 의존 그래프의 `MC`/`KIN`/`QD`/`AS` subgraph 가 실측 대상으로 올린 trnav 6 패키지가 들어있는 디렉토리다.)
> 즉 아래 표의 '0' 은 stub 여부에 대한 **어떤 근거도 되지 못한다**.
> **유효 명령**: `find <dir> -type f \( -name "*.py" -o -name "*.cpp" -o -name "*.hpp" -o -name "*.h" -o -name "*.c" \)`

**2026-07-27 유효 명령으로 재실측한 결과** (원표의 '0' 값은 위 사유로 미검증 → 아래 재계수로 대체):

| stub 디렉토리 | 코드 파일(원표, **무효**) | 2026-07-27 재실측 | 아키텍처 역할(2026-06-27 정본) |
|---------------|-----------|-----------|------------------------------|
| `Comm/CAN/can_relay` | ~~0~~ | 0 (전체 파일 0) ✓ | Black Panda CAN relay(Seer↔모터 인라인) |
| `Control/Seer` | ~~0~~ | 0 (전체 파일 0) ✓ | Seer 주행제어 leg |
| `Safety` | ~~0~~ | 0 (전체 파일 0) ✓ | Safety PLC 안전 인증 체인 |
| `Sensors/Lidar/3D` | ~~0~~ | 0 (전체 파일 0) ✓ | 3D 인지 |
| `Control/Motion_Control/2WS` | ~~0~~ | **67 코드파일 / 6 package.xml — ⚠ 오탐, stub 아님** | 2-Wheel Steer 운동학 플랫폼 |
| `Control/Motion_Control/4IS` | ~~0~~ | 0 (전체 파일 0) ✓ | 4-Independent Steer 운동학 플랫폼 |
| `Control/Motion_Control/DD` | ~~0~~ | 0 (전체 파일 0) ✓ | Differential Drive 운동학 플랫폼 |

> ⚠ **2026-07-27 정정 — `Control/Motion_Control/2WS` 행은 오탐이다(반증됨).**
> 실측 재계수: 코드파일 **67**, `package.xml` **6**(`trnav_2ws_core`/`_msgs`/`_action_server`/`_kinematics`/`_motion`/`_interfaces`).
> 파일 생성 시각 `find ... -printf '%T+'` → **2026-07-26 17:51:14** 로, 본 문서가 스스로 밝힌 작성 시각(:1 `2026-07-26 21:30 (KST)`) 보다 **3시간 39분 앞선다**.
> 예: `src/Control/Motion_Control/2WS/trnav_2ws_core/src/localization_monitor.cpp`.
> 0 이 나온 이유는 위 무효 `find` 패턴이다. 본 문서 ①~⑤ 는 2WS 를 다루지 않으므로 **커버리지 공백**으로 표기한다.
> 2WS 구조는 신규 날짜 파일로 별도 작성할 것(rule 9 이중기록).

→ ~~아키텍처 정본의 Black Panda relay·Seer 주행·Safety PLC·3D 인지·타 운동학 플랫폼은 **본 날짜(2026-07-26) 기준 코드 미구현**.~~
→ ⚠ **2026-07-27 감사 정정(조건 명시)**: 위 판정은 문서 :7 이 선언한 범위, 즉 **`src/` 아래 ROS2 패키지로는** 에서만 성립한다.
  - **Black Panda relay 는 저장소에 자산으로 존재·문서화돼 있다** — 구조 실측 기록 `docs/sw_structure/panda-relay-firmware/2026-07-20.md:5`(대상: amap-1 `~/T-Robotics/CAN_Relay/panda`, git HEAD `26524538` 중 CAN 릴레이 경로 서브셋), 개조 펌웨어 `Tools/Can_Relay/panda-firmware/board/main.c:116`(`set_intercept_relay(true);  // CAN-Relay: intercept(릴레이 ON) 테스트`), 동 `:257-262`(`// CAN-Relay fail-open: 이상 상태에서는 릴레이를 intercept 로 두지 않는다.` → `set_intercept_relay(false);`).
  - **2WS 운동학 플랫폼은 미구현이 아니다** — 위 표 정정 참조(6 패키지 / 67 코드파일).
  - 나머지(Seer 주행·Safety PLC·3D 인지·4IS·DD)는 `src/` ROS2 패키지 기준 미구현이 2026-07-27 재실측으로 유지된다.
  구현 시 신규 날짜 파일로 갱신(rule 9 이중기록).
