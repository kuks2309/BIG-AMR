...
2. 참조 자료(PDF 등)는 프로젝트 루트 `References/` 에 보관(docs 와 분리)하고, 인용 시 출처·페이지·버전을 명시한다.
   - 폴더명은 **복수·대문자 `References/`** 고정. `references/`·`reference/`·`Reference/` 로 쓰지 말 것 —
     리눅스는 대소문자를 구분해 폴더가 둘로 갈라진다(2026-08-04 실제 발생·병합, §1).
   - 조회는 `ls References` 또는 `find . -maxdepth 1 -iname 'reference*'` 로 한다.
     소문자로 `ls` 해서 실패한 것을 "자료 부재"로 판정하지 말 것 (실제 오판 사례 있음).
   - **타 저장소의 참조 폴더는 철자를 고치지 말고 저장소 이름을 접두로** 붙인다 (예: `T-Robot_seer_gui/references/…`).
     접두 없는 `references/…` 인용은 본 저장소 자산 주장으로 읽힌다.
