...
- **철자·대소문자는 `References` 하나로 고정 (복수·대문자 R)** — 루트 폴더명은 정확히 `References/` 다. **소문자 `references/`·단수 `Reference/`·단수소문자 `reference/` 는 금지**이며 발견 즉시 §1 정규화 대상이다. 리눅스 파일시스템은 대소문자를 구분하므로 표기가 갈리면 **같은 이름의 폴더가 둘로 갈라져 공존**한다(실제 발생: 2026-08-04 `references/` + `References/` 공존 병합 — 아래 CHANGELOG 2.2.0). `ls References` 가 성공하는지로 확인하고, 실패하면 `find . -maxdepth 1 -iname 'reference*'` 로 전수 조사한 뒤 병합한다.
- **저장 폴더가 없으면 만든다 (승인 불요)** — 첫 보관·다운로드 시 `References/<vendor>/<product>/` 경로를 자동 생성한다.
- **저장 폴더가 표준과 다르면 표준 경로로 정규화한다** — 기존 참조 문서가 비표준 위치(예: 소문자 `references/`, 단수 `reference/`·`Reference/`, `docs/references/`, `docs/manual/`, `manuals/`, `datasheets/`, 루트 산재)에 있으면 `References/<vendor>/<product>/` 로 이동하고, 그 문서를 가리키던 인용·링크를 **모두 갱신**한 뒤 이동 내역을 작업 보고에 남긴다. 모든 프로젝트가 동일 경로 규약을 따르게 하기 위함이다.
- **타 저장소의 참조 폴더는 정규화 대상이 아니다** — 다른 저장소(예: `~/T-Robot_seer_gui/references/…`)의 경로 규약은 본 룰이 미치지 않으므로 **철자를 고치지 않는다.** 대신 인용할 때 **저장소 이름을 접두로 붙여**(`T-Robot_seer_gui/references/…`) 본 저장소 경로로 오독되지 않게 한다. 접두 없는 `references/…` 인용은 "본 저장소에 그 파일이 있다"는 주장으로 읽히므로, 실재하지 않으면 §10 등급을 ⚠ 로 낮춘다.
...
grep -nE "\]\((docs/[Rr]eferences|docs/manual|manual|manuals|datasheets|reference)/" $TARGET

# 6. 철자·대소문자 위반 탐지 (references/ · reference/ · Reference/) — §1
#    타 저장소 접두(<repo>/references/)는 정규화 대상이 아니므로 제외한다.
grep -nE "(^|[^/[:alnum:]_-])(references|reference|Reference)/" $TARGET \
  | grep -vE "[A-Za-z0-9_-]+/(references|reference)/"

# 7. 폴더 실재 확인 (룰 활성 저장소 전체) — §1 대소문자 분기 방지
find . -maxdepth 1 -iname 'reference*' -type d   # 결과가 References/ 1건이 아니면 병합 필요
```
...
2. **보관 폴더는 `References/` 하나** — 복수·대문자 R 고정. `references/`·`reference/`·`Reference/` 금지, 발견 시 병합·정규화 (§1). 타 저장소 경로는 고치지 말고 저장소 이름을 접두로 붙인다.
3. **1차 source 직접 확인 강제** — SDK / 드라이버 default / docstring 에서 1차 spec 추론 금지 (§3).
4. **인용 형식 강제** — `[문서명 v버전, Table N, page P](경로)` (§2).
5. **검증 등급 ✓/ⓦ/⚠ 표기 의무** — 모든 spec 주장에 등급 부착 (§10).
6. **강한 단정어 사용 조건** — `위반/fail/non-compliance` 는 primary source 인용 첨부 시에만 (§8).
7. **추정 금지 · 실측 검증** (§4).
8. **다중 1차 source 분리** — DataSheet ≠ User Manual ≠ SDK ≠ 표준 ≠ 논문 (§3.2).
9. **도메인 Add-on 은 선택 sub-file** — 5요건 충족 시 추가 (§13).
10. **자체 점검 grep 통과 의무** — §14 + 활성 sub-file grep.
...
- **2.2.0** (2026-08-04) — §1 **철자·대소문자 규약 명문화**: 루트 폴더를 `References/`(복수·대문자 R) 로 고정하고 `references/`·`reference/`·`Reference/` 를 금지·정규화 대상으로 등재. 타 저장소 참조 폴더는 정규화 대상에서 제외하되 **저장소 이름 접두 의무**를 신설(접두 없는 인용은 본 저장소 주장으로 읽히므로 실재하지 않으면 ⚠). §14 에 자체 점검 grep **#6(철자 위반 탐지)·#7(폴더 실재 확인)** 추가, #5 를 대소문자 양쪽 탐지로 확장. 요약 룰 2번 신설(이하 번호 이동).
  - 계기: 본 저장소에 `references/`(미추적 7파일)와 `References/`(추적 18파일)가 **동시에 존재**했다. 리눅스 파일시스템이 대소문자를 구분해 같은 뜻의 폴더가 갈라진 것으로, 2026-06-24 비교 문서의 "1차 source 부재" 판정도 이 분기에서 비롯됐다(`ls references` 실패 → 대문자 폴더를 못 봄). 2026-08-04 `References/` 로 병합.
- **2.1.0** — 2.0.0 + §1 선택적 제품분야(category) 계층 — 다품목 벤더용 3단계 `<vendor>/<category>/<product>` 경로 규약 + `docs/` 파생물 동일 taxonomy 명문화
