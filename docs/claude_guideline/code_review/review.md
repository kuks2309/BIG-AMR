...
- `<패키지루트>` = 리뷰 대상 파일이 속한 패키지/툴 루트(예: `Tools/<도구명>/`, `src/<도메인>/<pkg>/`). ROS2 패키지는 `package.xml`, 그 외는 대상 파일이 속한 최상위 도구/모듈 디렉토리로 식별.
  - 본 저장소에서 비-ROS2 도구의 툴 루트는 **반드시 `Tools/<도구명>/`**(대문자 T·복수) 이다 — 규약: [README.md §루트 도구 폴더명](../../../README.md). 소문자 `tools/` 는 저장소에 존재하지 않는다.
