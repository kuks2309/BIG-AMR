...
저는 처음에 iAHRS가 ttyUSB0으로 잡혀서 이렇게 했어요. 만약 번호가 다르면 그 번호를 입력하세요.
...
**주의: idVendor, idProduct, serial 값은 제품/칩셋에 따라 다릅니다. 반드시 위 명령어로 확인한 값을 사용하세요.**

예시 1) FTDI 칩 기반 iAHRS:

    KERNEL=="ttyUSB*", ATTRS{idVendor}=="0403", ATTRS{idProduct}=="6015", ATTRS{serial}=="DM03L0C6", MODE:="0666", GROUP:="dialout", SYMLINK+="IMU"

예시 2) Silicon Labs CP210x 칩 기반 iAHRS:

    KERNEL=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", ATTRS{serial}=="0001", MODE:="0666", GROUP:="dialout", SYMLINK+="IMU"

**참고: rplidar 등 다른 장치도 CP210x(10c4:ea60)를 사용할 수 있으므로, serial 번호로 구분이 필수입니다.**
