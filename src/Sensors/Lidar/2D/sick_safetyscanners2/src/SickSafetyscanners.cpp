    ...
    std::vector<rclcpp::Parameter> parameters)
{
    ...
    for (const auto &param : parameters)
    {
        ...
        {
            m_config.m_communications_settings.publishing_frequency = skipToPublishFrequency(param.as_int());
            update_sensor_config = true;
        }
        ...
        {
            m_config.m_communications_settings.end_angle = sick::radToDeg(param.as_double()) - m_config.m_angle_offset;
            update_sensor_config = true;
        }
        ...
        else if (param.get_name() == "general_system_state")
        {
            ...
            m_config.m_communications_settings.features =
                (m_config.m_communications_settings.features & ~(1UL << 0)) | (param.as_bool() << 0);
            update_sensor_config = true;
        }
        else if (param.get_name() == "derived_settings")
        {
            m_config.m_communications_settings.features =
                (m_config.m_communications_settings.features & ~(1UL << 1)) | (param.as_bool() << 1);
            update_sensor_config = true;
        }
        else if (param.get_name() == "measurement_data")
        {
            m_config.m_communications_settings.features =
                (m_config.m_communications_settings.features & ~(1UL << 2)) | (param.as_bool() << 2);
            update_sensor_config = true;
        }
        else if (param.get_name() == "intrusion_data")
        {
            m_config.m_communications_settings.features =
                (m_config.m_communications_settings.features & ~(1UL << 3)) | (param.as_bool() << 3);
            update_sensor_config = true;
        }
        else if (param.get_name() == "application_io_data")
        {
            m_config.m_communications_settings.features =
                (m_config.m_communications_settings.features & ~(1UL << 4)) | (param.as_bool() << 4);
            update_sensor_config = true;
        }
...
void SickSafetyscanners::setupCommunication(std::function<void(const sick::datastructure::Data &)> callback)
{
    // Create a sensor instance
    if (m_config.m_communications_settings.host_ip.is_multicast())
    {
        m_device = std::make_unique<sick::AsyncSickSafetyScanner>(m_config.m_sensor_ip, m_config.m_tcp_port,
                                                                  m_config.m_communications_settings,
                                                                  m_config.m_interface_ip, callback);
    }
    else
    {
        m_device = std::make_unique<sick::AsyncSickSafetyScanner>(m_config.m_sensor_ip, m_config.m_tcp_port,
...
void SickSafetyscanners::sensorDiagnostics(diagnostic_updater::DiagnosticStatusWrapper &diagnostic_status)
{
    const sick_safetyscanners2_interfaces::msg::DataHeader &header = m_last_raw_msg.header;
    if (header.timestamp_time == 0 && header.timestamp_date == 0)
    {
        diagnostic_status.summary(diagnostic_msgs::msg::DiagnosticStatus::STALE, "Could not get sensor state");
        return;
    }
    ...
    diagnostic_status.addf("Firmware version", "%s", m_config.m_firmware_version.getFirmwareVersion().c_str());
    ...
    diagnostic_status.addf("App checksum", "%08X", m_config.m_metadata.getAppChecksum());
    diagnostic_status.addf("Overall checksum", "%08X", m_config.m_metadata.getOverallChecksum());
    ...
    const sick_safetyscanners2_interfaces::msg::GeneralSystemState &state = m_last_raw_msg.general_system_state;
    ...
    {
        diagnostic_status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Application error");
    }
    else if (state.contamination_error)
    {
        diagnostic_status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Contamination error");
    }
    else if (state.contamination_warning)
    {
        diagnostic_status.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN, "Contamination warning");
    }
    else
    {
    ...
    std::shared_ptr<sick_safetyscanners2_interfaces::srv::FieldData::Response> response)
{
    ...
    for (size_t i = 0; i < fields.size(); i++)
    {
        ...
        field_msg.start_angle = degToRad(field.getStartAngle() + m_config.m_angle_offset);
