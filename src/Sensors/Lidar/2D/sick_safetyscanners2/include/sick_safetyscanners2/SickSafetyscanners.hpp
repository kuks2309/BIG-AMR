...
class SickSafetyscanners
{
  public:
    ...
    struct Config
    {
        ...
        {
            m_msg_creator = std::make_unique<sick::MessageCreator>(m_frame_id, m_time_offset, m_range_min, m_range_max,
                                                                   m_angle_offset, m_min_intensities);
        }
    ...
    template <typename NodeT> void loadParameters(NodeT &node)
    {
        ...
        m_config.m_interface_ip = boost::asio::ip::address_v4::from_string(interface_ip);
        ...
        m_config.m_communications_settings.host_ip = boost::asio::ip::address_v4::from_string(host_ip);
        ...
        m_config.m_communications_settings.publishing_frequency = skipToPublishFrequency(skip);
        ...
        {
            m_config.m_communications_settings.start_angle = sick::radToDeg(angle_start) - m_config.m_angle_offset;
            m_config.m_communications_settings.end_angle = sick::radToDeg(angle_end) - m_config.m_angle_offset;
        }
        ...
        m_config.m_communications_settings.features = sick::SensorDataFeatures::toFeatureFlags(
    ...
    void startCommunication(NodeT *node, rclcpp::Publisher<sensor_msgs::msg::LaserScan>::SharedPtr publisher)
    {
        // Set-up diagnostics
        ...
        diagnostic_updater::FrequencyStatusParam frequency_param(
            &m_config.m_expected_frequency, &m_config.m_expected_frequency, m_config.m_frequency_tolerance);

        diagnostic_updater::TimeStampStatusParam timestamp_param(m_config.m_timestamp_min_acceptable,
                                                                 m_config.m_timestamp_max_acceptable);

        m_diagnosed_laser_scan_publisher = std::make_shared<DiagnosedLaserScanPublisher>(
    ...
    bool getFieldData(const std::shared_ptr<sick_safetyscanners2_interfaces::srv::FieldData::Request> request,
