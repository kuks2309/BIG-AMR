...
- 수정 `include/…/localization_monitor.hpp` — 「상류 원본의 trnav_pose_publisher 는 이 저장소에 없다」는
  조사 결과 보고를 빼고, 운영 제약만 남겼다: 실기 `/robot_pose` 발행자 미정(debt-068) → 기동 시 `pose_topic:=` 명시,
  SIL 은 `src/Sim/sil_pose_adapter`.
  `Params::pose_qos` 주석의 옵션 서술도 코드(0/1/2 분기)와 맞췄다.
- 수정 `CMakeLists.txt` — 계층 taxonomy 에서 「(not present in this repository)」 부재 서술 제거.
  DD 슬롯 이름은 ADR-012 「분리」 근거의 한쪽 항이라 유지.
- 수정 `package.xml` — `<description>` 에서 부재 서술·이력 문서 안내 제거.
- 수정 `test/test_motion_profile.cpp` — 프로파일 경계 서술을 구현식으로 재도출한 값에 맞춤.
...
2026-05-21 / 22:30 - (pending commit) / **삭제** (ADR-012 QD/DD 분리): 5 cpp + 6 hpp 를 `trnav_2ws_motion` 패키지로 이전. core 는 platform-agnostic 만 잔존.

- 삭제 cpp (→ trnav_2ws_motion): `src/inverse_kinematics.cpp`, `src/bicycle_model.cpp`, `src/path_controller.cpp`, `src/pure_pursuit_controller.cpp`, `src/wheel_set_packer.cpp`
- 삭제 hpp (→ trnav_2ws_motion): `include/trnav_2ws_core/inverse_kinematics.hpp`, `bicycle_model.hpp`, `path_controller.hpp`, `pure_pursuit_controller.hpp`, `wheel_set_packer.hpp`, `action_server_base.hpp` (header-only template 도 이전)
...
- 패키지 namespace `trnav_2ws_core` 무변경. consumer 의 잔존 심볼 (`ActionMutex`, `normalizeAngle`, `RobotGeometry`, `Platform`, `parsePlatform`, `TrapezoidalProfile`, `TransientGuard`, `LocalizationMonitor`, `RecursiveMovingAverage`) 호출부 무변경.
- 빌드: `colcon build --packages-select trnav_2ws_core trnav_2ws_motion trnav_motion_dd trnav_2ws_action_server --symlink-install` PASS (0 failures, 0 stderr).
...
- `include/trnav_2ws_core/pure_pursuit_controller.hpp`: 두 선언 추가 (`reset()` 다음 줄)
...
- `include/trnav_2ws_core/path_controller.hpp`: setMode 다음 줄에 선언 추가
...
- 호출자: `trnav_2ws_action_server` 의 translate_forward / translate_reverse `add_on_set_parameters_callback` (별도 commit)
...
- 수정: `include/trnav_2ws_core/localization_monitor.hpp`
...
검증: `colcon build --packages-up-to trnav_2ws_action_server --symlink-install` PASS (motion_core 21.3s + 6 server 재컴파일 5min 14s).
...
빌드: `colcon build --packages-up-to trnav_2ws_action_server --symlink-install` PASS.

영향: 6 action server (translate_forward/reverse, pure_pursuit/_reverse, yaw_control/_reverse) 가 본 enum 으로 finish_abort 코드 -4/-5/-6 분기 가능 (`trnav_2ws_action_server` 별도 patch).
...
- 검증: `colcon build --packages-up-to trnav_2ws_action_server` PASS
...
- `include/trnav_2ws_core/pure_pursuit_controller.hpp` (신규)
...
빌드 검증: `colcon build --base-paths src --packages-select trnav_2ws_core` PASS, `libpure_pursuit_controller.a` 정상 install.
...
- 사용처: `trnav_2ws_action_server::yaw_control` 의 yaw error / offset 계산 (기존 익명 namespace 의 while-loop wrapPi/wrapDeg 대체)
...
- `include/trnav_2ws_core/path_controller.hpp`
...
빌드: `colcon build --packages-select trnav_2ws_core` 성공 (~1.4s).
...
- `include/trnav_2ws_core/recursive_moving_average.hpp` (header-only, 원본 포팅)
- `include/trnav_2ws_core/path_controller.hpp` + `src/path_controller.cpp`
...
- `include/trnav_2ws_core/transient_guard.hpp` + `src/transient_guard.cpp` (원본 그대로 포팅, namespace 교체)
- `include/trnav_2ws_core/localization_monitor.hpp` + `src/localization_monitor.cpp`
...
빌드 검증: `colcon build --packages-select trnav_2ws_core` 성공 (12.0s, 경고 없음).
...
- `include/trnav_2ws_core/bicycle_model.hpp` (원본 `amr_motion_control::BicycleModel` 포팅, namespace·include guard 교체, 로직 동일)
- `src/bicycle_model.cpp` (원본 로직 그대로, include 경로만 `trnav_2ws_core/bicycle_model.hpp` 로 교체)
- `CMakeLists.txt` — STATIC 라이브러리 `bicycle_model` 추가, `inverse_kinematics` PUBLIC 링크, install TARGETS 에 포함
- `docs/trnav_2ws_core_code_updates.md` 초기 생성

빌드 검증: `colcon build --packages-select trnav_2ws_core` 성공 (1.9s, 경고 없음, `libbicycle_model.a` 설치).
