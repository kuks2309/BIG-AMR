# SIL — 후진 원호(turn_reverse). 전진판 `sil_turn.launch.py` 의 짝.
#   mux source 12 = turn_reverse (10·11 은 stanley 예약).
#   ADR: docs/adr/2026-08-09-turn-reverse.md
"""
SIL(Software In the Loop) closed-loop launch for amr_turn_reverse_node.

체인:
  amr_turn_reverse_node ─ /motion/wheel_cmd/turn_reverse ─→ trnav_motion_mux
  trnav_motion_supervisor ─ service ─→ trnav_motion_mux  (active source = turn_reverse, id=12)
...
def generate_launch_description():
    ...
    supervisor_node = Node(
        ...
        output='screen',
        parameters=[{'target_source_id': 12}],  # source 12 = turn_reverse
    )
    ...
    turn_node = Node(
        ...
        executable='amr_turn_reverse_node',
        name='trnav_turn_reverse_node',
        output='screen',
        parameters=[PathJoinSubstitution([
            FindPackageShare('trnav_2ws_action_server'),
            'config', 'turn_reverse_params.yaml'])],
    )
