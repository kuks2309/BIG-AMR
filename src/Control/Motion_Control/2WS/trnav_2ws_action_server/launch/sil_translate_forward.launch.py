"""
SIL closed-loop launch for amr_translate_forward_node.

체인:
  amr_translate_forward_node ─ /motion/wheel_cmd/translate_forward ─→ trnav_motion_mux
  trnav_motion_supervisor ─ service ─→ trnav_motion_mux  (active source = translate_forward, id=1)
  ...
  translate_sim_odom_node ─→ TF + /rtabmap/localization_pose + /imu/data + /wheel_motor_state
                            ─→ amr_translate_forward_node (피드백)
...
def generate_launch_description():
    ...
    # (translate_forward_action_server.cpp:408).
    # ⚠ 실차 `/robot_pose` 발행자는 미정이다(debt-068) — 기동 시 `pose_topic:=` 을 명시하라.
    #   저장소 유일 후보 seer_pose_publisher 의 기본 발행은 `/seer/robot_pose` 라 그대로는 맞지 않는다.
    #   배선: docs/code_review/pose-topic-wiring/2026-08-10.md
    ...
    supervisor_node = Node(
        ...
        output='screen',
        parameters=[{'target_source_id': 1}],  # source 1 = translate_forward
    )
    ...
    translate_forward_node = Node(
        package='trnav_2ws_action_server',
        executable='amr_translate_forward_node',
        name='trnav_translate_forward_node',
        output='screen',
        parameters=[PathJoinSubstitution([
            FindPackageShare('trnav_2ws_action_server'),
            'config', 'translate_forward_params.yaml'])],
    )
