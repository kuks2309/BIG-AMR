"""
SIL(Software In the Loop) closed-loop launch for amr_yaw_control_reverse_node.

체인:
  amr_yaw_control_reverse_node ─ /motion/wheel_cmd/yaw_control_reverse ─→ trnav_motion_mux
  trnav_motion_supervisor ─ service ─→ trnav_motion_mux  (active source = yaw_control_reverse, id=7)
...
⚠ `yaw_control_reverse` 의 종료 조건도 **거리**다(헤딩 수렴이 아니다). 헤딩은 추종 목표일 뿐이며
...
def generate_launch_description():
    ...
    supervisor_node = Node(
        ...
        output='screen',
        parameters=[{'target_source_id': 7}],  # source 7 = yaw_control_reverse
    )
    ...
    return LaunchDescription(args + [
        ...
        dummy_estop,
        dummy_lidar,
        yaw_control_reverse_node,
    ])

# ⚠ `vx_max` 는 **magnitude**(>0)만 받는다 — 후진 의미는 액션 자체가 갖는다.
