#include "trnav_2ws_action_server/mpc/mpc_action_server.hpp"
...
MpcActionServer::MpcActionServer(rclcpp::Node::SharedPtr node, ActionMutex action_mutex)
    : trnav::motion::two_ws::TwoWsActionServerBase<Mpc>(
...
{
    trnav::motion::two_ws::TwoWsActionServerBase<Mpc>::wheelStateCallback(msg);
...
void MpcActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "/robot_pose 미수신 또는 낡음(신선도 초과) — TF 문제가 아니다. 측위 발행자와 pose_topic 결속을 확인하라. aborting mpc");
        finish_abort(-3, 0.0, 0.0, 0.0, start_time);
        return;
    }
