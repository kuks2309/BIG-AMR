    ...
    while (rclcpp::ok() && !reached)
    {
        ...
        if ((node_->now() - start_time).seconds() > max_timeout_sec_)
        {
            RCLCPP_WARN(node_->get_logger(), "YawControlReverse global timeout (%.1f s), dist=%.3f m",
                        max_timeout_sec_.load(), current_distance);
            double calibrated_yaw = normalizeAngle(last_yaw_rad_.load() + yaw_offset);
        ...
        //   그 계산의 「축 10.3 °/s」는 성립하지 않는다 — **조향 속도는 축의 성질이 아니라
        //   `0x6081`(profile velocity) 설정값이 정한다.** 시간에서 역산해 「축 속도」라 부른 것
        //   자체가 잘못이었다. 게다가 큰 각 조향은 미검증 영역이 아니다 —
        //   **호밍이 매번 최대 회전을 돌고 10/10 성공한다.**
        //   실제 고장은 지령 −20.2° 대 실제 0.00°(오차 20.2° **고정**)였고
        ...
        if (guard_out.gate_blocked)
        {
            ...
            {
                RCLCPP_ERROR(node_->get_logger(),
                             "YawControlReverse 조향 미도달 %.1f s 지속 — 발행 F=%.2f°/R=%.2f° 대 "
                             "실제 F=%.2f°/R=%.2f°. 조향축이 지령을 실행하지 못한다. abort(-8)",
