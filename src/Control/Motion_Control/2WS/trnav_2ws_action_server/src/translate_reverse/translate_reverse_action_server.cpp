...
#include "trnav_2ws_core/math_utils.hpp" // normalizeAngle (effective_yaw 보정)
...
using trnav::motion::two_ws::TravelDirection;
...
void TranslateReverseActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "/robot_pose 미수신 또는 낡음(신선도 초과) — TF 문제가 아니다. 측위 발행자와 pose_topic 결속을 확인하라. aborting translate_reverse");
        finish_abort(-3, 0.0, 0.0, 0.0, start_time);
        return;
    }
