...
TwoWsCrabIK::TwoWsCrabIK(std::size_t num_wheels, double wheel_radius, double gear_walk)
    : num_wheels_(num_wheels), wheel_radius_(wheel_radius), gear_walk_(gear_walk)
{
}

void TwoWsCrabIK::setInitial(double initial_base_steer, int initial_walk_dir)
{
...
//
// ⚠ 2026-07-27 감사 정정 — 위 2줄의 수치가 **아래 실제 상수와 다르다**(주석이 코드보다 5° 작다).
//   (원문은 이력 보존용으로 남김. 코드·상수 변경 0건.)
//   실제: WRAP_MARGIN = 25° (바로 아래 행) → wrapSteer() 는 ±(π/2 + WRAP_MARGIN) 를 임계로 쓰므로
//   ✔ 실제 wrap threshold = **±115°** (±110° 아님)
//   ✔ ±90° strict 대비 최대 어긋남 = **±25°** (±20° 아님)
//   ⇒ 정정 후 서술: 「±25° 마진 — wrap threshold ±115°, ±90° strict 대비 최대 ±25° wheel cmd 어긋남」.
//   ※ 첫 줄의 「모터 ±90° 한계」 전제도 본 기체(Foil_A082)에서는 미판정이다 —
//     qd_inverse_kinematics.cpp:60 부근의 2026-07-27 감사 정정 블록 참조(ADR 의 >90° 확인은 Carrier AGV 건).
...
IKResult TwoWsCrabIK::compute(double vx_path, double theta_body, double delta_cte,
