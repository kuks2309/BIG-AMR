...
void TwoWsPathController::setMode(ControlMode mode)
{
    mode_ = mode;
    prev_e_theta_ = 0.0;
    first_update_ = true;
...
void TwoWsPathController::setGains(double Kp_heading, double Kd_heading, double K_stanley, double K_soft,
...
void TwoWsPathController::setPath(double start_x, double start_y, double end_x, double end_y)
{
    start_x_ = start_x;
    start_y_ = start_y;
    end_x_ = end_x;
...
double TwoWsPathController::computeStanleyCTE(double e_d, double vx) const
{
    if (vx <= 1e-6)
    {
        return 0.0;
...
double TwoWsPathController::computeHeadingPD(double e_theta, double de_theta) const
{
...
PathControlOutput TwoWsPathController::computeBicycle(double e_d, double e_theta, double de_theta, double vx,
                                                   TravelDirection dir)
{
...
PathControlOutput TwoWsPathController::update(double robot_x, double robot_y, double robot_yaw, double vx, double dt,
                                           TravelDirection dir)
{
    ...
    const double e_theta_raw = trnav_2ws_core::normalizeAngle(robot_yaw - theta_path_);
...
int TwoWsPathController::validateInitialPose(double robot_x, double robot_y, double robot_yaw) const
{
    const double rx = robot_x - start_x_;
    const double ry = robot_y - start_y_;
    ...
    const double e_theta = trnav_2ws_core::normalizeAngle(robot_yaw - theta_path_);
