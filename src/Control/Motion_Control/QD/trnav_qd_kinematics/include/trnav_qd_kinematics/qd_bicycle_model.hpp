#ifndef TRNAV_QD_KINEMATICS__QD_BICYCLE_MODEL_HPP_
#define TRNAV_QD_KINEMATICS__QD_BICYCLE_MODEL_HPP_
...
/// QdBicycleModel: Abstraction layer over QdDualSteerIK for QD diagonal platform.
///
...
class QdBicycleModel
{
  public:
    /// Construct from wheel positions (same as QdDualSteerIK constructor params)
    /// Stores wheels internally for use in fromIKResult().
    /// @param wheels  Wheel positions in robot frame (expects 2 wheels)
    explicit QdBicycleModel(const std::vector<WheelPosition> &wheels);
    ...
    explicit QdBicycleModel(double wheelbase_m);
    ...
    IKResult toIKResult(const BicycleCommand &cmd, const QdDualSteerIK &ik) const;
    ...
    IKResult toIKResult(const DualBicycleCommand &cmd, const QdDualSteerIK &ik) const;
...
#endif // TRNAV_QD_KINEMATICS__QD_BICYCLE_MODEL_HPP_
