#include "trnav_qd_kinematics/qd_bicycle_model.hpp"
...
QdBicycleModel::QdBicycleModel(const std::vector<WheelPosition> &wheels) : wheels_(wheels), has_wheels_(true)
{
    if (wheels_.size() < 2)
    {
        throw std::runtime_error("QdBicycleModel requires at least 2 wheel positions");
    }
    L_ = std::abs(wheels_[0].x - wheels_[1].x);
    if (L_ < kSpeedEpsilon)
    {
        throw std::runtime_error("QdBicycleModel: computed wheelbase is too small (wheels at same X?)");
    }
}

QdBicycleModel::QdBicycleModel(double wheelbase_m) : L_(wheelbase_m), has_wheels_(false)
{
    if (L_ < kSpeedEpsilon)
    {
        throw std::runtime_error("QdBicycleModel: wheelbase_m must be > 0");
    }
}

VelocityCommand QdBicycleModel::toVelocityCommand(const BicycleCommand &cmd) const
{
...
IKResult QdBicycleModel::toIKResult(const BicycleCommand &cmd, const QdDualSteerIK &ik) const
{
    return ik.compute(toVelocityCommand(cmd));
}

VelocityCommand QdBicycleModel::toVelocityCommand(const DualBicycleCommand &cmd) const
{
...
IKResult QdBicycleModel::toIKResult(const DualBicycleCommand &cmd, const QdDualSteerIK &ik) const
{
    return ik.compute(toVelocityCommand(cmd));
}

BicycleState QdBicycleModel::fromVelocityCommand(const VelocityCommand &vel) const
{
    double v = vel.vx;
...
BicycleState QdBicycleModel::fromIKResult(const IKResult &result) const
{
    if (!has_wheels_)
    {
        throw std::runtime_error("QdBicycleModel::fromIKResult requires wheel positions "
...
double QdBicycleModel::curvature(const BicycleCommand &cmd) const
{
    double delta_clamped = std::clamp(cmd.delta, -kDeltaMax, kDeltaMax);
    return std::tan(delta_clamped) / L_;
}

double QdBicycleModel::turningRadius(const BicycleCommand &cmd) const
{
    double delta_clamped = std::clamp(cmd.delta, -kDeltaMax, kDeltaMax);
...
double QdBicycleModel::omega(const BicycleCommand &cmd) const
{
    if (std::abs(cmd.v) < kSpeedEpsilon)
    {
        return 0.0;
...
double QdBicycleModel::deltaFromOmega(double vx, double omega_val) const
{
