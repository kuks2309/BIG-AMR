...
- 관련 action server: `src/Control/AMR-Motion/trnav_motion_action_server/docs/amr_motion_action_server_code_updates.md` [2026-05-21]
...
빌드 검증: `colcon build --base-paths src --packages-select trnav_interfaces` 성공.
`ros2 interface show trnav_interfaces/action/AMRMotionPurePursuit` 노출 확인 (nav_msgs/Path 정상 expand).
...
빌드 검증: `colcon build --packages-select trnav_interfaces` 성공 (no stderr).
...
빌드 검증: `colcon build --packages-select trnav_interfaces` 성공 (27.2s, 경고 없음).
`ros2 interface show trnav_interfaces/action/AMRMotionTranslate` 로 타입 생성 확인.
...
- `package.xml` (원본 그대로, `<name>trnav_interfaces</name>`)
- `CMakeLists.txt` (원본에서 Turn/Translate/Crab/YawControl/PurePursuit 제외, Spin + ControlStop 만 활성)
- `docs/trnav_interfaces_code_updates.md` 초기 기록

결정 배경: `docs/request/2026-04-23_trnav_interfaces_porting.md` (Wave 별 점진 이식, Wave 1 = Spin)
