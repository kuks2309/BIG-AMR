#ifndef TRNAV_MOTION_QD__QD_ACTION_SERVER_BASE_HPP_
#define TRNAV_MOTION_QD__QD_ACTION_SERVER_BASE_HPP_
...
//
// QD-specific: instantiates QdDualSteerIK + QdWheelSetPacker as members. DD or other
// platforms must use a sibling base class (e.g., dd_action_server_base.hpp when added).
template <typename ActionT> class QdActionServerBase
{
  public:
    using GoalHandle = rclcpp_action::ServerGoalHandle<ActionT>;

    QdActionServerBase(rclcpp::Node::SharedPtr node, trnav_motion_core::ActionMutex action_mutex,
        ...
        : node_(node), action_mutex_(std::move(action_mutex)), packer_(loadGeometry(node))
    {
        ...
        ik_ = std::make_unique<QdDualSteerIK>(wheels, geom.wheel_radius, geom.gear_walk);
    ...
    virtual ~QdActionServerBase() = default;
  ...
  protected:
    ...
    std::unique_ptr<QdDualSteerIK> ik_;
    trnav_motion_core::ActionMutex action_mutex_;
  ...
  private:
    ...
    static trnav_motion_core::RobotGeometry loadGeometry(rclcpp::Node::SharedPtr node)
    {
        ...
        trnav_motion_core::RobotGeometry geom;
        geom.platform = trnav_motion_core::parsePlatform(get_s("platform", "QD_DIAGONAL"));
    ...
    QdWheelSetPacker packer_;
};

} // namespace trnav::motion::qd

#endif // TRNAV_MOTION_QD__QD_ACTION_SERVER_BASE_HPP_
