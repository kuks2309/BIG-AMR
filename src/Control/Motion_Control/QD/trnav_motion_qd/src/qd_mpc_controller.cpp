...
QdMpcController::QdMpcController(const Params &params) : params_(params) {}

void QdMpcController::reset()
{
    initialized_ = false;
    wp_.clear();
    seg_len_.clear();
...
bool QdMpcController::setPath(const std::vector<MpcPose> &poses)
{
    if (poses.size() < 2)
    {
        initialized_ = false;
...
void QdMpcController::closestSegmentError(double rx, double ry, double ryaw,
...
double QdMpcController::computeCost(const std::vector<double> &deltas,
...
{
    const QdMpcController *self;
    double rx, ry, ryaw;
};

double QdMpcController::nloptObjective(unsigned n, const double *x, double *grad, void *data)
{
...
MpcOutput QdMpcController::update(double rx, double ry, double ryaw,
                                   double /*current_speed*/, double /*dt*/)
{
    ...
    opt.set_min_objective(QdMpcController::nloptObjective, &ud);
...
int QdMpcController::validateInitialPose(double rx, double ry, double /*ryaw*/) const
{
