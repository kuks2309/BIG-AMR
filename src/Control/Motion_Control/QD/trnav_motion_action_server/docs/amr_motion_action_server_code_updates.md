# trnav_motion_action_server — code updates

2026-07-04 / 09:52 - (pending) / **QD 운동학 include·link 경로 이관** (AD-012 개정 — `trnav_qd_kinematics` 분리)

- 수정 헤더 8종 include 경로 `trnav_motion_qd/qd_{bicycle_model,crab_inverse_kinematics}.hpp` → `trnav_qd_kinematics/...`: `mpc`, `mpc_reverse`, `yaw_control`, `yaw_control_reverse`, `translate_forward`, `translate_reverse`, `turn` (bicycle), `crab_linear` (crab). `spin` 은 `qd_action_server_base` 만 사용 — 무변경
- 수정 `CMakeLists.txt` — `find_package(trnav_qd_kinematics)` 추가 + link 16곳 `trnav_motion_qd::qd_{inverse_kinematics×9,bicycle_model×6,crab_inverse_kinematics×1}` → `trnav_qd_kinematics::`
- 수정 `package.xml` — `<depend>trnav_qd_kinematics</depend>` 추가 (동작·로직 무변경, 경로만)
...
- 의존: `trnav_interfaces` action(TranslateForward/Reverse)에 `bool skip_initial_pose_check false` 신규 필드 + `acs_gui/ros/goal_builder.py` 가 `on_position_mismatch=='none'` 시 true 전파.
...
- offset 처리(1a): 주행 시작 후 **첫 10 control sample(50Hz, 0.2s) 로 `(pose_yaw − imu_yaw)` 평균 → 고정**(RMA10). capture 중엔 pose yaw 유지(시작 안전), 고정 후 `robot_yaw = normalizeAngle(imu_yaw + offset)`. iAHRS yaw 가 지자기로 유계(§9)라 고정 offset 성립. `#include trnav_motion_core/math_utils.hpp`(normalizeAngle).
...
- **dir rename**: `src/pure_pursuit/` → `src/mpc/`, `src/pure_pursuit_reverse/` → `src/mpc_reverse/` (헤더 동일: `include/trnav_motion_action_server/{pure_pursuit,pure_pursuit_reverse}/` → `{mpc,mpc_reverse}/`)
...
- **수정** `src/mpc/mpc_action_server.cpp` + `src/mpc_reverse/mpc_reverse_action_server.cpp`:
  - controller 교체 `QdPurePursuitController` → `QdMpcController` (NLopt SLSQP)
...
검증: `colcon build --packages-select trnav_motion_action_server` 8.33s PASS (1 format warning std::size_t %d). 전체 `colcon build` 49 packages PASS.
...
2026-05-26 / 17:00 - (pending commit) / **변경**: crab_linear Phase 0 끝 시점 `QdCrabIK::setInitial` 호출 (state-aware, L3)
...
- `src/crab_linear/crab_linear_action_server.cpp` (line 466 직후):
  ...
  ```
- 의도: Phase 0 (DualSteerIK ±π/2 strict wrap) 의 결정 (align_steer_f, walk dir) 을 cruise 의 QdCrabIK 에 전달. cruise compute() 가 본 state 기준 ±25° clamp + dir 고정 → 두 단계 부호 일관.
...
- 관련: [trnav_motion_qd_code_updates.md](../../trnav_motion_qd/docs/trnav_motion_qd_code_updates.md), [docs/issues_fixes/issues_and_fixes.md](../../../../../docs/issues_fixes/issues_and_fixes.md) [2026-05-26 17:00].
...
- 관련: [trnav_motion_qd_code_updates.md](../../trnav_motion_qd/docs/trnav_motion_qd_code_updates.md) QdCrabIK 인터페이스 확장.

---

2026-05-24 / 22:00 - (pending commit) / **변경**: crab_linear cruise 경로 `QdDualSteerIK` → `QdCrabIK` 교체 (±90° normalize edge sweep 제거, L3)
...
- `include/.../crab_linear_action_server.hpp` — include `trnav_motion_qd/qd_crab_inverse_kinematics.hpp` 추가 + 멤버 `crab_ik_: std::unique_ptr<QdCrabIK>` 추가.
- `src/crab_linear/crab_linear_action_server.cpp`:
  - constructor (path_ctrl_ 초기화 직후): `crab_ik_ = std::make_unique<QdCrabIK>(geom.num_wheels, geom.wheel_radius, geom.gear_walk)` 추가.
...
- `CMakeLists.txt` — `amr_crab_linear_node` target_link_libraries 에 `trnav_motion_qd::qd_crab_inverse_kinematics` 추가.
- **검증**:
  - Static/Regression: `colcon build --packages-select trnav_motion_qd trnav_motion_action_server` PASS (warning 0)
...
- 관련: [docs/issues_fixes/issues_and_fixes.md](../../../../../docs/issues_fixes/issues_and_fixes.md) [2026-05-24 22:00], [trnav_motion_qd_code_updates.md](../../trnav_motion_qd/docs/trnav_motion_qd_code_updates.md).
...
- `src/spin/spin_action_server.cpp` — 4 hunk:
  - 진입부: `const double target_angle_deg = trnav_motion_core::normalizeAngleDeg(goal->target_angle);` 신규 (L82)
...
- `src/turn/turn_action_server.cpp` — 3 hunk:
  - 신규 include `trnav_motion_core/math_utils.hpp` (L2)
  - 진입부: 정규화 변수 신규 (L99)
...
- **검증**:
  ...
  - 빌드: `colcon build --packages-select trnav_motion_action_server --symlink-install` PASS (1m 37s, 0 stderr) + launch 추가 후 재빌드 PASS
...
- **include path 갱신 (6 종)**: `#include "trnav_motion_core/{bicycle_model,inverse_kinematics,path_controller,pure_pursuit_controller,wheel_set_packer,action_server_base}.hpp"` → `#include "trnav_motion_qd/qd_*.hpp"`. 잔존 (`action_mutex`, `math_utils`, `motion_profile`, `transient_guard`, `localization_monitor`, `robot_geometry`, `recursive_moving_average`) 무변경.
- **qualified type 갱신 (17 종)**: `trnav_motion_core::{ActionServerBase, BicycleModel, DualSteerIK, PathController, PurePursuitController, WheelSetPacker, BicycleCommand, DualBicycleCommand, BicycleState, ControlMode, TravelDirection, PathControlOutput, PurePursuitOutput, WheelPosition, VelocityCommand, WheelOutput, IKResult}` → `trnav::motion::qd::Qd*` / `trnav::motion::qd::*`. 잔존 (`ActionMutex`, `normalizeAngle`, `RobotGeometry`, `Platform`, `parsePlatform`, `TrapezoidalProfile`, `ProfilePhase`, `ProfileOutput`, `TransientGuard`, `LocalizationMonitor`, `RecursiveMovingAverage`) 무변경.
- **base class** 모든 9 action server: `trnav_motion_core::ActionServerBase<T>` → `trnav::motion::qd::QdActionServerBase<T>`. 호출 site 의 ctor + `wheelStateCallback` 슈퍼 콜 갱신.
- **CMakeLists.txt**: `find_package(trnav_motion_qd REQUIRED)` 추가, `target_link_libraries` 5 target rename (`trnav_motion_core::bicycle_model` → `trnav_motion_qd::qd_bicycle_model` 외), `ament_target_dependencies` 9 블록에 `trnav_motion_qd` 추가.
- **package.xml**: `<depend>trnav_motion_qd</depend>` 추가 (`<depend>trnav_motion_core</depend>` 다음 줄).
- **main.cpp 9 파일**: 변경 없음 (`action_mutex.hpp` 만 include — core 잔존).
- 빌드: 4 패키지 (trnav_motion_core, trnav_motion_qd, trnav_motion_dd, trnav_motion_action_server) PASS, 0 failures, 0 stderr. trnav_motion_action_server 빌드 5m 9s.
...
- 빌드: `colcon build --packages-select trnav_motion_core trnav_motion_action_server --symlink-install` PASS (1m 52s)
...
- 빌드: `colcon build --packages-select trnav_motion_core trnav_motion_action_server --symlink-install` PASS (1m 51s)
...
2026-05-21 / 19:10 - (pending commit) / 변경: `trnav_motion_core` `PathController::setGains` 신규 (5 스칼라 게인 핫스왑 setter)

- `include/trnav_motion_core/path_controller.hpp`: `void setGains(double Kp_h, double Kd_h, double K_s, double K_so, double max_delta_rad)` 선언 추가 (setMode 다음 줄)
...
- 삭제 (8):
  - `include/trnav_motion_action_server/crab/` (hpp 1)
  ...
  - (별도 패키지) `trnav_interfaces/action/AMRMotionCrab.action` + entry
...
- 추가 (8):
  - `include/trnav_motion_action_server/crab_linear/crab_linear_action_server.hpp`
  ...
  - (별도 패키지) `trnav_interfaces/action/AMRMotionCrabLinear.action` + entry
...
- Static: `colcon build --packages-select trnav_motion_action_server --symlink-install` PASS (45.6s, exit 0)
...
- 빌드 검증: `colcon build --packages-select trnav_motion_action_server` PASS (3min 8s, exit 0)
...
관련 core: `src/Control/AMR-Motion/trnav_motion_core/docs/amr_motion_core_code_updates.md` [2026-05-18 22:10].
관련 .action: `src/Control/AMR-Motion/trnav_interfaces/docs/amr_interfaces_code_updates.md` [2026-05-18 22:10].
...
관련 core: `src/Control/AMR-Motion/trnav_motion_core/docs/amr_motion_core_code_updates.md` [2026-05-18 21:30].
...
`LocalizationMonitor::getLastFailReason()` (trnav_motion_core 측 patch 동반) 을 사용해 fail 원인별 status code 반환.
...
- 수정 (cpp `.cpp` 6 개, 각 2 분기 + include 1):
  ...
  - `src/pure_pursuit/pure_pursuit_action_server.cpp` (동일 2 분기, pp_out 사용) + `#include "trnav_motion_core/localization_monitor.hpp"` 추가
...
- 수정 (.action 6 개, status 주석):
  - `trnav_interfaces/action/AMRMotionTranslateForward.action`
  - `trnav_interfaces/action/AMRMotionTranslateReverse.action`
  - `trnav_interfaces/action/AMRMotionPurePursuit.action`
  - `trnav_interfaces/action/AMRMotionPurePursuitReverse.action`
  - `trnav_interfaces/action/AMRMotionYawControl.action`
  - `trnav_interfaces/action/AMRMotionYawControlReverse.action`
...
빌드 검증: `colcon build --packages-up-to trnav_motion_action_server --symlink-install` PASS (4 packages, 7min, exit 0).
...
관련 core: `src/Control/AMR-Motion/trnav_motion_core/docs/amr_motion_core_code_updates.md` [2026-05-18 16:30].
...
- 수정: `include/trnav_motion_action_server/pure_pursuit/pure_pursuit_action_server.hpp:64` — `motion_source_id_{7}` → `{8}`
- 수정: `src/pure_pursuit/pure_pursuit_action_server.cpp:104` — `safeParam("motion_source_id", 7)` → `8`
- 수정: `include/trnav_motion_action_server/pure_pursuit_reverse/pure_pursuit_reverse_action_server.hpp:66` — `motion_source_id_{10}` → `{9}`
- 수정: `src/pure_pursuit_reverse/pure_pursuit_reverse_action_server.cpp:101` — `safeParam("motion_source_id", 10)` → `9`
- 수정: `include/trnav_motion_action_server/yaw_control_reverse/yaw_control_reverse_action_server.hpp:49` — `motion_source_id_{9}` → `{7}`
...
검증: `colcon build --packages-up-to trnav_motion_action_server` + 각 action_server smoke run 시 `/select_motion_source` 호출 success 확인.
...
- 빌드 검증: `colcon build --base-paths src --packages-select trnav_motion_action_server` PASS, `hil_pure_pursuit.launch.py` install 확인.
...
- **PurePursuit** (신규 작성):
  - `include/trnav_motion_action_server/pure_pursuit/pure_pursuit_action_server.hpp`
  - `src/pure_pursuit/pure_pursuit_action_server.cpp`
  - `src/pure_pursuit/pure_pursuit_main.cpp`
  - 액션: `/amr_motion_pure_pursuit_abstract` (`trnav_interfaces/action/AMRMotionPurePursuit`)
...
빌드 검증: `colcon build --base-paths src --packages-up-to trnav_motion_action_server` 4 packages PASS, stderr 0.
설치 확인: `install/trnav_motion_action_server/lib/trnav_motion_action_server/amr_pure_pursuit_node` symlink + yaml/launch share 디렉터리 정상.
...
- **YawControlReverse** (신규 작성):
  - `include/trnav_motion_action_server/yaw_control_reverse/yaw_control_reverse_action_server.hpp`
  - `src/yaw_control_reverse/yaw_control_reverse_action_server.cpp`
  - `src/yaw_control_reverse/yaw_control_reverse_main.cpp`
  - 액션: `/amr_motion_yaw_control_reverse_abstract` (`trnav_interfaces/action/AMRMotionYawControlReverse`)
...
빌드: `colcon build --base-paths src --packages-up-to trnav_motion_action_server trnav_motion_mux` 5 packages finished, no stderr.
검증:
- `install/trnav_motion_action_server/lib/trnav_motion_action_server/amr_yaw_control_reverse_node` 산출물 확인
- `ros2 interface show trnav_interfaces/action/AMRMotionYawControlReverse` 노출 확인
...
- **Turn** (T-Robot ROS2 원본 그대로 이식, 알고리즘 무변경):
  - `include/trnav_motion_action_server/turn/turn_action_server.hpp`
  - `src/turn/turn_action_server.cpp` (~428 LOC)
  - `src/turn/turn_main.cpp`
  - 변경: namespace `amr_motion_control` → `trnav_motion_action_server::turn`,
    include `amr_motion_control/...` → `trnav_motion_action_server/turn/...` + `trnav_motion_core/...`,
    ActionServerBase 인자 `(node, mutex, "amr_motion_turn")` → `trnav_motion_core::ActionServerBase<Turn>(node, mutex, "amr_motion_turn_abstract", "/motion/wheel_cmd/turn")`
  - 알고리즘/파라미터/Phase 분할 모두 동일 (라인별 검증)
- **YawControl** (ROS1 개념 + bicycle model + 거리 종료, 신규 작성):
  - `include/trnav_motion_action_server/yaw_control/yaw_control_action_server.hpp`
...
- **중복 정리** (CLAUDE.md 규칙 5):
  - 작성 중 익명 namespace 의 `wrapPi/wrapDeg` 사용 → `trnav_motion_core::normalizeAngle/normalizeAngleDeg` 재사용으로 교체 (11개 호출 위치)

빌드: `colcon build --packages-up-to trnav_motion_action_server` 4 packages finished, no stderr (4회 반복).
...
- `trnav_interfaces/action/AMRMotionCrab.action` 변경 (별도 패키지지만 본 노드와 한 쌍):
...
- 빌드: `colcon build --packages-select trnav_interfaces trnav_motion_action_server --symlink-install` (실행 중).
...
- 빌드: `colcon build --packages-select trnav_motion_action_server --symlink-install` 성공.
...
- `src/translate/` → `src/translate_forward/` rename, namespace `trnav_motion_action_server::translate_forward`
...
- `include/trnav_motion_action_server/crab/crab_action_server.hpp`
  - `trnav_motion_core::ActionServerBase<AMRMotionCrab>` 상속, namespace `trnav_motion_action_server::crab`
...
빌드 검증: `colcon build --packages-select trnav_interfaces trnav_motion_action_server trnav_motion_mux --symlink-install` 성공 (2min 10s, 경고 없음).
사전 단계: stale build symlink 충돌(trnav_interfaces) → `rm -rf build/trnav_interfaces install/trnav_interfaces` 후 재빌드.
...
- `include/trnav_motion_action_server/translate/translate_action_server.hpp`
  - `trnav_motion_core::ActionServerBase<AMRMotionTranslate>` 상속
...
빌드 검증: `colcon build --packages-select trnav_motion_action_server` 성공 (46.8s, 경고 없음).
