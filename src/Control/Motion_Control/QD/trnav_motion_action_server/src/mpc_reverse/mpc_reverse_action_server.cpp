...
MpcReverseActionServer::MpcReverseActionServer(rclcpp::Node::SharedPtr node,
    ...
    : trnav::motion::qd::QdActionServerBase<MpcReverse>(node, std::move(action_mutex),
...
{
    trnav::motion::qd::QdActionServerBase<MpcReverse>::wheelStateCallback(msg);
...
void MpcReverseActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "TF2 map->base_link not available, aborting mpc_reverse");
        finish_abort(-3, 0.0, 0.0, 0.0, start_time);
        return;
    }
