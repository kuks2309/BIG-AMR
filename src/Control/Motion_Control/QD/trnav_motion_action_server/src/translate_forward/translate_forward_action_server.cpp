...
void TranslateForwardActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "TF2 map->base_link not available, aborting translate_forward");
        finish_abort(-3, 0.0, 0.0, 0.0, start_time);
        return;
    }
    ...
    while (rclcpp::ok() && !reached)
    {
        ...
        if (heading_source_ == 1 && imu_received_.load())
        {
            ...
            {
                imu_offset_sum += trnav_motion_core::normalizeAngle(robot_yaw - imu_yaw);
                imu_offset_n++;
            ...
            {
                robot_yaw = trnav_motion_core::normalizeAngle(imu_yaw + imu_heading_offset);
            }
        }
