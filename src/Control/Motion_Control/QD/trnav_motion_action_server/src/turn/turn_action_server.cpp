...
#include "trnav_motion_core/math_utils.hpp"  // normalizeAngleDeg — target_angle 입력 정규화
...
TurnActionServer::TurnActionServer(rclcpp::Node::SharedPtr node, trnav_motion_core::ActionMutex action_mutex)
    : trnav::motion::qd::QdActionServerBase<Turn>(node, std::move(action_mutex), "amr_motion_turn_abstract",
...
void TurnActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    trnav_motion_core::TrapezoidalProfile profile(target_abs, max_omega_deg, accel_dps2);
    ...
    while (rclcpp::ok() && !profile.isComplete(accumulated_angle))
    {
        ...
        {
        case trnav_motion_core::ProfilePhase::ACCEL:
            phase_id = 1;
            break;
        case trnav_motion_core::ProfilePhase::CRUISE:
            phase_id = 2;
            break;
        case trnav_motion_core::ProfilePhase::DECEL:
            phase_id = 3;
            break;
        default:
            phase_id = 3;
