          ...
          "/motion/wheel_cmd/crab_linear")
{
    ...
    // ── Crab IK (DualSteerIK 의 ±90° normalize edge 회피 — 양 휠 동일 steer 직접 출력) ──
    {
        ...
        crab_ik_ = std::make_unique<trnav::motion::qd::QdCrabIK>(
...
{
    trnav::motion::qd::QdActionServerBase<CrabLinear>::wheelStateCallback(msg);
...
void CrabLinearActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    if (goal->entry_speed <= 0.0)
    {
        ...
        // ── state-aware: Phase 0 가 결정한 wheel steer + walk dir 를 QdCrabIK 에 저장.
        ...
        RCLCPP_INFO(node_->get_logger(),
                    "CrabLinear Phase 0 → QdCrabIK setInitial(steer=%.1f deg, dir=%d)",
    ...
    while (rclcpp::ok() && !reached)
    {
        ...
        //   QdCrabIK 가 rear wheel 만 (-dir × δ_heading) offset 적용 → yaw 능동 보정
