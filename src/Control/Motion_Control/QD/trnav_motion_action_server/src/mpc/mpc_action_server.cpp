...
#include "trnav_motion_core/localization_monitor.hpp"
...
namespace trnav_motion_action_server::mpc
{
...
using MpcController = trnav::motion::qd::QdMpcController;
using trnav::motion::qd::MpcOutput;
using trnav::motion::qd::MpcPose;
...
namespace
{
// Path helper — MpcOutput 에 없는 projection/lookahead/remaining_x 자체 계산
// (2026-05-28: PurePursuitController 의 알고리즘 차용 — MPC controller 는 cost 기반이라 path 위치 정보 비공개).
...
MpcActionServer::MpcActionServer(rclcpp::Node::SharedPtr node, ActionMutex action_mutex)
    : trnav::motion::qd::QdActionServerBase<Mpc>(
          node, std::move(action_mutex), "amr_motion_mpc_abstract", "/motion/wheel_cmd/mpc")
{
    // ── IK / geometry ──
    ...
    // ── MpcController (kinematic bicycle MPC, NLopt SLSQP) ──
    ...
    // MPC 전용 — yaml 외화 후속 (현재는 default 사용)
    // pp_params.v_const = ... (Trapezoidal Profile 의 cmd_vx 가 별도 — controller 의 v_const 는 prediction 용)
    pp_ctrl_ = std::make_unique<MpcController>(pp_params);
    (void)heading_threshold_deg;   // MPC validateInitialPose 는 lat 만 검사 — heading 검사 폐기
    (void)cte_filter_window;       // MPC 는 RMA 필터 미사용
    ...
    // ── Extra publishers ──
    path_viz_pub_ = node_->create_publisher<nav_msgs::msg::Path>("mpc_path", rclcpp::QoS(10).transient_local());
    debug_pub_ = node_->create_publisher<std_msgs::msg::Float64MultiArray>("mpc_debug", rclcpp::QoS(10));
    ...
    motion_source_id_ = safeParam("motion_source_id", 8);
    select_source_client_ = node_->create_client<trnav_msgs::srv::SelectMotionSource>("/select_motion_source");

    RCLCPP_INFO(node_->get_logger(),
                "MpcActionServer initialized (TF-only, loc_watchdog=%s, "
                "lookahead=%.2f m, wheelbase=%.3f m, max_delta=%.1f deg)",
                enable_localization_watchdog_ ? "ON" : "OFF", lookahead_distance_,
                wheelbase_, max_delta_ * 180.0 / M_PI);

    // ── Hot-reload param callback (lookahead + max_delta) ──
    params_cb_handle_ = node_->add_on_set_parameters_callback(
        [this](const std::vector<rclcpp::Parameter> &params) -> rcl_interfaces::msg::SetParametersResult {
            ...
            if ((touched_la || touched_md) && pp_ctrl_)
            {
                if (touched_la)
                {
                    // MpcController 는 lookahead setter 없음 — cpp 측 lookahead_distance_ 만 갱신 (helper 가 사용).
                }
                if (touched_md)
                {
                ...
                RCLCPP_INFO(node_->get_logger(),
                            "Mpc gains hot-reloaded: lookahead=%.3f m, max_delta=%.1f deg",
...
// ════════════════════════════════════════════════════════
void MpcActionServer::wheelStateCallback(const trnav_msgs::msg::WheelMotor::SharedPtr msg)
{
    trnav::motion::qd::QdActionServerBase<Mpc>::wheelStateCallback(msg);
...
bool MpcActionServer::validateGoal(std::shared_ptr<const Mpc::Goal> goal)
{
    if (goal->path.poses.size() < 2)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: path.poses.size()=%zu (<2)", goal->path.poses.size());
        return false;
    }
    if (goal->path.header.frame_id != "map")
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: path.frame_id='%s' (must be 'map')",
                    goal->path.header.frame_id.c_str());
        return false;
    }
    if (goal->max_linear_speed <= 0.0)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: max_linear_speed <= 0");
        return false;
    }
    if (goal->acceleration <= 0.0)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: acceleration <= 0");
        return false;
    }
    if (goal->entry_speed < 0.0)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: entry_speed < 0");
        return false;
    }
    if (!goal->hold_steer && (goal->exit_steer_angle < -90.0 || goal->exit_steer_angle > 90.0))
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: exit_steer_angle out of [-90, +90]");
        return false;
    }
    if (goal->control_mode != 0 && goal->control_mode != 1)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: control_mode=%u 미지원 (BICYCLE=1 만 지원)",
                    goal->control_mode);
        return false;
    }

    // path 총 길이 검증 (인접 점 간 거리 합)
    ...
    if (total_len < 1e-6)
    {
        RCLCPP_WARN(node_->get_logger(), "Mpc rejected: zero-length path");
        return false;
    }

    RCLCPP_INFO(node_->get_logger(),
                "Mpc goal accepted: poses=%zu, total_len=%.3f m, v_max=%.2f m/s, lookahead=%.2f m",
                goal->path.poses.size(), total_len, goal->max_linear_speed, lookahead_distance_);
    return true;
}
...
void MpcActionServer::execute(std::shared_ptr<GoalHandle> goal_handle)
{
        ...
        if (future.wait_for(std::chrono::milliseconds(500)) == std::future_status::ready)
        {
            ...
            if (!resp->success)
                RCLCPP_WARN(node_->get_logger(),
                            "Mpc: SelectMotionSource(id=%d) failed: %s",
                            motion_source_id_, resp->message.c_str());
            else
                RCLCPP_INFO(node_->get_logger(),
                            "Mpc: mux active source → %d (mpc)",
                            motion_source_id_);
        }
        else
        {
            RCLCPP_WARN(node_->get_logger(),
                        "Mpc: SelectMotionSource(id=%d) timeout 500ms",
                        motion_source_id_);
        }
    }
    else
    {
        RCLCPP_WARN(node_->get_logger(),
                    "Mpc: /select_motion_source service not ready — mux 전환 skip");
    }

    const auto goal = goal_handle->get_goal();
    loc_monitor_->setEnableWatchdog(goal->enable_localization_watchdog && enable_localization_watchdog_);
    auto feedback = std::make_shared<Mpc::Feedback>();
    auto result = std::make_shared<Mpc::Result>();
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "Mpc setPath failed (invalid waypoints)");
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "TF2 map->base_link not available, aborting mpc");
        finish_abort(-3, 0.0, 0.0, 0.0, start_time);
        return;
    }
    ...
    {
        RCLCPP_WARN(node_->get_logger(),
                    "Mpc initial pose validation failed (code=%d): robot=(%.2f,%.2f,%.1f deg)", validate_result,
                    robot_x, robot_y, robot_yaw * 180.0 / M_PI);
        finish_abort(-2, 0.0, 0.0, 0.0, start_time);
        return;
    }

    // ── Phase 0: Steer Align (entry_speed <= 0.0 일 때만) ──
    // 첫 update 의 delta_f 를 steer alignment 목표로 사용
    if (goal->entry_speed <= 0.0)
    {
        ...
        const double align_steer_f = pp_first.delta_f;
        const double align_steer_r = 0.0; // 전륜만 조향

        feedback->phase = 0;
        auto phase0_start = node_->now();

        RCLCPP_INFO(node_->get_logger(), "Mpc Phase 0: steer alignment (target F=%.1f R=%.1f deg)",
        ...
        while (rclcpp::ok())
        {
            ...
            {
                RCLCPP_INFO(node_->get_logger(), "Mpc cancelled during Phase 0");
            ...
            {
                RCLCPP_WARN(node_->get_logger(), "Mpc Phase 0 steer timeout");
                finish_abort(-3, 0.0, 0.0, 0.0, start_time);
                return;
            }
        ...
        RCLCPP_INFO(node_->get_logger(), "Mpc Phase 0 complete, starting trapezoidal profile");
    }

    // Steer rate limiter / walk profile state
    ...
    // ── Phase 1-3: TrapezoidalProfile + Mpc + TransientGuard ──
    ...
    while (rclcpp::ok() && !reached)
    {
        ...
        {
            pp_out = pp_ctrl_->update(robot_x, robot_y, robot_yaw, 0.0, dt); helper = computePathHelper(waypoints, robot_x, robot_y, robot_yaw, lookahead_distance_);
            RCLCPP_INFO(node_->get_logger(), "Mpc cancelled at dist=%.3f m", helper.projection);
        ...
        {
            pp_out = pp_ctrl_->update(robot_x, robot_y, robot_yaw, 0.0, dt); helper = computePathHelper(waypoints, robot_x, robot_y, robot_yaw, lookahead_distance_);
            RCLCPP_WARN(node_->get_logger(), "Mpc global timeout (%.1f s), dist=%.3f m", max_timeout_sec_,
                        helper.projection);
        ...
        bool near_goal = (remaining < goal_reach_threshold_) || (helper.remaining_x < goal_reach_threshold_);
        ...
        if (prof_out.phase == ProfilePhase::DONE || projection >= target_distance ||
            helper.remaining_x < goal_reach_threshold_)
        {
            reached = true;
            if (goal->exit_speed > 0.0)
            {
                RCLCPP_INFO(node_->get_logger(), "Mpc reached with velocity continuity (exit_speed=%.3f)",
                            goal->exit_speed);
            }
            else
            {
        ...
        // ── BicycleModel: 전륜만 조향 (delta_r = 0) ──
        IKResult ik_expected;
        double vy_for_guard, omega_for_guard;
        {
            DualBicycleCommand dual_cmd{vx_profile, pp_out.delta_f, 0.0};
        ...
        {
            RCLCPP_INFO(node_->get_logger(),
                        "[Mpc] seg=%d steer: F=%.1f/%.1f° R=%.1f/%.1f° err=%.1f° %s | "
                        "vx=%.3f proj=%.3fm rem_x=%.3fm lat=%.3fm hdg=%.1f° L=(%.2f,%.2f)",
                        ...
                        guard_out.gate_blocked ? "BLOCKED" : "OK", cmd_vx, helper.projection, helper.remaining_x,
                        pp_out.e_d, pp_out.e_theta * 180.0 / M_PI, helper.lookahead_x, helper.lookahead_y);
        }
    ...
    RCLCPP_INFO(node_->get_logger(),
                "Mpc Phase 1-3 complete: dist=%.3f m, lat_err=%.4f m, head_err=%.2f deg, rem_x=%.3f m",
                final_helper.projection, final_pp.e_d, final_pp.e_theta * 180.0 / M_PI, final_helper.remaining_x);

    if (goal_handle->is_canceling())
    {
        RCLCPP_INFO(node_->get_logger(), "Mpc cancelled after Phase 1-3");
    ...
    if (!goal->hold_steer)
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "Mpc Phase 4: steer return to %.1f deg", goal->exit_steer_angle);

        while (rclcpp::ok())
        {
            if (goal_handle->is_canceling())
            {
                RCLCPP_INFO(node_->get_logger(), "Mpc cancelled during Phase 4");
                finish_abort(-1, final_helper.projection, final_pp.e_d, final_pp.e_theta * 180.0 / M_PI, start_time);
                return;
            }
            ...
            {
                RCLCPP_WARN(node_->get_logger(), "Mpc Phase 4 steer timeout (non-critical)");
                break;
            }
    ...
    RCLCPP_INFO(node_->get_logger(),
                "Mpc complete: dist=%.3f/%.3f m, lat_err=%.4f m, head_err=%.2f deg, time=%.1f s",
