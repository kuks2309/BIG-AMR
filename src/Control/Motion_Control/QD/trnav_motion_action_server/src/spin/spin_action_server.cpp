#include "trnav_motion_action_server/spin/spin_action_server.hpp"
...
SpinActionServer::SpinActionServer(rclcpp::Node::SharedPtr node, trnav_motion_core::ActionMutex action_mutex)
    : trnav::motion::qd::QdActionServerBase<Spin>(node, std::move(action_mutex), "amr_motion_spin_abstract",
...
{
    trnav_motion_core::ActionMutexGuard mutex_guard(action_mutex_);
    ...
    const double target_angle_deg = trnav_motion_core::normalizeAngleDeg(goal->target_angle);
    ...
    auto remaining_signed_deg = [&](double cur_yaw) -> double {
        double e = trnav_motion_core::normalizeAngle(target_imu_yaw - cur_yaw);
    ...
    trnav_motion_core::TrapezoidalProfile profile(target_abs, max_omega_dps, accel_dps2);
    ...
    while (rclcpp::ok())
    {
        ...
        if (remaining_deg <= pid_band_deg_ || prof_out.phase == trnav_motion_core::ProfilePhase::DONE)
            break;
        ...
        if (prof_out.phase != trnav_motion_core::ProfilePhase::DONE && omega_dps < min_speed_dps_)
        ...
        uint8_t phase_id = (prof_out.phase == trnav_motion_core::ProfilePhase::ACCEL)    ? 1
                           : (prof_out.phase == trnav_motion_core::ProfilePhase::CRUISE) ? 2
                                                                                         : 3;
    ...
    double final_traveled =
        sign * (target_abs - std::fabs(trnav_motion_core::normalizeAngle(target_imu_yaw - final_yaw)) * 180.0 / M_PI);
    ...
    double final_error = trnav_motion_core::normalizeAngle(target_imu_yaw - last_yaw_rad_.load()) * 180.0 / M_PI;
    RCLCPP_INFO(node_->get_logger(),
