...
## 2026-09-02 (3) — 3055/3056 은 「필드명 미확인」이 아니었다

`docs/seer/2026-08-08-seer-path-control.md` 가 벤더 원문의 필드 표 위치
(`robotkit-netprotocol-l-1.2.1.txt:3192-3246`·`:3248-3300`)를 인용해 두었는데, 코드 주석과
`debt-111` 은 「필드명 자체가 미확인이라 저수준 dict 만 받는다」고 적고 있었다. 원문을 열어 보니
표가 완전하다.

| 편호 | 필드 |
|---|---|
| 3055 평동 | `dist`(m, **절대값**, 필수) · `vx`(m/s, 부호가 방향, 필수) · `mode`(선택) |
| 3056 전동 | `angle`(rad, **절대값**, 2π 초과 가능, 필수) · `vw`(rad/s, +반시계, 필수) · `mode`(선택) |

`mode` 는 `0`=오도메트리(기본), `1`=자기측위인데 **벤더가 `1` 을 「目前不可用」(현재 사용 불가)**
라 적는다. 그래서 `translateBy`·`turnBy` 는 `mode` 를 아예 보내지 않는다 — 기본 오도메트리
모드만 쓴다. 그 귀결이 중요하다: **이 두 기동은 측위 폐루프가 아니다.** 거리·각도가 커질수록
오차가 커지고, 3055 와 3056 은 **동시 수행 불가**다. 측위 폐루프가 필요하면 3050/3051 이다.

| 변경 | 내용 |
|---|---|
| `api.hpp` / `api.cpp` | `translateBy(distM, vxMps)`·`turnBy(angleRad, vwRadPerSec)` 신설. 음수 거리·각도와 속도 0 을 **송신 전에** 거부 |
| `api.hpp` 편호 주석 | `// 필드명 미확인` → 실제 필드 목록 |
| `test/test_api.cpp` | 부호 규약·`mode` 미송신 확인 시험 14건. 164 → **178건** |

저수준 `translate(body)`·`turn(body)` 는 그대로 둔다 — `mode` 를 직접 넣어야 할 날이 오면 그 경로다.

부채: `debt-111` ③ 해소.

## 2026-09-02 (4) — 유령 경로 정리: 지도 다운로드·편집·업로드·전환을 실기로 닫았다

현재 지도 `260709_test` 의 경로 목록 6건 중 **4건이 유령**이었다 — 끝점 좌표가 스테이션 정본과
어긋나거나(`LM1(-16, 2.4)`, `LM2(-16, 15.5)`), 아예 존재하지 않는 `LM4` 를 가리켰다. 게다가
`LM1-LM2`·`LM2-LM1` 이라는 **같은 이름이 두 벌씩** 있어, 이름으로 경로를 고르는 3051(고정 경로
내비게이션)이 어느 것을 잡을지 정의되지 않는 상태였다.

정리 기준은 **스테이션 정본과의 대조** 하나다 — 끝점이 `advancedPointList` 에 있고 좌표가 그
스테이션과 일치하는 경로만 남긴다. 결과 6 → 2, 나머지(점군 17,711 · 반사판 112 · 스테이션 2 ·
`header`)는 무변경.

| 편호 | 실기 결과 |
|---|---|
| **4011** 다운로드 | 백업 확보, `1300 current_map_md5` 와 대조 통과 |
| **4010** 업로드 | 수리 후 되받아 **값 일치** |
| **2022** 전환 | `current_map_md5` `92e73074…` → `0319831d…`, `1022 loadmap_status=1` 유지 |

전후로 `1021 reloc_status=1` 유지, 스테이션 LM1·LM2 보존, fatal·error 0.

### 바이트 대조는 쓸 수 없다 — 로봇이 키를 재정렬한다

첫 업로드에서 도구가 `✗ 불일치` 를 내고 종료코드 5(되돌리라)를 냈다. 조사해 보니 길이는 같고
(420,109 B) 내용도 같은데 **키 순서만 달랐다** — 우리는 `header` 부터 쓰고 로봇은
`advancedCurveList` 부터 쓴다(알파벳순). 즉 로봇은 받은 JSON 을 재직렬화해 저장한다.
대조를 **파싱 후 값 비교**로 바꿨다. 바이트 차이는 정보로만 표시한다.

| 변경 | 내용 |
|---|---|
| `api.hpp` / `api.cpp` | `kCtrlLoadMap = 2022`·`kCtrlLoadMapObj = 2023` 상수, `loadMap(mapName)` 신설(빈 이름 거부) |
| `tools/map_upload.cpp` (신설) | 백업 실패 시 업로드 중단 · 경로 외 변경(점군·반사판·스테이션·header) 거부 · 기본 예행(`--yes` 필요) · 업로드 후 값 대조 · `--activate` 로 2022 |
| `test/test_api.cpp` | `loadMap` 4건. 178 → **182건** |
| `References/Seer-Driver/robokit_tcp_api.md` | `2022`/`2023` 정정 + `4010`/`4011` 추가 + 「스캔은 6100/6101@19210」 경고 |

부채: `debt-110` **해결**(2022 의 뜻 확정), `debt-095` ①③ 상환, `debt-111` 에서 4010·4011·2022 제외.
