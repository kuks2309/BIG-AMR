...
포트 상수 출처: 공식 SDK `netprotocol/rbkNetProtoEnums.py` 의 `API_PORT_*`
(`References/Seer-Driver/github_sdk/…`), 가이드 `seer_api_guide.md` §2.

⚠ **동시연결 한도는 문서 상수가 아니라 로봇의 런타임 파라미터다** — 2026-08-07 확정.
   `libNetProtocol.so` 가 `Robot<카테고리>APITCPServerMaxConnections` 파라미터를 읽어 쓰며,
   API 1400 응답이 `type=uint32, minValue=1, maxValue=20, advanced=True` 로 신고한다.
   즉 **판본으로 정해지는 값이 아니라 기체마다 바뀔 수 있는 설정값**이다.
   아래 표는 **정본이 아니라 관측 기본값**이다 — 정확한 값은 `SeerApi.get_max_connections()` 로
   로봇에 직접 물어본다.
"""
...
#: **[실측 2026-08-07]** Foil_A082(rbk 3.4.5.22) 관측 기본값. 정본 아님 — 위 docstring 참조.
#: 출처 ① 실기 API 1400 (192.168.44.82, 6개 파라미터 전부 `value == defaultValue`)
#:      ② 원본 하드 `robot.param` SQLite `NetProtocol` 테이블 (amap-server, 동일 값)
#: ⚠ 이전 판(2026-08-07 오전)은 문서 v1.2.1 을 근거로 배타 포트를 **1** 로 적었다 — **틀렸다**.
...
#: 한도 초과 시 로봇이 내는 `ret_code`. 응답 편호는 요청+10000 이 아니라 **포트 번호 그대로**다.
#: [실측 2026-08-07] 19204 에 9번째 연결 → `type=19204`,
#: `{"ret_code":61001,"err_msg":"reach the maximum of status api connection limitation", …}`.
CONNECTION_LIMIT_RET_CODE = 61001

#: 라이브러리 직결을 막는 포트 — **연결 수가 부족해서가 아니라 지령이 겹치면 위험해서**다.
#:
#: ⚠ 근거 정정 2026-08-07 — 최초 설계는 "동시연결 1 이라 선점 사고가 난다"였으나 **반증됐다**:
#:    한도는 5 이고, 초과 시 거동은 **거부형**이다(기존 연결 유지, 신규만 `ret_code 61001`.
#:    19204 실측으로 기존 연결이 살아남는 것을 확인). 즉 선점 위험은 없다.
#:    그럼에도 이 집합을 유지하는 이유는 **중재**다 — 두 주체가 동시에 로봇을 움직이거나
#:    설정을 쓰면 소켓이 남아돌아도 사고가 난다. 소켓 희소성이 아니라 권한 문제다.
