...
| 12 | `BackendBase.estop` | `on` | `None` | — | backend_base.py:100 |
...
| 30 | `RelayClient.send_estop` | `engage` | `—` | — | backend_ros2.py:163 |
...
| 43 | `Ros2Backend.estop` | `on` | `None` | — | backend_ros2.py:276 |
...
| 60 | `DirectBackend.home` | `—` | `tuple` | 조향 2축 호밍. 원본 `gui.py:946-995` 과 같은 3프레임 + 2상 판정. | backend_direct.py:253 |
| 61 | `DirectBackend._wait_homed` | `—` | `tuple` | 상태워드 bit15 **2상** 판정 — 먼저 0(진행 중)을 보고, 그 다음 1(완료)을 기다린다. | backend_direct.py:278 |
| 62 | `DirectBackend.estop` | `on` | `None` | 원본에 E-stop 이 없다 — UI 도 노출하지 않으므로 호출되지 않는다. | backend_direct.py:304 |
...
| # | 사용처(함수) | 기능 | 위치 |
| --- | --- | --- | --- |
| G1 | #16·#17·#76 | `CAP_SCAN`·`CAP_USB`·`CAP_ENGAGE`·`CAP_ESTOP`·`CAP_STOP`·`CAP_HOME`·`CAP_STEER_AXIS`·`CAP_STEER_ALL`·`CAP_DRIVE`·`CAP_MOTOR_TABLE` (상수 10종) | backend_base.py:29-38 |
| G2 | #18·#23 | `MEAS_TTL_S = 1.0` (상수) — ROS2 백엔드 기본 TTL | backend_ros2.py:32 |
| G3 | #18 | `LATCHED_QOS` (상수) — estop 전용 TRANSIENT_LOCAL | backend_ros2.py:34-37 |
...
| G7 | #60·#61 | `HOMING_SPEED`·`HOMING_TIMEOUT_S`·`HOMING_START_S`·`BIT15` (상수) | backend_direct.py:53-56 |
...
| `/estop` | `std_msgs/Bool` | **1 · RELIABLE · TRANSIENT_LOCAL** | `send_estop` (#30) | backend_ros2.py:74 |
...
| `/estop` | UI 1·RELIABLE·**TRANSIENT_LOCAL** | 드라이버 1·RELIABLE·**TRANSIENT_LOCAL** | ✅ 연결 (offered ≥ requested) |
...
can_relay_gui ─/estop(TRANSIENT_LOCAL)──────────────────▶ can_relay_node
...
조치 현황(2026-08-04 갱신): **Medium 2건·Low 2건 완료**, Medium 2건(E-stop 경로)·Low 2건 미착수.
미착수 4건은 각 항목 말미에 사유를 적었다 — E-stop 2건은 **사용자 판단 대기**(노출 vs 삭제).
Verdict: COMMENT
...
**함수 #62 — [논리] `DirectBackend.estop` 이 예외를 던진다 (Medium)**
...
① ~~Medium 4건 조치~~ → **M1·M2 완료**(회귀가 만든 대수 소실 + 정본 출처 통일).
   남은 M3·M4 는 E-stop 을 **UI 에 노출할지 죽은 경로를 삭제할지**가 갈려 사용자 판단이 필요하다.
