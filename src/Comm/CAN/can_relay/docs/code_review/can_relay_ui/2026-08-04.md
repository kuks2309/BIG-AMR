...
| 104a | `MainWindow._engaged` | `—` | `bool` | **2026-08-14 신설** — 제어권 보유 여부(백엔드 `status()` 세 번째 값). 조그 경로 분기 기준 | app.py:703 |
| 104b | `MainWindow._seer_velocity` (staticmethod) | `steer_deg,sign,mmps` | `(vx, vy)` m/s | **2026-08-14 신설** — `JOG` 표를 차체 속도로 환산. `v = −sign × (cos θ, −sin θ)` · mm/s → m/s | app.py:711 |
| 104c | `MainWindow._seer_ctrl_client` | `—` | `RobokitClient` | **2026-08-14 신설** — 제어용 Seer 클라이언트(상태 폴링과 별개 연결 — 19205 는 배타 포트) | app.py:723 |
| 104d | `MainWindow._jog_seer` | `label` | `None` | **2026-08-14 신설** — 제어권 없을 때의 조그. `정지` 는 2000 즉시, 그 외는 #104f 워커 기동 | app.py:731 |
| 104e | `MainWindow._run_seer_ctrl` | `method,*args` | `None` | **2026-08-14 신설** — Seer 제어 포트 호출 1회를 작업 스레드에서. 결과·실패를 로그로 | app.py:752 |
| 104f | `MainWindow._jog_seer_run` | `label,vx,vy` | `None` | **2026-08-14 신설** — Seer 조그 워커. 1060 소유자 조회 → 4005 획득 → 2010 재송신 → 2000 → 4006 반납 | app.py:762 |
| 113a | `acquire_single_instance` | `path` | `(파일객체\|None, 선점자 PID\|None)` | **2026-08-14 신설** — `flock` 으로 한 번에 하나만 띄운다. **단독 GUI 와 같은 잠금 파일**이라 둘 중 하나만 뜬다 | gui_node.py:34 |
...
> ⚠ **#100 `_jog` 계약 변경(2026-08-14)** — 제어권 보유 시 백엔드 경로, **미보유 시 Seer API
> 경로**(#104d)로 갈린다. 종전에는 미보유 상태에서 백엔드가 거부하는 것으로 끝났다.
>
> ⚠ **#? `_check_seer_agreement` 계약 변경(2026-08-14, app.py:520)** — 대조 상대를
> **잡기 직전 Seer 각도**(`_seer_at_take`)로 바꾸고 **축별 세션당 1회**만 본다. 종전에는 살아
> 있는 `_seer_deg` 와 매 표본 대조했는데, **제어권을 쥐면 Seer 는 모터를 못 읽어**(intercept)
> 굳은 값과 비교하는 것이라 거짓 경보만 났다(단독 GUI 실기 11:16:03 에서 실증). 실측 갱신
> 자체가 제어권 보유 중에만 도니 그 대조에는 유효한 창이 없었다.
> 죽은 상수 `SEER_MATCH_STREAK`·`SEER_MATCH_REWARN_S` 와 연속 카운터 상태 2개를 함께 제거했다.
...
| G15 | #104d·#104f | `SEER_JOG_DURATION_MS=600`·`SEER_JOG_PERIOD_S=0.2`·`SEER_CONTROL_NICK` (상수) — Seer 조그 시한·재송신 주기·제어권 식별 이름 | app.py:40-42 |
| G16 | #113a | `SINGLE_INSTANCE_LOCK` (상수) — 중복 실행 가드 잠금 파일. 단독 GUI 와 공유 | gui_node.py:31 |
