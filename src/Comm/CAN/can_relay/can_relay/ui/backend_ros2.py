...
판다를 열지 않는다. 두 프로세스가 각자 USB 를 열면 제어권과 heartbeat 가 겹치므로
`판다 검색`·`USB 연결` capability 를 갖지 않는다(드라이버가 소유한다).

그 대신 드라이버의 보호를 받는다 — 20 Hz 재송신 · `cmd_timeout_s` 워치독 ·
...
DRIVE_REPUBLISH_HZ = 20.0   # 드라이버 `cmd_hz` 와 같은 주기로 구동 지령을 유지한다
...
class RelayClient(Node):
    ...
    화면 없이 시험할 수 있어야 하므로 UI 와 분리한다.

    구독 `/joint_states`(조향 실측) · `/diagnostics`(드라이버 상태) ·
    `/motor/low_state`(모터 표, `trnav_msgs` 있을 때만).
    발행 `~/steer_deg` · `~/steer_axis_deg` · `~/drive_mmps`.
    서비스 클라이언트 `~/engage` · `~/stop` · `~/home`.
    """
    ...
    def _load_msgs(self):
        """`MotorStateArray` 타입을 얻는다. 없으면 `None` — 모터 표는 각도만 채운다."""
        try:
    ...
    def _on_joints(self, msg: JointState):
        """`steer_<node>` 관절만 골라 각도(rad→°)와 수신 시각을 남긴다."""
        now = time.monotonic()
        with self._lock:
    ...
    def _on_diag(self, msg: DiagnosticArray):
        """`can_relay` 진단 항목의 level·message·key/value 와 수신 시각을 남긴다."""
    ...
    def meas_angle(self, node: int) -> Optional[float]:
        """그 축의 조향 실측(°). `meas_ttl_s` 밖이면 `None`.

        TTL 은 파라미터를 매번 읽는다 — 스냅샷을 쓰면 런타임 param set 이 무시된다.
        """
    ...
    def diagnostics(self) -> tuple:
        """`(level, message, 신선한가(3초), key/value dict)`."""
        with self._lock:
    ...
    def _republish_drive(self):
        """구동 지령을 유지한다. 비영이면 계속, 0 이면 `ZERO_HOLD_S` 동안만, 그 뒤에는 조용히.

        드라이버 워치독(`cmd_timeout_s`)이 갱신 없는 지령을 0 으로 수렴시키므로 유지하려는
        쪽이 계속 발행해야 한다. 반대로 유휴에도 0 을 계속 내면 이 GUI 가 토픽을 점유해
        다른 지령자를 즉시 덮어쓰므로, 정지 직후 잠깐만 반복하고 그친다.
        조그 중 GUI 가 죽으면 재발행이 끊겨 드라이버가 0 으로 만든다.
        """
        with self._lock:
    ...
    def call(self, client, request, timeout_s: float = 5.0, discovery_s: float = 5.0):
        """서비스를 1회 호출한다. 반환 `(성공, 메시지)`. **블로킹** — 작업 스레드에서 부른다.

        `discovery_s` 는 서비스가 나타나기를 기다리는 한도다. 첫 호출이 기동 직후에
        몰리므로 여유를 둔다.
        """
...
class Ros2Backend(BackendBase):
    ...
    @property
    def cfg(self) -> dict:
        """UI 가 읽는 파라미터(Seer 접속 등 백엔드와 무관한 부분)."""
        return self.node.cfg

    def why_not(self, cap: str) -> str:
        """`검색`·`USB` 버튼 툴팁 — 판다는 드라이버가 소유한다."""
    ...
    def shutdown(self, reason: str = "") -> None:
        """구동 0 → 정지 서비스 → 제어권 반환 → 노드·실행기 해제. 멱등이다."""
        if self._released:
            return
        self._released = True
    ...
    # ── 조회 ──────────────────────────────────────────────────────────
    def meas_angle(self, node: int) -> Optional[float]:
        """그 축의 조향 실측(°). TTL 밖이면 `None`."""
        return self.node.meas_angle(node)

    def motor_rows(self) -> dict:
        """`{node: (각도°, 회전, 전류 A)}`. `low_state` 가 없으면 조향축 각도만 채운다."""
        rows = {}
