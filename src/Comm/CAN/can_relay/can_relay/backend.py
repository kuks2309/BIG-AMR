...
class RelayBackend:
            ...
            if mid in self.cfg.drive_nodes:
                ...
                if mode != P.MODE_POSITION:
                    notes.append(f"node{mid} 조향축에 mode={P.MODE_NAME.get(mode, mode)} "
                                 f"— POSITION 만 허용, 거부")
                    continue
                # set_steer_deg(:264)·set_steer_axis_deg(:301) 와 동일하게 homed_effective()
                # 를 쓴다 — 원시 self._homed 만 보면 Seer 가 호밍해 둔 상태(0x6041 bit15=1)
                # 를 놓쳐 raw 경로 조향이 통째로 거부된다(2026-08-04 실기 사고와 동형).
                if self.cfg.require_homed_for_steer and not self.homed_effective():
    ...
    def stop_all(self, reason: str = "") -> bool:
        ...
        ⚠ **이름과 달리 「조향을 세우는」 함수가 아니다.** `release_steer_target` 은 2026-08-05 결정으로
