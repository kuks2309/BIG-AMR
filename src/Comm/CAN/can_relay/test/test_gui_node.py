...
def test_home_result_is_logged_not_only_returned(rig, monkeypatch):
    ...
    없었다. 결과가 `res.message` 에만 담겨 있어 무엇이 왜 끝났는지를 로그만으로 판정할
    수 없었다. 조용한 실패는 조사할 수 없다.

    성패로 severity 가 갈리므로 두 단계를 다 잡는다 — rclpy 가 호출 지점별로 severity 를
    캐시해 거부하는 함정이 이 분기에 그대로 있다.
    """
    driver, client = rig
    _engage(client)
    lines = []
