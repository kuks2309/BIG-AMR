...
def test_low_cmd_steer_allowed_when_drive_reports_homed():
    ...
    :264·:301 조향 경로는 homed_effective() 로 고쳤지만 raw 경로(:365)는 원시 self._homed
    만 봐, Seer 가 호밍해 둔 상태에서 상위 모션이 /motor/low_cmd 로 조향하면 통째로 거부됐다
    (2026-08-04 실기 사고와 동형, 이 경로만 남아 있던 누락). 우리가 호밍하지 않아도 드라이브가
    「완료」를 보고하면 raw 조향이 통과해야 한다.
    """
    link, be = make(require_homed_for_steer=True)
