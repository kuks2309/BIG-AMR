...
namespace feature_localizer_test
{

// T_station_lidar 자세의 라이다에서 벽 선분들을 레이캐스트한 거리 배열.
// sigma_m > 0 이면 거리 방향 가우시안 잡음(rng 필수). 미충돌 빔은 +inf.
inline std::vector<float> simulateScan(const std::vector<feature_localizer_core::FeatureRef> &features,
                                       const feature_localizer_core::Pose2D &T_station_lidar,
