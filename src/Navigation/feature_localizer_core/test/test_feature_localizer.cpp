// FeatureLocalizer 파사드 통합검증 — 합성 스캔 E2E 정밀도·퇴화·점프 게이트·복귀.
...
const std::vector<FeatureRef> kStationWalls = {{"front", {2.0, -1.5}, {2.0, 1.5}},
...
std::vector<float> scanAt(const std::vector<FeatureRef> &features, const Pose2D &T_station_base,
                          double sigma_m, std::mt19937 *rng)
{
    return simulateScan(features, compose(T_station_base, kLaserInBase), kAngleMin, kAngleInc,
...
int main()
{
    const FeatureLocalizerParams params;  // 전 기본값

    // 1) 무잡음 — OK, 오차 < 1 mm · 0.05°.
    {
        FeatureLocalizer loc(kStationWalls, params, kLaserInBase, kInitialGuess);
        ...
        for (const auto &f : r.feature_fits)
        {
            CHECK(f.matched);
        }
    }
    ...
    // 3) 좌측 벽 가림 — DEGRADED 로 강등되지만 자세는 유지 (전방+우측이 3자유도 구속).
    {
        FeatureLocalizer loc(kStationWalls, params, kLaserInBase, kInitialGuess);
        const Pose2D truth{0.5, 0.1, 2.0 * M_PI / 180.0};
        const std::vector<FeatureRef> visible = {kStationWalls[0], kStationWalls[2]};
        const LocalizeResult r =
    ...
    // 4) 전방 벽만 보임 — 대응 부족으로 LOST, 자세 출력 없음.
    {
        ...
        const std::vector<FeatureRef> visible = {kStationWalls[0]};
        const LocalizeResult r =
            loc.update(scanAt(visible, truth, 0.0, nullptr), kAngleMin, kAngleInc);
        CHECK(r.status == Status::LOST);
    ...
    {
        const std::vector<FeatureRef> parallel_only = {kStationWalls[1], kStationWalls[2]};
        FeatureLocalizer loc(parallel_only, params, kLaserInBase, kInitialGuess);
    ...
    //    (거리 임계는 3σ 규칙대로 조정 — 임계 < 1.5σ 의 극한 붕괴는 본 수정의 범위 밖)
    {
        ...
        FeatureLocalizer loc(kStationWalls, hp, kLaserInBase, kInitialGuess);
        const Pose2D truth{0.5, 0.1, 2.0 * M_PI / 180.0};
        std::mt19937 rng(11);
        int n_fix = 0;
        double worst = 0.0;
    ...
    std::printf("test_feature_localizer: PASS\n");
    return 0;
}
