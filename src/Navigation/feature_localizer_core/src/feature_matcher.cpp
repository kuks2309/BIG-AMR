                                               ...
                                               const Pose2D &T_station_lidar)
{
    ...
    {
        const LineNormalForm &ls = features[i].line_station;
        PredictedWall pw;
        pw.feature_idx = i;
        ...
        pw.p1 = transformPoint(T_lidar_station, features[i].ref.p1);
        pw.p2 = transformPoint(T_lidar_station, features[i].ref.p2);
                                  ...
                                  const std::vector<OrientedWall> &features, const MatchParams &p)
{
    ...
    for (const Candidate &c : candidates)
    {
        ...
        FeatureMatch m;
        m.feature_idx = predicted[c.pred_idx].feature_idx;
        m.ref_line_station = features[m.feature_idx].line_station;
                                                ...
                                                const MatchParams &p)
{
    ...
    std::vector<FeatureAxis> axes(predicted.size());
    for (std::size_t pi = 0; pi < predicted.size(); ++pi)
    {
        const PredictedWall &pw = predicted[pi];
        const double ux0 = pw.p2.x_m - pw.p1.x_m;
    ...
    std::vector<FeatureCandidateGroup> groups;
    for (std::size_t pi = 0; pi < predicted.size(); ++pi)
    {
        if (members[pi].empty())
        {
        ...
        FeatureCandidateGroup g;
        g.feature_idx = predicted[pi].feature_idx;
        g.ref_line_station = features[g.feature_idx].line_station;
