...
struct LocalizeResult
{
    ...
    std::vector<FeatureFit> feature_fits;
...
class FeatureLocalizer
{
  public:
    ...
    // 로봇 초기 추정(대응 게이트 시드 + 기준 특징면 법선 정향 기준).
    FeatureLocalizer(const std::vector<FeatureRef> &features, const FeatureLocalizerParams &params,
  ...
  private:
    ...
    std::vector<OrientedWall> oriented_features_;
    FeatureLocalizerParams params_;
