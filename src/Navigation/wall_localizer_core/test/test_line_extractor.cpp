...
std::vector<ExtractedSegment> extractFromWalls(const std::vector<WallRef> &walls,
                                               const ExtractParams &p)
{
    const Pose2D lidar_at_origin{0.0, 0.0, 0.0};
    const std::vector<float> scan =
        simulateScan(walls, lidar_at_origin, kAngleMin, kAngleInc, kBeams, 10.0, 0.0, nullptr);
...
int main()
{
    ...
    {
        const std::vector<WallRef> walls = {{"front", {1.0, -1.0}, {1.0, 1.0}},
                                            {"left", {-1.0, 1.0}, {1.0, 1.0}}};
        const auto segs = extractFromWalls(walls, p);
        CHECK(segs.size() == 2);
        int n_front = 0;
        int n_left = 0;
    ...
    {
        const std::vector<WallRef> walls = {{"short", {1.0, -0.15}, {1.0, 0.15}}};
        const auto segs = extractFromWalls(walls, p);
        CHECK(segs.empty());
    }
    ...
    {
        const std::vector<WallRef> walls = {{"a", {1.0, -1.0}, {1.0, -0.3}},
                                            {"b", {1.0, 0.3}, {1.0, 1.0}}};
        const auto segs = extractFromWalls(walls, p);
        CHECK(segs.size() == 2);
    }
    ...
    {
        const std::vector<WallRef> walls = {{"front", {2.0, -1.5}, {2.0, 1.5}}};
        std::mt19937 rng(7);
        const std::vector<float> scan = simulateScan(walls, {0.0, 0.0, 0.0}, kAngleMin,
