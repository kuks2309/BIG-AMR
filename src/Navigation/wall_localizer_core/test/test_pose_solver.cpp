...
WallMatch exactMatch(const OrientedWall &w, const Pose2D &T_station_lidar, int num_points)
{
...
int main()
{
    ...
    // 1) 벽 3면 정확 대응 → 참 자세 복원 (해석해 수준 오차).
    {
        ...
        const std::vector<WallMatch> matches = {exactMatch(front, truth, 120),
    ...
    // 2) 비평행 2면으로도 해석 가능.
    {
        ...
        const std::vector<WallMatch> matches = {exactMatch(front, truth, 100),
    ...
    // 3) 평행 벽만(좌·우) → 접선 방향 미관측, 해를 내지 않는다.
    {
        ...
        const std::vector<WallMatch> matches = {exactMatch(left, truth, 100),
                                                exactMatch(right, truth, 100)};
        const SolveResult r = solvePose(matches, sp);
        CHECK(!r.ok);
