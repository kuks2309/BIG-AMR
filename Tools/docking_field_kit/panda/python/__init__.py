...
class Panda:
  ...
  # CAN-Relay: per-bus CAN health (bxCAN ESR) — firmware health.h can_health_t 와 동기
  ...
  # CAN-Relay: per-bus CAN 에러 상태(bxCAN ESR) — firmware 0xc3
  def can_health(self, bus):
    dat = self._handle.controlRead(Panda.REQUEST_IN, 0xc3, bus, 0, self.CAN_HEALTH_STRUCT.size)
    ...
    return {
      ...
      "error_warning": a[2],
      "last_error_code": a[3],
      "receive_error_cnt": a[4],
      "transmit_error_cnt": a[5],
      "esr_reg": a[6],
    }
