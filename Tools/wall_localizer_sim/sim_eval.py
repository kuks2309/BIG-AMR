#!/usr/bin/env python3
"""wall_localizer 시나리오 시뮬레이션 평가.

U자 스테이션(전방 x=2, 좌·우 y=±1.5, 구간 x∈[0,2])의 합성 LaserScan 을 발행하고,
wall_localizer_node 의 /wall_pose·진단을 스캔 스탬프로 페어링해 오차 통계를 낸다.
...
# 기준 벽 (config/wall_localizer.yaml 의 예시 스테이션과 동일해야 한다)
...
class SimEval(Node):
    def __init__(self, args):
        ...
        self.walls = list(REF_WALLS) + (CLUTTER_WALLS if args.clutter else [])
        self.scan_pub = self.create_publisher(LaserScan, "/wl_sim_scan", 10)
        self.pose_sub = self.create_subscription(PoseStamped, "/wall_pose", self.on_pose, 10)
        self.diag_sub = self.create_subscription(DiagnosticArray, "/wall_localizer/diagnostics",
                                                 self.on_diag, 10)
        self.truth_by_stamp = {}
        self.records = []
    ...
    def tick(self):
        ...
        for i in range(N_BEAMS):
            ...
            r, hit = raycast(self.walls, ox, oy, dx, dy)
...
def main():
    ...
    ap.add_argument("--occlude-wall", type=int, default=-1,
                    help="가릴 기준 벽 인덱스 (0=front,1=left,2=right, -1=없음)")
    ap.add_argument("--clutter", action="store_true")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--out-dir", default="Log/wall_localizer_sim")
    args = ap.parse_args()
