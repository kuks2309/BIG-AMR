#!/bin/bash
# feature_localizer 시나리오 시뮬레이션 일괄 실행.
...
OUT="$REPO/Log/feature_localizer_sim"
cd "$REPO" || exit 1
source /opt/ros/humble/setup.bash
source install/setup.bash 2>/dev/null

# 노드 실행 파일 기준으로 잔류를 판정한다 — `ros2 run` 래퍼 PID 만 죽이면
# 실제 노드가 살아남아 /feature_pose 발행자가 누적되고 이후 측정이 전부 오염된다.
NODE_PAT='feature_localizer_ros2/feature_localizer_node'
...
run_one() {
    ...
    ros2 run feature_localizer_ros2 feature_localizer_node --ros-args \
        --params-file src/Navigation/feature_localizer_ros2/config/feature_localizer.yaml \
    ...
    python3 Tools/feature_localizer_sim/sim_eval.py --scenario "$name" \
