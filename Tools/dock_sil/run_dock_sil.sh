#!/bin/bash
# 도킹 폐루프 SIL — 플랜트 → feature_localizer → dock 서버 전 체인.
...
PATS='dock_sil_plant.py|feature_localizer_ros2/feature_localizer_node|dock_approach_action_server'
cleanup() { pkill -f 'dock_sil_plant.py' 2>/dev/null; pkill -f 'feature_localizer_ros2/feature_localizer_node' 2>/dev/null; pkill -f 'dock_approach_action_server' 2>/dev/null; sleep 1; }
...
run_case() {
    ...
    ros2 run feature_localizer_ros2 feature_localizer_node --ros-args \
        --params-file Tools/dock_sil/features_sil.yaml \
        -p initial_x_m:="$x0" -p initial_y_m:="$y0" -p initial_yaw_deg:="$yaw0" \
        -r scan:=/dock_sil/scan -r feature_pose:=/dock_sil/feature_pose \
        -r feature_localizer/diagnostics:=/dock_sil/diag > "$OUT/${name}_wall.log" 2>&1 &
    ros2 run trnav_2ws_dock_ros dock_approach_action_server --ros-args \
        ...
        -r /feature_pose:=/dock_sil/feature_pose \
    ...
    for pat in 'dock_sil_plant.py' 'feature_localizer_node' 'dock_approach_action_server'; do
