...
⚠ **호밍 중에도 각도는 실시간으로 갱신된다.** 탐색 구간에서 `0x6064` 는 실제 엔코더 값을
계속 보고하고(2026-07-29 실기: node3 7,871,817 / node4 7,840,084 대다수), **정확한 0 이
간헐적으로** 섞인다. 그 0 만 글리치로 걸러내고(`decode_frames`) 나머지는 그대로 그린다 —
바퀴 그림이 ①②의 스윙을 따라간다.

  2026-07-27 캡처에서는 같은 구간 3,105/3,105 표본이 전부 0 이었지만, 그것은 **Seer 의
  폴링에 대한 응답**을 수동청취한 것이라 우리 폴링 경로와 조건이 다르다. 그 성질을 우리
  경로로 옮겨 표시를 통째로 끄던 것이 과보수적이었다
  ([claude-mistake 2026-07-29-004](../../docs/claude-mistake/2026-07-29-004_frozen-readout-from-transferred-observation.md)).
  간헐 0 의 발생원은 **미확정**(debt-017) — 어느 쪽이든 0 필터로 해결된다.
