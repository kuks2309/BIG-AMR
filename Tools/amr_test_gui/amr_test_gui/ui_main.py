...
class MainWindow(QWidget):
    ...
    def _build_control(self):
        ...
        # 호밍 = 원점(리밋) 탐색 → 조향 0° 복귀. 위 '조향 0° 이동'은 기준점을 새로 잡지 않는다.
        self.btn_homing = QPushButton("호밍  (원점 탐색 → 조향 0° 복귀)")
        self.btn_homing.setMinimumHeight(34)
        self.btn_homing.setToolTip(
            "판다 펌웨어가 지휘한다(0xea). 조향축이 −리밋까지 이동해 원점을 잡고 조향 0° 로 돌아온다.\n"
    ...
    def _on_homing(self):
        ...
        if not self._confirm(
                "호밍 실행",
                "조향축이 −리밋까지 이동한 뒤 조향 0° 로 복귀합니다 (약 35초).\n\n"
                "· 조향 이동 구역이 비었습니까?\n"
                "\n계속할까요?"):
            return
