...
feature_localizer 기준 특징면 YAML 을 생성한다.

기준 특징면을 도면 좌표가 아니라 **라이다 실측 맵**에서 뽑는다(.smap 은 Seer 가 라이다로
만든 맵). 좌표는 맵 프레임이므로 출력 YAML 은 station_frame: "map" 이고, /feature_pose 도
맵 프레임 자세가 된다. 초기 추정(initial_*)은 스테이션 진입 지점에 맞춰 별도 설정할 것.

사용:
  비대화형: python3 teach_features.py --smap map/260709_test.smap \
              --roi front:1.0,2.0,1.4,5.0 --roi left:2.0,5.0,5.0,5.4 --out features.yaml
  대화형:   python3 teach_features.py --smap map/260709_test.smap --interactive
            (사각형 드래그 → 콘솔에 벽 이름 입력, 빈 입력=버림. 창 닫으면 YAML 기록)
  미리보기: python3 teach_features.py --smap ... --preview-only   (맵 PNG 만 출력)
...
def fit_line_in_roi(points, name, roi, trim_sigma=3.0, min_points=30, max_iters=5,
    ...
    fit = FeatureFit(name, [lo[0], lo[1], hi[0], hi[1]])
    ...
    if fit.length_m < min_length_m:
        fit.warnings.append(f"길이 {fit.length_m:.2f}m < {min_length_m}m — 기준 특징면으로 부적합(ANT 최소 세그먼트 준용)")
    return fit


def write_features_yaml(fits, out_path, smap_name):
    ok = [f for f in fits if f.p1 is not None]
    lines = [
        "# feature_localizer 기준 특징면 — teach_features.py 가 라이다 실측 맵에서 생성",
        f"# 소스 맵: {smap_name} (맵 프레임 좌표, station_frame=map)",
        "# 초기 추정(initial_x_m 등)은 스테이션 진입 지점의 맵 좌표로 별도 설정할 것.",
        "feature_localizer:",
        "  ros__parameters:",
        '    station_frame: "map"',
        f"    feature_names: [{', '.join(repr(f.name) for f in ok)}]",
    ]
    for f in ok:
        lines.append(f"    # {f.name}: 점 {f.n_points}개, rms {1000*f.rms_m:.1f}mm, 길이 {f.length_m:.2f}m")
        lines.append(f"    features.{f.name}: [{f.p1[0]:.4f}, {f.p1[1]:.4f}, {f.p2[0]:.4f}, {f.p2[1]:.4f}]")
...
def run_interactive(points, smap_name, out_yaml, out_png):
    ...
    del selector
    if fits:
        write_features_yaml(fits, out_yaml, smap_name)
...
def main():
    ...
    ap.add_argument("--out", default="features_taught.yaml")
    ap.add_argument("--png", default="features_taught.png")
    ...
    if not ok:
        ...
        return 1
    write_features_yaml(fits, args.out, smap_name)
