...
## ⚠ 2026-08-10 안전 보강 — 종전 주장 정정

이 도구는 **실기를 회전시킨다.** 감사에서 아래가 드러나 고쳤다.

| 종전 | 지금 |
| --- | --- |
| `--w` 에 **상한이 없었다** — `--w 5.0` 오타(0.05 의도)면 **286 °/s** 가 개루프로 나간다 | `--w-limit`(기본 0.2 rad/s = 11.5 °/s) 초과면 **연결 전에 종료코드 2 로 거부** |
| 라이다 여유 판정이 **전혀 없었다** | 매 주기 **전방위 최소** 여유 확인(≥ 1.3 m). 스캔이 없거나 0.5 s 이상 낡으면 `0.0`(fail-closed) |
| 맵 적산이 멈추면 `tmax` 70 s 를 채우며 계속 돌았다(`w=0.100` 이면 약 **400° 회전**) | 5 s 동안 적산 변화가 없으면 측위 두절로 보고 정지(종료코드 3) |
| README `:51` 「제자리 회전이므로 위치 이동은 수십 mm 수준」 | **코드가 검증하지 않는 경험칙**이었다. 여전히 위치는 감시하지 않는다 — 전방위 여유로 대신한다 |

전방위 최소를 보는 이유: 제자리 회전은 진행 방향이 없고, 축거 1.2 m 차체는 모서리가
반경 0.75~0.85 m 를 쓸고 지나간다. `/scan_merged` 는 배제영역(`x∈[-0.96,0.98]`)을 삭제하고
`use_inf: True` 라 그보다 가까운 점은 관측되지 않으므로 임계를 그 위(1.3 m)로 잡았다.

**여전히 막지 못하는 것**: `SIGKILL`. 벤더 개루프 지령에는 워치독도 지속시간 필드도 없다.
