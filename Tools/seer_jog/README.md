...
| 종전 주장 | 실제였던 것 | 지금 |
| --- | --- | --- |
| 「매 주기 진행방향 여유를 확인하고 임계 미만이면 정지」 | 임계 0.5 m 가 **물리적으로 관측 불가**였다 — `/scan_merged` 가 배제영역 `x∈[-0.96,0.98]`·`y∈[-0.68,0.71]` 안을 삭제하고 `use_inf: True` 라 전방 최소 관측거리가 0.98 m 다. **한 번도 발화할 수 없었다** | 기본 임계 **1.3 m** |
