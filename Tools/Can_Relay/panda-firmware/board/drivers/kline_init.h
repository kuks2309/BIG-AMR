...
void setup_timer5(void) {
  ...
  register_set(&(TIM5->PSC), (48-1), 0xFFFFU);
  register_set(&(TIM5->CR1), TIM_CR1_CEN, 0x3FU);
  register_set(&(TIM5->ARR), (5000-1), 0xFFFFFFFFU);

  NVIC_EnableIRQ(TIM5_IRQn);

  register_set(&(TIM5->DIER), TIM_DIER_UIE, 0x5F5FU);
  TIM5->SR = 0;
}
...
void TIM5_IRQ_Handler(void) {
  if ((TIM5->SR & TIM_SR_UIF) && (kline_data != 0U)) {
    if (kline_bit_count < kline_data_len) {
      ...
      register_clear_bits(&(TIM5->DIER), TIM_DIER_UIE);
      register_set(&(TIM5->CR1), 0U, 0x3FU);
      setup_kline(false);
      kline_data = 0U;
      USB_WritePacket(NULL, 0, 0);
...
bool bitbang_five_baud_addr(bool k, bool l, uint8_t addr) {
  ...
  if (kline_data == 0U) {
    k_init = k;
    l_init = l;
    kline_data = (addr << 1) + 0x200U;
    kline_data_len = 10U;
    kline_bit_count = 0;
    kline_tick_count = 0;
    kline_ticks_per_bit = 40U;
    setup_kline(true);
    setup_timer5();
    result = true;
  }
