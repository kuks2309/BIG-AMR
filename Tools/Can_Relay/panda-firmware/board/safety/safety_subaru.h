...
static int subaru_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 0U)) {
    ...
    if (addr == 0x13a) {
      int subaru_speed = ((GET_BYTES_04(to_push) >> 12) & 0x1FFFU) + ((GET_BYTES_48(to_push) >> 6) & 0x1FFFU);
      subaru_speed /= 2;
...
static int subaru_legacy_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 0U)) {
    ...
    if (addr == 0xD4) {
      int subaru_speed = ((GET_BYTES_04(to_push) >> 16) & 0xFFFFU) + (GET_BYTES_48(to_push) & 0xFFFFU);
      subaru_speed /= 2;
      vehicle_moving = subaru_speed > SUBARU_STANDSTILL_THRSLD;
    }
