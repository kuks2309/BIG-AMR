...
>
> - **조향 0° 채택 정본 = `[7871815, 7840086]` (실측 역산 추정치는 `[7871816, 7840087]` — 1c 차, 근거로만 인용)** (node3 / node4). **공학적 채택값 = Seer 좌표계의 조향 영점**(물리 0° 실측 아님 · 非-Seer 앵커 부재 — 아래 08-03 15:40·17:00 정정에서 CAN↔Seer 독립 교차검증 불성립으로 「실측 확정」 표현을 강등). 값 `[7871815, 7840086]` 및 아래 CAN `0x6064` 판독치는 실기 검증 데이터·현행 config 정본으로 유지한다.
...
**[H11] 함수 #23 `seer_homing_tick` — [논리][신규] `DONE` 만 RstStart 를 내리지 않는 비대칭 (품질/대칭성 재평가 — [동작] 「2회차 상승에지 미획득」 귀결은 아래 08-03 15:40 정정에서 폐기)**
   재현: ABORT(`:550`)·TIMEOUT(`:639`)은 `seer_home_cancel_frames()` 를 호출하는데 **DONE(`:690`)·ERR_GOZERO(`:699`)·`default`(`:707`) 3경로가 호출하지 않는다.** 특히 ERR_GOZERO 는 축이 아직 이동 중일 수 있어 더 위험하다. (❌ [동작] 귀결 폐기: 「`0x60FB.4` 가 1 로 남아 2회차 `SEER_HOME_START`(`:600`)이 상승에지를 못 만들어 `seen_active` 미셋 → 120초 뒤 `ERR_TIMEOUT`」는 아래 08-03 15:40 정정에서 반증됨 — `seer_home_cancel_frames()` 미호출 비대칭이라는 [존재] 사실만 유지.)
