...
int can_loopback = 0;
int can_silent = ALL_CAN_LIVE;  // CAN-Relay: 기본 LIVE (TX/ACK 항상 활성)
...
// CAN-Relay 기본 비트레이트 = 250 kbps (2500U, 단위 0.1 kbps).
// ⚠ 스톡 기본값 5000U(=500 kbps)를 쓰면 안 된다:
//   harness.h:91 이 부팅 시 릴레이를 물리 통과로 두어 Seer↔모터가 직결되고, main.c 가
//   트랜시버를 강제 live 로 올리므로, 판다가 250 kbps 버스에 500 kbps 로 붙는다.
//   그 상태의 판다는 모든 프레임을 오독해 에러 프레임을 방출하며 버스를 파괴한다
//   (실측 2026-07-27: Seer 52106 odo lost / 52111 motor driver connection error /
//    54022 Bit Recessive error 지속 증가 → 250 kbps 정합만으로 전량 소멸).
//   호스트 도구가 take() 에서 set_can_speed_kbps(250) 을 부르기 때문에 지금까지 가려져 있었다.
//   ADR: docs/adr/2026-07-27-panda-boot-bitrate-and-failsafe.md
// bus1(CAN2)은 본 배선에서 미사용이나 기본값 일관성을 위해 동일하게 둔다.
bus_config_t bus_config[] = {
  { .bus_lookup = 0U, .can_num_lookup = 0U, .can_speed = 2500U, .can_data_speed = 20000U, .canfd_enabled = false, .brs_enabled = false },
  { .bus_lookup = 1U, .can_num_lookup = 1U, .can_speed = 2500U, .can_data_speed = 20000U, .canfd_enabled = false, .brs_enabled = false },
  { .bus_lookup = 2U, .can_num_lookup = 2U, .can_speed = 2500U, .can_data_speed = 20000U, .canfd_enabled = false, .brs_enabled = false },
