...
void set_safety_mode(uint16_t mode, uint16_t param) {
  ...
  switch (mode_copy) {
    ...
    case SAFETY_ALLOUTPUT:
      set_intercept_relay(true);  // CAN-Relay: intercept(릴레이 ON) 테스트
      heartbeat_counter = 0U;
      heartbeat_lost = false;
      if (current_board->has_obd) {
        current_board->set_can_mode(CAN_MODE_NORMAL);
    ...
    if (loop_counter == 0U) {
      ...
      if (!heartbeat_disabled) {
        ...
        if (heartbeat_counter >= (check_started() ? HEARTBEAT_IGNITION_CNT_ON : HEARTBEAT_IGNITION_CNT_OFF)) {
          ...
          // CAN-Relay fail-open: 이상 상태에서는 릴레이를 intercept 로 두지 않는다.
          // heartbeat 상실 = PC 주도가 더 이상 유효하지 않은 상태이므로, 물리 통과로 되돌리고
          // PC 주도권을 내려 Seer 가 모터를 직접 보게 한다. (사용자 요구 2026-07-27:
          // "모든 이상상태가 되면 relay 를 연결하면 안됨")
...
int main(void) {
  ...
  // ===== CAN-Relay TX debug: 강제 live + relay passthrough =====
  // docking: boot passthrough + heartbeat fail-safe (relay=0xe8, gate=0xe9)
  can_silent = ALL_CAN_LIVE;
  can_init_all();
  ...
  for (cnt=0;;cnt++) {
    current_board->enable_can_transceivers(true);  // CAN-Relay: 트랜시버 항상 ON
