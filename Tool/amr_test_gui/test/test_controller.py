...
def test_stuck_steer_node_faults_and_homes():
    """node4 가 지령을 안 따라오면 FAULT → 지령 홈·구동 금지 (사고 유형 회귀 방지)."""
    ...
    try:
        ...
        assert st["ramp_cmd_deg"] == 0.0 and st["raw_units"] == 0
        assert not st["drive_allowed"]
...
def test_worst_node_gates_ramp_not_average():
    """한 축만 뒤처져도 램프가 전진하면 안 된다(평균이 아니라 최악값 사용)."""
    c, bus, _ = make(steer_rate_dps=2000.0)
    try:
        ...
        while time.monotonic() - t0 < 2.0:
            st = c.tick()
            assert abs(st["ramp_cmd_deg"]) <= 30.0 + 1e-9, \
                "node4 고착인데 램프가 다음 단계로 전진했다"
            time.sleep(0.02)
    finally:
        c.disconnect()
