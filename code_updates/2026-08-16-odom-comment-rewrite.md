...
## 구 브랜치 `session/5466b21a` 정리 (같은 날)

`main` 이 `dual_laser_merger` 병합 스캔 단일 구독으로 전환하면서, 두 스캔 콜백 위에 지어진
그 브랜치의 코드는 되살릴 수 없게 됐다. **라이다 쪽은 되살리지 않고 폐기**하되, 라이다와 무관한
원본 분석·검증 두 건만 건져 왔다.

| 산출물 | 판정 | 근거 |
| --- | --- | --- |
| 대조 문서 §8 — `ScanLostTimeThresh` 원본 실측 | **건짐** | 63G 원본 해석 결과. 노드 구조와 무관 |
| 오라클 `-O0` 고정 + debt-043 원인 확정 | **건짐** | 시험 타깃 컴파일 옵션 1줄. 노드 구조와 무관 |
| 2-rate 분리(`advanceWithOdom`/`updateWithScan`) | **폐기** | 두 스캔 콜백 전제. 병합 스캔에서는 재설계 대상 |
| 입력 두절 워치독 + `/diagnostics` 발행 | **폐기** | 위 노드 구조 위에 배선됨 → `debt-106` 으로 재등록 |
| ADR 2건(2-rate · 워치독) | **폐기** | 폐기된 구현의 결정 기록 |

번호 정리: 그 브랜치가 쓰던 `debt-048` 은 `main` 에서 **다른 부채**(turn 액션 IMU 누적)에 이미
배정돼 있어, 워치독 미이식 건을 `debt-106` 으로 새로 등록했다.
